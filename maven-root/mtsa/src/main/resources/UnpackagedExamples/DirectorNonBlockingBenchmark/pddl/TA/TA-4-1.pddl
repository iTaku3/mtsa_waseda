(define
  ...
  (:constants
    ...
    $query-2 $query-fail-0 $committed-1 $query-fail-1 $query-3 $committed-0 $agency-request $query-0 $query-1 $steps-2-0 $query-fail-2 $query-fail-3 $steps-0-0 $cancel-0 $cancel-1 $cancel-2 $cancel-3 $purchase-0 $agency-succ $purchase-3 $purchase-2 $purchase-1 $purchase-fail-0 $available-0 $purchase-fail-2 $purchase-fail-1 $available-3 $available-1 $available-2 $steps-1-0 $steps-3-0 $unavailable-1 $unavailable-0 $unavailable-3 $unavailable-2 $agency-fail $purchase-fail-3 $query-succ-2 $purchase-succ-2 $purchase-succ-1 $query-succ-3 $purchase-succ-0 $query-succ-0 $query-succ-1 $purchase-succ-3 $uncommitted-1 $uncommitted-2 $uncommitted-0 $uncommitted-3 $committed-3 $committed-2 - Label
  ...
  (:action do$committed-1
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $5 $Service-1)
          (and (not (at $5 $Service-1)) (at $9 $Service-1))
        )
      )
  )
  ...
  (:action do$uncommitted-2
    ...
    :effect
      (and
        ...
        (when (at $5 $Service-2)
          (and (not (at $5 $Service-2)) (at $9 $Service-2))
        )
      )
  )
  ...
  (:action do$committed-3
    ...
    :effect
      (and
        ...
        (not (status busy))
        (when (at $5 $Service-3)
          (and (not (at $5 $Service-3)) (at $8 $Service-3))
        )
      )
  )
