(define
  ...
  (:constants
    $Crew-1 $Crew-2 $Document $Crew-0 - LTS
    $-1 $0 $1 $2 $3 $4 $5 $6 $7 - State
    $accept-2 $accept-0 $assign-0 $accept-1 $reject-2-3 $assign-1 $assign-2 $reject-0-3 $reject-1-2 $reject-2-1 $reject-1-3 $reject-2-2 $reject-0-1 $reject-0-2 $reject-1-1 $refuse $approve - Label
  ...
  (:action do$accept-2
    ...
    :effect
      (and
        ...
        (when (at $3 $Crew-2)
          (and (not (at $3 $Crew-2)) (at $7 $Crew-2))
        )
        (when (at $5 $Crew-2)
  ...
  (:action do$accept-0
    ...
    :effect
      (and
        ...
        (when (at $1 $Crew-0)
          (and (not (at $1 $Crew-0)) (at $7 $Crew-0))
        )
        (when (at $3 $Crew-0)
          (and (not (at $3 $Crew-0)) (at $7 $Crew-0))
        )
        (when (at $5 $Crew-0)
          (and (not (at $5 $Crew-0)) (at $7 $Crew-0))
        )
      )
  )
