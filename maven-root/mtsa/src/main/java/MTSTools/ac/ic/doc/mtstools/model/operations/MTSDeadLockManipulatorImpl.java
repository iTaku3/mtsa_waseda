...
public class MTSDeadLockManipulatorImpl<State, Action>
    implements MTSDeadLockManipulator<State, Action> {
   ...
   *      java.util.List)
   */
  public boolean getTransitionsToDeadlock(
  ...
  public boolean buildTraceToDeadlock(
      State actualState,
    ...
    Iterator<Pair<Action, State>> it =
        mts.getTransitions(actualState, TransitionType.POSSIBLE).iterator();
    while (it.hasNext()) {
      Pair<Action, State> nextState = it.next();
      if (!visited.contains(nextState.getSecond())) {
        MTSTransition<Action, State> newTransition =
            MTSTransitionImpl.createMTSEventState(
                actualState, nextState.getFirst(), nextState.getSecond());
  ...
  // FIXME: Change name of method if does not always delete the trace or add a comment. It deletes
  // the trace partially sometimes. Add a comment with the criteria. Could this be added to
  // MTSUtils?
  private void deleteTrace(MTS<State, Action> mts, MTSTrace<Action, State> trace) {
    for (ListIterator<MTSTransition<Action, State>> it =
            (ListIterator<MTSTransition<Action, State>>) trace.listIterator(trace.size());
        it.hasPrevious(); ) {
      ...
      if (mts.getTransitions(transition.getStateTo(), TransitionType.POSSIBLE).isEmpty()) {
        mts.removePossible(
            transition.getStateFrom(), transition.getEvent(), transition.getStateTo());
      }
    }
  }
  ...
  private boolean unavoidableDeadlock(
      MTS<State, Action> mts, MTSTrace<Action, State> trace, Set<State> reachableByRequired) {
    ...
    for (ListIterator<MTSTransition<Action, State>> it = trace.listIterator(trace.size());
        it.hasPrevious(); ) {
      MTSTransition<Action, State> transition = it.previous();
      if (MTSUtils.stateWithoutOutgoingChoices(
          mts, transition.getStateFrom(), TransitionType.POSSIBLE)) {
  ...
  // FIXME: check if there is a bug here. I think not all traces to deadlock are returned. Is this
  // what has to be done? What is the result. Add a comment if it is not returning all of them.
  ...
  private Set<State> buildReachableByRequired(MTS<State, Action> mts) {
    Set<State> reachableStates =
        new HashSet<State>((int) (mts.getStates().size() / .75f + 1), 0.75f);
    ...
    while (!toProcess.isEmpty()) {
      for (Pair<Action, State> transition :
          mts.getTransitions(toProcess.poll(), TransitionType.REQUIRED)) {
  ...
  // FIXME: This seems to be the same as MTSUtils. Check if there is a difference. If not use
  // MTSUtils.cloneMTS
    ...
    for (TransitionType type : TransitionType.values()) {
      ...
      for (State source : mts.getStates()) {
        for (Pair<Action, State> transition : mts.getTransitions(source, type)) {
          result.addTransition(source, transition.getFirst(), transition.getSecond(), type);
        }
      }
    }
    return result;
