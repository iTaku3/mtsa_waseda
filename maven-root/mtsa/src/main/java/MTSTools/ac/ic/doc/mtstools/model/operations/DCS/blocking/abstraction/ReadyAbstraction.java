...
public class ReadyAbstraction<State, Action> implements Abstraction<State, Action> {
   ...
   */
  private Map<HState<State, Action>, Map<HState<State, Action>, Map<HAction<Action>, Integer>>>
      manyStepsReachableStates;

  /**
   * Subset of the manyStepsReachableState map, containing only which marked states can be reached
   * from a given source. This map keeps aliasing/sharing with the other and it is used only to
   * speed up iteration while looking for marked states to reach.
   */
  private Map<HState<State, Action>, Map<HState<State, Action>, Map<HAction<Action>, Integer>>>
      markedReachableStates;
   ...
   */
  private Map<HState<State, Action>, Map<HAction<Action>, Map<HAction<Action>, Integer>>>
  ...
  public ReadyAbstraction(
      List<LTS<State, Action>> ltss, List<Set<State>> defaultTargets, Alphabet<Action> alphabet) {
    this.ltss = ltss;
  ...
  private void buildRA(Compostate<State, Action> compostate) {
    ...
    for (HAction<Action> t : vertices) {
      for (Integer lts : actionsToLTS.get(t)) {
        HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
        Map<HAction<Action>, Integer> actionsLeadingToTfromS =
      ...
      for (Pair<Action, State> transitions : s.getTransitions()) {
        ...
        if (!(mt == 0 || (mt == 1 && targetStates.isEmpty()))) { // already best, skip search
          ...
          for (HState<State, Action> g :
              markedReachableStatesFromSource.keySet()) { // search for best
    ...
    while (!fresh.isEmpty()) {
      ...
      for (HAction<Action> l : edges.getK(t)) {
        ...
        for (int lts = 0; lts < ltss.size(); ++lts) {
          ...
          HDist newlDist =
              dl == DirectedControllerSynthesisBlocking.INF
                  ? HDist.chasm
                  : getHDist(tDist.getFirst(), dl);
          if (newlDist.compareTo(lDist) < 0) {
            estimates.get(l).set(lts, newlDist);
            fresh.add(l);
            if (compostate.getTransitions().contains(l)) registerShort(lts, newlDist);
          }
        }
      }
    }
  ...
  private Integer gap(Compostate<State, Action> compostate, HAction<Action> l, HAction<Action> t) {
    ...
    for (Integer lts : actionsToLTS.get(l)) {
      ...
      HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
      if (s.contains(l)) {
        Map<HAction<Action>, Integer> actionFromSourceToTarget =
            manyStepsReachableActions.get(s).get(t);
    ...
    if (result != DirectedControllerSynthesisBlocking.INF) result += score;
  ...
  private Integer addDist(Integer d1, Integer d2) {
    return (d1 == DirectedControllerSynthesisBlocking.INF
            || d2 == DirectedControllerSynthesisBlocking.INF)
        ? DirectedControllerSynthesisBlocking.INF
        : d1 + d2;
  }
  ...
  private void reconcilateShort(Compostate<State, Action> compostate) {
    for (int lts = 0;
      ...
      if (shortLts == HDist.chasm) {
        ...
        Map<HState<State, Action>, Map<HAction<Action>, Integer>> markedStatesReachableFroms =
            markedReachableStates.get(s);
        for (Map.Entry<HState<State, Action>, Map<HAction<Action>, Integer>> entry :
          ...
          if (m < shortLts.getFirst()) {
            for (Integer d : entry.getValue().values()) {
              if (d < shortLts.getSecond()) shortLts = getHDist(m, d);
            }
          }
        }
    ...
    for (HAction<Action> l :
      ...
      for (int lts = 0; lts < ltss.size(); ++lts) {
        ...
        if (lDist == HDist.chasm) el.set(lts, shortest.get(lts).inc());
      }
    }
  }
  ...
  private void extractRecommendations(Compostate<State, Action> compostate) {
    ...
    if (first != null && !first.getEstimate().isConflict()) {
      ...
      for (int i = 0; i < compostate.recommendations.size(); ++i) {
        ...
        if (!ri.getAction().isControllable()) ei.values.add(0, getHDist(0, 0));
        else if (ei.equals(ef)) ei.values.add(0, getHDist(0, 1));
        else if (!ei.isConflict()) ei.values.add(0, getHDist(0, 2));
        else ei.values.add(0, HDist.chasm);
      }
    }
  ...
  private void computeActionsToLTS() {
    for (int i = 0; i < ltss.size(); ++i) {
      for (Action l : ltss.get(i).getActions()) {
        Set<Integer> set =
            actionsToLTS.computeIfAbsent(alphabet.getHAction(l), k -> new HashSet<>());
        set.add(i);
      }
    }
  }
  ...
  private void computeReachableStates() {
    Map<HState<State, Action>, Map<HState<State, Action>, Set<HAction<Action>>>>
    ...
    for (int lts = 0; lts < ltss.size(); ++lts) { // this loop populates one step reachable states
      for (State state : ltss.get(lts).getStates()) {
        ...
        Map<HState<State, Action>, Set<HAction<Action>>> oneReachableFromSoruce =
        ...
        for (Pair<Action, State> transition : ltss.get(lts).getTransitions(state)) {
          ...
          Map<HAction<Action>, Integer> manyStepsToDestination =
    ...
    while (statesPopulated) { // this loop extends the reachable states in the transitive closure
      ...
      for (Set<Pair<HAction<Action>, HState<State, Action>>> set : lastStates.values()) set.clear();
      Map<HState<State, Action>, Set<Pair<HAction<Action>, HState<State, Action>>>> swap =
          lastStates;
  ...
  /**
   * Computes for each state in each LTS which other *marked* states can reach and in how many
   * steps.
   */
  ...
  private void computeReachableActions() {
    for (int lts = 0; lts < ltss.size(); ++lts) {
      for (State state :
        ...
        // relations (one step)
        HState<State, Action> source = buildHState(lts, state);
        Map<HAction<Action>, Map<HAction<Action>, Integer>> reachableActionsFromSource =
            manyStepsReachableActions.get(source);
        for (Pair<Action, State> transition : source.getTransitions()) {
          HAction<Action> label = alphabet.getHAction(transition.getFirst());
          Map<HAction<Action>, Integer> reachableActionsThroughLabel =
              reachableActionsFromSource.computeIfAbsent(label, k -> new HashMap<>());
      ...
      for (State state :
        ...
        HState<State, Action> source = buildHState(lts, state);
        Map<HState<State, Action>, Map<HAction<Action>, Integer>> statesReachableFromSource =
            manyStepsReachableStates.get(source);
        Map<HAction<Action>, Map<HAction<Action>, Integer>> actionsReachableFromSource =
            manyStepsReachableActions.get(source);
        for (HState<State, Action> destination : statesReachableFromSource.keySet()) {
          for (Map.Entry<HAction<Action>, Integer> entry :
            ...
            for (Pair<Action, State> transition : destination.getTransitions()) {
              ...
              Map<HAction<Action>, Integer> actionsLeadingToTarget =
                  actionsReachableFromSource.computeIfAbsent(target, k -> new HashMap<>());
              putmin(
  ...
  private HState<State, Action> buildHState(int lts, State state) {
    ...
    if (index == null) {
      HState<State, Action> hstate =
          new HState<>(
              lts, state, this.stash.size(), this.defaultTargets.get(lts).contains(state), ltss);
      stash.add(hstate);
