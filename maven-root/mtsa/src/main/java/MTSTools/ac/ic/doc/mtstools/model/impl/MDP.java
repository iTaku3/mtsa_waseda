...
public class MDP extends AbstractTransitionSystem<Long, String> implements LTS<Long, String> {
  private Map<Long, HashSet<ProbabilisticTransition>> transitions =
      new HashMap<Long, HashSet<ProbabilisticTransition>>();
  ...
  public boolean addTransition(Long from, String label, Long to, double probability, int bundle) {
    if (!getStates().contains(from)) addState(from);
    if (!getStates().contains(to)) addState(to);
    if (!getActions().contains(label)) addAction(label);
  ...
  public Map<Long, BinaryRelation<String, Long>> getTransitions() {
    ...
    for (Long s : getStates()) ts.put(s, getTransitions(s));
    return ts;
  }
  ...
  protected void removeTransitions(Collection<Long> unreachableStates) {
    for (long state : unreachableStates) transitions.remove(state);
  }
  ...
  public boolean removeTransition(Long from, String label, Long to) {
    ...
    for (ProbabilisticTransition t : transitions.get(from))
      if (t.getAction().equals(label) && t.getTo().equals(to)) removes.add(t);
  ...
  public List<ProbabilisticTransition> getBundle(long state, ProbabilisticTransition t) {
    ...
    for (ProbabilisticTransition t2 : getTransitionsFrom(state))
      if (t2.getAction().equals(t.getAction()) && t2.getBundle() == t.getBundle()) bundle.add(t2);
    return bundle;
  }

  public void addStateLabel(Long state, String label) {
    System.out.println("Adding label " + state + " " + label);
    if (!stateLabels.containsKey(label)) stateLabels.put(label, new Vector<Long>());
    if (!stateLabels.get(label).contains(state)) stateLabels.get(label).add(state);
  }
  ...
  public List<String> getLabels(Long state) {
    ...
    for (String label : stateLabels.keySet())
      if (stateLabels.get(label).contains(state)) labels.add(label);
    return labels;
  }
  ...
  public static MDP compose(MDP mdp1, MDP mdp2) {
    composition =
        new MDP(compositionStateNumber(mdp1, mdp1.getInitialState(), mdp2.getInitialState()));
  ...
  private static long compositionStateNumber(MDP mdp1, long state1, long state2) {
    if (state2 == exceptionState) return exceptionState + state1;
    return (state2 * mdp1.getStates().size()) + state1; // y*width + x
  }

  private static void composeStates(
      MDP mdp1, MDP mdp2, long state1, long state2) // deal with taus***
      {
    long stateC = compositionStateNumber(mdp1, state1, state2);
    if (visited.contains(stateC)) return;
    visited.add(stateC);
    ...
    for (ProbabilisticTransition t : mdp2.getTransitionsFrom(state2)) {
      if (mdp1.getActions().contains(t.getAction())) // need to sync
      {
        ...
        if (!synchronised) isDeadlock = true;
      } else {
        composition.addTransition(
            stateC,
            t.getAction(),
    ...
    if (isDeadlock) System.out.println("Deadlock detected in MDP composition");
  }
  ...
  public static MDP composeAbstraction(MDP r, MDP a, BinaryRelation<Long, Long> simul) {
    ...
    if (simul.size() == 0) System.out.println("WARNING: simulation empty");
    ...
    for (ProbabilisticTransition t : r.getTransitionsFrom(state1)) {
      System.out.println(
          "state ("
              ...
              + ") got a "
              + t.getAction()
              + " transition, looking for bundle "
              + t.getBundle());
      if (!processed.contains(t)) {
        // find the bundle
        List<ProbabilisticTransition> bundle = new Vector<ProbabilisticTransition>();
        for (ProbabilisticTransition t2 : r.getTransitionsFrom(state1))
          if (t2.getBundle() == t.getBundle()
              && t2.getAction().equals(t.getAction())
              && !bundle.contains(t2)) bundle.add(t2);
        processed.addAll(bundle);

        System.out.println("bundle size is " + bundle.size());

        // find all the matching A states
        Map<ProbabilisticTransition, Vector<Long>> aStates =
            new HashMap<ProbabilisticTransition, Vector<Long>>();
        for (ProbabilisticTransition t2 : bundle) {
          ...
          for (Pair<String, Long> t3 : a.getTransitions(state2))
            if (t2.getAction().equals(t3.getFirst())
                ...
                && !aStates.get(t2).contains(t3.getSecond())) aStates.get(t2).add(t3.getSecond());
          ...
          if (aStates.get(t2).size()
              == 0) // if there are no ways to do this part of bundle, make it go to
            // exception
  ...
  private static void composeAbstractionStates_replicateBundle(
      MDP r,
      MDP a,
      long stateC,
      long state2,
      List<ProbabilisticTransition> bundle,
      Map<ProbabilisticTransition, Vector<Long>> aStates) {
    composeAbstractionStates_replicateBundle(
        r, a, stateC, state2, bundle, aStates, new long[bundle.size()], 0);
  }
  ...
  private static void composeAbstractionStates_replicateBundle(
    ...
    if (index < assignment.length /* && aStates.size() > 0 */) // not filled the bundle
    {
      for (long s :
          aStates.get(
              bundle.get(index))) // iterate over aStates to fill a slot of the bundle, recurse
      // to next slot of bundle
      {
        assignment[index] = s;
        composeAbstractionStates_replicateBundle(
            r, a, stateC, state2, bundle, aStates, assignment, index + 1);
      }
    } else // filled, can add the bundle and recurse back into main method
    {
      System.out.println(
          "state "
              + stateC
              + " ASSIGNMENT OF A-STATES IN THIS BUNDLE "
              + bundle.get(0).getBundle()
              + " IS "
      ...
      for (int i = 0; i < bundle.size(); i++) {
        ...
        if (next != exceptionState)
          for (String label :
              a.getLabels(state2)) // copy labels over only if destination is not an exception
          composition.addStateLabel(stateC, label);

        System.out.println(
            " adding " + stateC + " -" + t.getAction() + "-> (" + t.getTo() + "," + next + ")");
        composition.addTransition(stateC, t.getAction(), nextC, t.getProbability(), bundleID);
        if (next == exceptionState) composition.addStateLabel(nextC, "exception");
  ...
  private static void composeEnactmentStates(
      MDP r, MDP a, long state1, long state2, List<String> controlledActions) {
    long stateC = compositionStateNumber(r, state1, state2);
    if (visited.contains(stateC)) return;
    visited.add(stateC);
    composition.addState(stateC);
      ...
      if (state2 == exceptionState) // we're already in exception, allow R to move -- RULE 3
      {
        ...
        for (Pair<String, Long> t2 : a.getTransitions(state2))
          if (t2.getFirst().equals(t.getAction())) // RULE 1
          {
            ...
            for (String label :
                a.getLabels(state2)) // copy labels over only if destination is not an
              // exception
              composition.addStateLabel(stateC, label);
            ...
            composition.addTransition(
                stateC,
                t.getAction(),
                compositionStateNumber(r, t.getTo(), t2.getSecond()),
                t.probability,
                t.bundle);
            composeEnactmentStates(
                r, a, t.getTo(), t2.getSecond(), controlledActions); // depth first
          } else if (t.getAction().equals("tau")) // RULE 2
          {
            ...
            composition.addTransition(
                stateC,
                t.getAction(),
                compositionStateNumber(r, t.getTo(), state2),
                t.probability,
                t.bundle);
            composeEnactmentStates(r, a, t.getTo(), state2, controlledActions); // depth first
          }
        if (!synced) {
          if (!controlledActions.contains(
              t.getAction())) // A cannot handle R's uncontrolled action -- RULE 4
          {
            long next = compositionStateNumber(r, t.getTo(), exceptionState);
            composition.addTransition(
                stateC, t.getAction(), next, t.getProbability(), t.getBundle());
            composition.addStateLabel(next, "exception");
            composeEnactmentStates(
                r, a, t.getTo(), exceptionState, controlledActions); // depth first;
            // recurse to allow
            // R to continue
            // after exception
          } else // A cannot handle R's controlled action -- RULE 5
          {
            ...
            if (!syncPossible) {
              ...
              composition.addTransition(
                  stateC, t.getAction(), stateC, t.getProbability(), t.getBundle()); // nop
  ...
  public void writePrismFile(String filename, String modName) {
    try {
      ...
      for (long state : this.getStates()) {
        for (ProbabilisticTransition t : getTransitionsFrom(state)) {
          if (!visited.contains(t)) {
            ...
            String line =
                "[" + prismifyLabel(t.getAction()) + "] " + stateVar + "=" + state + " -> ";
            for (ProbabilisticTransition t2 : bundle) {
      ...
      for (String label : stateLabels.keySet()) {
        ...
        for (Long state : stateLabels.get(label)) line += stateVar + "=" + state + " | ";
