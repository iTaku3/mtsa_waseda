...
/**
 * This class contains the logic to synthesize a controller for a deterministic environment using an
 * informed search procedure.
 */
public class DirectedControllerSynthesisBlocking<State, Action>
    extends DirectedControllerSynthesis<State, Action> {
  ...
  private HashMap<Integer, HashMap<Integer, HashMap<Integer, Set<Compostate<State, Action>>>>>
      stratY;

  /** HashMap<loopID, X[j][r][i] -> Set<Compostate> > */
  private HashMap<
          Integer,
          HashMap<Integer, HashMap<Integer, HashMap<Integer, Set<Compostate<State, Action>>>>>>
      stratX;
  ...
  private final Logger logger =
      Logger.getLogger(DirectedControllerSynthesisBlocking.class.getName());
   ...
   * @return the controller for the environment in the form of an LTS that when composed with the
   *     environment reaches the goal, only by enabling or disabling controllable actions and
   *     monitoring uncontrollable actions. Or null if no such controller exists.
   */
  public LTS<Long, Action> synthesize(
    ...
    /**
     * Indicates whether the procedure tries to reach a goal state (by default we look for live
     * controllers).
     */
    statistics.clear();
    statistics.start();
    ...
    assertFalse(
        "Finished because open was empty, shouldn't be the case", initial.isStatus(Status.NONE));
  ...
  /**
   * Creates (or retrieves from cache) a state in the composition given a list with its base
   * components.
   *
   * @param states, a list of states with one state per LTS in the environment, the position of each
   *     state in the list (its index) reflects to which LTS that state belongs.
   */
  ...
  private Compostate<State, Action> buildInitialState() {
    ...
    for (LTS<State, Action> lts : ltss) states.add(lts.getInitialState());
    Compostate<State, Action> initialComp =
        buildCompostate(
            states); // for non-deterministic LTS I need the tau-closure as a set of initial states
    if (initialComp.markedByGuarantee.size() > 0) initialComp.addTargets(initialComp);
  ...
  /**
   * Expands the next recommendation of an open state and opens its child. If no further
   * recommendations are available the state is marked as an error, and the error is propagated to
   * its ancestors. If the state is controllable and has further recommendations the parent state is
   * left open and competes with its exploredChildren in the open queue. This avoids a DFS type of
   * search. On the other hand, uncontrollable states are never left opened since all its
   * exploredChildren need to be explored eventually, thus a DFS to a failure is used instead.
   */
  private void doOpen(Compostate<State, Action> compostate) {
    ...
    assertTrue(
        "compostate to open doesn't have valid recommendation",
        compostate.hasValidRecommendation());
    if (compostate.hasValidRecommendation()) {
      ...
      if (compostate.isControlled()
          && compostate.hasValidRecommendation()
          && compostate.isStatus(Status.NONE)) compostate.open();
    }
  }

  /**
   * Expands a state following a given recommendation from a parent compostate. Internally this
   * populates the transitions and expanded lists.
   */
  private void expand(Compostate<State, Action> compostate, Recommendation<Action> recommendation) {
    ...
    if (childCompostate.markedByGuarantee.size() > 0) childCompostate.addTargets(childCompostate);
    if (!childCompostate.isEvaluated()) abstraction.eval(childCompostate);
  ...
  private List<State> getChildStates(Compostate<State, Action> compostate, HAction<Action> action) {
    ...
    for (int i = 0; i < size; ++i) {
      Set<State> image =
          ltss.get(i).getTransitions(parentStates.get(i)).getImage(action.getAction());
      if (image == null || image.isEmpty()) {
        if (ltss.get(i)
            .getActions()
            .contains(
                action.getAction())) { // tau does not belong to the lts alphabet, yet it may have a
          // valid image, but I do not want taus at this stage
          transitions.clear();
    ...
    for (Set<State> singleton :
        transitions) // Note: Cartesian product for non-deterministic systems
  ...
  /**
   * Opens a given child state following a recommendation from a parent. This method distinguishes
   * between reachability and liveness. If the child to "open" is a goal (or closes a loop over a
   * marked state) it propagates the goal through the ancestors. If the child is an error (or closes
   * a loop over non-marked states) it propagates an error. If the child is already live this method
   * does nothing, otherwise the child is evaluated with the heuristic and opened.
   */
  private void explore(
      Compostate<State, Action> parent,
      Recommendation<Action> recommendation,
      Compostate<State, Action> child) {
    assert (!isError(parent)
        && !isGoal(parent)); // the parent should be NONE, otherwise exploring is useless
  ...
  private Set<Compostate<State, Action>> playerCanForceWinMax(
      Set<Compostate<State, Action>> c, boolean allowNones) {
    ...
    logger.fine(
        "playerCanForceWin with allowNones = "
            + allowNones
            + " and c = "
            + c.toString()
            + "\n\t\t\t - is = "
  ...
  /**
   * checks if the player can force any of set target, Goals or (if allowNones) any Nones outside of
   * nonesNotWanted
   */
  private boolean playerCanForce(
      Compostate<State, Action> v,
    ...
    for (Compostate<State, Action> child : v.getChildrenExploredThroughUncontrollable()) {
      if (child.isStatus(Status.ERROR)) {
        ...
        if (target.contains(child) || child.isStatus(Status.GOAL))
          hasUncontrollableToTargetOrGoal = true;
        if (!nonesInLoop.contains(child) && child.isStatus(Status.NONE)) {
          if (!unexploredAreGoals) return false;
          hasUncontrollableToUnexplored = true;
        }
        if (nonesInLoop.contains(child) && !target.contains(child) && child.isStatus(Status.NONE))
          return false;
      }
    }
    ...
    for (Compostate<State, Action> child : v.getChildrenExploredThroughControllable()) {
      ...
      if (!nonesInLoop.contains(child) && child.isStatus(Status.NONE) && unexploredAreGoals)
        return true;
    }
  ...
  private Set<Compostate<State, Action>> playerCanForceSetIn1Step(
      Set<Compostate<State, Action>> c,
      Set<Compostate<State, Action>> target,
      boolean allowNones,
    ...
    //        logger.fine("playerCanForceSetIn1Step with allowNones = " + allowNones + ", target = "
    // + target.toString() + " and c = " + c.toString() + "\n is = " + result.toString());
    return result;
  }
  ...
  /**
   * funciona porque ya se sabe que o no tiene todos los markings dentro del loop, o gatherGoal dio
   * vacio
   */
  private Set<Compostate<State, Action>> findNewErrors() {
    ...
    if (canBeWinningLoop(
        c)) { // if our optimization to avoid the fixPoint doesn't work, we do the fixpoint
  ...
  /**
   * adapted idea from Synthesis of Reactive(1) Designs Nir Piterman, Amir Pnueli, and Yaniv Sa’ar
   * also used in buildController
   */
  private Set<Compostate<State, Action>> tripleFixPoint(
      Set<Compostate<State, Action>> c, boolean setR, boolean unexploredAreGoals) {
    ...
    while (previous_sizeZ != z.size()) {
      ...
      for (int j = 0; j < n; ++j) {
        ...
        while (previous_size_y != y.size()) {
          ...
          Set<Compostate<State, Action>> coxY =
              playerCanForceSetIn1Step(c, y, unexploredAreGoals, c);
          ...
          for (int i = 0; i < assumptions.size(); ++i) {
            ...
            if (setR) {
              ...
              if (!stratX.get(loopID).get(j).containsKey(r))
                stratX.get(loopID).get(j).put(r, new HashMap<>());
  ...
  private void propagateGoal(
      Set<Compostate<State, Action>> newGoals, Set<Compostate<State, Action>> parent) {
    ...
    while (!toCheck.isEmpty() || !toCheckLoop.isEmpty()) {
      while (!toCheck.isEmpty()) {
        ...
        if (playerCanForce(
            current, new HashSet<>(), false, new HashSet<>())) { // player can force Goal
  ...
  private void propagateError(
      Set<Compostate<State, Action>> newErrors, Set<Compostate<State, Action>> parent) {
  ...
  private Set<Compostate<State, Action>> ambientCanForceLoseMax(
      Set<Compostate<State, Action>> c, boolean unexploredAreGoals) {
    Set<Compostate<State, Action>> k = new HashSet<>(c);
    auxiliarListStates.addAll(c);
    ...
    auxiliarListStates.clear();
    logger.fine(
        "ambientCanForceLose with allowNones = "
            + unexploredAreGoals
            + " and c = "
            + c.toString()
            + "\n\t\t\t - is = "
  ...
  private boolean ambientCanForce(
      Compostate<State, Action> v,
      Set<Compostate<State, Action>> target,
      boolean unexploredAreGoals,
      Set<Compostate<State, Action>> nonesInLoop) {
    ...
    for (Compostate<State, Action> child : v.getChildrenExploredThroughControllable()) {
      if (target.contains(child)
          || child.isStatus(Status.ERROR)
          || (!nonesInLoop.contains(child) && !unexploredAreGoals && child.isStatus(Status.NONE))) {
  ...
  private boolean canForceOutside(
      Compostate<State, Action> state, Set<Compostate<State, Action>> ancestors) {
  ...
  /**
   * Note that the logic of this function is seeing if having memory in our controller we could
   * disable a controllable transition that stays in the loop after 1 lap. Otherwise the ambient
   * could keep us here for an infinite time.
   */
  private boolean hasControllableWayOut(
      Compostate<State, Action> state, Set<Compostate<State, Action>> c) {
    ...
    for (Compostate<State, Action> child : state.getChildrenExploredThroughUncontrollable()) {
      if (c.contains(child) || child.isStatus(Status.ERROR)) {
        ...
        hasUncontrollableOut =
            true; // we don't return true here bc there can still be one uncontrollable that stays
      }
    }
  ...
  private boolean closingALoop(Compostate<State, Action> child, Compostate<State, Action> parent) {
    if (child
        .isEvaluated()) // if the child has already been considered then we might be closing a loop
  ...
  private void expandChild(Compostate<State, Action> child) {
    if (!child.isEvaluated()) abstraction.eval(child);
  ...
  /**
   * Returns a map representing a DAG from parent to child. The child state in a terminal node
   * indicates a loop.
   */
  private void buildAncestorsDAG(
      Compostate<State, Action> child, Compostate<State, Action> parent) {
    dag.clear();
    ...
    for (int i = 0; i < auxiliarListStates.size(); ++i) {
      ...
      for (Pair<HAction<Action>, Compostate<State, Action>> predecesor : state.getParents()) {
        ...
        if (isError(predState) || isGoal(predState)) continue;
  ...
  /**
   * Gathers the states enclosed by a loop over parent and child states. Additionally gathers the
   * marked states in the loop.
   */
  ...
  private boolean isError(Compostate<State, Action> compostate) {
    return compostate.isStatus(Status.ERROR)
        || compostate.getStates().contains(-1L)
        || compostate.isDeadlock();
  }
  ...
  private void broadcastNewClosedChildToParents(Compostate<State, Action> state, Status status) {
    for (Pair<HAction<Action>, Compostate<State, Action>> ancestorActionAndState :
        state.getParents()) {
  ...
  private void setGoal(Compostate<State, Action> state, HAction<Action> action) {
    ...
    if (action == null) {
      for (Pair<HAction<Action>, Compostate<State, Action>> transition :
          state.getExploredChildren()) {
        action = transition.getFirst();
        Compostate<State, Action> child = transition.getSecond();

        if (child.isStatus(Status.GOAL))
          break; // fixme una mejor manera de hacer esto sin recorrer todo
  ...
  private void setDistanceToGoal(Compostate<State, Action> compostate, int distance) {
    if (distance < compostate.getDistance()) compostate.setDistance(distance);
    setFinal(compostate);
  }
  ...
  /**
   * After the synthesis procedure this method builds a controller in the form of an LTS by starting
   * in the initial state and following all the non-closed descendants. If there is no controller
   * for the given environment this method returns null.
   */
  private LTS<Long, Action> buildController() {
    class ContComp {
      ...
      // if we dont know that we can avoid an assumption (i.e. c is not in a strategy x[i][r]) then
      // i=-1
    ...
    // ids takes as a key a Compostate a "j" the guarantee (and "i" the assumption) wich strategy we
    // follow with this state.
    // The same Compostate can be present with different strategies being represented with multiple
    ...
    while (!controllerDescendants.isEmpty()) {
      ...
      if (current.getLoopID() == 0) {
        ...
        for (Pair<HAction<Action>, Compostate<State, Action>> transition :
          ...
          includeTransition |=
              (child.getLoopID() == 0)
                  && (child.getBestDistToWinLoop() < current.getBestDistToWinLoop())
                  && child.isStatus(Status.GOAL);
          ...
          if (includeTransition) {
            ...
            result.addTransition(
                ids.get(new ContComp(current, 0)),
                action.getAction(),
                ids.get(new ContComp(child, 0)));
          }
        }
      } else {
        ...
        for (Pair<HAction<Action>, Compostate<State, Action>> transition :
          ...
          if (child.getLoopID() == thisLoop) {
            if (current.markedByGuarantee.contains(j)) { // like asking that J^2_j
              ...
              includeTransition |=
                  minRForChildInY != -1 && minRForChildInY < minRInStratYFor(current, j, thisLoop);
            }
            ...
            if (minRIForCurrentInX != null) {
              ...
              if (current.markedByAssumption.contains(currentI)) {
                includeTransition |=
                    stratX.get(thisLoop).get(j).get(currentR).get(currentI).contains(child);
              }
            }
          }
          ...
          if (includeTransition) {
            ...
            assertTrue(
                "succ: "
                    ...
                    + " i2: "
                    + i2,
                i1 != null && i1 >= 0 && i2 != null && i2 >= 0);

            result.addTransition(
                ids.get(new ContComp(current, j)),
                action.getAction(),
                ids.get(new ContComp(child, childJ)));
          }
        }
      }
    }
  ...
  private Pair<Integer, Integer> minRIInStratXFor(
      Compostate<State, Action> state, int j, int thisLoop) {
