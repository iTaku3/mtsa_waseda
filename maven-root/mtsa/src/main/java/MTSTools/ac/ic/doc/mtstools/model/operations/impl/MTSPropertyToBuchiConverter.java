...
public class MTSPropertyToBuchiConverter {
  ...
  public static <State, Action> void convert(
      MTS<State, Action> property, State trapState, Action specialAction) {
    ...
    for (Iterator it = property.getStates().iterator(); it.hasNext(); ) {
      ...
      for (Iterator it2 = property.getTransitions(state, TransitionType.REQUIRED).iterator();
          it2.hasNext(); ) {
