...
public class ParallelComposer<S, A, D> {
  ...
  /**
   * This implementation of parallel composition assumes that controllers is an LTS that has the
   * same alphabet as environment and the environment can always simulate the controller.
   */
  public LTS<Pair<S, D>, A> compose() {
      ...
      for (A a : controller.getActions()) {
        if (!environment.getActions().contains(a) && !"-1".equals(a)) {
          throw new RuntimeException(
              "This implementation of parallel composition assumes that you are composing a"
                  + " controller with the associated environment: "
                  + environment.getActions()
                  + " != "
    ...
    Pair<S, D> initialState =
        new Pair<S, D>(environment.getInitialState(), controller.getInitialState());
    ...
    while (!pending.isEmpty()) {
      ...
      for (Pair<A, D> cTrans : cTransitions) {
        ...
        //					throw new RuntimeException("The environment couldn't imitate the taking the
        // transition " + a.toString() + " at the state " + d.toString());
        //				}

        if (envSuccesors == null || envSuccesors.isEmpty()) {
          //					System.out.println("The environment couldn't imitate the taking the transition " +
          // a.toString() + " at the state " + d.toString());
        } else {
