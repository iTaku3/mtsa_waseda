...
public class MonotonicAbstraction<State, Action> extends Abstraction<State, Action> {
  ...
  /** */
  private final DirectedControllerSynthesisNonBlocking<State, Action>
      directedControllerSynthesisNonBlocking;
  ...
  /**
   * Mapping of states to the reachable states and the actions leading to them plus in how many
   * steps.
   */
  private Map<
  ...
  public MonotonicAbstraction(
      DirectedControllerSynthesisNonBlocking<State, Action>
          directedControllerSynthesisNonBlocking) {
        ...
        new InitMap<>(
            new Factory<HEstimate<State, Action>>() {
              ...
              public HEstimate<State, Action> newInstance() {
                return new HEstimate<State, Action>(
                    MonotonicAbstraction.this.directedControllerSynthesisNonBlocking.ltssSize,
                    HDist.chasm);
              }
            });
    ...
    while (!freshStates.isEmpty()) {
      ...
      for (HAction<State, Action> action : freshActions) {
        for (HState<State, Action> state : pending.get(action)) {
          if (!stateGenerations.containsKey(state)) freshStates.add(state);
        }
  ...
  private void update(int iteration) {
    for (HState<State, Action> s : freshStates) {
      ...
      for (Pair<Action, State> transition : current.getTransitions(s.state)) {
        ...
        HAction<State, Action> l =
            this.directedControllerSynthesisNonBlocking.alphabet.getHAction(transition.getFirst());
        pending.get(l).add(d);
  ...
  public void evaluateMA(Compostate<State, Action> compostate) {
    ...
    for (Integer lts : selected) {
      ...
      Map<HState<State, Action>, Map<HAction<State, Action>, Integer>> pathsFromSource =
          paths.get(s);
      for (HState<State, Action> t : pathsFromSource.keySet()) {
        if (!t.marked /*isMarked()*/) continue;
        ...
        for (Entry<HAction<State, Action>, Integer> entry : pathsToTarget.entrySet()) {
          ...
          if (newDist.compareTo(lDist) < 0) estimates.get(l).set(lts, newDist);
        }
      }
    }
  ...
  private boolean select(Compostate<State, Action> compostate) {
    ...
    for (int lts = 0; lts < this.directedControllerSynthesisNonBlocking.ltssSize; ++lts) {
      ...
      if (isTarget) {
        if (includeTargets) selected.add(lts);
      } else if (isMarked) {
        if (includeTargets) {
          selected.clear();
  ...
  private void reconcilateCrossLTS(Compostate<State, Action> compostate) {
    for (int lts = 0; lts < this.directedControllerSynthesisNonBlocking.ltssSize; ++lts) {
      ...
      for (HAction<State, Action> l : compostate.getTransitions()) {
        if (!selected.contains(lts)) estimates.get(l).set(lts, HDist.zero);
        if (!this.directedControllerSynthesisNonBlocking
                .ltss
                .get(lts)
                .getActions()
            ...
            || s.isSelfLoop(l)) {
          HDist lDist = estimates.get(l).get(lts);
          if (lDist.compareTo(HDist.chasm) == 0) estimates.get(l).set(lts, HDist.zero);
        }
      }
    }
  }
  ...
  private Integer minDist(HState<State, Action> source, Set<HAction<State, Action>> labels) {
    ...
    for (HAction<State, Action> action : labels) result = Math.min(result, dist(source, action));
    return result;
  }

  /** Adds to distances (maxing at overflows). */
  private Integer addDist(Integer d1, Integer d2) {
    if (d1 == DirectedControllerSynthesisNonBlocking.INF
    ...
    for (int lts = 0; lts < this.directedControllerSynthesisNonBlocking.ltssSize; ++lts) {
      for (State state : this.directedControllerSynthesisNonBlocking.ltss.get(lts).getStates()) {
        ...
        for (Pair<Action, State> transition :
            this.directedControllerSynthesisNonBlocking.ltss.get(lts).getTransitions(state)) {
          HAction<State, Action> label =
              this.directedControllerSynthesisNonBlocking.alphabet.getHAction(
                  transition.getFirst());
        ...
        for (Pair<HAction<State, Action>, HState<State, Action>> pair : lastStates.get(source)) {
          ...
          for (HState<State, Action> target : oneStepReachableStates.get(intermediate).keySet()) {
            ...
            Integer newDist =
                addDist(
                    distIntermediate,
                    minDist(intermediate, oneStepReachableStates.get(intermediate).get(target)));
            if (newDist != DirectedControllerSynthesisNonBlocking.INF) { // !!!
