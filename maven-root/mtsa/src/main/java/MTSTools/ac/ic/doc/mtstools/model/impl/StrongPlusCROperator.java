...
public class StrongPlusCROperator {
  ...
  public <S1, S2, A> MTS<Pair<S1, S2>, A> compose(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
    ...
    Set<Pair<S1, S2>> consistencyRelation =
        this.getStrongConsistency().getConsistencyRelation(mtsA, mtsB);
  ...
  private <S1, S2, A> void processRule(
      Pair<S1, S2> startState,
      BinaryRelation<A, S1> transitionsA,
      BinaryRelation<A, S2> transitionsB,
      TransitionType newTranstitionsType,
      MTS<Pair<S1, S2>, A> mtsCR) {
