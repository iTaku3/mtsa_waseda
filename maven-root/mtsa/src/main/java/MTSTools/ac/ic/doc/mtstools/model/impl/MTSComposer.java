...
public class MTSComposer {
  ...
  public <Action> MTS<Long, Action> compose(MTS<Long, Action> mtsA, MTS<Long, Action> mtsB) {
    ...
    while (!toProcess.isEmpty()) {
      ...
      Collection<Transition<Action>> composableTransitions =
          getComposableTransitions(mtsA, mtsB, actual);
      for (Iterator<Transition<Action>> it = composableTransitions.iterator(); it.hasNext(); ) {
        ...
        retValue.addTransition(
            actual, transition.getAction(), transition.getTo(), transition.getType());
  ...
  private static <Action> Collection<Transition<Action>> getComposableTransitions(
    ...
    Map<Action, List<Pair<Long, TransitionType>>> fromA =
        groupByAction(getTransitions(mtsA, state.getFirst()));
    Map<Action, List<Pair<Long, TransitionType>>> fromB =
        groupByAction(getTransitions(mtsB, state.getSecond()));
    ...
    for (Iterator<Action> it = allActions.iterator(); it.hasNext(); ) {
      ...
      if (isInA && isInB) {
        if (isInFromA && isInFromB) {
          for (Iterator<Pair<Long, TransitionType>> itA = fromA.get(action).iterator();
              itA.hasNext(); ) {
            Pair<Long, TransitionType> toA = itA.next();
            for (Iterator<Pair<Long, TransitionType>> itB = fromB.get(action).iterator();
                itB.hasNext(); ) {
        ...
        if (fromA.get(action) != null) {
          for (Iterator<Pair<Long, TransitionType>> it2 = fromA.get(action).iterator();
              it2.hasNext(); ) {
          ...
          for (Iterator<Pair<Long, TransitionType>> it2 = fromB.get(action).iterator();
              it2.hasNext(); ) {
            Pair<Long, TransitionType> transition = it2.next();
  ...
  private static <Action> Map<Action, List<Pair<Long, TransitionType>>> groupByAction(
    ...
    Map<Action, List<Pair<Long, TransitionType>>> retValue =
        new HashMap<Action, List<Pair<Long, TransitionType>>>();
    for (Iterator<Transition<Action>> it = transitions.iterator(); it.hasNext(); ) {
      ...
      retValue
          .get(transition.getAction())
          .add(Pair.create(transition.getTo(), transition.getType()));
    }
    return retValue;
  }
  ...
  private static <Action> Collection<Transition<Action>> getTransitions(
    ...
    Collection<Transition<Action>> retValue = new ArrayList<Transition<Action>>();
    for (Iterator<Pair<Action, Long>> iter = mts.getTransitions(state, type).iterator();
        iter.hasNext(); ) {
  ...
  private static <Action> Long code(
      MTS<Long, Action> mtsA, MTS<Long, Action> mtsB, Long a, Long b) {
    return a * mtsB.getStates().size() + b;
  }

  private static <Action> Pair<Long, Long> decode(
      MTS<Long, Action> mtsA, MTS<Long, Action> mtsB, Long x) {
