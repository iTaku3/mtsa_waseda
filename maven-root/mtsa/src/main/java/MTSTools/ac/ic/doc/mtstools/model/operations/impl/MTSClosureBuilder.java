...
public class MTSClosureBuilder implements MTSClosure {
  ...
  private <Action, State> void addTransitions(
      MTS<State, Action> mts,
      State state,
      Action action,
      TransitionType transitionType,
      Set<Action> silentActions) {
    Iterator<List<Pair<Action, State>>> iterator =
        getIterator(mts, state, action, transitionType, silentActions);
  ...
  protected <State, Action> Iterator<List<Pair<Action, State>>> getIterator(
      ...
      Action action,
      TransitionType transitionType,
      Set<Action> emptySilentActions) {
    return getClosurePathBuilder()
        .getProjectionIterator(mts, state, action, transitionType, emptySilentActions);
  }
