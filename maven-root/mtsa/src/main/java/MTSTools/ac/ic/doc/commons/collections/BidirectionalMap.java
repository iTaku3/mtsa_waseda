...
/**
 * This class implements a Bidirectional Map, that is, keys map to sets of values and values map to
 * sets of keys.
 */
public class BidirectionalMap<K, V> {
  ...
  public boolean put(K key, V value) {
    ...
    if (keys == null) imap.put(value, keys = new HashSet<>());
    keys.add(key);
    return result;
  }
  ...
  public void putAll(BidirectionalMap<K, V> other) {
    for (K key : other.keySet()) putAll(key, other.getV(key));
  }
  ...
  public void removeAll(Collection<K> keys, Collection<V> values) {
    if (keys != null) {
      for (K key : keys) removeAll(key, values);
    }
  }
