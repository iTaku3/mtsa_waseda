...
public class Compostate<State, Action> {
  ...
  /**
   * An identifier used by CompleteFrontierExplorationHeuristic to know how old the values of the
   * estimates are, and whether they need to be recomputed
   */
  public Integer seq = -1;

  // These 3 fields are used by the Ready Abstraction as a cache, to avoid recomputing the graph
  // every time
  ...
  /**
   * todo esto es para poder guardarme las transiciones a usar en setGoal sin tener pares
   * state/action en gather
   */
  ...
  public Compostate(
      DirectedControllerSynthesisNonBlocking<State, Action> directedControllerSynthesisNonBlocking,
      List<State> states) {
  ...
  private Set<HAction<State, Action>>
      ...
      for (int i = 0; i < states.size(); ++i)
        if (!dcs.facilitators.get(i).equals(states.get(i)))
          for (Pair<Action, State> transition :
              dcs.ltss.get(i).getTransitions(dcs.facilitators.get(i))) {
            HAction<State, Action> action = dcs.alphabet.getHAction(transition.getFirst());
  ...
  public int getChildDistance(HAction<State, Action> action) {
    int result =
        DirectedControllerSynthesisNonBlocking
            .UNDEF; // exploredChildren should never be empty or null
  ...
  public void addTargets(Compostate<State, Action> compostate) {
    ...
    if (targets.isEmpty()) {
      ...
      for (int lts = 0; lts < dcs.ltssSize; ++lts) targets.add(new HashSet<>());
    }
    for (int lts = 0; lts < dcs.ltssSize; ++lts) targets.get(lts).add(states.get(lts));
  }
  ...
  public boolean addRecommendation(
      HAction<State, Action> action, HEstimate<State, Action> estimate) {
    boolean controllableAction = action.isControllable();
    controlled &= controllableAction; // may not work with lists of recommendations

    if (!estimate.isConflict()) recommendations.add(new Recommendation<>(this, action, estimate));
