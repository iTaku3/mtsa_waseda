...
/**
 * This class represents a set of synchronized transitions, that is to say, that a transition
 * belongs to the set if all intervening LTSs allow the action.
 */
public class TransitionSet<State, Action> implements Cloneable {
  ...
  public TransitionSet(List<LTS<State, Action>> ltss, Alphabet<State, Action> alphabet) {
    this.ltss = ltss;
    if (alphabet != null) init(alphabet);
  }

  /**
   * Initialization for a Transition Set, marks as enabled for each LTS all the actions that do not
  ...
  private void init(Alphabet<State, Action> alphabet) {
    ...
    for (int i = 0; i < this.ltss.size(); ++i) enabledByLTS.add(new BitSet(alphabet.size()));
    for (HAction<State, Action> hAction : alphabet) {
      ...
      if (count == this.ltss.size()) enabledActions.add(hAction);
    }
  }

  /**
   * Adds an action enabled by the given LTS. Returns true if the action was not enabled and it
   * became enabled as a consequence of this addition.
   */
  public boolean add(int lts, HAction<State, Action> action) {
    ...
    if (result) enabledActions.add(action);
    return result;
  }

  /**
   * Removes an action enabled by the given LTS. Returns true if the action was enabled and it
   * became disabled as a consequence of this removal.
   */
  public boolean remove(int lts, HAction<State, Action> action) {
    ...
    if (result) enabledActions.remove(action);
    return result;
  }
  ...
  /**
   * Returns whether an action belongs to this Transition Set. That is, if all intervening LTSs
   * allow the action.
   */
  public boolean contains(HAction<State, Action> action) {
    return enabledCount[action.hash /*hashCode()*/]
        == this.ltss.size(); // replaced during aggressive inlining
  }
  ...
  /**
   * Overrides the internal representation of this Transition Set to reflect that of another set.
   */
