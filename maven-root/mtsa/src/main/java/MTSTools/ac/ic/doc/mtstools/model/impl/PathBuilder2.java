...
/**
 * PathBuilder2 calculates all required or possible paths in a MTS. It only calculates loop free
 * paths, and it does not assume any implicit self-loop transition via silent actions. The advantage
 * of this class over the original PathBuilder is that it calculates all the paths once and caches
 * those values. The other two differences are that PathBuilder returns path that might contain a
 * loop (the last transition ends in an intermediate node), and always assumes a self-loop
 * transition by the silent actions.
 *
 * @author Dario
 * @param <S> States type
 * @param <A> Labels type
...
public class PathBuilder2<S, A> {
  ...
  public PathBuilder2(
      final MTS<S, A> mts, final TransitionType transitionType, final Set<A> silentActions) {
    ...
    for (A label : noSilent) {
      for (Map.Entry<S, Collection<List<Pair<A, S>>>> entry : tauPaths.entrySet()) {
        Collection<List<Pair<A, S>>> extendedPaths =
            extenedPath(mts, transitionType, entry.getValue(), label);
  ...
  private Collection<List<Pair<A, S>>> extenedPath(
      MTS<S, A> mts, TransitionType transitionType, Collection<List<Pair<A, S>>> paths, A label) {
    ...
    for (List<Pair<A, S>> path : paths) {
      ...
      Collection<S> endStates =
          mts.getTransitions(mn, transitionType)
              .getImage(
                  label); // collection of nodes reachable via label from the last state of the path
      for (S s0 : endStates) {
  ...
  private Map<S, Collection<List<Pair<A, S>>>> getTauPaths(
      MTS<S, A> mts, TransitionType transitionType, Set<A> silentActions) {
  ...
  private void addPath(Map<S, Collection<List<Pair<A, S>>>> result, List<Pair<A, S>> path) {
