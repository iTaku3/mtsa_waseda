...
public class MTSImpl<State, Action> extends AbstractTransitionSystem<State, Action>
    implements MTS<State, Action> {
  ...
  public boolean addRequired(State from, Action label, State to) {
    ...
    this.getTransitionsForInternalUpdate(from, TransitionType.POSSIBLE).addPair(label, to);
    boolean added =
        this.getTransitionsForInternalUpdate(from, TransitionType.REQUIRED).addPair(label, to);
    if (added) {
  ...
  protected EnumMap<TransitionType, Map<State, BinaryRelation<Action, State>>>
      getTransitionsByType() {
    if (this.transitionsByType == null) {
      this.setTransitionsByType(
          new EnumMap<TransitionType, Map<State, BinaryRelation<Action, State>>>(
  ...
  protected void setTransitionsByType(
      EnumMap<TransitionType, Map<State, BinaryRelation<Action, State>>> transitionsByType) {
    this.transitionsByType = transitionsByType;
  }

  private void validateExistingTransition(
      State from, Action label, State to, TransitionType possible) {
    Validate.isTrue(getTransitions(from, possible).contains(Pair.create(label, to)));
  }

  /**
   * Elimina la transicion si existe y ademas elimina los posibles estados que quedaran huerfanos
   * luego de borrar la transicion.
   */
  ...
  public boolean removePossible(State from, Action label, State to) {
    boolean removed =
        this.getTransitionsForInternalUpdate(from, TransitionType.POSSIBLE).removePair(label, to);
    if (removed) {
      this.getTransitionsForInternalUpdate(from, TransitionType.MAYBE).removePair(label, to);
  ...
  public String toString() {
    ...
    sb.append("Required Transitions: ")
        .append(this.getTransitions(TransitionType.REQUIRED))
        .append("\r\n");
    sb.append("Maybe Transitions: ")
        .append(this.getTransitions(TransitionType.MAYBE))
        .append("\r\n");
    return sb.toString();
  }
  ...
  public boolean equals(Object o) {
    ...
    boolean maybeEquals =
        !(getTransitions(TransitionType.MAYBE) != null
            ? !getTransitions(TransitionType.MAYBE).equals(mts.getTransitions(TransitionType.MAYBE))
            : mts.getTransitions(TransitionType.MAYBE) != null);

    boolean requiredEquals =
        !(getTransitions(TransitionType.REQUIRED) != null
            ? !getTransitions(TransitionType.REQUIRED)
                .equals(mts.getTransitions(TransitionType.REQUIRED))
            : mts.getTransitions(TransitionType.REQUIRED) != null);
  ...
  public int hashCode() {
    ...
    result =
        31 * result
            + (getTransitions(TransitionType.MAYBE) != null
                ? getTransitions(TransitionType.MAYBE).hashCode()
                : 0)
            + (getTransitions(TransitionType.REQUIRED) != null
                ? getTransitions(TransitionType.REQUIRED).hashCode()
                : 0);
    return result;
  }
