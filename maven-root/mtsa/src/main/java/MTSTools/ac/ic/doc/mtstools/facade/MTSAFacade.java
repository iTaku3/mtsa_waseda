...
public class MTSAFacade {
  ...
  public static <State, Action> void deleteTransitionsToDeadlock(
    ...
    MTSDeadLockManipulator<State, Action> deadlockManipulator =
        new MTSDeadLockManipulatorImpl<State, Action>();
    ...
    for (Long state : mts.getStates()) {
      ...
      for (Pair<String, Long> transition : mts.getTransitions(state, transitionType)) {
        ...
        result.addTransition(
            state, transition.getFirst(), transition.getSecond(), TransitionType.REQUIRED);
      }
    }
    return result;
  }
  ...
  /**
   * Returns deadlock status: 1 if every implementation have a deadlock state, 2 if every
   ...
   * @return
   */
  public static MTS<Long, String> getComponent(
      MTS<Long, String> mts, Set<String> componentAlphabet) {
   ...
   * @return
   */
  public static <State, Action> void applyClosure(
      MTS<State, Action> mts, Set<Action> silentActions) {
  ...
  public static <State, Action> void hide(
      MTS<State, Action> mts, Action internalAction, Set<Action> silentActions) {
