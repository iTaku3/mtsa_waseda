...
public class BFSAbstraction<State, Action> extends Abstraction<State, Action> {
  ...
  /**
   * This is a mock Abstraction, we only want to make the compostates usable But we dont want to
   * have a good heuristic, we want to order the recommedations as we see fit. Everything else the
   * other Abstractions have, we are omitting it here
   */
  ...
  public void eval(
    ...
    // this needs to set "recommendations", "recommendations", "recommendit" so the compostate can
    // work as usual
    if (!compostate.isEvaluated()) {
      ...
      for (HAction<State, Action> action : compostate.getTransitions()) {
        ...
        HEstimate<State, Action> estimate =
            new HEstimate<State, Action>(1, new HDist(priorityOfAction, 1));
        compostate.addRecommendation(action, estimate);
      }
      // compostate.rankRecommendations(); //we dont use the regular rankRecommendations because the
      // ranker is
      ...
      compostate.initRecommendations(); // initializing recommendit and recommendation, necesary for
      // compostates
    }
  }

  /**
   * Ranker that doesnt care if the action is controllable or not, it only returns lexicografical
   * order.
   */
  private class BFSRanker implements Comparator<Recommendation<State, Action>> {

    /**
     * Compares two recommendations for ranking purposes. Returns: -1 if r1 is smaller than r2; 0 if
     * r1 and r2 are equal; and 1 if r1 is greater than r2.
     */
    @Override
