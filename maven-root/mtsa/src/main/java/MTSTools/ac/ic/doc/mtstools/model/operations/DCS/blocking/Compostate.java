...
/**
 * This class represents a state in the parallel composition of the LTSs in the environment. These
 * states are used to build the fragment of the environment required to reach the goal on-the-fly.
 * The controller for the given problem can then be extracted directly from the partial construction
 * achieved by using these states.
 */
public class Compostate<State, Action> implements Comparable<Compostate<State, Action>> {
  private final DirectedControllerSynthesisBlocking<State, Action>
      directedControllerSynthesisBlocking;
  ...
  public Compostate(
      DirectedControllerSynthesisBlocking<State, Action> directedControllerSynthesisBlocking,
      List<State> states) {
    ...
    this.inOpen = false;
    this.controlled =
        true; // we assume the state is controlled until an uncontrollable recommendation is
    // obtained
    ...
    for (Map.Entry<Integer, Integer> entry :
        directedControllerSynthesisBlocking.guarantees.entrySet()) {
      int gNumber = entry.getKey();
      int gIndex = entry.getValue();

      if (directedControllerSynthesisBlocking
          .defaultTargets
          .get(gIndex)
          .contains(states.get(gIndex))) {
    ...
    for (Map.Entry<Integer, Integer> entry :
        directedControllerSynthesisBlocking.assumptions.entrySet()) {
  ...
  /**
   * Indicates whether this state has been evaluated, that is, if it has a valid ranking of
   * recommendations.
   */
  ...
  /**
   * Returns the target states to be reached from this state as a list of sets, which at the i-th
   * position holds the set of target states of the i-th LTS.
   */
  ...
  /**
   * Sets the given set as target states for this state (creates aliasing with the argument set).
   */
  ...
  public void setStatus(Status status) {
    //            logger.fine(this.toString() + " status was: " + this.status + " now is: " +
    // status);
    if (this.status != Status.ERROR || status == Status.ERROR) this.status = status;
  }
  ...
  /**
   * Adds a new recommendation to this state and returns whether an error action has been introduced
   * (no other recommendations should be added after an error is detected). Recommendations should
   * not be added after they have been sorted and with an iterator in use.
   */
  public boolean addRecommendation(HAction<Action> action, HEstimate estimate) {
    ...
    if (controlled) { // may not work with lists of recommendations
      if (uncontrollableAction) controlled = false;
    }
  ...
  public boolean hasValidRecommendation() {
    return /*isEvaluated()*/ recommendations != null /**/
        && recommendation != null; // replaced during aggressive inlining
  }
  ...
  public boolean hasValidUncontrollableRecommendation() {
    return /*hasValidRecommendation()*/ recommendation != null /**/
        && !recommendation.getAction().isControllable(); // replaced during aggressive inlining
  }

  /** Returns whether the iterator points to a valid controllable recommendation. */
  public boolean hasValidControllableRecommendation() { // strongly relies on recommendations being
    // uncontrollable first and controllable last
    return recommendation != null
        && recommendations.get(recommendations.size() - 1).getAction().isControllable();
  }
  ...
  private void updateRecommendation() {
    if (recommendit.hasNext()) {
      ...
      estimate =
          recommendation.getEstimate(); // update this state estimate in case the state is reopened
    } else {
      recommendation = null;
    }
  }
  ...
  private Set<HAction<Action>>
      buildTransitions() { // Note: here I can code the wia and ia behavior for non-deterministic
    // ltss
    Set<HAction<Action>> result = new HashSet<>();
    if (directedControllerSynthesisBlocking.facilitators == null) {
      for (int i = 0; i < states.size(); ++i) {
        for (Pair<Action, State> transition :
            directedControllerSynthesisBlocking.ltss.get(i).getTransitions(states.get(i))) {
          HAction<Action> action =
              directedControllerSynthesisBlocking.alphabet.getHAction(transition.getFirst());
        ...
        if (!directedControllerSynthesisBlocking.facilitators.get(i).equals(states.get(i)))
          for (Pair<Action, State> transition :
              directedControllerSynthesisBlocking
                  .ltss
                  .get(i)
                  .getTransitions(directedControllerSynthesisBlocking.facilitators.get(i))) {
            HAction<Action> action =
                directedControllerSynthesisBlocking.alphabet.getHAction(transition.getFirst());
            directedControllerSynthesisBlocking.allowed.remove(
                i, action); // remove old non-shared facilitators transitions
          }
      for (int i = 0; i < states.size(); ++i)
        if (!directedControllerSynthesisBlocking.facilitators.get(i).equals(states.get(i)))
          for (Pair<Action, State> transition :
            ...
            HAction<Action> action =
                directedControllerSynthesisBlocking.alphabet.getHAction(transition.getFirst());
            directedControllerSynthesisBlocking.allowed.add(
                i, action); // add new non-shared facilitators transitions
          }
    }
    ...
    // this removes mixed compostates, if there are uncontrollable transitions we ignore
    // controllable ones
    boolean hasU = false;
  ...
  public int getChildDistance(HAction<Action> action) {
    int result =
        DirectedControllerSynthesisBlocking.UNDEF; // exploredChildren should never be empty or null
    for (Compostate<State, Action> compostate :
        exploredChildren.getImage(action)) { // Note: maximum of non-deterministic exploredChildren
      if (result < compostate.getDistance()) result = compostate.getDistance();
    }
    return result;
  }
  ...
  public int compareTo(Compostate o) {
    ...
    if (result == 0) result = this.depth - o.depth;
    return result;
  }
