...
public class HierarchicalMap<K, V> extends AbstractMap<K, V> implements Serializable {
  ...
  /**
   * Establishes if a value is contained in the local map, or if it not, checks if the value is in
   * the parent map related with an inherited key.
   *
    ...
    if (this.getLocalMap().containsValue(value)) {
      ...
      if (this.getParentOrEmptyMap().containsValue(value)) {
        ...
        while (parentEntryIt.hasNext()) {
          ...
          if (((value != null && value.equals(parentEntry.getValue()))
                  || (value == null && parentEntry.getValue() == null))
  ...
  /**
   * Returns a <b>unmodifiable </b> set view of the mappings contained in this map. Each element in
   * this set is a Map.Entry.
   *
  ...
  /**
   * Establishes if a Hierarchical Map is equals to the received object based on the comparisson of
   * both the Local Map and the Parent Map. Both Local and Parents Maps must be equal for two
   * Hierarchical Maps to be equals, for example, two Hierarchical Maps can have equal entrySets yet
   * some particular values of one of them can be inherited while not being the case in the other.
   */
  @Override
  ...
  /**
   * Retrieves a map entry for a given key from the local map first and if not found from the parent
   * map.
   *
  ...
  /**
   * Associates the specified value with the specified key in the local map . If the hierarchy
  ...
  /**
   * Copies all of the mappings from the specified map to the local map. These mappings will replace
   * any mappings that this map had for any of the keys currently in the local map.
   *
   ...
   * Sets the localMap
   *
   * @param localMap Map with the localMap
   */
   ...
   * Sets the parent
   *
   * @param parent Map with the parent
   */
