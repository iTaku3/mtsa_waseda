...
/**
 * Built from an MTS mts. The predicate returns true when evaluated with a state that has non
...
public class HasNonDeterministicTransitionsStatePredicate<S, A> implements Predicate<S> {
  ...
  public HasNonDeterministicTransitionsStatePredicate(
      MTS<S, A> mts, TransitionType transitionType) {
    this.mts = mts;
