...
public class InitMap<K, V> extends HashMap<K, V> implements Map<K, V> {
  ...
  /**
   * Returns the value associated to the given key (initializes the value with the factory if not
   * set).
   */
  @Override
  public V get(Object key) {
    V result = super.get(key);
    if (result == null && key != null) put((K) key, result = factory.newInstance());
    return result;
  }
