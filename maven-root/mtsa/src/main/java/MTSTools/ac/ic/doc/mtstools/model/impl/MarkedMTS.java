...
public class MarkedMTS<S, A> extends MTSImpl<S, A> {
  ...
  public MarkedMTS(MTS<S, A> mts, S initialState, Set<Set<S>> terminalSet) {
    ...
    trasitionsByType =
        new EnumMap<TransitionType, Map<S, BinaryRelation<A, S>>>(TransitionType.class);
    for (TransitionType type : TransitionType.values()) {
      Map<S, BinaryRelation<A, S>> transitions =
          new HashMap<S, BinaryRelation<A, S>>(mts.getTransitions(type));
   ...
   * @param currentState this state is used to determine with which of the terminal sets to work
   *     with (if there is more than one)
   * @return
   */
  ...
  private Set<S> selectMarkedStates(S currentState) {
    ...
    if (currentTerminalSet == null) {
      ...
      for (Iterator<Pair<A, S>> it = transitions.iterator();
          it.hasNext() && currentTerminalSet == null; ) {
        Pair<A, S> transition = it.next();
        // TODO: Problematic situation_
        // currentState has 2 transitions. With one it goes to one terminal set, with the other, to
        // another one.
