...
public class DirectedControllerSynthesisPartialOrderReduction<State, Action> {
  ...
  // For each j: if j != isomorphismGroup[j] = i: f_a[j] contains a biyective function that shows
  // that E_i and E_j are isomorphic
  ...
  private class IsomorphismComputator {
    ...
    // In case they are it computes the renaming function for the actions and renames the states of
    // E_j to satisfy that equivalent states are represented with the same number in both components
    private boolean areIsomorphic(int i, int j) {
      if (!satisfyEasyToCheckNecessaryConditionsForIsomorphism(i, j)) return false;
      Set<Pair<Action, Action>> haveSameEffectInOtherComponents =
          computePairsOfActionsThathaveSameEffectInOtherComponents(i, j);
      ...
      Set<Action> left = new TreeSet<>(E_i.getActions());
      left.removeAll(E_j.getActions());
      Set<Action> right = new TreeSet<>(E_j.getActions());
      right.removeAll(E_i.getActions());
      return aBiyectiveFunctionForActionsExist(
          haveSameEffectInOtherComponents, E_i, E_j, i, j, left, right);
    }

    private boolean aBiyectiveFunctionForActionsExist(
        Set<Pair<Action, Action>> haveSameEffectInOtherComponents,
        MarkedWithIllegalLTSImpl<Long, Action> E_i,
        MarkedWithIllegalLTSImpl<Long, Action> E_j,
        int i,
        int j,
        Set<Action> left,
        Set<Action> right) {
      ...
      for (Action a2 : possible) {
        ...
        if (aBiyectiveFunctionForActionsExist(
            haveSameEffectInOtherComponents, E_i, E_j, i, j, left, right)) return true;
        f_a.get(j).remove(a1);
        right.add(a2);
      }
      left.add(a1);
    ...
    private void renameStates(
        MarkedWithIllegalLTSImpl<Long, Action> E, int j, Map<Long, Long> f_s) {
      Long initial = f_s.get(E.getInitialState());
      MarkedWithIllegalLTSImpl<Long, Action> Ep =
          new MarkedWithIllegalLTSImpl<Long, Action>(initial);
    ...
    private boolean areEquivalent(
      ...
      for (Pair<Action, Long> step : trans_ei) {
        Action l = step.getFirst();
        Long ep = step.getSecond();
        Action fl = f_a.get(j).get(l);
        if (fl == null) fl = l;
        if (trans_ej.getImage(fl).isEmpty()) return false;
        if (!areEquivalent(
            E_i, ep, E_j, trans_ej.getImage(fl).iterator().next(), f_s, f_s_inverse, j))
          return false;
      }
      return true;
    }

    private Set<Pair<Action, Action>> computePairsOfActionsThathaveSameEffectInOtherComponents(
        int i, int j) {
    ...
    private void removePairWithDifferentEffect(
        Set<Pair<Action, Action>> sameEffect, MarkedWithIllegalLTSImpl<Long, Action> lts) {
      for (Action a1 : alphabet)
        for (Action a2 : alphabet) {
  ...
  private class CompositionalState {
    ...
    public boolean enabled(Action l) {
      for (int i = 0; i < ltss.size(); i++) {
        ...
        Long ei = state.get(i);
        if (lts.getActions().contains(l) && lts.getTransitions(ei).getImage(l).isEmpty())
          return false;
      }
      return true;
    }
    ...
    public CompositionalState destination(Action l) {
      ...
      for (int i = 0; i < ltss.size(); i++) {
        MarkedWithIllegalLTSImpl<Long, Action> lts = ltss.get(i);
        Long ei = state.get(i);
        if (!lts.getTransitions(ei).getImage(l).isEmpty())
          ep.state.set(i, lts.getTransitions(ei).getImage(l).iterator().next());
      }
      return ep;
    }
  ...
  public LTS<Long, Action> synthesize(
      List<MarkedWithIllegalLTSImpl<Long, Action>> inputLTSs, Set<Action> inputControllable) {
    ltss = inputLTSs;
    ...
    DirectedControllerSynthesisMonolithicDirector<Long, Action> dcs =
        new DirectedControllerSynthesisMonolithicDirector<Long, Action>();
  ...
  private void expand(LTS<Long, Action> compactDirector) {
    ...
    Set<CompositionalState> processing = new HashSet<>();
    processing.add(compositionInitial());
    Map<List<Long>, Long> id = new HashMap<>();
    id.put(compositionInitial().state, 0L);
    Long states = 1L;
    while (!processing.isEmpty()) {
      ...
      for (Action l : alphabet) {
        if (e.enabled(l)) {
          ...
          if (!compactDirector
              .getTransitions(compactId.get(e.representator().state))
              .getImage(fl)
              .isEmpty()) {
  ...
  private Action rename(CompositionalState _e, Action l) {
    ...
    for (int i = 0; i < e.state.size(); i++) {
      ...
      for (int j = i + 1; j < e.state.size(); j++) {
        if (isomorphismGroup.get(i).equals(isomorphismGroup.get(j))
            && e.state.get(j) < e.state.get(minpos)) minpos = j;
      }
  ...
  private MarkedWithIllegalLTSImpl<Long, Action> partialOrderReductionParallelComposition() {
    ...
    Set<CompositionalState> processing = new HashSet<>();
    processing.add(compositionInitial());
    compactId = new HashMap<>();
    compactId.put(compositionInitial().state, 0L);
    Long states = 1L;
  ...
  private boolean isControllable() {
    ...
    while (!queue.isEmpty()) {
      ...
      for (Action l : alphabet) {
        if (!controllables.contains(l)
            && e2.enabled(l)
            && director.getTransitions(e1).getImage(l).isEmpty()) return false;
        if (!director.getTransitions(e1).getImage(l).isEmpty()) {
          assertTrue(
              "Director has a transition that does not exist in the parallel composition",
              e2.enabled(l));
  ...
  private MarkedWithIllegalLTSImpl<Long, Action> computeReversedLTS(
      MarkedWithIllegalLTSImpl<Long, Action> lts) {
    MarkedWithIllegalLTSImpl<Long, Action> reverse =
