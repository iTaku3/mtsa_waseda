 ...
 */
public class DirectedControllerSynthesisNonBlocking<State, Action>
  ...
  // TrainedAgent
  // PreMarking
  // GeneralizedPreMarking
  
  public static HeuristicMode mode = HeuristicMode.PreMarking;
  ...
  /**
   * Indicates whether the procedure consider a non-blocking requirement (by default we consider a
   * stronger goal).
   */
  ...
  public LTS<Long, Action> synthesize(
    ...
    if (mode == HeuristicMode.Dummy) return (LTS<Long, Action>) ltss.get(0);
    ...
    assertTrue(
        "Finished because there was nothing left to explore, shouldn't be the case", isFinished());
  ...
  void setupHeuristic() {
    if (mode == HeuristicMode.TrainedAgent) {
      ...
      // 'CompleteFrontierExplorationHeuristic' no usa la Open Queue, le deja a la heuristica
      // manejar la cola
   ...
   */
  private Compostate<State, Action> buildCompostate(
      List<State> states, Compostate<State, Action> parent) {
  ...
  private Compostate<State, Action> buildInitialState() {
    ...
    Compostate<State, Action> result =
        buildCompostate(
            states,
            null); // for non-deterministic LTS I need the tau-closure as a set of initial states
    result.setDepth(0);
    return result;
  }
  ...
  /**
   * Opens a given child state following an action selected by the heuristic. This method
   * distinguishes between reachability and liveness. If the child to "open" is a goal (or closes a
   * loop over a marked state) it propagates the goal through the ancestors. If the child is an
   * error (or closes a loop over non-marked states) it propagates an error.
   */
  private void open(
    ...
    // System.err.printLn("opening from " + parent.toString() + " using " + action.toString() + " to
    // " + child.toString());
    ...
    if (isError(child) || child.heuristicStronglySuggestsIsError) {
      ...
      // only if we discovered a new Compostate for the loop or if the action was uncontrollable we
      // can find new goals.
      // if (probablyWinningStates.size()>0 && (!recommendation.action.isControllable() ||
      // !newLoopIsTheOldOne )){
  ...
  private void findNewGoals() {
      ...
      while (!reached_fixpoint) {
        ...
        while (it.hasNext()) {
          ...
          if (forcedByEnvironmentToLeave(state, c)) {
            ...
            for (Pair<HAction<State, Action>, Compostate<State, Action>> ancestorActionAndState :
                state.getParents()) {
    ...
    for (int i = 0; i < auxiliarListStates.size(); ++i) {
      ...
      for (Pair<HAction<State, Action>, Compostate<State, Action>> predecesor :
          state.getParents()) {
    ...
    for (Compostate<State, Action> state :
        mccc) { // fixme en realidad "performe" a ver si se puede hacer mÃ¡s performante
      for (Pair<HAction<State, Action>, Compostate<State, Action>> transition :
    ...
    // we start from marked states and check parents until all states in mccc have distance and
    // bestChild for the director
  ...
  private void updateDistances(
      Set<Compostate<State, Action>> alreadyChecked,
      Set<Compostate<State, Action>> goalsToUpdate,
      int amountToUpdate) {
    int allChecked = 0;
    if (alreadyChecked.isEmpty()) {
      for (Compostate<State, Action> s : goalsToUpdate) {
        assertEquals(
            "in updateDistances we have a set distance but we think we dont",
            (int) s.getBestControllable().getFirst(),
            -1);
        if (s.hasGoalChild()) {
          for (Pair<HAction<State, Action>, Compostate<State, Action>> childPair :
              s.getExploredChildren()) {
      ...
      for (Compostate<State, Action> s : alreadyChecked) {
        for (Pair<HAction<State, Action>, Compostate<State, Action>> p : s.getParents()) {
          ...
          if (goalsToUpdate.contains(parent) && parent.getBestControllable().getFirst() == -1) {
            // as explained in huang-kumar2008, if parent has controllable way of reaching
            // alreadyChecked, that is the bestChild
            // otherwise any controllable leading to Goal is valid
            toCheck.add(parent);
            if (action.isControllable()
                && (parent.getBestControllable().getFirst() == -1
                    || parent.getBestControllable().getFirst()
                        > s.getBestControllable().getFirst())) {
              parent.setBestControllable(s.getBestControllable().getFirst() + 1, s);
            } else {
              // el probl es que distancia le ponemos aca? porque si le ponemos una controlable
              // random, no puede haber probl despues?
  ...
  private void propagateGoal(
    ...
    while (previous_size != c.size()) {
      ...
      Set<Compostate<State, Action>> targetsAncestors =
          gatherWinningTargetsAncestorsForPropagateGoal(c);

      // sacar los que son forzados a irse o no llegan a targets
      c.removeIf(
          state -> forcedByEnvironmentToLeave(state, c) || !targetsAncestors.contains(state));
    }
    ...
    // newGoals already have their distances, we start from them and update their ancestors
    // distances
    if (!c.isEmpty()) {
  ...
  private void findNewErrors() {
    ...
    for (Compostate<State, Action> state : loop) {
      ...
      for (Pair<HAction<State, Action>, Compostate<State, Action>> transition :
          state.getExploredChildren()) { // hasChildNoneNotInLoop
        Compostate<State, Action> child = transition.getSecond();
  ...
  private void propagateError(
    ...
    while (previous_size != winners.size()) {
      ...
      Set<Compostate<State, Action>> targetsAncestors =
          gatherWinningTargetsForPropagateError(ancestors);
  ...
  private Set<Compostate<State, Action>> ancestorsUpToGoalOrError(
      Set<Compostate<State, Action>> states) {
  ...
  // fixme esta funcion y gatherWinningTargetsAncestorsForPropagateGoal son muy parecidas, se
  // podrÃ­an unir
  private Set<Compostate<State, Action>> gatherWinningTargetsForPropagateError(
  ...
  private Set<Compostate<State, Action>> gatherWinningTargetsAncestorsForPropagateGoal(
      Set<Compostate<State, Action>> c) {
    Set<Compostate<State, Action>> targets = new HashSet<>();
    for (Compostate<State, Action> state : c) {
  ...
  private Set<Compostate<State, Action>> gatherTargetAncestors(
      Set<Compostate<State, Action>> c, Set<Compostate<State, Action>> targets) {
    ...
    for (int i = 0; i < auxiliarListStates.size(); ++i) {
      ...
      for (Pair<HAction<State, Action>, Compostate<State, Action>> predecesorActionAndState :
          state.getParents()) {
        Compostate<State, Action> parent = predecesorActionAndState.getSecond();

        if (!isNone(parent) || !c.contains(parent)) continue;

        parent.setPotentiallyGoodTransition(
            predecesorActionAndState.getFirst()); // fixme no hay que chequear nada?
  ...
  private boolean forcedByEnvironmentToLeave(
      Compostate<State, Action> state, Set<Compostate<State, Action>> targets) {
  ...
  private boolean closingALoop(Compostate<State, Action> parent, Compostate<State, Action> child) {
    if (child
        .wasExpanded()) { // if the child has already been considered then we might be closing a
      // loop
  ...
  private void setGoal(Compostate<State, Action> state, HAction<State, Action> action) {
    ...
    if (distance < state.getDistance()) state.setDistance(distance);
    if (!wasGoal) setFinal(state);
  }
  ...
  /**
   * After the synthesis procedure this method builds a director in the form of an LTS by starting
  ...
  private LTS<Long, Action> buildDirector() {
    ...
    while (!descendants.isEmpty()) {
      ...
      for (Pair<HAction<State, Action>, Compostate<State, Action>> transition :
          successor.getExploredChildren()) {
        // we could simply add all uncontrollable childs but we need the "action"
        Action action = transition.getFirst().getAction();
        Compostate<State, Action> child = transition.getSecond();
        assertTrue(
            "All Goal states should know their best distance to a marked state",
            successor.getBestControllable().getFirst() != -1);
        boolean toAdd = false;
        if (!transition.getFirst().isControllable()) {
          assertTrue(
              "there is an uncontrollable transition to non-Goal state in the controller",
              isGoal(child));
          toAdd = true;
        } else {
  ...
  private void assertGoodDirector(
      ...
      Set<Compostate<State, Action>> directStates,
      Set<Compostate<State, Action>> directMarked) {
    ...
    for (int i = 0; i < auxiliarListStates.size(); ++i) {
      ...
      for (Pair<HAction<State, Action>, Compostate<State, Action>> ancestorActionAndState :
        ...
        if (!directStates.contains(ancestor)) continue;
        markedAncestors.add(ancestor);
        if (visited.add(ancestor)) auxiliarListStates.add(ancestor);
      }
    }
    visited.clear();
    auxiliarListStates.clear();
    assertEquals(
        "Not all states in the director can be extended to reach a marked state",
        markedAncestors,
        directStates);
  }
