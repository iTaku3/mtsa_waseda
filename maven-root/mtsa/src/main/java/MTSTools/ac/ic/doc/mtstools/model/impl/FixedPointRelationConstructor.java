...
class FixedPointRelationConstructor implements RelationConstructor {
  ...
  public <S1, S2, A> BinaryRelation<S1, S2> getLargestRelation(MTS<S1, A> mts1, MTS<S2, A> mts2) {
    ...
    BinaryRelation<S1, S2> result =
        new UniversalBinaryRelation<S1, S2>(mts1.getStates(), mts2.getStates());

    int oldSize, size = result.size();
    do {
      oldSize = size;
      Predicate<Pair<S1, S2>> predicate =
          new SimulationPredicate<S1, S2, A>(mts1, mts2, result, sim);
  ...
  public <S1, S2, A> boolean isValidRelation(
      MTS<S1, A> mts1, MTS<S2, A> mts2, BinaryRelation<S1, S2> relation) {
    boolean wasInterrupted = false;
    Simulation<?> sim = this.getSimulation();
    Predicate<Pair<S1, S2>> predicate =
        new SimulationPredicate<S1, S2, A>(mts1, mts2, relation, sim);
    List<Future<Pair<S1, S2>>> futures = new ArrayList<Future<Pair<S1, S2>>>(relation.size());

    CompletionService<Pair<S1, S2>> service =
        new ExecutorCompletionService<Pair<S1, S2>>(ExecutorServiceFactory.getExecutorService());
  ...
  public <S1, S2> BinaryRelation<S1, S2> filter(
      BinaryRelation<S1, S2> originalRelation, Predicate<Pair<S1, S2>> predicate) {
    BinaryRelation<S1, S2> result = new MapSetBinaryRelation<S1, S2>();
    boolean wasInterrupted = false;

    List<Future<Pair<S1, S2>>> futures =
        new ArrayList<Future<Pair<S1, S2>>>(originalRelation.size());
    CompletionService<Pair<S1, S2>> service =
        new ExecutorCompletionService<Pair<S1, S2>>(ExecutorServiceFactory.getExecutorService());
