...
public class Statistics {
  ...
  private int
      expandedStates; // expanded is usually smaller than the iterations due to the reopening of

  // states, we could also report the number of iterations
  ...
  public long getFindNewGoalsCalls() {
    return findNewGoalsCalls;
  }
  ...
  public long getFindNewErrorsCalls() {
    return findNewErrorsCalls;
  }
  ...
  public long getPropagateGoalsCalls() {
    return propagateGoalsCalls;
  }
  ...
  public long getPropagateErrorsCalls() {
    return propagateErrorsCalls;
  }
  ...
  public String toString() {
    return "ExpandedStates: "
        ...
        + "\n"
        + "UsedStates: "
        + controllerUsedStates
        + "\n"
        + "ExpandedTransitions: "
        + expandedTransitions
        + "\n"
        + "UsedTransitions: "
        + controllerUsedTransitions
        + "\n"
        + "Elapsed in Synthesis: "
        + (ended - started)
        + " ms\n"
        + "findNewGoalsCalls: "
        + findNewGoalsCalls
  ...
  /**
   * Returns tab separated values with statistic data (expanded, evaluated, revisited, used,
   * elapsed).
   */
  public String toTSV() {
  ...
  private String formatMemory(long memory) {
    ...
    String result;
    if (memory > giga) result = String.format("%02.2f", memory / giga) + " GB";
    else if (memory > mega) result = String.format("%02.2f", memory / mega) + " MB";
    else if (memory > kilo) result = String.format("%02.2f", memory / kilo) + " KB";
    else result = memory + " Bs";
    return result;
  }
}
