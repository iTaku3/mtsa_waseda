...
public class TauRemoval {
  ...
  private static class IncompleteSet<T> extends AbstractSet<T> {
    ...
    public boolean equals(Object o) {
      return o != null && o instanceof IncompleteSet;
    }
    ;
  ...
  public static <State, Action> void apply(MTS<State, Action> mts, Action tau) {
    ...
    Map<State, BinaryRelation<Action, State>> transitions =
        mts.getTransitions(TransitionType.REQUIRED);
    ...
    for (State state : conducing) {
      for (Pair<Action, State> pair : walks.get(state))
        mts.addTransition(state, pair.getFirst(), pair.getSecond(), TransitionType.REQUIRED);
    }
  }

  public static <State, Action> Set<State> getConducing(MTS<State, Action> mts, Action tau) {
    ...
    for (State state : mts.getStates()) {
      ...
      if (relation.isEmpty()) {
        ...
        for (Pair<Action, State> pair : relation) {
  ...
  public static <State, Action> Map<State, BinaryRelation<Action, State>> walkReach(
      Set<State> conducing, MTS<State, Action> mts, Action tau) {
    ...
    for (State state : conducing) {
      ...
      for (State tauReachable : taus.get(state)) relation.addAll(steps.get(tauReachable));
      walks.put(state, relation);
    }
    return walks;
  }

  public static <State, Action> Map<State, Set<State>> tauReach(
      Set<State> conducing, MTS<State, Action> mts, Action tau) {
  ...
  public static <State, Action> Set<State> tauReachRecursive(
      Set<State> conducing,
      MTS<State, Action> mts,
      State state,
      Action tau,
      Map<State, Set<State>> taus) {
    ...
    if (result != sink && conducing.contains(state)) result.add(state);
    taus.put(state, result);
    return result;
  }

  public static <State, Action> void muAddition(MTS<State, Action> mts, Action mu, Action tau) {
    State s0 = mts.getInitialState();
    BinaryRelation<Action, State> relation = mts.getTransitions(s0, TransitionType.REQUIRED);
    Set<State> tauReachables = relation.getImage(tau);
    if (!tauReachables.isEmpty()) mts.addAction(mu);
    for (State tauReachble : tauReachables) mts.addRequired(s0, mu, tauReachble);
  }

  public static <State, Action> void muRemoval(MTS<State, Action> mts, Action mu) {
