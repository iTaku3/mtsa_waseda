...
/** This class consists exclusively of static methods that operate on or return MTS and LTS. */
public class MTSUtils<S, A> {

  /**
   * Returns true if <code>mts</code> has tau transitions TODO: REFACTOR encapsulate this in a
   * Predicate
   */
   ...
   *
   * <p>Returns true if <code>mts</code> has no outgoing transitions from state <code>stateFrom
   * </code> with <code>transitionType</code> type.
   */
  ...
  /** */
  public static <State, Action> void hide(
      MTS<State, Action> mts, Set<Action> silentActions, Action internalAction) {
  ...
  public static <State, Action> void removeSilentTransitions(
      MTS<State, Action> mts, Set<Action> toHide) {
  ...
  public static <State, Action> void removeSilentTransitions(
      MTS<State, Action> mts, Action toHide) {
    ...
    for (TransitionType type : TransitionType.values()) {
      ...
      for (S state : mts.getStates()) {
        for (Pair<A, S> transition : mts.getTransitions(state, type)) {
          result.addTransition(state, transition.getFirst(), transition.getSecond(), type);
        }
      }
    }
    return result;
  ...
  /**
   * TODO: remove from here and create a Predicate Returns true if the MTS has no transitions
   *
   * @param mts
   * @return
   */
  ...
  /**
   * For each state satisfying the predicate there is exactly one trace leading to such state in the
   * result. The trace is minimal in the sense that there are no repeated states (i.e. no loops).
   * Note that there could be other traces leading to the same states that are not part of the
   * result.
   *
   * @param mts
   ...
   * @return
   */
  public static <S, A> Set<MTSTrace<A, S>> getAlltracesToStatesSatisfyingPredicate(
      MTS<S, A> mts, Predicate<S> predicateOnState, TransitionType transitionType) {
    Set<S> visited = new HashSet<S>();
    return buildTracesToStateSatisfyingPredicateAux(
        mts.getInitialState(), mts, visited, predicateOnState, transitionType);
  }
   ...
   * @return
   */
  private static <S, A> Set<MTSTrace<A, S>> buildTracesToStateSatisfyingPredicateAux(
      S currentState,
      MTS<S, A> mts,
      Set<S> visited,
      Predicate<S> predicateOnState,
    ...
    Iterator<Pair<A, S>> transitionsIt =
        mts.getTransitions(currentState, transitionType).iterator();
    while (transitionsIt.hasNext()) {
      ...
      if (!visited.contains(nextState)) {
        ...
        MTSTransition<A, S> transitionFromCurrentStateToNextState =
            new MTSTransitionWithModalityInfo<A, S>(
                currentState, transition.getFirst(), nextState, isMaybeTransition);
        ...
        Set<MTSTrace<A, S>> tracesToStateSatisfyingPredicateFromNextState =
            buildTracesToStateSatisfyingPredicateAux(
                nextState, mts, visited, predicateOnState, transitionType);
        ...
        appendTransitionToTracesAndAddToResult(
            transitionFromCurrentStateToNextState,
            tracesToStateSatisfyingPredicateFromNextState,
            result);
      }
    }
    ...
    if (predicateOnState.evaluate(currentState)) {
      ...
      result.add(
          new LinkedListMTSTrace<
              A, S>()); // LinkedList implementation is used to optimise transition appending at the
      // first position of the trace
    }

    return result;
  }

  private static <S, A> void appendTransitionToTracesAndAddToResult(
      MTSTransition<A, S> transitionCurrentStateNextState,
      Set<MTSTrace<A, S>> tracesToStateSatisfyingPredicateFromNextState,
      Set<MTSTrace<A, S>> result) {

    for (MTSTrace<A, S> traceToStateSatisfyingPredicateFromNextState :
        tracesToStateSatisfyingPredicateFromNextState) {
