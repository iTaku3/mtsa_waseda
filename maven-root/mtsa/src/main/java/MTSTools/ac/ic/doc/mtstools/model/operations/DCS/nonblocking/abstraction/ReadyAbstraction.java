...
public class ReadyAbstraction<State, Action> extends Abstraction<State, Action> {
  ...
  /**
   * Maps for each state which other states can be reached. It also stores which actions lead from
   * source to destination and in how many local steps.
   */
  private Map<
          HState<State, Action>, Map<HState<State, Action>, Map<HAction<State, Action>, Integer>>>
      manyStepsReachableStates;

  /**
   * Subset of the manyStepsReachableState map, containing only which marked or "goal" states can be
   * reached from a given source. This map keeps aliasing/sharing with the other and it is used only
   * to speed up iteration while looking for marked states to reach.
   */
  private Map<
          HState<State, Action>, Map<HState<State, Action>, Map<HAction<State, Action>, Integer>>>
      markedOrGoalReachableStates;

  /**
   * Maps for each state which actions can be reached. It also stores which actions lead from source
   * to desired action and in how many local steps.
   */
  private Map<
          HState<State, Action>, Map<HAction<State, Action>, Map<HAction<State, Action>, Integer>>>
      manyStepsReachableActions;
  ...
  private static final Boolean CONSIDER_GOALS = true;

  /** Whether to attempt to consider the plant's "structure" when comparing Compostates. */
  private static final Boolean CONSIDER_STRUCTURE = true;
  ...
  public ReadyAbstraction(
    ...
    estimates =
        new InitMap<>(
            new Factory<HEstimate<State, Action>>() {
              @Override
              public HEstimate<State, Action> newInstance() {
                HEstimate<State, Action> result =
                    new HEstimate<State, Action>(ltss.size() + 1, HDist.chasm);
  ...
  public void evalForHeuristic(
      Compostate<State, Action> compostate,
  ...
  private void buildRA(Compostate<State, Action> compostate) {
    ...
    for (HAction<State, Action> t : vertices) {
      for (Integer lts : actionsToLTS.get(t)) {
        HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
        Map<HAction<State, Action>, Integer> actionsLeadingToTfromS =
            manyStepsReachableActions.get(s).get(t);
        if (actionsLeadingToTfromS != null) {
          for (HAction<State, Action> l : actionsLeadingToTfromS.keySet()) {
            if (!l.equals(t)
                && s.contains(l)
                && !s.isSelfLoop(
                    l)) { // we need an efficient s.contains(l) that returns false for self-loops
              edges.put(l, t);
            }
          }
        }
  ...
  private void evaluateRA(Compostate<State, Action> compostate) {
    for (int lts = 0; lts < this.ltss.size(); ++lts) {
      ...
      Map<HState<State, Action>, Map<HAction<State, Action>, Integer>>
          markedOrGoalReachableStatesFromSource = markedOrGoalReachableStates.get(s);
      for (Pair<Action, State> transitions : s.getTransitions()) {
        ...
        if (!(mt == -1
            || (mt == 1
                && goals.isEmpty()
                && knownMarked.isEmpty()))) { // already best, skip search
          if (s.state.equals(t)) // a self-loop
          continue;
          for (HState<State, Action> g :
              markedOrGoalReachableStatesFromSource.keySet()) { // search for best
            Integer mg;
        ...
        if (newlDist.compareTo(currentDist) < 0) {
          ...
          if (compostate
              .getTransitions()
              .contains(l)) { // register only the shortest distances for enabled events
            // debugLog("New shortest for lts=%d at comp=%s is d=%d using l=%s", lts, compostate,
            // dt, l);
    ...
    while (!fresh.isEmpty()) {
      ...
      for (HAction<State, Action> l : edges.getK(t)) {
        ...
        for (int lts = 0; lts < this.ltss.size(); ++lts) {
          if (readyInLTS.get(l).contains(lts)) continue;
          HDist tDist = estimates.get(t).get(lts);
          HDist lDist = estimates.get(l).get(lts);
          Integer dl = addDist(tDist.getSecond(), dtl);
          HDist newlDist =
              dl == DirectedControllerSynthesisNonBlocking.INF
                  ? HDist.chasm
                  : getHDist(tDist.getFirst(), dl);
          if (newlDist.compareTo(lDist) < 0) {
            // CASES 2/3 (again): there's a path in the RA, update with the shortest one
            estimates.get(l).set(lts, newlDist);
            // debugLog("CLOSURE: Updated estimate for lts=%d at comp=%s,using l1=%s--l2=%s %d =
            // (%d)+%d", lts, compostate, l, t, dl, dtl, tDist.getSecond());
            fresh.add(l);
            if (compostate.getTransitions().contains(l)) {
              // debugLog("CLOSURE: New shortest for lts=%d at comp=%s,using l1=%s--l2=%s %d =
              // (%d)+%d", lts, compostate, l, t, dl, dtl, tDist.getSecond());
              registerShort(lts, newlDist);
            }
          }
  ...
  private HDist getHDist(Integer m, Integer d) {
    ...
    if (result == null) mCache.put(d, result = new HDist(m, d));
    return result;
  }

  /** Returns the maximum distance between two actions from the current state of every LTS. */
  private Integer gap(
      Compostate<State, Action> compostate, HAction<State, Action> l, HAction<State, Action> t) {
    Integer result = gapCache.get(l).get(t);
    if (result != null) return result;
    result = 0;
    for (Integer lts : actionsToLTS.get(l)) {
      if (!actionsToLTS.get(t).contains(lts)) continue;
      HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
      if (s.contains(l)) {
        Map<HAction<State, Action>, Integer> actionFromSourceToTarget =
            manyStepsReachableActions.get(s).get(t);
  ...
  private Integer addDist(Integer d1, Integer d2) {
    return (d1 == DirectedControllerSynthesisNonBlocking.INF
            || d2 == DirectedControllerSynthesisNonBlocking.INF)
        ? DirectedControllerSynthesisNonBlocking.INF
        : d1 + d2;
  }
  ...
  private void registerShort(Integer lts, HDist dist) {
    ...
    if (dist.compareTo(shortDist) < 0) shortest.put(lts, dist);
  }
  ...
  private void reconcilateShort(Compostate<State, Action> compostate) {
    for (int lts = 0;
        lts < this.ltss.size();
        ++lts) { // this loops sets any missing shortest information
      HDist shortLts = shortest.get(lts);
      if (shortLts == HDist.chasm) {
        HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
        Map<HState<State, Action>, Map<HAction<State, Action>, Integer>>
            markedStatesReachableFroms = markedOrGoalReachableStates.get(s);
        for (Entry<HState<State, Action>, Map<HAction<State, Action>, Integer>> entry :
            markedStatesReachableFroms.entrySet()) {
          ...
          if (m < shortLts.getFirst()) {
            for (HAction<State, Action> a : entry.getValue().keySet()) {
              ...
              if (d < shortLts.getSecond()) {
                ...
                // debugLog("New shortest for lts=%d at comp=%s is d=%d using l=%s", lts,
                // compostate, d, a.getAction());
              }
            }
          }
        }
    ...
    for (HAction<State, Action> l :
        compostate.getTransitions()) { // this loops fills missing goals with shortest paths
      HEstimate<State, Action> el = estimates.get(l);
      for (int lts = 0; lts < this.ltss.size(); ++lts) {
        HState<State, Action> s = buildHState(lts, compostate.getStates().get(lts));
        if (readyInLTS.get(l).contains(lts) && !s.isSelfLoop(l)) continue;
        HDist lDist = el.get(lts);
        if (lDist == HDist.chasm) {
          if (s.marked && (!actionsToLTS.get(l).contains(lts) || s.isSelfLoop(l))) {
            // CASES 4/5
            // Current state is marked and 'l' is either not in the component's alphabet or a
            // self-loop,
            ...
            // debugLog("Current is marked for lts=%s and l=%s is either not in alphabet or a
            // self-loop: (%d, 1)", lts, l, m);
          } else {
            // CASES 6/7 if shortest.get(lts) is not 'chasm'. If it is 'chasm', we are covering CASE
            // 8
            // debugLog("Updating dist for lts=%s l=%s using 1 + shortest=%s", lts, l,
            // shortest.get(lts));
  ...
  private void extractRecommendations(Compostate<State, Action> compostate) {
    ...
    for (Pair<HAction<State, Action>, Compostate<State, Action>> transition :
        compostate.getExploredChildren()) {
    ...
    for (HAction<State, Action> action : compostate.getTransitions()) {
      ...
      if (compostate.addRecommendation(action, estimate)) break;
    }
  ...
  private void extractRecommendationsForHeuristic(
      Compostate<State, Action> compostate,
      FeatureBasedExplorationHeuristic<State, Action> heuristic) {
    for (HAction<State, Action> action : compostate.getTransitions()) {
      HEstimate<State, Action> estimate = estimates.get(action);
  ...
  private void computeActionsToLTS() {
    for (int i = 0; i < this.ltss.size(); ++i) {
      for (Action l : this.ltss.get(i).getActions()) {
        Set<Integer> set =
            actionsToLTS.computeIfAbsent(this.alphabet.getHAction(l), k -> new HashSet<>());
        set.add(i);
      }
    }
  }
  ...
  private void computeReachableStates() {
    Map<HState<State, Action>, Map<HState<State, Action>, Set<HAction<State, Action>>>>
        oneStepReachableStates = new InitMap<>(HashMap.class);
    Map<HState<State, Action>, Set<Pair<HAction<State, Action>, HState<State, Action>>>>
        lastStates = new InitMap<>(HashSet.class);
    Map<HState<State, Action>, Set<Pair<HAction<State, Action>, HState<State, Action>>>>
        nextStates = new InitMap<>(HashSet.class);
    ...
    for (int lts = 0;
        lts < this.ltss.size();
        ++lts) { // this loop populates one step reachable states
      for (State state : this.ltss.get(lts).getStates()) {
        ...
        Map<HState<State, Action>, Set<HAction<State, Action>>> oneReachableFromSoruce =
            oneStepReachableStates.get(source);
        for (Pair<Action, State> transition : this.ltss.get(lts).getTransitions(state)) {
          ...
          Set<HAction<State, Action>> oneStepToDestination =
              oneReachableFromSoruce.get(destination);
          Map<HAction<State, Action>, Integer> manyStepsToDestination =
              manyStepsReachableFromSource.get(destination);
    ...
    int i = 2;
    while (statesPopulated) { // this loop extends the reachable states in the transitive closure
      // (each iteration adds the states reachable in i steps).
      ...
      for (HState<State, Action> source : lastStates.keySet()) {
        ...
        for (Pair<HAction<State, Action>, HState<State, Action>> pair : lastStates.get(source)) {
          ...
          for (HState<State, Action> target : oneStepReachableStates.get(intermediate).keySet()) {
            Map<HAction<State, Action>, Integer> manyStepsReachableFromSourceToTarget =
                manyStepsReachableFromSource.get(target);
            if (manyStepsReachableFromSourceToTarget == null)
              manyStepsReachableFromSource.put(
                  target, manyStepsReachableFromSourceToTarget = new HashMap<>());
      ...
      for (Set<Pair<HAction<State, Action>, HState<State, Action>>> set : lastStates.values())
        set.clear();
      Map<HState<State, Action>, Set<Pair<HAction<State, Action>, HState<State, Action>>>> swap =
          lastStates;
  ...
  /**
   * Computes for each state in each LTS which other *marked* or *goal* states can reach and in how
   * many steps.
   */
  private void computeMarkedOrGoalReachableStates() {
    ...
    for (HState<State, Action> source : manyStepsReachableStates.keySet()) {
      Map<HState<State, Action>, Map<HAction<State, Action>, Integer>>
          markedOrGoalStatesFromSource = markedOrGoalReachableStates.get(source);
      Map<HState<State, Action>, Map<HAction<State, Action>, Integer>> reachableStatesFromSource =
      ...
      for (HState<State, Action> destination : reachableStatesFromSource.keySet()) {
        if (destination.marked
            || (!destination.state.equals(-1L)
                && this.goals.get(destination.lts).contains(destination.state)))
  ...
  private void computeReachableActions() {
    for (int lts = 0; lts < this.ltss.size(); ++lts) {
      for (State state :
          this.ltss
              .get(lts)
              .getStates()) { // this loop populates the reachable action with the LTSs' transition
        // relations (one step)
        HState<State, Action> source = buildHState(lts, state);
        Map<HAction<State, Action>, Map<HAction<State, Action>, Integer>>
            reachableActionsFromSource = manyStepsReachableActions.get(source);
        for (Pair<Action, State> transition : source.getTransitions()) {
          HAction<State, Action> label = this.alphabet.getHAction(transition.getFirst());
          Map<HAction<State, Action>, Integer> reachableActionsThroughLabel =
              reachableActionsFromSource.get(label);
      ...
      for (State state :
          this.ltss
              .get(lts)
              .getStates()) { // this loop extends the reachable actions with the outgoing events
        // from reachable states (many steps)
        HState<State, Action> source = buildHState(lts, state);
        Map<HState<State, Action>, Map<HAction<State, Action>, Integer>> statesReachableFromSource =
            manyStepsReachableStates.get(source);
        Map<HAction<State, Action>, Map<HAction<State, Action>, Integer>>
            actionsReachableFromSource = manyStepsReachableActions.get(source);
        for (HState<State, Action> destination : statesReachableFromSource.keySet()) {
          for (Entry<HAction<State, Action>, Integer> entry :
              statesReachableFromSource.get(destination).entrySet()) {
            ...
            for (Pair<Action, State> transition : destination.getTransitions()) {
              ...
              Map<HAction<State, Action>, Integer> actionsLeadingToTarget =
                  actionsReachableFromSource.get(target);
              if (actionsLeadingToTarget == null)
                actionsReachableFromSource.put(target, actionsLeadingToTarget = new HashMap<>());
              DirectedControllerSynthesisNonBlocking.putmin(
                  actionsLeadingToTarget,
                  actionLeadingToDestination,
                  distanceFromSourceToDestination + 1);
            }
          }
        }
      }
