...
public abstract class AbstractForwardSimulation<A> implements Simulation<A> {
  ...
  public AbstractForwardSimulation(
      MTS.TransitionType actualTransitionType, MTS.TransitionType simulatorTransitionType) {
    this.setActualTransitionType(actualTransitionType);
    this.setSimulatorTransitionType(simulatorTransitionType);
  }

  public <S1, S2> boolean simulate(
      MTS<S1, A> mts1, S1 s1, MTS<S2, A> mts2, S2 s2, Set<Pair<S1, S2>> relation) {
  ...
  protected abstract <S1, S2> boolean simulate(
