...
public class WeakAlphabetMergeBuilder implements MergeBuilder {
  ...
  public <S1, S2, A> MTS<Object, A> merge(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
    ...
    Set<A> alphabetAminusB =
        new HashSet<A>(CollectionUtils.subtract(mtsA.getActions(), mtsB.getActions()));
    Set<A> alphabetBminusA =
        new HashSet<A>(CollectionUtils.subtract(mtsB.getActions(), mtsA.getActions()));
    ...
    Set<Set<A>> actionsPartition =
        this.createActionsPartition(mtsA.getActions(), mtsB.getActions());
      ...
      while ((state = stateToProcess.poll()) != null
        ...
        for (Set<A> actions : actionsPartition) {
          ...
          for (A reqAction : actions) {
            reqTransitions
                .getImage(reqAction)
                .addAll(bestCR.getTransitions(state, REQUIRED).getImage(reqAction));
          }
          ...
          for (Set<Pair<A, Object>> coverSet : new PowerSet<Pair<A, Object>>(reqTransitions)) {
            ...
            if (refA.isARefinement(mtsA, cr) && refB.isARefinement(mtsB, cr)) {
  ...
  private <A> boolean updateMCRsSet(
      Collection<MTS<Object, A>> mcrs,
      MTS<Object, A> mcrCandidate,
      RefinementByRelation refCR,
      Object state) {
    boolean addCr = true;
  ...
  private <A> void setAsRequiredTransitions(
      MTS<Object, A> cr, Object sourceSate, Set<Pair<A, Object>> targetTransitions) {
  ...
  private <A> void setAsPossibleTransitions(
      MTS<Object, A> cr, Object sourceState, Set<Pair<A, Object>> targetTransitions) {
    for (Pair<A, Object> reqTransition : targetTransitions) {
   ...
   * @return
   */
  private <A> MTS<Object, A> mergeMCRs(
      Collection<MTS<Object, A>> mcrs,
      Object stateToClone,
      Set<A> actions,
    ...
    Map<Object, Collection<Object>> state2newStates =
        this.createNewStatesMapping(mcr0, mcrs.size(), stateToClone, newStates);
    ...
    for (TransitionType type : TransitionType.values()) {
      ...
      for (Object source : mcr0.getStates())
        for (Pair<A, Object> transition : mcr0.getTransitions(source, type)) {
          ...
          // if the transition is one of the transitions over which the abstraction is taking place
          // it is ignored.
          ...
          for (Object newSource : state2newStates.get(source))
            for (Object newTarget : state2newStates.get(target)) {
              result.addTransition(newSource, transitionAction, newTarget, type);
            }
        }
    }
  ...
  /**
   * Returns a map, which maps every mts' state to a singleton with itself as a unique element
   * except the stateToClone. The mapping for the stateToClone is a set of new object instance, the
   * cardinality of that set if amountNewState.
   *
   * @param mts
  ...
  private Map<Object, Collection<Object>> createNewStatesMapping(
      MTS<Object, ?> mts, int amountNewState, Object stateToClone, Collection<Object> newStates) {
    Map<Object, Collection<Object>> result =
        new HashMap<Object, Collection<Object>>((int) (mts.getStates().size() / 0.75 + 1));
