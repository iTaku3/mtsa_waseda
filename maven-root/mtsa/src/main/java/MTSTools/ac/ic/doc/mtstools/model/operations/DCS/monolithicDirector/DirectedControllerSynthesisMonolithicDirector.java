...
/**
 * This class implements the algorithm presented in Directed Control of Discrete Event Systems for
 * Safety and Nonblocking Jing Huang, Student Member, IEEE, and Ratnesh Kumar, Fellow, IEEE. Thoug
 * it is not made by the authors.*
 */
public class DirectedControllerSynthesisMonolithicDirector<State, Action> {
  ...
  private void fromSupervisorToDirector(
      Set<State> X_mono, Set<State> X_k, Set<State> X_kprev, Set<State> X_pico) {
  ...
  private void backwardsDFS(
      State current, Set<State> states, Set<State> visited, Set<State> currentSCC) {
    visited.add(current);
  ...
  private void forwardDFS(
      State current, Set<State> states, Set<State> visited, Stack<State> SCCorder) {
    visited.add(current);
    Stack<State> stack = new Stack<>();
    stack.add(current);
    while (!stack.empty()) {
  ...
  private void assertGoodDirector(LTSImpl<Long, Action> result) {
    ...
    for (Long s : result.getStates()) {
      ...
      for (Pair<Action, State> transPlant : plant.getTransitions((State) s)) {
        if (!controllables.contains(transPlant.getFirst())) {
          ...
          assertTrue(
              "we are not returning a legal controller, it prohibits uncontrollable transitions",
              resultHasTrans);
        }
      }
    }
    ...
    for (int i = 0; i < auxiliarListStates.size(); ++i) {
      ...
      for (Pair<Action, State> ancestorActionAndState : reversedPlant.getTransitions((State) s)) {
        ...
        if (result.getStates().contains(ancestor)
            && result
                .getTransitions(ancestor)
