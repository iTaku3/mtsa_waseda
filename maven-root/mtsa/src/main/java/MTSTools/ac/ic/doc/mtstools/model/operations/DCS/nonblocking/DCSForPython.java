...
public class DCSForPython {
  ...
  public DCSForPython(String inputStartingDirectory, String labels_path, boolean using_ra_feature) {
    ...
    if (Objects.equals(labels_path, "mock")) DCSWithFeatures.readLabels(labels_path);
    else DCSWithFeatures.readLabels(inputStartingDirectory + labels_path);
  }
  ...
  public void startSynthesis(String problem, int n, int k) {
    ...
    String filename =
        inputStartingDirectory + "fsp/" + problem + "/" + problem + "-" + n + "-" + k + ".fsp";
    ...
    DirectedControllerSynthesisNonBlocking<Long, String> dcs =
        TransitionSystemDispatcher.hcsInteractive(c.getFirst(), c.getSecond());
  ...
  public void expandAction(int idx) {
    Pair<Compostate<Long, String>, HAction<Long, String>> stateAction =
        heuristic.removeFromFrontier(idx);
  ...
  public static void main(String[] args) throws OrtException {
    ...
    env.setFeaturesBuffer(
        ByteBuffer.allocateDirect(env.getNumberOfFeatures() * 4 * 100000).asFloatBuffer());
