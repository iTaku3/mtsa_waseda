...
public class TraceInclusionClosure implements MTSClosure {
  ...
  private <State, Action> void closure(
      State state, Set<Action> silentActions, MTS<State, Action> mts) {
  ...
  private <State, Action> void addPossibleSuccessors(
      State state,
      State succ,
    ...
    for (Pair<Action, State> tr : mts.getTransitions(succ, trType)) {
      ...
      if (!mts.getTransitions(state, TransitionType.POSSIBLE)
          .getImage(tr.getFirst())
          .contains(tr.getSecond())) {
        mts.addPossible(state, tr.getFirst(), tr.getSecond());
      }
    }
  }

  private <State, Action> void addRequiredSuccessors(
      State state, State succ, Set<Action> silentActions, MTS<State, Action> mts) {
  ...
  private <Action, State> void addSilentSuccessorsToVisit(
      State state,
      Set<Action> silentActions,
      MTS<State, Action> mts,
      Queue<State> toVisitMay,
      TransitionType type) {
