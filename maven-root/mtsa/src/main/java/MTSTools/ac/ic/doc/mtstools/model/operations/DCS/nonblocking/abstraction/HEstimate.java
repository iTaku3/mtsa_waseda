...
public class HEstimate<State, Action> implements Comparable<HEstimate<State, Action>>, Cloneable {
  ...
  public HEstimate(int size, HDist init) {
    this(size);
    for (int i = 0; i < size; ++i) values.add(init);
  }
  ...
  public int compareTo(HEstimate<State, Action> o) {
    int result = 0;
    for (int i = 0; result == 0 && i < values.size(); ++i) result = get(i).compareTo(o.get(i));
    return result;
  }
  ...
  public boolean set(int i, HDist d) {
    boolean result;
    if (result = (get(i).compareTo(d) > 0)) values.set(i, d);
    return result;
  }
  ...
  public boolean equals(Object obj) {
    ...
    if (obj instanceof HEstimate) result = this.compareTo((HEstimate<State, Action>) obj) == 0;
    return result;
  }
