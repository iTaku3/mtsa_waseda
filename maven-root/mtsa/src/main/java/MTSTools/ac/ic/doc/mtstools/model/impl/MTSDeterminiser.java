...
public class MTSDeterminiser {
  ...
  public MTSDeterminiser(MTS<Long, String> toDeterminize, boolean forkSemantics) {
    ...
    oldToNewStateMap =
        new Hashtable<MultipleState, Integer>(this.toDeterminize.getStates().size() * 2);
    newStateCode = 0;
  ...
  public MTS<Long, String> determinize() {
    ...
    MTS<Long, String> deterministic =
        new MTSImpl<Long, String>(Long.valueOf(addState(new MultipleState(newStateCode))));
  ...
  protected void compute(int currentStateNumber, MTS<Long, String> determinisedMTS) {
    if (currentStateNumber != ERROR) {
      ...
      Map<String, Map<Integer, List<Pair<Long, TransitionType>>>> transitionsByAction =
          groupTransitionsByAction(currentState);

      for (Entry<String, Map<Integer, List<Pair<Long, TransitionType>>>> transitionByAction :
          transitionsByAction.entrySet()) {
        String label = transitionByAction.getKey();
        Map<Integer, List<Pair<Long, TransitionType>>> transitionsByState =
            transitionByAction.getValue();
        ...
        determinisedMTS.addTransition(
            Long.valueOf(currentStateNumber), label, Long.valueOf(nextStateNumber), type);
      }
    }
  }

  private boolean buildNextState(
      MultipleState currentState,
      MultipleState nextState,
      Map<Integer, List<Pair<Long, TransitionType>>> transitionsByState) {
    boolean isRequired = false;
    for (Entry<Integer, List<Pair<Long, TransitionType>>> subStateTransition :
        transitionsByState.entrySet()) {
      ...
      for (Pair<Long, TransitionType> transition : subStateTransition.getValue()) {
        ...
        if (this.toDeterminize.getTransitions((long) toState, TransitionType.POSSIBLE).size()
            <= 0) {
        ...
        isRequired |=
            (TransitionType.REQUIRED.equals(type) && !currentState.hasMaybeMark(actualSubState));
  ...
  private Map<String, Map<Integer, List<Pair<Long, TransitionType>>>> groupTransitionsByAction(
    ...
    Map<String, Map<Integer, List<Pair<Long, TransitionType>>>> transitionsByAction =
        new HashMap<String, Map<Integer, List<Pair<Long, TransitionType>>>>();
    for (Integer subState : state) {
      Map<String, List<Pair<Long, TransitionType>>> tr =
          mtsAux.groupByAction(Long.valueOf(subState));
      for (Entry<String, List<Pair<Long, TransitionType>>> transitionByAction : tr.entrySet()) {
        if (!transitionsByAction.containsKey(transitionByAction.getKey())) {
          Map<Integer, List<Pair<Long, TransitionType>>> transitionsByState =
              new HashMap<Integer, List<Pair<Long, TransitionType>>>();
          ...
          transitionsByAction
              .get(transitionByAction.getKey())
              .put(subState, transitionByAction.getValue());
        }
      }
    }
