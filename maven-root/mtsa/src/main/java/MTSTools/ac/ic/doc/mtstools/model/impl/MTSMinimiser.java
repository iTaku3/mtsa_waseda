...
public class MTSMinimiser<Action> {
  ...
  public MTS<Long, Action> minimise(MTS<Long, Action> mts) {
    ...
    Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>> simulationRelation =
        computeMaximalSimulationRelation(mts);
  ...
  private void initalizeMinimiser(MTS<Long, Action> mts) {
    ...
    for (Long state : mts.getStates()) {
      ...
      for (Pair<Action, Long> transition : mts.getTransitions(state, TransitionType.REQUIRED)) {
        actualSuccessors.addSuccessor(
            transition.getSecond(), transition.getFirst(), TransitionType.REQUIRED);
      }
      for (Pair<Action, Long> transition : mts.getTransitions(state, TransitionType.POSSIBLE)) {
        actualSuccessors.addSuccessor(
            transition.getSecond(), transition.getFirst(), TransitionType.POSSIBLE);
      }
    ...
    simulationRelation.add(
        new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
  ...
  private MTS<Long, Action> buildMTS(
      MTS<Long, Action> mts,
      Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>> simulationRelation) {
    MTS<Long, Action> result =
        new MTSImpl<Long, Action>(code(getEquivalenceClassOf(mts.getInitialState())));
  ...
  private Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>>
      computeMaximalSimulationRelation(MTS<Long, Action> mts) {
    boolean change = true;
    while (change) {
      change = false;
      Set<EquivalenceClass<Long>> iterationEquivalenceClasses =
          new HashSet<EquivalenceClass<Long>>();
      Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>> iterationSimulationRelation =
          new HashSet<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>>();
      // refine
  ...
  private boolean update(
      MTS<Long, Action> mts,
      Set<EquivalenceClass<Long>> iterationEquivalenceClasses,
      Collection<Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>>
        ...
        for (EquivalenceClass<Long> equivalenceClassB : iterationEquivalenceClasses) {
          if (relationItem.getFirst().hasAll(equivalenceClassA)
            ...
            if (fi.containsAll(buildPi(equivalenceClassA, mts))
              ...
              iterationSimulationRelation.add(
                  new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
  ...
  private boolean classBRefinesClassA(
      EquivalenceClass<Long> equivalenceClassA, EquivalenceClass<Long> equivalenceClassB) {
    return stateBRefinesStateA(
        equivalenceClassA.iterator().next(), equivalenceClassB.iterator().next());
  }
  ...
  private Set<EquivalenceClass<Long>> buildFi(
    ...
    for (EquivalenceClass<Long> clazz : piEquivalenceClass) {
      for (Pair<EquivalenceClass<Long>, EquivalenceClass<Long>> simulationItem :
          simulationRelation) {
  ...
  private EquivalenceClass<Long> computeNewEquivalenceClass(
      Long state, EquivalenceClass<Long> equivalenceClass) {
  ...
  private boolean stateBRefinesStateA(Long stateA, Long stateB) {
    ...
    for (Entry<Action, BitSet> successorFromAByAction :
        aSucessors.getRequiredSuccessorsByAction().entrySet()) {
      BitSet successorsFromA = successorFromAByAction.getValue();
      BitSet successorsFromB =
          bSucessors.getRequiredSuccessorsByAction().get(successorFromAByAction.getKey());
    ...
    for (Entry<Action, BitSet> successorFromBByAction :
        bSucessors.getPossibleSuccessorsByAction().entrySet()) {
      BitSet successorsFromB = successorFromBByAction.getValue();
      BitSet successorsFromA =
          aSucessors.getPossibleSuccessorsByAction().get(successorFromBByAction.getKey());
  ...
  /**
   * Para todo sucesor de A s1 tengo que tener un sucesor de B s2 tal que ([s1], [s2]) pertenece a H
   *
   * @param successorsFromA
   * @param successorsFromB
   */
  private boolean successorsOfBRefinesSuccessorsOfA(
      BitSet successorsFromA, BitSet successorsFromB) {
    if (successorsFromB == null) {
      return false;
    }
    for (int succA = successorsFromA.nextSetBit(0);
        succA >= 0;
        succA = successorsFromA.nextSetBit(succA + 1)) {
  ...
  private boolean hasRefinesSuccesor(BitSet successorsFromB, Integer succA) {
    for (int succB = successorsFromB.nextSetBit(0);
        succB >= 0;
        succB = successorsFromB.nextSetBit(succB + 1)) {
      // TODO ISSUE Desing, no quiero construir pares
      if (simulationRelation.contains(
          new Pair<EquivalenceClass<Long>, EquivalenceClass<Long>>(
  ...
  private Map<EquivalenceClass<Long>, Long> equivalenceClassToLong =
      new HashMap<EquivalenceClass<Long>, Long>();
