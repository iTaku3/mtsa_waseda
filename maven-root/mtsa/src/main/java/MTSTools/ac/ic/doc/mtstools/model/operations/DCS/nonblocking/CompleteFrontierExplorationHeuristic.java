...
/**
 * This class implements ExplorationHeuristic by completely relying on the underlying heuristic to
 * rank the states/transitions of the entire frontier. In contrast to OpenSetExplorationHeuristic,
 * it does not restrict the states of the frontier in any way.
 */
public class CompleteFrontierExplorationHeuristic<State, Action>
    implements ExplorationHeuristic<State, Action> {
  ...
  /**
   * If true, the estimates for a state are recomputed every time they may change: when a new marked
   * state or goal is found.
   */
  // Ver Tesis de Nico Pazos para más detalles, capítulo "Mejoras propuestas"
  private static final Boolean RECOMPUTE_ESTIMATES_ON_CHANGE = true;

  /**
   * seq (as in 'sequence') is a number that is incremented every time a marked state or a goal is
   * reached. When the estimates for a state are computed, the seq is stored in the state; if at
   * some point a state's seq is lower than the current seq, it means that the state's estimates are
   * outdated and need to be recomputed.
   */
  ...
  public CompleteFrontierExplorationHeuristic(
    ...
    switch (mode) {
      ...
      case Debugging:
        ...
        break;
      case PreMarking:
        abstraction = new PreMarkingDirection<>(dcs);
        compostateRanker = new PreMarkingDirection.CompostateRanker<>();
        break;
      case GeneralizedPreMarking:
        abstraction = new GeneralizedPreMarkingDirection<>(dcs);
        compostateRanker = new GeneralizedPreMarkingDirection.CompostateRanker<>();
        break;
    }
  ...
  private void recomputeEstimates() {
    ...
    if (!abstraction.shouldRecomputeEstimatesOnChange()) return;
  ...
  public void notifyExpandingState(
      Compostate<State, Action> parent,
      HAction<State, Action> action,
      Compostate<State, Action> state) {
    if (state.wasExpanded()) { // todo: understand this, i am copying the behavior of the code pre
      // refactor
      state.setTargets(parent.getTargets());
  ...
  public void notifyExpansionDidntFindAnything(
      Compostate<State, Action> parent,
      HAction<State, Action> action,
      Compostate<State, Action> child) {}
