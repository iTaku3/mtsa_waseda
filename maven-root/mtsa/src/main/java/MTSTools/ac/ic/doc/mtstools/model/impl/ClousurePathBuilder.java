...
public class ClousurePathBuilder {
  ...
  public static ClousurePathBuilder getInstance() {
    return singleton;
  }
  ...
  private static class ClousurePathIterator<S, A> implements Iterator<List<Pair<A, S>>> {
    ...
    public ClousurePathIterator(
        MTS<S, A> mts, S state, A label, TransitionType type, Set<A> silentActions) {
    ...
    private void clousureSilentActions(
        S state, MTS.TransitionType transitionType, QueueType queueType) {
    ...
    private void processPreVisibleTransition() {
      ...
      while (this.unprocessed.get(QueueType.POSTACTION).isEmpty()
        ...
        this.enqueueToVisit(
            mts.getTransitions(state, this.type).getImage(this.label), QueueType.POSTACTION);
      }
    }
