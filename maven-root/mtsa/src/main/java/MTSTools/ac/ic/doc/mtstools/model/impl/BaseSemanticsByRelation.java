...
public class BaseSemanticsByRelation implements ImplementationNotion, RefinementByRelation {
  ...
  public <S1, S2, A> BinaryRelation<S1, S2> getRefinement(
      MTS<S1, A> mts1, MTS<S2, A> mts2, S1 state1, S2 state2) {
    Validate.isTrue(areComparable(mts1, mts2), "The models do not have the same alphabet");
    BinaryRelation<S1, S2> largestRelation =
        this.getRelationConstructor().getLargestRelation(mts1, mts2);
    if (largestRelation.contains(Pair.create(state1, state2))) return largestRelation;
    else return null;
  }
  ...
  public <S1, S2, A> boolean isAValidRelation(
      MTS<S1, A> m, MTS<S2, A> n, BinaryRelation<S1, S2> relation) {
  ...
  protected <S1, S2, A> boolean areComparable(MTS<S1, A> m, MTS<S2, A> n) {
    return (n.getActions()
            .containsAll(CollectionUtils.subtract(m.getActions(), this.getSilentActions())))
        && (m.getActions()
            .containsAll(CollectionUtils.subtract(n.getActions(), this.getSilentActions())));
  }
}
