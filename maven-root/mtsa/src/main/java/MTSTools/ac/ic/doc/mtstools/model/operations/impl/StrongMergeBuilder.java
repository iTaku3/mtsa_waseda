...
public class StrongMergeBuilder implements MergeBuilder {
  ...
  public <S1, S2, A> MTS<Object, A> merge(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
    ...
    BinaryRelation<Object, Object> refRelationCRCR =
        this.getRefinement()
            .getLargestRelation(cr, cr); // new IdentityRelation<Object>(cr.getStates());
    ...
    do {
      ...
      for (A action : cr.getActions())
        for (Object state : cr.getStates()) {
          ...
          reqReachableStates =
              new HashSet<Object>(
                  reqReachableStates); // clone the set in order to be able to modify cr without
          // impacting reqReachableStates
            ...
            if (mcrs.size() > 1) {
              ...
              refRelationCRCR =
                  this.getRefinement()
                      .getLargestRelation(
                          bestCR,
                          bestCR); // TODO prove that this relation can be keep and use it in order
              // to validate refinement between cr and bestCR.
  ...
  private <A> void setAsRequiredTransitions(
      MTS<Object, A> cr, Object sourceSate, A action, Set<Object> targetStates) {
  ...
  private <A> void setAsPossibleTransitions(
      MTS<Object, A> cr, Object sourceState, A action, Set<Object> targetStates) {
    for (Object toState : targetStates) {
  ...
  private <A> MTS<Object, A> mergeMCRs(
      Collection<MTS<Object, A>> mcrs, Object stateToClone, A action) {
    ...
    Map<Object, Collection<Object>> state2newStates =
        this.createNewStatesMapping(mcr0, mcrs.size(), stateToClone);
