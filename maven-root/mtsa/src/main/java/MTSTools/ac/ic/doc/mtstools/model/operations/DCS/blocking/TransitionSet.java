...
public class TransitionSet<State, Action> implements Cloneable {
  ...
  private void init(Alphabet<Action> alphabet) {
    ...
    for (int i = 0; i < ltss.size(); ++i) enabledByLTS.add(new BitSet(alphabet.size()));
    for (HAction<Action> hAction : alphabet) {
      ...
      if (count == ltss.size()) enabledActions.add(hAction);
    }
  }
  ...
  public boolean contains(HAction<Action> action) {
    return enabledCount[action.hash /*hashCode()*/]
        == ltss.size(); // replaced during aggressive inlining
  }
