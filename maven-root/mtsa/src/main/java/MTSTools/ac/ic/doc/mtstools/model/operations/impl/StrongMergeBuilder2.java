...
/**
 * This class implements a merge algorithm based on make a common refinement more abstract applying
 * a sequence of abstraction operations, but this implementations keeps the original states
 * structure (this means that it doesn't clone states).
 *
 * @author fdario
 */
public class StrongMergeBuilder2 implements MergeBuilder {
  ...
  public <S1, S2, A> MTS<?, A> merge(MTS<S1, A> mtsA, MTS<S2, A> mtsB) {
    ...
    BinaryRelation<S1, Pair<S1, S2>> refRelationACR =
        this.getRefinement().getLargestRelation(mtsA, cr);
    BinaryRelation<S2, Pair<S1, S2>> refRelationBCR =
        this.getRefinement().getLargestRelation(mtsB, cr);
    BinaryRelation<Pair<S1, S2>, Pair<S1, S2>> refRelationCRCR =
        new IdentityRelation<Pair<S1, S2>>(cr.getStates());
    ...
    do {
      ...
      for (Pair<S1, S2> state : cr.getStates()) {
        for (A action : cr.getActions()) {
          Set<Pair<S1, S2>> reqReachableStates =
              cr.getTransitions(state, REQUIRED).getImage(action);
          Set<Pair<S1, S2>> posReachableStates =
              cr.getTransitions(state, POSSIBLE).getImage(action);
