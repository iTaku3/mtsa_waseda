...
public class DCSWithFeatures<State, Action> {
  ...
  public void getActionFeatures(
    ...
    if (child != null) {
      ...
      this.input_buffer.put(
          ((float) child.uncontrollableTransitions) / (child.getTransitions().size() + 1));
      this.input_buffer.put(
          ((float) child.unexploredTransitions) / (child.getTransitions().size() + 1));
    } else {
