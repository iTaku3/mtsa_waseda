...
/**
 * This class serves as a proxy for the Action type argument. Internally it is represented simply by
 * its hash value. Together with the alphabet allows for a high-performance representation of
 * transitions.
 */
public class HAction<Action> implements Comparable<HAction<Action>> {
  ...
  public HAction(
      DirectedControllerSynthesisBlocking<?, Action> directedControllerSynthesisBlocking,
    ...
    if (directedControllerSynthesisBlocking.controllable.isEmpty()
        || directedControllerSynthesisBlocking.controllable.contains(action))
