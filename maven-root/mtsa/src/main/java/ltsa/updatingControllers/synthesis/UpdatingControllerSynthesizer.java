...
public class UpdatingControllerSynthesizer {
  ...
  private static void solveControlProblem(
    ...
    Pair<List<Formula>, Set<Fluent>> safetyFormulasAndFluents =
    ...
    fillTerminatingActions(
        E_u.getActions(), goalFluents); // set the action events fluents terminating with any action
    ...
    MTS<Long, String> safetyEnv =
        UpdatingControllerSafetySynthesizer.synthesizeSafety(
            metaEnvironment,
            goalFluents,
            safetyFormulas,
    ...
    CompactState compactSafetyEnv =
        MTSToAutomataConverter.getInstance().convert(safetyEnv, "E_u||G(safety)", false);
    //		CompactState compactMetaEnv = MTSToAutomataConverter.getInstance().convert(metaEnvironment,
    // "meta E_u", false);
  ...
  /**
   * get the list of safety formulas from old and new and the set of propositions used in every
   * formula
   *
   * @param newGoalDef
   * @param output
   * @return
   */
  private static Pair<List<Formula>, Set<Fluent>> getSafetyFormulas(
    ...
    for (Symbol safetyDefinition : newGoalDef.getSafetyDefinitions()) {
      ...
      if (def != null) {
        safetyFormulas.add(
            FormulaUtils.adaptFormulaAndCreateFluents(def.getFormula(false), safetyFluents));
  ...
  /**
   * Event actions fluents are define as <"actionName",{}, false>. This method fill the empty set
   * with all actions from the alphabet but not the action "actionName".
   *
   * @param actions
   * @param goalFluents
   */
  private static void fillTerminatingActions(Set<String> actions, Set<Fluent> goalFluents) {
    ...
    for (Fluent fl : UpdatingControllersUtils.ACTION_FLUENTS_FOR_UPDATE) {
      ...
      Fluent resultantFl =
          new FluentImpl(fl.getName(), fl.getInitiatingActions(), terminating, fl.isInitialValue());
