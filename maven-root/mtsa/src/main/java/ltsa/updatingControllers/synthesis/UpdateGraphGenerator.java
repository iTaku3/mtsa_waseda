...
public class UpdateGraphGenerator {
  ...
  private static Map<String, UpdateGraphDefinition> graphDefinitions =
      new HashMap<String, UpdateGraphDefinition>();
  private static Map<String, UpdateGraph> graphs = new HashMap<String, UpdateGraph>();

  private UpdateGraphGenerator() {}

  public static UpdateGraph generateGraph(String name, LTSCompiler comp, LTSOutput output)
    ...
    if (updateGraphDef != null) {
      ...
      for (Symbol controllerSpecSymbol : updateGraphDef.getTransitions()) {
        ...
        UpdatingControllerCompositeState updateController =
            (UpdatingControllerCompositeState)
                comp.continueCompilation(updateSpec.getName().getName());
        ...
        if (updateController.getComposition().getName().endsWith(ControlConstants.NO_CONTROLLER)) {
          throw new LTSException(
              "Graph could not be generated because controller "
                  + name
                  + " could "
                  + "not be generated");
        }
  ...
  */
  private static UpdatingControllersDefinition getUpdateControllerSpec(
      Symbol controllerSpecSymbol) {
  ...
  private static UpdateTransition buildTransition(
      UpdatingControllerCompositeState updtControllerCS) {
  ...
  private static UpdateNode buildFromNode(
      UpdatingControllersDefinition spe, UpdatingControllerCompositeState uccs) {
    String goalName = spe.getOldGoal().toString();
    //		return buildNode(uccs.getOldEnvironment(), uccs.getOldController(),
    // uccs.getControllableActions(), goalName);
    return null;
  }

  private static UpdateNode buildToNode(
      UpdatingControllersDefinition spec, UpdatingControllerCompositeState uccs) {
    // If there's a transition FROM this node, eventually this controller will be assigned.
    // If this node is a final one, then we don't need to generate the controller since it will run
    // the marked Cu.
    // However, we do need at least the new environment to map the states when cutting down the Cu
    // to C'.
    //		MTS<Long, String> newController = uccs.getNewEnvironment();
    String goalName = spec.getNewGoal().toString();
    //		return buildNode(uccs.getNewEnvironment(), newController, uccs.getControllableActions(),
    // goalName);
    return null;
  }

  private static UpdateNode buildNode(
