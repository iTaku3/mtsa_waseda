...
public class UpdatingControllersAnimatorUtils {
  ...
  public static Map<String, UpdatingControllersDefinition> updateDefinitions =
      new HashMap<String, UpdatingControllersDefinition>();
  ...
  public static Long getActualStateFromTheUpdateController(ArrayList<String> executedActions) {
    ...
    executedActions.remove(
        executedActions.size() - 1); // remove the last action to match the current execution

    for (String action : executedActions) {

      Set<Long> toStates =
          updateController
              .getTransitions(actualState, MTS.TransitionType.REQUIRED)
              .getImage(action);

      if (toStates.size() > 1 || toStates.size() < 1) {
        //                System.out.println("Executed Action: " + action +  " does not match with
