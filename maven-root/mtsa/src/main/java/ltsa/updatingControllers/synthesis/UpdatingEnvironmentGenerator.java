...
public class UpdatingEnvironmentGenerator {
  ...
  /**
   * pre: updEnv is E||C post: updEnv is E||C plus some states of E (only the one that has an
   * incoming beginUpdate transition) states from E has a stopOldSpec, startNewSpec actions
   */
  private void linkWithBeginUpdate() {
    ...
    Pair<Long, Long> firstState =
        new Pair(new Long(oldController.getInitialState()), new Long(mapping.getInitialState()));
  ...
  private ArrayList<Pair<Long, Long>> nextToVisitInParallelComposition(
      Pair<Long, Long> actual, Pair<String, Long> transition) {

    ArrayList<Pair<Long, Long>> toVisit = new ArrayList<Pair<Long, Long>>();

    for (Pair<String, Long> action_toStateInMapping :
  ...
  /**
   * pre: updEnv is E||C plus some states of E (only the one that has an incoming beginUpdate
   * transition) post: updEnv is E||C -> E -> E' states from E and E' has a stopOldSpec,
   * startNewSpec actions.
   */
   ...
   * @param state
   * @return empty set if not fresh state was created. A set with the new state if a fresh state was
   *     created
   */
  private Set<Long> addTransitionCreatingNewStates(Pair<String, Long> action_toState, Long state) {
    ...
    if (!mappingToUpdEnv.containsKey(action_toState.getSecond())) {
      ...
      updEnv.addTransition(
          state, action_toState.getFirst(), mappingToUpdEnv.get(action_toState.getSecond()));
    }
    return result;
  }
