...
public class UpdatingControllerGRSynthesizer {
  ...
  public static void synthesizeGR(
  ...
  private static void nonBlockingGR(
      UpdatingControllerCompositeState uccs, LTSOutput output, MTS<Long, String> safetyEnv) {
    ...
    FluentStateValuation<Set<Long>> valuation =
        fluentUtils.buildValuation(perfectInfoGame, uccs.getUpdateGRGoal().getFluents());
    Assumptions<Set<Long>> assumptions =
        formulasToAssumptions(
            perfectInfoGame.getStates(), uccs.getUpdateGRGoal().getAssumptions(), valuation);
    Guarantees<Set<Long>> guarantees =
        formulasToGuarantees(
            perfectInfoGame.getStates(), uccs.getUpdateGRGoal().getGuarantees(), valuation);
    Set<Set<Long>> faults = new HashSet<Set<Long>>();

    grGoal =
        new GRGoal<Set<Long>>(
            guarantees, assumptions, faults, uccs.getUpdateGRGoal().isPermissive());
    ...
    game =
        new KnowledgeGRGame<Long, String>(
            initialStates,
            safetyEnv,
            perfectInfoGame,
    ...
    GRRankSystem<Set<Long>> system =
        new GRRankSystem<Set<Long>>(
            game.getStates(),
            grGoal.getGuarantees(),
            grGoal.getAssumptions(),
            grGoal.getFailures());

    KnowledgeGRGameSolver<Long, String> solver =
        new KnowledgeGRGameSolver<Long, String>(game, system);
    solver.solveGame();

    if (solver.isWinning(perfectInfoGame.getInitialState())) {
      Strategy<Set<Long>, Integer> strategy = solver.buildStrategy();

      Set<Pair<StrategyState<Set<Long>, Integer>, StrategyState<Set<Long>, Integer>>> worseRank =
          solver.getWorseRank();
      MTS<StrategyState<Set<Long>, Integer>, String> result =
          GameStrategyToMTSBuilder.getInstance().buildMTSFrom(perfectInfoGame, strategy, worseRank);

      result.removeUnreachableStates();
      LTSAdapter<StrategyState<Set<Long>, Integer>, String> ltsAdapter =
          new LTSAdapter<StrategyState<Set<Long>, Integer>, String>(
              result, MTS.TransitionType.POSSIBLE);
      MTS<StrategyState<Set<Long>, Integer>, String> synthesised =
          new MTSAdapter<StrategyState<Set<Long>, Integer>, String>(ltsAdapter);
      ...
      CompactState compactState =
          MTSToAutomataConverter.getInstance().convert(plainController, uccs.getName(), false);
  ...
  private static Assumptions<Set<Long>> formulasToAssumptions(
      Set<Set<Long>> states, List<Formula> formulas, FluentStateValuation<Set<Long>> valuation) {
  ...
  private static Guarantees<Set<Long>> formulasToGuarantees(
      Set<Set<Long>> states, List<Formula> formulas, FluentStateValuation<Set<Long>> valuation) {
  ...
  private static void synthesizeGRDeterministic(
    ...
    if (solver.isWinning(safetyEnv.getInitialState())) {
      ...
      Set<Pair<StrategyState<Long, Integer>, StrategyState<Long, Integer>>> worseRank =
          grSolver.getWorseRank();
      MTS<StrategyState<Long, Integer>, String> result =
          GameStrategyToMTSBuilder.getInstance()
              .buildMTSFrom(safetyEnv, strategy, worseRank, uccs.getUpdateGRGoal().getLazyness());
      ...
      if (result == null) {
        ...
        output.outln(
            "Controller [" + plainController.getStates().size() + "] generated successfully.");
        CompactState convert =
            MTSToAutomataConverter.getInstance().convert(plainController, uccs.getName());
