...
public class UpdatingControllersUtils {
  ...
  static {
    HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol> beginAction =
        new HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol>();
    HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol> stopAction =
        new HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol>();
    HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol> reconfigureAction =
        new HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol>();
    HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol> startAction =
        new HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol>();
  ...
  public static ControllerGoalDefinition generateSafetyGoalDef(
    ...
    cgd.addAssumeDefinition(
        new Symbol(123, "BeginUpdate")); // is this useless?we use the assumption in GR not here
    cgd.addGuaranteeDefinition(
        new Symbol(123, "StopOldSpec")); // is this useless? we use Guarantee in GR not here
    cgd.addGuaranteeDefinition(
        new Symbol(123, "StartNewSpec")); // besides the symbol redirects to nothing.
    ...
    AssertDefinition.compileAll(
        output); // this is for filling the fac attribute in constraints added before
    return cgd;
  }
  ...
  public static boolean isNotUpdateAction(String action) {
    return !START_NEW_SPEC.equals(action)
  ...
  private static void addFailures(
      ControllerGoalDefinition cgd, ControllerGoalDefinition controllerGoalDef) {
  ...
  private static void addFluentAndAssumption(
      ControllerGoal<String> grcg, Set<Fluent> fluents, String action) {
   ...
   * @return
   */
  private static FluentPropositionalVariable generateAndAddFluent(
      Set<Fluent> fluents, String action) {
    Fluent turnOnFluent = createOnlyTurnOnFluent(action);
    fluents.add(turnOnFluent);
    FluentPropositionalVariable fluentPropositionalVariable =
        new FluentPropositionalVariable(turnOnFluent);
    return fluentPropositionalVariable;
  }

  /**
   * Changes the safety goals of each controller to (!StopOldSpec -> OLD) and (StartNewSpec -> NEW).
   * Adds T too.
   *
   * @param oldGoalDef
   * @param newGoalDef
  ...
  public static MarkedMTS<Long, String> markCuTerminalSet(MTS<Long, String> updateController) {
    Set<Set<Long>> stronglyConnectedComponents =
        GraphUtils.getStronglyConnectedComponents(updateController);
    Set<Set<Long>> terminalSets =
        GraphUtils.getTerminalSets(stronglyConnectedComponents, updateController);
    ...
    return new MarkedMTS<Long, String>(
        updateController, updateController.getInitialState(), terminalSets);
  }
  ...
  private static Fluent createOnlyTurnOnFluent(String initAction) {
    ...
    Fluent onlyTurnOnFluent =
        new FluentImpl(
            new String(initAction),
            initiatingActions,
            new HashSet<MTSSynthesis.ar.dc.uba.model.language.Symbol>(),
            false);
  ...
  public static void removeOldTransitions(CompositeState cs) {
    ...
    for (Long state : mts.getStates()) {
      ...
      for (Pair<String, Long> action_toState :
        ...
        if (!isOld(action_toState.getFirst())) {
          ...
          resultMts.addState(action_toState.getSecond());
          resultMts.addRequired(
              state, withoutOld(action_toState.getFirst()), action_toState.getSecond());
        }
      }
    }
    cs.composition =
        MTSToAutomataConverter.getInstance().convert(resultMts, cs.composition.getName(), false);
  }
