...
public class UpdatingControllerSafetySynthesizer {
  ...
  public static MTS<Long, String> synthesizeSafety(
    ...
    MTS<Long, String> safetyEnv =
        valuateSafety(safetyFormulas, metaEnvironment, fluentStateValuation);

    return getDontDoTwiceGoals(safetyEnv);
  }

  private static FluentStateValuation<Long> buildValuations(
    ...
    while (!toVisit.isEmpty()) {
      ...
      if (!discovered.contains(actualInMetaEnv)) {
        ...
        for (Pair<String, Long> action_toStateInMetaEnv :
            metaEnv.getTransitions(actualInMetaEnv, MTS.TransitionType.REQUIRED)) {
  ...
  private static void formulaToStateSet(
      Set<Long> toBuild, Formula formula, Long state, FluentStateValuation<Long> valuation) {
  ...
  private static MTS<Long, String> applySafetyInEnvironment(
      MTS<Long, String> metaEnvironment, HashSet<Long> toBuild) {
    ...
    for (Long state : metaEnvironment.getStates()) {
      ...
      if (!toBuild.contains(state)) {
        for (Pair<String, Long> transition :
            metaEnvironment.getTransitions(state, MTS.TransitionType.REQUIRED)) {
  ...
  private static void makeOldActionsUncontrollable(
      Set<String> controllableActions, MTS<Long, String> env) {
  ...
  private static CompactState dontDoTwiceModel(String dontDoTwiceAction, Set<String> alphabet) {
    ...
    return MTSToAutomataConverter.getInstance()
        .convert(model, "dontDo" + dontDoTwiceAction.toUpperCase());
  }
}
