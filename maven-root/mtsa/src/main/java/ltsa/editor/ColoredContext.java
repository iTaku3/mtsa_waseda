...
public class ColoredContext extends StyleContext implements ViewFactory {
  /**
   * View that uses the lexical information to determine the style characteristics of the text that
   * it renders. This simply colorizes the various tokens and assumes a constant font family and
   * size.
   */
  class ColoredView extends PlainView {
     ...
     * invalidate the lexical scanner after rendering so that the next request to drawUnselectedText
     * will set a new range for the scanner.
     *
     ...
     * colors based upon the token-to-color translations. To reduce the number of calls to the
     * Graphics object, text is batched up until a color change is detected or the entire requested
    ...
    /**
     * Update the scanner (if necessary) to point to the appropriate token for the given start
     * position needed for rendering.
     */
  ...
  // CONSTRUCTOR-DESTRUCTOR
  /**
   * Constructs a set of styles to represent darwin lexical tokens. By default there are no colors
   * or fonts specified.
   */
