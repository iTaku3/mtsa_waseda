        ...
        for (Object o : Collections.list(automaton.states[(int) actualState].elements())) {
          ...
          if (ev instanceof ProbabilisticEventState) {
            ...
            if (pev.getBundleTransitions() != null)
              for (Object o2 :
                  Collections.list(
                      pev.getBundleTransitions()
                          .elements())) // this ignores anything chained in .probTr
              {
                if (o2 != o) {
                  ...
                  // --"+action2+","+pev2.getProbability()+","+pev2.getBundle()+"--> "+next2);
                  mdp.addTransition(
                      actualState,
                      action2,
                      next2,
                      pev2.getProbability().doubleValue(),
                      pev2.getBundle());

                  while (pev2.getBundleTransitions()
                      != null) // catch the ones chained in .probTr that are ignored in the outer
                  // loop
                  {
            ...
            //          System.out.println("adding "+actualState+"
            // --"+action+","+pev.getProbability()+","+pev.getBundle()+"--> "+next);
            mdp.addTransition(
                actualState, action, next, pev.getProbability().doubleValue(), pev.getBundle());
          } else {
          ...
          if (EventState.hasNonDet(ev)) {
            //          System.out.println(indexToAction[ev.getEvent()]+"-->"+ev.getNext()+" has
            // nondet (mdpconverter)");
            for (Object o2 : Collections.list(ev.elements())) {
              if (o2 != o) {
                ...
                //              System.out.println("adding "+actualState+"
                // --"+indexToAction[ev2.getEvent()]+"--> "+next2);
                mdp.addTransition(
                    actualState, indexToAction[ev2.getEvent()], next2); // but if it's nondet+prob?
              }
            }
          } else
            HPWindow.instance.ltsOutput.outln(
                indexToAction[ev.getEvent()]
                    + "-->"
                    + ev.getNext()
