...
public class MDPToAutomataConverter {
  ...
  public CompactState convert(MDP mdp, String name) {
    ...
    return new CompactState(
        size, name, statemap, automataTransitionsList, alphabet.toArray(new String[0]), endState);
  }
  ...
  public MTS<Long, String> convert(MDP mdp) {
    ...
    for (String a : mdp.getActions()) res.addAction(a);
    for (long s : mdp.getStates()) res.addState(s);
