 ...
 *
 * <p>Opposed to the drawings of LTS, this animator can only show changes in either single machines
 * or in the composite machine, however not in submachines of a composite. This would require
 * tracking the current state within every submachine what this class does not do.
 */
public class MTSAnimator implements Animator {
  ...
  public MTSAnimator(CompositeState state, EventManager em) {
    ...
    if (state instanceof UpdatingControllerCompositeState) {
      ...
      for (Object cstate : state.machines) {
        if (cstate instanceof CompactState) mtss.add(mtsConverter.convert((CompactState) cstate));
      }
    }
    // Generate MTS composition if there is more than one machine

    if (mtss.size() == 1) this.mts = mtss.get(0);
    else {
      MTSMultipleComposer<Long, String> composer =
          new MTSMultipleComposer<Long, String>(new CompositionRuleApplier());
    ...
    else transitionIndeces = new int[state.getMachines().size() + 2];
  ...
  public BitSet initialise(Vector menu) {
    ...
    for (MTS.TransitionType type :
        new MTS.TransitionType[] {MTS.TransitionType.REQUIRED, MTS.TransitionType.MAYBE}) {
      ...
      for (Long from : transitions.keySet()) {
        ...
        for (Pair<String, Long> to : transitions.get(from)) {
          ...
          } else transitionsByAction.add(t);
        }

        ArrayList<Transition> existingTransitions = this.transitionsByStateTable.get(from);
        if (existingTransitions == null) this.transitionsByStateTable.put(from, transitionsByState);
        else existingTransitions.addAll(transitionsByState);
      }
    }
  ...
  public String actionNameChosen() {
    ...
    else return this.lastTransition.name().concat("?");
  }
  ...
  /** This is called after every animation step. TODO document properly. */
  public String[] getMenuNames() {
    ...
    ret[0] = "";
    int i = 1;
    for (String action : relabeled) ret[i++] = action;
    return ret;
  }
  ...
  /**
   * Change the current state to the next given a transition and make all necessary modifications
   * including setting the now possible transitions and notifying the event manager
   *
   * @return
   */
  ...
  private Transition transitionForIndex(int index) {
    ...
    for (Transition t : transitionsUnfiltered) {
      if (t.from().equals(this.currentState)) transitions.add(t);
    }
  ...
  /**
   * If for an action there are only maybe transitions, append a question mark However, default to
  ...
  private ArrayList<String> relabelMaybeTransitionNames(ArrayList<String> names) {
    ...
    for (String actionName : this.menuActionNames) {
      ...
      if (transitions != null) {
        ...
        if (isMaybeOnly && hasMaybe) relabeled.add(actionName);
        else relabeled.add(actionName);
      } else relabeled.add(actionName);
    }
    return relabeled;
  }
  ...
  public boolean traceChoice() {
    if (this.errorTrace.size() > 0) return (this.errorTrace.get(0).from() == this.currentState);
    return false;
  }
  ...
  /**
   * Return the correct string object of the action list which matches the argument
   *
   * @param s
   * @return
   */
  private String getActionNameByStringComparison(String s) {
    for (String str : this.mts.getActions()) {
      if (str.compareTo(s) == 0) return str;
    }
    return null;
  }
}
