...
public class HPWindow extends JFrame implements Runnable {
  ...
  MTSAEnactmentSimulation<Long, String> enactmentSimulation =
      new MTSAEnactmentSimulation<Long, String>();
  ...
  private FindDialog setFindDialog() {
    return new FindDialog(this) {
      JTextComponent currentTextComponent() {
        ...
        if (title.equals("Edit")) return input;
        else if (title.equals("Output")) return output;
        else return null;
      }
    };
  }
  ...
  private void buildEditResultPane() {
    ...
    JScrollPane outp =
        new JScrollPane(
            output, JScrollPane.VERTICAL_SCROLLBAR_ALWAYS, JScrollPane.HORIZONTAL_SCROLLBAR_NEVER);
  ...
  private void buildToolbar() {
    ...
    tools.add(
        composeTool = createTool("icon/compose.gif", "Compose", new DoAction(DO_doComposition)));
    tools.add(
        minimizeTool =
            createTool("icon/minimize.gif", "Minimize", new DoAction(DO_minimiseComposition)));
    ...
    tools.add(
        progressTool =
            createTool("icon/progress.gif", "Check Progress", new DoAction(DO_progress)));
    tools.add(stopTool = createTool("icon/stop.gif", "Stop", new StopAction()));
    stopTool.setEnabled(false);
    tools.addSeparator();
    tools.add(
        createTool(
            "icon/alphabet.gif", "Run Arranged Animation", new DoAction(DO_ARRANGED_ANIMATOR)));
  ...
  private void enactmentMenu(JMenuBar mb) {
    ...
    if (schedulerFactory != null) {
      ...
      if (schedulerNames.size() > 0) this.enactmentOptions.scheduler = schedulerNames.get(0);
    }
  }
  ...
  private void doOpenFile(String dir, String f, boolean resource) {
    if (f != null)
      try {
        ...
        InputStream fin;
        if (!resource) fin = new FileInputStream(dir + openFile);
        else fin = getClass().getClassLoader().getResourceAsStream(dir + "/" + openFile);
  ...
  private void saveFile() {
    if (openFile != null && openFile.equals("*.lts")) saveAsFile();
    else if (openFile != null)
      try {
        int i = openFile.indexOf('.', 0);
        if (i > 0) openFile = openFile.substring(0, i) + "." + "lts";
        else openFile = openFile + ".lts";
  ...
  private void exportFile() {
    ...
    if ((sn = fd.getFile()) != null)
      try {
        int i = sn.indexOf('.', 0);
        if (i == -1) sn += extension;
        else extension = sn.substring(i, sn.length());
        ...
        switch (extension) {
          case ".xml":
            ...
            break;
          case ".smv":
            translator = new SMVTranslator();
            break;
          case ".slugs":
            translator = new SlugsTranslator();
            break;
          case ".py":
            translator = new CTLPYTranslator();
            break;
          case ".pddl":
            translator = new PDDLTranslator();
            break;
          case ".fsp":
            translator = new FSPTranslator();
            break;
        }
        if (translator != null) translator.translate(current, myOutput);
        else current.composition.printAUT(myOutput);
        myOutput.close();
        fout.close();
  ...
  private boolean checkSave() {
    ...
    if (!savedText.equals(input.getText())) {
      int result =
          JOptionPane.showConfirmDialog(this, "Do you want to save the contents of " + openFile);
  ...
  class SaveFileAction implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (pp.equals("Edit") || pp.equals("Output")) saveFile();
      else if (pp.equals("Alphabet")) alphabet.saveFile();
      else if (pp.equals("Transitions")) prints.saveFile(currentDirectory, ".txt");
      else if (pp.equals("Draw")) draws.saveFile();
      else if (pp.equals("Layout")) layouts.saveFile();
    }
  }
  ...
  class ExportFileAction implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      String pp = textIO.getTitleAt(textIO.getSelectedIndex());
      if (pp.equals("Edit") || pp.equals("Transitions")) exportFile();
    }
  }
  ...
  class OptionAction implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      if (source == setWarnings) Diagnostics.warningFlag = setWarnings.isSelected();
      else if (source == setWarningsAreErrors)
        Diagnostics.warningsAreErrors = setWarningsAreErrors.isSelected();
      else if (source == setFair) ProgressCheck.strongFairFlag = setFair.isSelected();
      else if (source == setAlphaLTL) AssertDefinition.addAsterisk = !setAlphaLTL.isSelected();
      else if (source == setSynchLTL) FormulaFactory.normalLTL = !setSynchLTL.isSelected();
        ...
        if (setParallelSynthesis.isSelected()) {
          ComputerOptions.getInstance()
              .setAllowedThreads(Runtime.getRuntime().availableProcessors());
        } else {
          ComputerOptions.getInstance().setAllowedThreads(1);
        }
      } else if (source == setObsEquiv) Analyser.preserveObsEquiv = setObsEquiv.isSelected();
      else if (source == setReduction) CompositeState.reduceFlag = setReduction.isSelected();
  ...
  class LayoutOptionListener implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      ...
      final JSpinner KK_length_factor_spinner =
          new JSpinner(new SpinnerNumberModel(LTSJUNGCanvas.KK_length_factor, 0.1, 10.0, 0.1));
      ...
      final JSpinner kk_it_spinner =
          new JSpinner(new SpinnerNumberModel(LTSJUNGCanvas.KK_max_iterations, 1, 10000, 1));
      ...
      frpanel.setBorder(
          BorderFactory.createTitledBorder(EnumLayout.FruchtermanReingold.toString()));
      frpanel.setLayout(new GridLayout(0, 2));

      frpanel.add(new JLabel("Attraction"));
      final JSpinner fr_attraction_spinner =
          new JSpinner(new SpinnerNumberModel(LTSJUNGCanvas.FR_attraction, 0.1, 10.0, 0.05));
      frpanel.add(fr_attraction_spinner);

      frpanel.add(new JLabel("Repulsion"));
      final JSpinner fr_repulsion_spinner =
          new JSpinner(new SpinnerNumberModel(LTSJUNGCanvas.FR_repulsion, 0.1, 10.0, 0.05));
      frpanel.add(fr_repulsion_spinner);

      frpanel.add(new JLabel("Max iterations"));
      final JSpinner fr_it_spinner =
          new JSpinner(new SpinnerNumberModel(LTSJUNGCanvas.FR_max_iterations, 1, 10000, 1));
      ...
      class ButtonOptionListener implements ActionListener {
        ...
        private void updateValues() {
          try {
            ...
            if (o == null) return;
            LTSJUNGCanvas.KK_length_factor =
                Double.parseDouble(o.toString()) < 0
                    ? 0
                    : (Double.parseDouble(o.toString()) > 10
                        ? 10
          ...
          //		  	    	  if (o==null) return;
          //		  	    	  LTSJUNGCanvas.KK_distance = Double.parseDouble(o.toString()) < 0 ? 0 :
          ...
          try {
            ...
            if (o == null) return;
            LTSJUNGCanvas.KK_max_iterations =
                Integer.parseInt(o.toString()) < 0
                    ? 0
                    : (Integer.parseInt(o.toString()) > 10000
                        ? 10000
          ...
          try {
            ...
            if (o == null) return;
            LTSJUNGCanvas.FR_max_iterations =
                Integer.parseInt(o.toString()) < 0
                    ? 0
                    : (Integer.parseInt(o.toString()) > 10000
                        ? 10000
          ...
          //		  	    	  if (o==null) return;
          //		  	    	  LTSJUNGCanvas.Tree_distX = Integer.parseInt(o.toString()) < 0 ? 0 :
          // (Integer.parseInt(o.toString()) > 1000 ? 1000 : Integer.parseInt(o.toString()));
          //		  	    	  //if (layouts!=null)
          ...
          //		  	    	  if (o==null) return;
          //		  	    	  LTSJUNGCanvas.Tree_distY = Integer.parseInt(o.toString()) < 0 ? 0 :
          // (Integer.parseInt(o.toString()) > 1000 ? 1000 : Integer.parseInt(o.toString()));
          //		  	    	  //if (layouts!=null)
          // layouts.setFR_max_iterations(LTSLayoutWindow.FR_max_iterations);
          //		    	  } catch(NumberFormatException nfe) {}
          ...
          //		  	    	  if (o==null) return;
          //		  	    	  LTSJUNGCanvas.Radial_distX = Integer.parseInt(o.toString()) < 0 ? 0 :
          // (Integer.parseInt(o.toString()) > 1000 ? 1000 : Integer.parseInt(o.toString()));
          //		  	    	  //if (layouts!=null)
          // layouts.setFR_max_iterations(LTSLayoutWindow.FR_max_iterations);
          //		    	  } catch(NumberFormatException nfe) {}
          ...
          //		  	    	  if (o==null) return;
          //		  	    	  LTSJUNGCanvas.Radial_distY = Integer.parseInt(o.toString()) < 0 ? 0 :
          // (Integer.parseInt(o.toString()) > 1000 ? 1000 : Integer.parseInt(o.toString()));
          //		  	    	  //if (layouts!=null)
          // layouts.setFR_max_iterations(LTSLayoutWindow.FR_max_iterations);
          //		    	  } catch(NumberFormatException nfe) {}
  ...
  class EditCopyAction implements ActionListener {
    public void actionPerformed(ActionEvent e) {
      String pp = textIO.getTitleAt(textIO.getSelectedIndex());
      if (pp.equals("Edit")) input.copy();
      else if (pp.equals("Output")) output.copy();
      else if (pp.equals("Manual")) manual.copy();
      else if (pp.equals("Alphabet")) alphabet.copy();
      else if (pp.equals("Transitions")) prints.copy();
    }
  }
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      if (run_items != null && run_enabled != null) {
        if (run_items.length == run_enabled.length)
          for (int i = 0; i < run_items.length; ++i) run_items[i].setEnabled(run_enabled[i]);
      }
    }
  }
  ...
  private boolean compile() {
    ...
    postState(current);
    int[] current_states =
        current.machines.size() > 0 ? new int[current.machines.size()] : new int[] {0};
    for (int i = 0; i < current.machines.size(); i++) current_states[i] = 0;
  ...
  private void liveness() {
    ...
    CompositeState not_ltl_property =
        AssertDefinition.compile(new EmptyLTSOuput(), AssertDefinition.NOT_DEF + asserted);
    if (current != null && ltl_property != null) {
      TransitionSystemDispatcher.checkFLTL(
          current, ltl_property, not_ltl_property, this.setFair.isSelected(), ltsOutput);
      postState(current);
    }
  ...
  private void doComposition() {
    ...
    if (current != null) {
      ...
      for (int i = 0; i < current.machines.size() + 1; i++) current_states[i] = 0;
      layouts.setCurrentState(current_states);
    }
  }
  ...
  private void exploration_new() {
    ...
    ViewNextConfiguration[] view_configurations =
        new ViewNextConfiguration[current.machines.size() - 1];
    for (int i = 1; i < components.length; i++) {
      ...
      } else view_configurations[i] = new ViewNextConfigurationRandom();
    }

    View view = new View(components, view_configurations);

    // Model
    CompactState[] knowledge_configurations =
        new CompactState[explorerDefinition.getModel().size()];
    for (int i = 0; i < explorerDefinition.getView().size(); i++)
      for (int j = 0; j < current.machines.size(); j++)
        if (explorerDefinition
            .getModel()
    ...
    for (String anAction : goal.getControllableActions())
      if (!anAction.contains("[")) controlableActions.add(anAction);
    goal.setControllableActions(controlableActions);

    // Strategy manager
    Strategy[] strategies = new Strategy[1];
    strategies[0] =
        new StrategySynthesisNewAction(
            new StrategySynthesis(knowledge, goal.copy()),
            new StrategyNewAction(knowledge, goal.copy()));
    ...
    while (state.length() < 3) state = " " + state;
  ...
  private void exploration_manual() {
    if (this.explorer == null) throw new UnsupportedOperationException("Primero hay que explorar");
  ...
  private String exploration_stepover() {
    ...
    for (int i = 0; i < steps_count; i++) {
      ...
      if (mtsControlProblemAnswer.equals("RESET")) explorer.reset();

      if (mtsControlProblemAnswer.equals("SOME")) this.explorer.explore();
  ...
  private void exploration_output() {
    ...
    for (int j = 0; j < traceStates.size(); j++) {
      ...
      while (action.length() < 6) action = action + " ";

      String state = traceStates.get(j);
      while (state.length() < 3) state = " " + state;

      if (Objects.equals(state.trim(), "")) ltsOutput.outln(action);
      else ltsOutput.outln(action + "  ->  " + state);
    }
  ...
  private Integer getStepsCount() {
    ...
    if (count_integer < 1) count_integer = 1;
  ...
  private void animate() {
    ...
    if (current != null) {
      ...
      Animator anim = TransitionSystemDispatcher.generateAnimator(animatedCurrent, ltsOutput, eman);
  ...
  private Frame createCustom(
      Animator anim, String params, Relation actions, Relation controls, boolean replay) {
  ...
  private void doRefinement() {
    if (compileIfChange() && current != null) {
      ...
      final RefinementWindow refinementWindow =
          new RefinementWindow(
              this,
              refinementOptions,
              models,
              SemanticType.values(),
      ...
      if (refinementOptions.isValid()) {
        ...
        if (isThereAProb) {
          ltsOutput.outln(
              "One of the models is probabilistic. Nondeterministic-based Bisimulation will be"
                  + " applied.");
        }

        boolean areRefinement =
            TransitionSystemDispatcher.isRefinement(
                refines, refined, refinementOptions.getRefinementSemantic(), ltsOutput);
        if (refinementOptions.isBidirectionalCheck()) {
          areRefinement &=
              TransitionSystemDispatcher.isRefinement(
                  refined, refines, refinementOptions.getRefinementSemantic(), ltsOutput);
          if (areRefinement) {
            ltsOutput.outln(" ");
  ...
  private void doBisimulation() {
    if (compileIfChange() && current != null) {
      ...
      final BisimulationWindow refinementWindow =
          new BisimulationWindow(this, refinementOptions, models, semanticTypes);
      refinementWindow.setLocation(this.getX() + 100, this.getY() + 100);
      refinementWindow.pack();
      refinementWindow.setVisible(true);
      if (refinementOptions.isValid()) {
        ...
        if (!(new HashSet<String>(Arrays.asList(refines.getAlphabet()))
            .equals(new HashSet<String>(Arrays.asList(refined.getAlphabet()))))) {
          ltsOutput.outln(
              "Models "
                  + refines.getName()
                  + " and "
                  + refined.getName()
                  + " have different alphabet. Cannot proceed with bisimulation check.");
        } else {
          ltsOutput.outln(" ");
          BinaryRelation<?, ?> refRel =
              TransitionSystemDispatcher.getRefinement(
                  refines, refined, refinementOptions.getRefinementSemantic(), ltsOutput);
          ...
          if (refRel != null) {
            ltsOutput.outln(
                "Models "
                    ...
                    + refined.getName()
                    + " are ("
                    + refinementOptions.getRefinementSemantic().toString()
                    + ") bisimilar.");
  ...
  private void doLegality() {
    if (compileIfChange() && current != null) {
      ...
      if (legalityOptions.isValid()) {
        ...
        LegalityAnalyser a =
            new LegalityAnalyser(
                current, ltsOutput, legalityOptions.source, legalityOptions.target, actions);
        a.composeNoHide();
        ...
        if (a.isLegal())
          resultMessage =
              String.format(
                  "Result: Yes, %s is legal w.r.t %s with controlled actions %s",
          ...
          resultMessage =
              String.format(
                  "Result: No, %s is not legal w.r.t %s with controlled actions %s",
                  current.machines.get(legalityOptions.source).name,
                  current.machines.get(legalityOptions.target).name,
                  legalityOptions.actionSet);
    ...
    if (compileIfChange() && current != null) {
      ...
      if (refinementOptions.isValid()) {
        ...
        TransitionSystemDispatcher.areConsistent(
            ...
            refinementOptions.getRefinementSemantic(),
            ltsOutput);
      }
  ...
  private void doRunEnactors() {
    ...
    if (current == null || current.composition == null || current.goal == null) {
  ...
  private void doEnactorOptions() {
    final EnactorOptionsWindows enactorOptionsWindow =
        new EnactorOptionsWindows(enactmentOptions, this);

    if (this.enactmentOptions.scheduler != null && !this.enactmentOptions.scheduler.isEmpty())
      enactorOptionsWindow.getCbSchedulers().setSelectedItem(enactmentOptions.scheduler);

    if (this.enactmentOptions.enactors != null) {
      List<Integer> selected = new ArrayList<Integer>();
      for (String enactor : this.enactmentOptions.enactors) {
        for (int i = 0; i < enactorOptionsWindow.getEnactorList().getModel().getSize(); i++)
          if (enactor.equals(enactorOptionsWindow.getEnactorList().getModel().getElementAt(i)))
            selected.add(i);
      }
      int[] selectIndices = new int[selected.size()];
      for (int i = 0; i < selected.size(); i++) selectIndices[i] = selected.get(i).intValue();

      enactorOptionsWindow.getEnactorList().setSelectedIndices(selectIndices);
    }

    enactorOptionsWindow
        .getOkButton()
        .addActionListener(
            new ActionListener() {
              public void actionPerformed(ActionEvent e) {
                ...
                enactmentOptions.scheduler =
                    (String) enactorOptionsWindow.getCbSchedulers().getSelectedItem();
                int[] selectedIndices = enactorOptionsWindow.getEnactorList().getSelectedIndices();
                enactmentOptions.enactors.clear();
                for (int i = 0; i < selectedIndices.length; i++) {
                  enactmentOptions.enactors.add(
                      enactorOptionsWindow
                          .getEnactorList()
                          .getModel()
  ...
  public boolean parse(Hashtable cs, Hashtable ps, Hashtable ex) {
    ...
    eman.post(
        new LTSEvent(
            LTSEvent.NEWLABELSETS, (labelSetConstants = LabelSet.getConstants()).keySet()));
    // <<< AMES
  ...
  /** Updates the various display windows and animators with the given machines. */
  ...
  /**
   * Returns the set of actions which correspond to the label set definition with the given name.
   */
  public Set<String> getLabelSet(String name) {
    if (labelSetConstants == null) return null;
    ...
    for (String a : ls.getActions(null)) s.add(a);
