...
//                ltsOutput.outln("\n");
    ...
    private void stepwiseControllerSynthesis() {
        ...
        //　遷移数カウントを追加
        printModelTransitionCounts(unsynthesized_env_list);
        // 全環境モデルのトランジション合計を計算して出力
        Map<String, Integer> globalCounts = countTotalTransitionsPerAction(unsynthesized_env_list);
        ltsOutput.outln("--- Global Transition Counts ---");
        for (String actionName : globalCounts.keySet()) {
            ltsOutput.outln("  - " + actionName + " : " + globalCounts.get(actionName));
        }
        ltsOutput.outln("--------------------------------");
    ...
    private void analysisSynthesisProcess(List<CompactState> synthesisProcess, List<CompactState> unsynthesized_req_list, List<CompactState> unsynthesized_env_list, String final_model_name) {
        List<CompactState> this_step_req_list = new ArrayList<>();
        if (unsynthesized_req_list.size()!=0)
        {
            // Step 1 : 各監視モデルの合成コストを更新し，合成コストの一番小さなモデルをthis_step_req_listに格納．
            ...
            // Step 2 : 一番costの小さなモデルと同プロセスで合成できる要求も分析
            findSameStepReq(unsynthesized_req_list, this_step_req_list);

            // Step 3 : Step2の要求と対応する環境モデルから部分制御機"this_step_partController"の情報を構築
            CompactState this_step_partController = new CompactState(); //本ステップで合成される部分制御器（空）
            this_step_partController.inputModels = new ArrayList<>();
            this_step_partController.componentModels = new ArrayList<>(this_step_req_list.get(0).actual_monitoredModels);
            ...
            unsynthesized_env_list.add(this_step_partController);
    ...
    private void synthesisFromSynthesisProcess(List<CompactState> synthesisProcess, List<CompactState> all_models, List<CompactState> all_output_models, String final_model_name){
        for (CompactState partController : synthesisProcess){
            ...
            ltsOutput.outln("[info] do_minimise  : " + do_minimise);
            ltsOutput.outln("---------------------------------------------------");
    ...
    private void calculationCost(List<CompactState> unsynthesized_req_list, List<CompactState> unsynthesized_env_list, List<CompactState> this_step_req_list) {
        ...
        CompactState this_step_req = new CompactState();
        List<List<String>> partControllers = new ArrayList<>();
        
        // unsynthesized_env_listのうち，部分制御機となるenvを構成する環境モデルを，partControllersに格納
        for (CompactState env : unsynthesized_env_list) {
            if (env.componentModels!=null)
        ...
        // 部分制御器を踏まえたcostの再計算と，costが最小のreqをthis_step_reqに格納
        for (CompactState req : unsynthesized_req_list) {
        ...
        // this_step_reqのreqをthis_step_req_listに格納し，unsynthesized_req_listから削除する
        for (CompactState req : unsynthesized_req_list) {
            if (this_step_req.name.equals(req.name)){
    ...
    /**
     * モデルのリストを受け取り、各モデルの遷移数をプリントする
     */
    private void printModelTransitionCounts(List<CompactState> models) {
        ltsOutput.outln("--- Transition Counts per Action ---");
        for (CompactState model : models) {
            ltsOutput.outln("[Model] " + model.name);
            Map<String, Integer> counts = model.countTransitionsAction();
            List<String> actionNames = new ArrayList<>(counts.keySet());
            Collections.sort(actionNames);
            for (String actionName : actionNames) {
                ltsOutput.outln("  - " + actionName + " : " + counts.get(actionName));
            }
            ltsOutput.outln("");
        }
        ltsOutput.outln("------------------------------------");
    ...
    /**
     * 環境モデルのリスト全体で、各アクションが合計何遷移分あるかをカウントする
     */
    ...
    private void sortModel(List<String> model_name_list) {
        ...
        model_name_list.addAll(env_list);
        model_name_list.addAll(req_list);
    }
