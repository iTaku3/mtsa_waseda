...
public class ExplorationAnimator implements Animator {
  ...
  public ExplorationAnimator(
      CompositeState _environmentView,
      CompositeState _environmentModel,
    ...
    for (int i = 0; i < transitionIndeces.length; i++) transitionIndeces[i] = 0;
  ...
  /**
   * Initialize the animator, generating all dictionaries for easier use in this class, setting the
   * inital state etc.
   *
  ...
  public BitSet initialise(Vector menu) {
    ...
    // Now finally create the initial BitSets for each MTS with the indeces of all outgoing
    // transitions of state 0, set to 1
  ...
  private HashMap<String, Long> firstState() {
    ...
    for (java.util.Map.Entry<String, MTS<Long, String>> entry :
        environmentModel.getMTSs().entrySet()) {
  ...
  private TransitionsComparator calculateIntersections() {
    ...
    // Calculate the intersection of actions available in the environmentView with the actions
    // available in the environment model

    ArrayList<Transition> environmentViewTransitions =
        environmentView.getTransitionsByState().get(currentStates.get("Environment View"));
    ...
    for (String mtsName : currentStates.keySet()) {
      ...
      if (environmentModelTransitions.isEmpty()) {
        environmentModelTransitions =
            environmentModel.getTransitionsByState().get(mtsName).get(currentStates.get(mtsName));
      } else {
        environmentModelTransitions =
            new TransitionsComparator(
                    environmentModelTransitions,
                    environmentModel
                        .getTransitionsByState()
                        .get(mtsName)
                        .get(currentStates.get(mtsName)),
                    true)
                .intersection;
      }
    }
    TransitionsComparator transitionsComparator =
        new TransitionsComparator(environmentModelTransitions, environmentViewTransitions, false);
  ...
  public String[] getMenuNames() {
    ...
    ret[0] = "";
    int i = 1;
    for (String action : menuActionNames) ret[i++] = action;
    return ret;
  }
  ...
  private HashMap<String, Transition> transitionForIndex(int index) {
    ...
    ArrayList<Transition> viewTransitionsUnfiltered =
        environmentView.getTransitionsByName().get(transitionName);
    ArrayList<Transition> viewTransitions = new ArrayList<Transition>();
    for (Transition t : viewTransitionsUnfiltered) {
      if (t.from() == currentStates.get("Environment View")) viewTransitions.add(t);
    }

    choices.put("Environment View", chooseTransition(viewTransitions));

    for (Entry<String, HashMap<String, ArrayList<Transition>>> mtsNameTransitions :
        environmentModel.getTransitionsByName().entrySet()) {
      ArrayList<Transition> transitionsUnfiltered =
          mtsNameTransitions.getValue().get(transitionName);
      ArrayList<Transition> transitions = new ArrayList<Transition>();
      for (Transition t : transitionsUnfiltered) {
        if (t.from() == currentStates.get(mtsNameTransitions.getKey())) transitions.add(t);
      }
  ...
  /**
   * Choose a transition from the list of transitions to follow based on the MTSA behavior; assumes
   * that all transitions are valid, i.e. 'from' matches 'current state'
   *
   * @return
   */
  private Transition chooseTransition(ArrayList<Transition> transitions) {
    ...
    for (Transition t : transitions) {
      if (t.type() == MTS.TransitionType.REQUIRED) req.add(t);
    }
  ...
  private void performTransition(HashMap<String, Transition> chosenTransition) {
    ...
    if (size == 1) transitionIndeces = new int[size + 1];
    else transitionIndeces = new int[size + 2];
    int i = 0;
    for (; i < transitionIndeces.length - 2; i++) transitionIndeces[i] = 0;
  ...
  public BitSet singleStep() {
    ...
    for (Map.Entry<String, HashMap<Long, ArrayList<Transition>>> mtsNameWithMTS :
        environmentModel.getTransitionsByState().entrySet()) {
      ArrayList<Transition> mtsTransitions =
          mtsNameWithMTS.getValue().get(currentStates.get(mtsNameWithMTS.getKey()));
  ...
  public String actionNameChosen() {
    if (lastTransition.type() == MTS.TransitionType.REQUIRED) return lastTransition.name();
    else return lastTransition.name().concat("?");
  }
  ...
  public boolean isEnd() {
    return (environmentView
            .getTransitionsByState()
            .get(currentStates.get("Environment View"))
            .size()
        == 0);
  }
  ...
  /**
   * Check whether the next part in the trace is still valid in the current state
   *
   * @return
   */
  ...
  /**
   * Perform the next step in the trace by using the first step in the trace to generate the BitSet
   * to return
   *
   * @return
   */
