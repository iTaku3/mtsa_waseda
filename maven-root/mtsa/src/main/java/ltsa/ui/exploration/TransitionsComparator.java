...
public class TransitionsComparator {
  ...
  public TransitionsComparator(
      ArrayList<Transition> _modelTransitions,
      ArrayList<Transition> _viewTransitions,
    ...
    if (doIntersection) intersection = makeIntersection();
  }
  ...
  public ArrayList<Transition> makeIntersection() {
    ...
    for (Map.Entry<String, Transition> labelTransition : modelLabels.entrySet()) {
  ...
  private ArrayList<Pair<Integer, Transition>> rules() throws Exception {
    ...
    for (Transition mTransition : modelTransitions) {
      ...
      if (mTransition.type() == MTS.TransitionType.REQUIRED
          && !viewLabels.containsKey(mTransition.getEvent())) { // RULE 2

        throw new Exception(
            "Model and View are inconsistent in "
                + mTransition.toString()
                + "transition from Model View");
        ...
        if (viewLabels.containsKey(mTransition.getEvent())) { // RULE 5
          ...
          Pair<Integer, Transition> pair =
              new Pair<Integer, Transition>(new Integer(1), mTransition);
          result.add(pair);

        } else { // RULE 6

          Pair<Integer, Transition> pair =
              new Pair<Integer, Transition>(new Integer(2), mTransition);
          result.add(pair);
        }
      }
    }
    for (Transition vTransition : viewTransitions) {
      if (!modelTransitions.contains(vTransition)) { // RULE 3
        throw new Exception(
            "Model and View are inconsistent in "
                + vTransition.toString()
                + "transition from Environment View");
