...
public class LTSABatch implements LTSManager, LTSInput, LTSOutput, LTSError {
  ...
  public LTSABatch(
      String fileTxt, int modulesCount, boolean memo, boolean ref, String heur, boolean proj) {
    // SymbolTable.init();
    //		compositionBatch =
  ...
  public Set<String> getLabelSet(String name) {
    ...
    for (String a : (Vector<String>) ls.getActions(null)) s.add(a);
  ...
  private static void showUsage() {
    String usage =
        "MTSA usage:\n"
            ...
            + "-o <OUTPUT FILE>: Specifies the output format (*.aut for Aldebaran, *.pddl for"
            + " Planning Domain Definition Language, *.xml for a Supremica project, *.smv for MBP,"
            + " and *.py for Party-elli).\n\n"
            + "-m: Use monotonic abstraction during directed synthesis (ready abstraction by"
            + " default).\n\n"
  ...
  private static void process(final String fileTxt, String controller, String outputFile) {
    ...
    switch (extension) {
      case ".xml":
        ...
        break;
      case ".pddl":
        translator = new PDDLTranslator(true);
        break;
    ...
    try {
      ...
      if (translator != null) translator.translate(compositeState, outStream);
      else compositeState.composition.printAUT(outStream);
      outStream.close();
  ...
  private static CompositeState compileCompositeState(String inputString, String modelName)
      throws IOException {
