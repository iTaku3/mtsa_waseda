...
public class UpdateGraphGUI extends JPanel implements UpdateGraphEventDispatcher {
  ...
  private final Set<UpdateGraphEventListener> eventListeners =
      new HashSet<UpdateGraphEventListener>();
  ...
  private void buildGraphUI(
      UpdateGraph updateGraph, NonEditableGraphView graph, mxGraphComponent graphComponent) {
    if (updateGraph != null) {
  ...
  private BidiMap<UpdateNode, Object> insertNodes(
      NonEditableGraphView graph, UpdateGraph updGraph) {
  ...
  private void insertEdges(NonEditableGraphView graph, UpdateGraph updateGraph) {
    ...
    for (UpdateTransition updateTransition : updateGraph.getEdges()) {
      ...
      StringBuilder b =
          new StringBuilder(fromUpdNode.getGoalName()).append("-").append(toUpdNode.getGoalName());
      graph.getModel().beginUpdate();
  ...
  private void addNodeClickHandler(
      ...
      final NonEditableGraphView graph,
      final UpdateGraph updtGraph) {
  ...
  private void fireNodeClickedEvent(UpdateTransition updateTransition) {
    NodeClickedEvent clickedEvent =
        UpdateGraphEvent.getNodeClickedEvent(updateTransition, currentNode, nextNode);
    for (UpdateGraphEventListener eventListener : this.eventListeners) {
