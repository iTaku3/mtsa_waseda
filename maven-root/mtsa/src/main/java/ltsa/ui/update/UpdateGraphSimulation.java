...
public class UpdateGraphSimulation implements Runnable {
  ...
  public UpdateGraphSimulation(
      HPWindow hpWindow, String currentDirectory, ApplicationContext applicationContext) {
    this.input = hpWindow;
  ...
  private void initUI() {
    ...
    UpdateGraphBidirectionalManager dispatcher =
        new UpdateGraphBidirectionalManager(schedulerFactory, graphGUI);
    final UpdateGraphWindow graphWindow =
        new UpdateGraphWindow(input, directory, running, graphGUI);
