...
public class StrategyNewAction extends Strategy {
  ...
  public String chooseNextAction(HashSet<String> availableActions) {
    ...
    for (int anEvent :
        this.knowledge.getCmponents()[0].states[this.knowledge.getCurrentStates()[0]].getEvents())
      for (String anAvailableAction : availableActions)
        if (alphabet[anEvent].equals(anAvailableAction + '?')) return anAvailableAction;
  ...
  private Boolean fullyExplored(EventState aState, Boolean onlyControllable) {
    for (int anEvent : aState.getEvents())
      if (isPossibleAction(anEvent, onlyControllable)) return false;
  ...
  private Boolean isPossibleAction(int i, Boolean onlyControllable) {
    ...
    if (onlyControllable)
      for (int componentNumber = 1;
          componentNumber < this.knowledge.getCmponents().length;
          componentNumber++)
        for (String anAction : this.knowledge.getCmponents()[componentNumber].alphabet)
          if (Objects.equals(action, anAction)) return false;
  ...
  private String getControllerNextAction(HashSet<String> availableActions) {
    ...
    if (!this.fullyExplored(machines[0].states[stateZero], true)) {
      ...
      if (!controllerAction.equals("WAIT")) return controllerAction;
    }
  ...
  private CompactState getControllerToState(CompactState machine, int stateNumber) {
    ...
    for (String anAction : goal.getControllableActions()) controllableActions.add(anAction);

    Set<String> goalControllableActions = new HashSet<>();
    goalControllableActions.add("fluent_on");
    for (String anAction : controllableActions) goalControllableActions.add(anAction);
    ...
    for (String anAction : goalControllableActions)
      if (!anAction.equals("fluent_on")) off.add(new SingleSymbol(anAction));
    ...
    compositionMachine.alphabet =
        Arrays.copyOf(compositionMachine.alphabet, compositionMachine.alphabet.length + 2);
    ...
    compositionMachine.states[stateNumber] =
        EventState.addEvent(
            compositionMachine.states[stateNumber],
            compositionMachine.alphabet.length - 1,
            stateNumber);
  ...
  private String getActionFromController(
      CompactState machine, int stateNumber, HashSet<String> availableActions) {
