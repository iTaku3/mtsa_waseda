...
public class Knowledge {
  ...
  public Knowledge(CompactState[] components) {
    ...
    for (int i = 0; i < components.length; i++) {
      ...
      this.components[i].states =
          Arrays.copyOf(components[i].states, components[i].states.length + 1);
      this.components[i].states[this.components[i].states.length - 1] =
          EventState.copy(this.components[i].states[0]);
      this.currentStates[i] = this.components[i].states.length - 1;
      this.stateEquivalence.add(new ArrayList<StateEquivalence>(1));
      this.stateEquivalence
          .get(i)
          .add(new StateEquivalence(0, this.components[i].states.length - 1));
    }
  }
  ...
  public void updateKnowledgeFromCurrentStateActions(
      Integer componentNumber, HashSet<String> actions) {
    // List of available possible actions
    HashSet<String> currentStateActions = new HashSet<>(0);
    for (String action : actions) currentStateActions.add(action + "?");

    int[] currentStateEvents =
        this.components[componentNumber].states[this.currentStates[componentNumber]].getEvents();
  ...
  public void execute(
      ...
      String nextAction,
      Integer nextViewState,
      EventState modelNewEventState) {
    if (nextAction.equals("WAIT")) return;
  ...
  public CompactState[] cloneForSynthesisFromCurrentState() {
    ...
    for (int i = 0; i < this.components.length; i++) clone[i].swapStates(0, this.currentStates[i]);
  ...
  private void removeEventFromCurrentState(int compunentNumber, int event) {
    this.components[compunentNumber].states[this.currentStates[compunentNumber]] =
        EventState.removeEvent(
            this.components[compunentNumber].states[this.currentStates[compunentNumber]], event);
  }

  private void changeState(
      Integer componentNumber, EventState modelNewEventState, Integer nextViewState) {
    Integer nextKnowledgeState = null;
    for (int i = 0; i < this.stateEquivalence.get(componentNumber).size(); i++)
      if (this.stateEquivalence
          .get(componentNumber)
          .get(i)
          .getViewStateNumber()
          .equals(nextViewState))
        nextKnowledgeState =
            this.stateEquivalence.get(componentNumber).get(i).getKnowledgeStateNumber();

    if (nextKnowledgeState != null) this.currentStates[componentNumber] = nextKnowledgeState;
    else {
      this.currentStates[componentNumber] = this.components[componentNumber].states.length;
      this.stateEquivalence
          .get(componentNumber)
          .add(new StateEquivalence(nextViewState, this.currentStates[componentNumber]));
  ...
  private void addState(Integer componentNumber, int nextState, EventState modelNewEventState) {
    this.components[componentNumber].states =
        Arrays.copyOf(this.components[componentNumber].states, nextState + 1);
    this.components[componentNumber].maxStates = nextState + 1;
    this.components[componentNumber].states[nextState] = modelNewEventState;
  }

  private void conifirmAction(
      Integer componentNumber, int oldEvent, int newEvent, int fromState, int toState) {
    this.components[componentNumber].states[fromState].updateEventAndNext(
        oldEvent, newEvent, toState);
  }
  // endregion
}
