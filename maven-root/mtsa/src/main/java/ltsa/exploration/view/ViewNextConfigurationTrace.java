...
public class ViewNextConfigurationTrace extends ViewNextConfiguration {
  ...
  public String nextAction(List<String> availableActions) {
    ...
    this.next++;
    if (this.next == trace.length) this.next = 0;

    if (availableActions.contains(nextInTrace)) return nextInTrace;
    else return availableActions.get(randomGenerator.nextInt(availableActions.size()));
  }
  // endregion
}
