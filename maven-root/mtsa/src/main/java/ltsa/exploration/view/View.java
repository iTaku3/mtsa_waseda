...
public class View {
  ...
  public View(CompactState[] components, ViewNextConfiguration[] nextConfigurations) {
    ...
    for (int i = 0; i < components.length; i++) this.currentStates[i] = 0;
  }
  ...
  public HashSet<String> getCurrentStateActions() {
    ...
    for (int i = 1; i < this.components.length; i++)
      for (String anAction : this.getCurrentStateActions(i)) currentStateActions.add(anAction);
  ...
  public HashSet<String> getCurrentStateAvailableActions(int component) {
    ...
    for (String anAction : currentStateActions)
      if (!lockedActions.contains(anAction)) currentStateAvaibleActions.add(anAction);

    return currentStateAvaibleActions;
  }

  public Pair<Integer, HashSet<String>> executeControllable(
      String nextAction, int componentNumber) {
    if (!nextAction.equals("WAIT"))
      if (this.moveComponent(componentNumber, nextAction))
        return new Pair<>(
            currentStates[componentNumber], this.getCurrentStateActions(componentNumber));

    return null;
  }

  public Pair<String, Pair<Integer, HashSet<String>>> executeUncontrollable(int componentNumber) {
    String nextUncontrollableAction =
        this.nextConfigurations[componentNumber].nextAction(
            new ArrayList<>(this.getCurrentStateAvailableActions(componentNumber)));
    this.moveComponent(componentNumber, nextUncontrollableAction);
    return new Pair<>(
        nextUncontrollableAction,
        new Pair<>(currentStates[componentNumber], this.getCurrentStateActions(componentNumber)));
  }
  ...
  private HashSet<String> getCurrentStateActions(int component) {
    int[] currentStateEvents =
        this.components[component].states[this.currentStates[component]].getEvents();
  ...
  private Boolean moveComponent(Integer component, String nextAction) {
    ...
    for (String anAction : this.components[component].alphabet)
      if (anAction.equals(nextAction)) isInAlphabet = true;

    if (!isInAlphabet) return false;
    ...
    for (Integer anEvent :
        this.components[component].states[this.currentStates[component]].getEvents())
      if (anEvent == nextEvent) isInState = true;

    if (!isInState) return false;

    int nextState =
        this.components[component].states[this.currentStates[component]].getNext(nextEvent);
