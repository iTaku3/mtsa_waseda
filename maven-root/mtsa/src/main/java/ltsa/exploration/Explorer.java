...
public class Explorer {
  ...
  public String getMTSControlProblemAnswer() {
    Synthesis synthesis =
        new Synthesis(this.knowledge.cloneForSynthesisFromCurrentState(), this.goal.copy());
    ...
    if (mtsControlProblemAnswer.equals("NONE")) {
      ...
      if (!mtsControlProblemAnswer.equals("NONE")) return "RESET";
    }
  ...
  public String[] getAviableActions() {
    HashSet<String> currentStateAvailableControllableActions =
        this.view.getCurrentStateAvailableActions(0);
  ...
  private void execute(String anAction) {
    ...
    for (int i = 0; i < this.view.getComponents().length; i++) {
      ...
      if (viewNextState != null)
        this.executeInModelAndKnowledge(
            i, anAction, viewNextState.getFirst(), viewNextState.getSecond());
      if (i == 0) this.updateTracesControllable(anAction, this.knowledge.getCurrentStates()[i]);
    }

    // Uncontrollable actions execution
    for (int i = 1; i < this.view.getComponents().length; i++) {
      Pair<String, Pair<Integer, HashSet<String>>> viewComponentNextState =
          this.view.executeUncontrollable(i);
  ...
  private void executeInModelAndKnowledge(
      Integer componentNumber,
      String nextAction,
      Integer nextViewState,
      HashSet<String> stateActions) {
  ...
  private void updateTracesControllable(String nextAction, int nextState) {
    ...
    if (!nextAction.equals("WAIT")) this.traceStatesCurrentStrategy.add(nextState);
  }
  ...
  private void updateKnowledgeFromCurrentStateActions(
      Integer componentNumber, HashSet<String> stateActions) {
