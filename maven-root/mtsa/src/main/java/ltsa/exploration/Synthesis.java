...
public class Synthesis {
  ...
  public Synthesis(CompactState[] mts, ControllerGoal<String> goal_original) {
    ...
    for (String anAction : goal.getControllableActions())
      if (anAction.contains("[")) throw new UnsupportedOperationException("Goal corrupted");
    ...
    for (int i = 1; i < mts.length; i++)
      for (String anAction : mts[i].alphabet)
        if (!anAction.contains("?")
            && !anAction.contains("tau")
            && !controllableActions.contains(anAction)
            && !uncontrollableActions.contains(anAction)) uncontrollableActions.add(anAction);
    ...
    if (needOrder) {
      ...
      order.name = "ORDER";
      order.alphabet =
          Arrays.copyOf(allActions.toArray(new String[allActions.size()]), allActions.size());
      order.maxStates = mts[0].states.length - 1;
      if (order.maxStates < 2) order.maxStates = 2;
