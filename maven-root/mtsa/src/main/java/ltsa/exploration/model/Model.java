  ...
  public EventState execute(int componentNumber, String nextViewAction) {
    if (nextViewAction.equals("WAIT"))
      return EventState.copy(
          this.components[componentNumber].states[this.currentStates[componentNumber]]);

    String nextAction = nextViewAction;

    int event = -1;
    for (int i = 0; i < this.components[componentNumber].alphabet.length; i++)
      if (nextAction.equals(this.components[componentNumber].alphabet[i])) event = i;
      ...
      for (int anEvent : events) {
        if (event == anEvent) {
          this.currentStates[componentNumber] =
              this.components[componentNumber].states[this.currentStates[componentNumber]].getNext(
                  event);
          return EventState.copy(
              this.components[componentNumber].states[this.currentStates[componentNumber]]);
        }
