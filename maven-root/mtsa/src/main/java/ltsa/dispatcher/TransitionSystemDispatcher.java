...
/**
 * This class consists exclusively of static methods that operate on or return CompactState,
 * CompositeState and MTS.
 *
 * <p>In the MTSA architecture this class is the communication layer between the graphical
 * interface, LTSA Core and MTSACore. None of them communicates each other directly, they do it
 * through this class.
 */
public class TransitionSystemDispatcher {
  ...
  public static CompactState getOptimisticModel(CompactState compactState, LTSOutput output) {
    ...
    output.outln(
        "MTS Representation: Optimistic operator applied to "
  ...
  /**
   * Given a CompositeState model this method builds the optimistic version of it. If the
   * composition field of compositeState parameter it's not null then it's replaced for its
   * optimistic version. Else every compactState inside compositeState parameter it's replaced for
  ...
  public static void makeOptimisticModel(CompositeState compositeState, LTSOutput ltsOutput) {
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      if (compositeState.getComposition() == null) {
        long initialTime = System.currentTimeMillis();
        compositeState.setMachines(
            TransitionSystemDispatcher.getOptimistModels(
                compositeState.getMachines(), new EmptyLTSOuput()));
        ltsOutput.outln(
            "MTS Representation: Optimistic model generated for all automatas in: "
                + (System.currentTimeMillis() - initialTime)
                + "ms.");
      } else {
        compositeState.setMachines(
        ...
        compositeState.setComposition(
            TransitionSystemDispatcher.getOptimisticModel(
                compositeState.getComposition(), ltsOutput));
      }
    }
  }
    ...
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      if (compositeState.getComposition() == null) {
        if (MTSUtils.isMTSRepresentation(compositeState)) {
          ...
          ltsOutput.outln(
              "MTS Representation: Pessimistic model generated for all automatas in: "
                  + (System.currentTimeMillis() - initialTime)
                  + "ms.");
        }
        ...
        long initialTime = System.currentTimeMillis();
        compositeState.setComposition(
            TransitionSystemDispatcher.getPessimistModel(compositeState.getComposition()));
        ltsOutput.outln(
            "MTS Representation: Pessimistic model generated for composition in: "
                + (System.currentTimeMillis() - initialTime)
                + "ms.");
      }
    }
  ...
  private static void checkSafetyOrDeadlock(
      boolean checkDeadlocks, CompositeState compositeState, LTSOutput ltsOutput) {
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      ...
      if (hasCompositionDeadlockFreeImplementations(compositeState, ltsOutput)) {
        ...
        ltsOutput.outln(
            "*****************************************************************************************");
        ltsOutput.outln(
            "Model must have at least one deadlock free implementation for a Safety or Deadlock"
                + " check.");
        ltsOutput.outln(
            "*****************************************************************************************");
      }
  ...
  /**
   * This method applies composition over the parameter model. If the <code>toCompose</code> model
   * it's a LTS, then it can only be composed using parallel composition, it means that the
   * compositionType field of <code>toCompose</code> should be set to Symbol.OR. If <code>toCompose
   * </code> is an MTS then there are different kinds of model composition might be applied
   * depending on the value of the compositionType field.
   *
   * @param toCompose composite model to be composed
   * @param ltsOutput used for process output
   */
  public static void applyComposition(
      CompositeState toCompose, LTSOutput ltsOutput, Statistics... stats) {
  ...
  private static void applyUpdatingController(CompositeState toCompose, LTSOutput output) {
    if (!(toCompose instanceof UpdatingControllerCompositeState)) {
      output.outln(
          "MTSA tool is trying to solve an updatingController problem but the CompositeState given "
              + "is not appropiated");
    }
    UpdatingControllerSynthesizer.generateController(
        (UpdatingControllerCompositeState) toCompose, output);
  }
  ...
  private static void merge(CompositeState composition, LTSOutput ltsOutput) {
    if (composition.getComposition() == null) {
      ...
      if (toCompose.size() > 2) {
        ltsOutput.outln(
            "Warning: Merge is being applied to more than two models. Pair-wise merging will be"
                + " performed. See [FM06] for details on associativity of merge");
      }
      ...
      while (i < toCompose.size()) {
        ...
        try {
          ...
          if (CollectionUtils.isEqualCollection(mtsB.getActions(), mtsA.getActions())) {
            ...
            ltsOutput.outln(
                "Try merging them on their common alphabet. If they are still inconsistent,");
        ...
        ltsOutput.outln(
            "Merge operator applied in " + (System.currentTimeMillis() - initialTime) + "ms.");

        Set<String> alphabetAminusB =
            new HashSet<String>(CollectionUtils.subtract(mtsA.getActions(), mtsB.getActions()));
        Set<String> alphabetBminusA =
            new HashSet<String>(CollectionUtils.subtract(mtsB.getActions(), mtsA.getActions()));
      ...
      composition.setComposition(
          MTSToAutomataConverter.getInstance().convert(mts, composition.name));
    }
  }
  ...
  private static void mdpComposeAbstraction(CompositeState toCompose, LTSOutput ltsOutput) {
    ...
    if (toCompose.machines.size() >= 2) {
      for (int i = 1; i < toCompose.machines.size(); i++) {
        ...
        BinaryRelation<Long, Long> simulation =
            new LTSSimulationSemantics().getLargestRelation(mts1, mts2);
        if (simulation.size() == 0) ltsOutput.outln("WARNING: simulation is empty");
        ...
        toCompose.composition =
            MDPToAutomataConverter.getInstance().convert(lastMachine, toCompose.getName());
      }
    } else {
   ...
   */
  private static void mdpComposeEnactment(
      CompositeState toCompose, List<String> controlledActions, LTSOutput ltsOutput) {
    long initialTime = System.currentTimeMillis();
    ...
    if (toCompose.machines.size() >= 2) {
      for (int i = 1; i < toCompose.machines.size(); i++) {
        ...
        for (Long s : mdp2.getStates()) {
          ...
          for (String lab : labels) mdp2.addStateLabel(s, lab);
        }
  ...
  /**
   * Applies plus CR merge operator over the models in <code>machines</code> field of <code>
   * compositeState</code>. The result of +CR is set to the <code>composition</code> field of <code>
   * compositeState</code> .
   *
   * @param compositeState
   * @param ltsOutput
  ...
  public static void applyPlusCROperator(CompositeState compositeState, LTSOutput ltsOutput) {
    if (compositeState.getComposition() == null) {
      ...
      ltsOutput.outln(
          "+CR operator applied in " + (System.currentTimeMillis() - initialTime) + "ms.");
      compositeState.setComposition(
          MTSToAutomataConverter.getInstance().convert(merge, compositeState.name));
      ltsOutput.outln(""); // leave an empty line
  ...
  public static void applyPlusCAOperator(CompositeState compositeState, LTSOutput ltsOutput) {
    if (compositeState.getComposition() == null) {
      ...
      initialTime = System.currentTimeMillis();
      MTS<Long, String> merge =
          new MTSMultipleComposer<Long, String>(plusCARulesApplier).compose(toCompose);
  ...
  /**
   * Applies determinisation to <code>composition</code> parameter. The determinisation semantics
   * depends on the model type. If <code>compositeState</code> it's an MTS then MTS semantics is
   * applied, otherwise LTS is applied. If <code>compositeState</code> it's an MTS then before
   * applying determinisation, composition is applied.
   *
   * <p>The result of determinisation is setted to <code>composition</code> field of <code>
   * compositeState</code>/
   *
   * @param compositeState
   * @param ltsOutput
  ...
  /**
   * Applies determinisation over <code>lts</code> depending on the model type it can be used LTS or
   * MTS semantic.
   *
   * @param lts
  ...
  public static CompactState determinise(CompactState lts, LTSOutput ltsOutput) {
    ...
    if (MTSUtils.isMTSRepresentation(compactState)) {
      ...
      return MTSToAutomataConverter.getInstance()
          .convert(mts, compactState.name, MTSUtils.isMTSRepresentation(compactState));
    } else {
  ...
  public static void monolithicDirectorSynthesis(
      CompositeState compositeState, final LTSOutput output, Statistics... stats) {
    ControllerGoal<String> goal = compositeState.goal;

    if (!goal.getGuarantees().isEmpty() || !goal.getAssumptions().isEmpty()) {
      output.outln(
          "monolithic director implements a nonblocking director for marked states, not goals and"
    ...
    MarkedWithIllegalLTSImpl<Long, String> plant =
        new MarkedWithIllegalLTSImpl<>(lts.getInitialState());
    ...
    DirectedControllerSynthesisMonolithicDirector<Long, String> dcs =
        new DirectedControllerSynthesisMonolithicDirector<Long, String>();
    ...
    if (controller != null) {
      CompactState compState =
          MTSToAutomataConverter.getInstance()
              .convert(new MTSAdapter<Long, String>(controller), compositeState.getName());
      ...
      output.outln(
          "There is no controller for model " + compositeState.name + " for the given setting.");
      output.outln(statistics.toString());
    }
  }

  public static void partialOrderReductionSynthesis(
      CompositeState compositeState, final LTSOutput output, Statistics... stats) {
    ControllerGoal<String> goal = compositeState.goal;

    if (!goal.getGuarantees().isEmpty() || !goal.getAssumptions().isEmpty()) {
      output.outln(
          "partial order reduction director implements a nonblocking director for marked states,"
              + " not goals andassumptions as in GR1");
      return;
    }
    ...
    for (CompactState automata : compositeState.getMachines()) {
      ...
      MarkedWithIllegalLTSImpl<Long, String> markedWithIllegalLTS =
          new MarkedWithIllegalLTSImpl<>(lts.getInitialState());
    ...
    Set<MTSSynthesis.ar.dc.uba.model.language.Symbol> initiating = new HashSet<>(),
        terminating = new HashSet<>();
    for (String action : actions)
      (goal.getMarking().contains(action) ? initiating : terminating).add(new SingleSymbol(action));
    formulaLTS =
        ftm.translate(
            new FluentPropositionalVariable(
                new FluentImpl("Goal", initiating, terminating, false)));
    MarkedWithIllegalLTSImpl<Long, String> markedWithIllegalLTS =
        new MarkedWithIllegalLTSImpl<>(formulaLTS.getInitialState());
    ...
    DirectedControllerSynthesisPartialOrderReduction<Long, String> dcs =
        new DirectedControllerSynthesisPartialOrderReduction<Long, String>();
  ...
  public static void hcs(
    ...
    if (!(goal.getGuarantees().isEmpty() ^ goal.getMarking().isEmpty())) {
      output.outln(
          "Marking or liveness requirements (only one of them) are required for heuristic"
              + " analysis.");
      return;
    }
    if (!goal.getMarking().isEmpty() && goal.getAssumptions().size() > 1) {
      output.outln(
          "Multiple assumptions are not supported by the heuristic analysis when using marking as"
              + " goals.");
      return;
    }
    ...
    HashMap<Integer, Integer> guarantees =
        new HashMap<>(); // HashMap < guaranteeNumber, ltsIndexInLtss >
    HashMap<Integer, Integer> assumptions =
        new HashMap<>(); // HashMap < assumptionNumber, ltsIndexInLtss >
    ...
    if (!goal.getGuarantees().isEmpty()) { // blocking
      ...
      Set<MTSSynthesis.ar.dc.uba.model.language.Symbol> initiating = new HashSet<>(),
          terminating = new HashSet<>();
      for (String action : actions)
        (goal.getMarking().contains(action) ? initiating : terminating)
            .add(new SingleSymbol(action));
      formulaLTS =
          ftm.translate(
              new FluentPropositionalVariable(
                  new FluentImpl("Goal", initiating, terminating, false)));
    ...
    if (!goal.getGuarantees().isEmpty()
        && goal.getFluents().stream().anyMatch(f -> f.getName().equals("tick_a"))) {
      // remove potential unwanted "*" actions related to "tick". This is a hack to optimize cases
      // translated from synchronous systems.
      ...
      output.outln(
          "CAREFUL: when using the \"tick\" action as an event while modeling synchronoussystems,"
              + " do not use other actions as events, since to optimize \"tick\", other actionswill"
              + " ignore actions of the plant not in the LTL_property.");
    }
    ...
    plantActions.remove(
        "tau"); // tau represents internal computation, it doesn't matter for these checks
    ...
    if (!plantActions.containsAll(propertiesActions)) {
      output.outln(
          "Assumptions and Guarantees cant include actions that are not permited by the plant");
  ...
  public static boolean isLTSRefinement(
      CompactState refines, CompactState refined, LTSOutput output) {
  ...
  /*stats is meant to be modified with relevant data to output*/
  public static DirectedControllerSynthesisNonBlocking<Long, String> hcsInteractive(
      CompositeState compositeState, final LTSOutput output, Statistics... stats) {
    ControllerGoal<String> goal = compositeState.goal;
    ...
    DirectedControllerSynthesisNonBlocking<Long, String> dcs =
        new DirectedControllerSynthesisNonBlocking<>();
  ...
  /**
   * Checks if <code>refines</code> model it's a refinement of refined <code>semantic</code> as the
   * semantic for the refinement check.
   *
   * @param refines
   * @param refined
   * @param semantic
  ...
  public static boolean isRefinement(
      CompactState refines, CompactState refined, SemanticType semantic, LTSOutput ltsOutput) {

    Refinement refinement = semantic.getRefinement();
    MTS<Long, String> refinedMTS =
        AutomataToMTSConverter.getInstance().convert((CompactState) refined);
    MTS<Long, String> refinesMTS =
        AutomataToMTSConverter.getInstance().convert((CompactState) refines);
    return isRefinement(refinesMTS, refines.name, refinedMTS, refined.name, refinement, ltsOutput);
  }

  /**
   * Returns <code>refinement relation</code> between refines and refined using <code>semantic
   * </code> as the semantic for the refinement check.
   *
   * @param refines
   * @param refined
   * @param semantic
   ...
   * @return
   */
  public static BinaryRelation<?, ?> getRefinement(
      CompactState refines, CompactState refined, SemanticType semantic, LTSOutput ltsOutput) {
  ...
  /**
   * Checks if <code>refines</code> model is a refinement of <code>refined</code> using <code>
   * refinement</code> parameter as the refinement notion
   *
   * @param refines
   * @param refinesName
   * @param refined
   ...
   */
  public static <A> boolean isRefinement(
      MTS<?, A> refines,
      String refinesName,
      MTS<?, A> refined,
      String refinedName,
  ...
  public static <A> BinaryRelation<?, ?> getRefinement(
      MTS<?, A> refines,
      String refinesName,
      MTS<?, A> refined,
      String refinedName,
  ...
  public static <A> boolean isWeaklyBisimilar(
      MTS<?, A> refines,
      String refinesName,
      MTS<?, A> refined,
      String refinedName,
      LTSOutput ltsOutput) {
    return isRefinement(
        refines, refinedName, refined, refinedName, SemanticType.WEAK.getRefinement(), ltsOutput);
  }

  public static <A> boolean isStronglyBisimilar(
      MTS<?, A> refines,
      String refinesName,
      MTS<?, A> refined,
      String refinedName,
      LTSOutput ltsOutput) {
    return isRefinement(
        refines, refinedName, refined, refinedName, SemanticType.STRONG.getRefinement(), ltsOutput);
  }

  public static boolean areConsistent(
      CompactState csA, CompactState csB, SemanticType semantic, LTSOutput ltsOutput) {
  ...
  public static <A> boolean areConsistent(
      MTS<?, A> mtsA,
      String mtsAName,
      MTS<?, A> mtsB,
      String mtsBName,
    ...
    if (!areConsistent && semantic == SemanticType.WEAK_ALPHABET) {
      if (!CollectionUtils.isSubCollection(mtsB.getActions(), mtsA.getActions())
        ...
        ltsOutput.outln(
            "Try checking consistency on their common alphabet. If they are inconsistent,");
  ...
  /**
   * Applies minimisation to <code>composition</code> parameter. The minimisation semantics depends
   * on the model type. If <code>compositeState</code> it's an MTS then MTS semantics is applied,
   * otherwise LTS is applied. If <code>compositeState</code> it's an MTS then before applying
   * minimisation, composition is applied.
   *
   * <p>The result of minimisation is setted to <code>composition</code> field of <code>
   * compositeState</code>/
   *
   * @param compositeState
   * @param ltsOutput
  ...
  public static void minimise(CompositeState compositeState, LTSOutput ltsOutput) {
    ...
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      Validate.isTrue(
          compositeState.getComposition() != null,
  ...
  private static MTS<Long, String> mtsMinimise(CompactState compactState, LTSOutput ltsOutput) {
    ...
    ltsOutput.outln(
        compactState.name + " minimised in " + (System.currentTimeMillis() - initialTime) + "ms.");

    // minimisation sanity check
    ltsOutput.outln(
        "Internal sanity check: Validating minimised and original are equivalent by simulation...");
  ...
  /**
   * Minimise <code>compactState</code> model using MTS or LTS semantic depending on the model type.
   *
   * @param compactState
   * @param ltsOutput
   * @return
  ...
  /**
   * Returns a vector with the optimistic representation of every model in the <code>
   * originalMachines</code> parameter.
   *
   * @param originalMachines
   * @param output
   * @return optimistic version of the original models.
   */
  private static Vector<CompactState> getOptimistModels(
      Vector<CompactState> originalMachines, LTSOutput output) {
  ...
  /**
   * Returns a vector with the pessimistic representation of every model in the <code>
   * originalMachines</code> parameter.
   *
   * @param originalMachines
  ...
  private static void checkSafety(
      CompositeState compositeState, LTSOutput ltsOutput, boolean checkDeadlocks) {
    ...
    if (compositeState.getErrorTrace() == null || compositeState.getErrorTrace().isEmpty()) {
      ...
      printLine(
          ...
          ltsOutput);
      printLine(
          "NO ERRORS FOUND: All implementations of "
              + compositeState.name
              + " do not have errors."
              + reference,
          ltsOutput);
      ...
      printLine(
          "This means that some implementations of " + compositeState.name + " have errors.",
          ltsOutput);
      printLine("", ltsOutput);
      ...
      if (!MTSUtils.isEmptyMTS(pessimisticModel)) {
        ...
        if (compositeState.getErrorTrace() == null || compositeState.getErrorTrace().isEmpty()) {
          ...
          printLine("*****************************************************************", ltsOutput);
          printLine(
              "Model " + compositeState.name + " has some implementations with errors." + reference,
              ltsOutput);
          printLine("*****************************************************************", ltsOutput);
        } else {
          // M- !|= FI
          ...
          printLine("*****************************************************************", ltsOutput);
          printLine(
              "All implmentations of  " + compositeState.name + " have errors." + reference,
              ltsOutput);
          printLine("*****************************************************************", ltsOutput);
        }
      } else {
        ...
        CompactState property =
            TransitionSystemDispatcher.buildBuchiFromProperty(compositeState, ltsOutput);
        ...
        if (compositeState.getErrorTrace() == null || compositeState.getErrorTrace().isEmpty()) {
          ...
          printLine("*****************************************************************", ltsOutput);
          printLine(
              "Model " + compositeState.name + " has implementations with errors." + reference,
              ltsOutput);
          printLine("*****************************************************************", ltsOutput);
        }
      }
    ...
    printLine("*******************************************", ltsOutput);
    printLine(
        "Total safety analysis time: " + (System.currentTimeMillis() - initialCurrentTimeMillis),
        ltsOutput);
    printLine("*******************************************", ltsOutput);
  ...
  /**
   * Returns true if the <code>compositeState</code> parameter has any deadlock free implementation.
   *
   * @param compositeState
   * @param ltsOutput
   */
  public static boolean hasCompositionDeadlockFreeImplementations(
    ...
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      ...
      if (deadlockStatus == 1) {
        String output =
            "All implementations of " + compositeState.name + " have a deadlock state." + reference;
        ltsOutput.outln(output);

        return false;
      } else if (deadlockStatus == 2) {
        String output =
            "All implementations of " + compositeState.name + " are deadlock free." + reference;
        ltsOutput.outln(output);
      } else {
        String output =
   ...
   * . If <code>compositeState</code> is an LTS, then the traditional LTS model checking algorithm
   * is applied. Otherwise MTS model checking algorithm is applied.
   *
   * @param compositeState
   * @param ltlProperty
   ...
   */
  @SuppressWarnings("unchecked")
  public static void checkFLTL(
      CompositeState compositeState,
      CompositeState ltlProperty,
      CompositeState not_ltl_property,
      boolean fairCheck,
      LTSOutput ltsOutput) {
    ...
    if (MTSUtils.isMTSRepresentation(compositeState)) {
      ...
      if (hasCompositionDeadlockFreeImplementations(compositeState, ltsOutput)) {
        ...
        printLine(" ", ltsOutput);
        printLine(
            "Phase I: Does " + compositeState.name + "+ satisfy " + ltlProperty.name + "?",
            ltsOutput);
        ...
        if (compositeState.getErrorTrace() == null || compositeState.getErrorTrace().isEmpty()) {
          ...
          printLine("*****************************************************************", ltsOutput);
          printLine(
              "All deadlock-free implementations of "
                  + compositeState.name
                  + " satisfy "
                  + ltlProperty.name
                  + " "
          ...
          printLine(" ", ltsOutput);
          printLine(
              "No. " + compositeState.name + "+ does not satisfy " + ltlProperty.name, ltsOutput);
          printLine(
          ...
          if (!MTSUtils.isEmptyMTS(pessimisticModel)) {
            ...
            printLine(" ", ltsOutput);
            printLine(
                "Phase II: Does " + compositeState.name + "- satisfy " + ltlProperty.name + "?",
                ltsOutput);
            ...
            if (compositeState.getErrorTrace() == null
              ...
              printLine(
                  "*****************************************************************", ltsOutput);
              printLine(
                  "No deadlock-free implementation of "
                      + compositeState.name
                      + " satisfies "
                      + ltlProperty.name
                      + " "
            ...
            printLine(
                ...
                ltsOutput);
            printLine(
                "Does " + compositeState.name + "+ satisfy " + not_ltl_property.name + "?",
                ltsOutput);
      ...
      if (compositeState.makeController
          || compositeState.checkCompatible
          || compositeState.makeRTCController
   ...
   */
  private static void checkControllerFLTL(
      CompositeState compositeState, CompositeState ltlProperty, LTSOutput ltsOutput) {
    if (compositeState.getComposition() == null) {
      applyComposition(compositeState, ltsOutput);
    }
  ...
  /**
   * Builds the <b>abstract</b> version of the original model. The result is builded following the
   * following procedure: <br>
   * Firstly a state (called trap state) with loop transitions on every label in the alphabet is
   * added to the abstract model. Then from every state in <code>compactState</code> and for every
   * label in <code>compactState</code>s alphabet for which there are no outgoing transition from
   * it, it is added one transition to trap state.
   *
   * @param compactState
   * @param output
   * @return
  ...
  /**
   * Sets the result of <code>getAbstractModel</code> method applied to composition field of <code>
   * compositeState</code> to the composition field again.
   *
   * @param compositeState
   * @param output
   */
  ...
  /**
   * Computes the transitive tau-closure for compactState. It propagates may transitions.
   *
   * @param compactState
   * @param output
   * @return
  ...
  /**
   * Sets the result of <code>getClousureModel</code> method applied to composition field of <code>
   * compositeState</code> to the composition field again.
   *
   * @param compositeState
   * @param output
  ...
  public static void makeClosureModel(CompositeState compositeState, LTSOutput output) {
    long initialTime = System.currentTimeMillis();
    if (compositeState.getComposition() != null) {
      CompactState tauClosure =
          TransitionSystemDispatcher.getTauClosure(compositeState.getComposition(), output);
      compositeState.setComposition(tauClosure);
      output.outln(
          "Clousure model generated for "
              + compositeState.name
              + " in: "
              + (System.currentTimeMillis() - initialTime)
              + "ms.");
  ...
  /**
   * For every state in which are choices transform those transitions to maybe transitions.
   *
   * @param compactState
   * @param output
   * @return
  ...
  public static CompactState makeMTSConstraintModel(CompactState compactState, LTSOutput output) {
    ...
    output.outln(
        "Constrained model generated for "
            + compactState.name
            + " in: "
            + (System.currentTimeMillis() - initialTime)
            + "ms.");
  ...
  public static void checkCompatible(CompositeState compositeState, LTSOutput output) {
    ...
    if (composition != null) {
      ...
      ControllerSynthesisFacade<Long, String, Integer> instance =
          new ControllerSynthesisFacade<Long, String, Integer>();
  ...
  public static void makePlant(CompositeState compositeState, LTSOutput output) {
    ...
    if (composition != null) {
      if (compositeState.goal != null) {
        ...
        compositeState.setComposition(
            MTSToAutomataConverter.getInstance().convert(plant, compositeState.getName()));
      } else {
  ...
  public static void makeControlledDeterminisation(
      CompositeState compositeState, LTSOutput output) {
    CompactState composition = compositeState.getComposition();
    if (composition != null) {
      ControllerGoal<String> goal = compositeState.goal;
      MTS<Long, String> env = AutomataToMTSConverter.getInstance().convert(composition);
      IsDeterministicMTSPredicate<Long, String> pred =
      ...
      if (!pred.evaluate(env)) {
        if (goal != null) {
          MTS<Pair<Set<Long>, String>, String> controlledDet =
              ControllerUtils.getControllableDeterminisationFor(
                  env, goal.getControllableActions(), output);
          IsDeterministicMTSPredicate<Pair<Set<Long>, String>, String> pred2 =
              new IsDeterministicMTSPredicate<Pair<Set<Long>, String>, String>(
                  TransitionType.POSSIBLE);
          if (!pred2.evaluate(controlledDet)) {
            Diagnostics.fatal("Controlled determinisation error.");
          }
          MTS<Long, String> det =
              new GenericMTSToLongStringMTSConverter<Pair<Set<Long>, String>, String>()
                  .transform(controlledDet);
          compositeState.setComposition(
              MTSToAutomataConverter.getInstance().convert(det, composition.getName()));
        } else {
    ...
    if (composition != null) {
      if (compositeState.goal != null) {
        CompactState synthesiseController =
            synthesiseRTCController(compositeState, compositeState.goal, output);
  ...
  public static CompactState synthesiseRTCController(
      CompositeState compositeState, ControllerGoal<String> goal, LTSOutput output) {
    ...
    if (c.hasTau()) {
      ...
      if (c.isNonDeterministic()) output.outln("Solving the nondeterministic control problem.");
    }
    ...
    LTS<StrategyState<Long, Integer>, String> synthesised =
        new ControllerSynthesisFacade<StrategyState<Long, Integer>, String, Integer>()
    ...
    if (synthesised == null) {
      ...
      MTSAdapter<StrategyState<Long, Integer>, String> mtsAdapter = new MTSAdapter<>(synthesised);
      ...
      output.outln("Analysis time: " + (System.currentTimeMillis() - initialTime) + "ms.");
      output.outln(
          "Controller [" + plainController.getStates().size() + "] generated successfully.");
  ...
  public static void synthesiseRTCAnalysisController(
      CompositeState compositeState, LTSOutput output) {
    CompactState composition = compositeState.getComposition();
    if (composition != null) {
      if (compositeState.goal != null) {
        ...
        if (c.hasTau()) {
          ...
          if (c.isNonDeterministic()) output.outln("Solving a nondeterministic control problem.");
        }
        ...
        compositeState
            ...
            .removeIf(
                (c1) ->
                    recorder.getRecords().stream()
                        .anyMatch((c2) -> c1.getName().equals(c2.getName())));
        compositeState.getMachines().addAll(recorder.getRecords());

        // process the output
        if (synthesised == null) {
          output.outln(
              "There is no controller for model "
                  + compositeState.name
    ...
    if (composition != null) {
      if (compositeState.goal != null) {
        ...
        if (synthesiseController != null) {
          ...
          // compositeState.setComposition(synthesiseController);
          compositeState.machines.set(
              0, synthesiseController); // replace environment with controller
          compositeState.compose(output);
  ...
  public static void synthesiseGRNoText(CompositeState compositeState, LTSOutput output) {
    ...
    if (composition != null) {
      if (compositeState.goal != null) {
        CompactState synthesiseController =
            synthesiseGRNoText(compositeState, compositeState.goal, output);
  ...
  private static CompactState applyLatencyHeuristic(
    ...
    ControllerGoal<String> goal = compositeState.goal;
    boolean PRUNNING_HEURISTIC_DEFINED =
        goal.isNonTransient() || !goal.getConcurrencyFluents().isEmpty();
    if (PRUNNING_HEURISTIC_DEFINED || goal.isReachability()) {
      ...
      HeuristicControllerSynthesiser<Long, String> synthesiser =
          new HeuristicControllerSynthesiser<Long, String>();
      MTS<Long, String> plainController;
      ...
      synthesiseController =
          MTSToAutomataConverter.getInstance().convert(plainController, compositeState.getName());
    }
    return synthesiseController;
  }

  public static CompactState synthesise(
      CompositeState compositeState, ControllerGoal<String> goal, LTSOutput output) {
    ...
    if (!isMTS && (c.isNonDeterministic() || c.hasTau())) {
      ...
      if (goal.isNonBlocking()) {
        ...
        LTS<StrategyState<Set<Long>, Integer>, String> synthesisResult =
            new ControllerSynthesisFacade<StrategyState<Set<Long>, Integer>, String, Integer>()
                .synthesiseController(cp);
        if (synthesisResult != null) {
          synthesised = new MTSAdapter<StrategyState<Set<Long>, Integer>, String>(synthesisResult);
          if (synthesised != null && c.hasTau()) TauRemoval.muRemoval(synthesised, "-2");
        }
      } else {
        ...
        if (synthesiseController != null) {
          MTS<Long, String> temp =
              AutomataToMTSConverter.getInstance().convert(synthesiseController);
          TraceInclusionClosure.getInstance().applyMTSClosure(temp, Collections.singleton("-1"));
          if (c.hasTau()) TauRemoval.muRemoval(temp, "-2");
      ...
      if (synthesised == null) {
        ...
        MTS<Long, String> plainController =
            new GenericMTSToLongStringMTSConverter<StrategyState<Set<Long>, Integer>, String>()
        ...
        CompactState compactState =
            MTSToAutomataConverter.getInstance()
                .convert(plainController, compositeState.getName(), isMTS);
      ...
      if (goal.getBuchi().size() > 0) {
        ...
        ControllerSynthesisFacade<StrategyState<Long, Integer>, String, Integer> facade =
            new ControllerSynthesisFacade<>();
        ...
        if (synthesised == null) {
          ...
          recordControllerStateLabels(
              transformer.getStateMapping(), facade.getGame(), goal); // needed
          // for
          // MDP
          // translation
      ...
      if (c.isNonDeterministic()) {
        Diagnostics.fatal(
            "The domain model [" + compositeState.getName() + "] must be deterministic [FM-2012].");
      }
      ...
      MTS<Long, String> plant =
          new GenericMTSToLongStringMTSConverter<Pair<Long, Set<String>>, String>()
              .transform(envStar);
      ...
      MTS<StrategyState<Long, Integer>, String> synthesised =
          new ControllerSynthesisFacade<Long, String, Integer>().synthesiseController(plant, goal);
  ...
  public static CompactState synthesiseGRNoText(
      CompositeState compositeState, ControllerGoal<String> goal, LTSOutput output) {
    ...
    if (!isMTS && (c.isNonDeterministic() || c.hasTau())) {
      ...
      if (goal.isNonBlocking()) {
        ...
        env = ControllerUtils.embedFluents(env, goal, output);
        NondetControlProblem<Long, String> cp = new NondetControlProblem<Long, String>(env, goal);
        ControllerSynthesisFacade<StrategyState<Set<Long>, Integer>, String, Integer> facade =
            new ControllerSynthesisFacade<>();
        LTS<StrategyState<Set<Long>, Integer>, String> synthesisResult =
            facade.synthesiseController(cp);
        if (synthesisResult != null) {
          synthesised = new MTSAdapter<StrategyState<Set<Long>, Integer>, String>(synthesisResult);
          if (synthesised != null && c.hasTau()) TauRemoval.muRemoval(synthesised, "-2");
        }
      ...
      ControllerSynthesisFacade<Long, String, Integer> facade =
          new ControllerSynthesisFacade<Long, String, Integer>();
      MTS<StrategyState<Long, Integer>, String> synthesised =
          facade.synthesiseController(plant, goal);
  ...
  private static void recordControllerStateLabels(
      Map<StrategyState<Long, Integer>, Long> mapping,
      GRGame<Long> lastGame,
    ...
    for (StrategyState<Long, Integer> ss : mapping.keySet()) {
      //            //System.out.println("Environment's " + ss.getState() + " is controller's " +
      // mapping.get(ss));
    ...
    // reason
    {
      for (Long s : actualMapping.keySet()) // controller states
      {
        ...
        // to
        {
          ...
          String formula =
              (goal.getAssumptions().size() >= j
                  ? goal.getAssumptions().get(j - 1).toString()
                  : "assu" + j);
          labels.add(formula);
        }
      }
    ...
    for (int j = 1; j <= guarantees.size(); j++) // 1-based for some reason
    {
      for (Long s : actualMapping.keySet()) // controller states
      {
        ...
        // to
        {
          ...
          String formula =
              (goal.getGuarantees().size() >= j
                  ? goal.getGuarantees().get(j - 1).toString()
                  : "guar" + j);
          labels.add(formula);
        }
      }
  ...
  public static ltsa.lts.Animator generateAnimator(
      CompositeState compositeState, LTSOutput output, ltsa.lts.EventManager eventManager) {
  ...
  /**
   * Returns the number of machines n in the CompositeState, adding one for MTS because of the
   * capability to draw the composite
   *
  ...
  public static int numberMachinesToDraw(CompositeState compositeState) {
    ...
    // if (MTSUtils.isMTSRepresentation(compositeState)) {
    if (isComposite) return numMachines + 1;
    else return numMachines;
    // } else
    //    return numMachines;
  }

  /**
   * Tries to apply the distribution. calculatedDistributedComponents is a parameter that should be
   * an empty set. Returns true if the distribution is possible and the resulting distributed
   * components are added to calculatedDistributedComponents.
   *
   * @param systemModelCS
  ...
  public static boolean tryDistribution(
      CompactState systemModelCS,
      DistributionDefinition distributionDefinition,
      LTSOutput output,
      Collection<CompactState> calculatedDistributedComponents) {
    Validate.notNull(
        calculatedDistributedComponents,
        "A set to store the calculated components must be provided");
    ...
    try {
      componentsNameAndAlphabet =
          distributionDefinition.calculateAndGetComponentsNameAndAlphabet(output);
      ...
      AlphabetDistribution<String> alphabetDistribution =
          new AlphabetDistribution<String>(allAlphabets);

      long initialTime = System.currentTimeMillis();

      // try to distribute
      DistributionResult<Long, String> distributionResult =
          distributionFacade.tryDistribute(systemModelMTS, alphabetDistribution, MTSConstants.TAU);

      output.outln(
          "Distribution attempt finished in "
              + (System.currentTimeMillis() - initialTime)
              + "ms. Model is distributable?: "
              + distributionResult.isDistributable());

      if (distributionResult.isDistributable()) {
        Set<Entry<String, Set<String>>> componentsNameAndAlphabetEntrySet =
            componentsNameAndAlphabet.entrySet();

        Set<String> componentAlphabet;
        String componentName;
        MTSToAutomataConverter converter = MTSToAutomataConverter.getInstance();
        GenericMTSToLongStringMTSConverter<Set<Long>, String> toLongStringMTSConverter =
        ...
        for (Entry<String, Set<String>> entry : componentsNameAndAlphabetEntrySet) {
          ...
          MTS<Long, String> mTSComponentLongString =
              toLongStringMTSConverter.transform(mTSComponent);

          CompactState compactStateComponent =
              converter.convert(mTSComponentLongString, componentName);
  ...
  public static void makeComponentModel(CompositeState compositeState, LTSOutput output) {
    ...
    if (compositeState.getComposition() != null) {
      if (compositeState.getComponentAlphabet() != null) {
        compositeState.setComposition(
            getComponentModel(
                compositeState.getComposition(), compositeState.getComponentAlphabet(), output));
      } else {
   ...
   * @return
   */
  private static CompactState getComponentModel(
      CompactState composition, Vector<String> componentAlphabet, LTSOutput output) {

    MTS<Long, String> compositionMTS = AutomataToMTSConverter.getInstance().convert(composition);

    MTS<Long, String> componentMTS =
        MTSAFacade.getComponent(compositionMTS, new HashSet<String>(componentAlphabet));
      ...
      if (!MTSUtils.isMTSRepresentation(compositeState)) {
        ...
        for (Long state : states) {
          if (state != -1) {
            ...
            BinaryRelation<String, Long> transitionsFromState =
            ...
            for (String transitionLabel : posibleTransitions) {
              if (compositeState.actionsToErrorSet == null
                  || compositeState.actionsToErrorSet.contains(transitionLabel)) {
