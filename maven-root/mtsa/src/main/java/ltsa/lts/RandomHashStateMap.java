...
public class RandomHashStateMap implements StateMap, StackCheck, Iterable {
    ...
    public int compareTo(Object arg0) {
      if (arg0 instanceof RandomHashEntry) {
        ...
        if (depth < other.depth) return -1;
          ...
          else return compareKeys(key, other.key);
        }
      } else
        throw new ClassCastException(
            "Cannot compare RandomHashEntry objects with "
  ...
  public void add(byte[] state) throws UnsupportedOperationException {
    ...
    if (table[hash] != null) table[hash].prev = entry;
    table[hash] = entry;
    count++;
  ...
  // @Override
  public void add(byte[] state, int action, byte[] parent) throws UnsupportedOperationException {
    throw new UnsupportedOperationException();
  }
