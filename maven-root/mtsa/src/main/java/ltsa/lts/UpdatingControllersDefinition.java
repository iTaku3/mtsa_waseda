...
public class UpdatingControllersDefinition extends CompositionExpression {
  ...
  public Set<String> generateUpdatingControllableActions(
      ControllerGoalDefinition oldGoalDef, ControllerGoalDefinition newGoalDef) {
  ...
  protected CompositeState compose(Vector<Value> actuals) {
    ...
    // Symbol controllableSetSymbol = LTSCompiler.saveControllableSet(controllableSet,
    // this.getName().getName());
    ControllerGoal<String> grGoal =
        UpdatingControllersUtils.generateGRUpdateGoal(
            this, oldGoalDef, newGoalDef, controllableSet);
    ControllerGoalDefinition safetyGoal =
        UpdatingControllersUtils.generateSafetyGoalDef(
            this, oldGoalDef, newGoalDef, controllableSet, output);

    UpdatingControllerCompositeState ucce =
        new UpdatingControllerCompositeState(oldC, mapping, safetyGoal, grGoal, name.getName());
    return ucce;
  }
  ...
  private HashSet<String> compileSet(Vector<String> actions) {
    if (actions == null) Diagnostics.fatal("Set not defined.");
