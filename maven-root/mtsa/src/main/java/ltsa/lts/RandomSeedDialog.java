...
public class RandomSeedDialog extends JFrame {
  ...
  class ButtonListener implements ActionListener {
    ...
    public void actionPerformed(ActionEvent evt) {
      ...
      if (src.getName().equals(CANCEL_BTN)) {
        ...
        if (seed == 0) Options.setUseGeneratedSeed(false);
        else Options.setRandomSeed(mainWindow.getRandomSeed());
        mainWindow.dispose();
      }
    }
  }
  ...
  private void initialize() {
    ...
    this.getContentPane()
        .add(new JLabel("Set seed for randomization (0 to clear seed): "), constraints);

    randomSeedSpinner =
        new JSpinner(
            new SpinnerNumberModel(
                new Long(0), new Long(0), new Long(Long.MAX_VALUE), new Long(10)));
