...
public class DistributionDefinition {
   ...
   * @return
   */
  public static DistributionDefinition getDistributionDefinitionContainingComponent(
      Symbol component) {
    ...
    if (result == null) {
      throw new IllegalArgumentException(
          "Component " + component.toString() + " not found in any distribution.");
    } else {
      return result;
    }
  }
  ...
  /**
   * Calculates the alphabets from the symbols and returns the pairs: C, Alphabet. where C is the
   * component name and Alphabet as a set of string. The alphabets are obtained from the id and
   ...
   */
  public Map<String, Set<String>> calculateAndGetComponentsNameAndAlphabet(LTSOutput output)
      throws DistributionTransformationException {
    ...
    for (Entry<Symbol, Symbol> nameAndAlphabetDef : componentsNameAndAlphabetDef) {
      ...
      if (labelSet == null) {
        throw new DistributionTransformationException(
            "Set of actions for alphabet "
                + nameAndAlphabetDef.getValue().toString()
                + " of component "
                + nameAndAlphabetDef.getKey().toString()
                + " is not defined.");
      }
  ...
  */
  ...
  public DistributionDefinition(
      Symbol systemMTS,
      List<Symbol> componentsName,
      List<Symbol> alphabets,
   ...
   */
  public DistributionDefinition(
      Symbol systemMTS, List<Symbol> componentsName, List<Symbol> alphabets) {
    this.initialise(componentsName, alphabets);
    this.systemModel = systemMTS;
  }
  ...
  private void initialise(List<Symbol> componentsName, List<Symbol> alphabets) {
    Validate.isTrue(
        componentsName.size() == alphabets.size(),
        "Components and alphabets should have the same size");
    ...
    for (int i = 0; i < componentsName.size(); i++) {
      ...
      Map.Entry<Symbol, Symbol> entry =
          new ComponentAndAlphabet(currentComponentName, alphabets.get(i));
  ...
  public String toString() {
    ...
    result
        .append("System model: ")
        .append(this.getSystemModel().toString())
        .append(".\n Distribution Specification: ")
        .append("{ ");
    for (Map.Entry<Symbol, Symbol> componentAndAlphabet : this.componentsNameAndAlphabet) {
      result
          .append("<")
          .append(componentAndAlphabet.getKey())
          .append(",")
          .append(componentAndAlphabet.getValue())
          .append("> ");
    }
    result.append("}");
  ...
  public boolean equals(final Object obj) {
    if (obj instanceof DistributionDefinition) {
      ...
      return new EqualsBuilder()
          .append(this.getComponentsNameAsString(), other.getComponentsNameAsString())
          .isEquals();
    } else {
      return false;
    }
