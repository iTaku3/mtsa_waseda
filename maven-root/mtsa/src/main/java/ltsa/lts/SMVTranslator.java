...
/**
 * This class includes the logic to export a directed control problem to an SMV model as supported
 * by MBP.
 */
public class SMVTranslator extends AbstractTranslator {
  ...
  /**
   * Performs the SMV translation storing the representation in an internal buffer. This completes
  ...
  protected void genConrolableVar() {
    open(CONTROLLABLE + ": {");
    add(LAMBDA + ",");
    for (String label : alphabet) add(action(label) + ",");
    removeLast(",");
    close("};");
  }
  ...
  protected void genUncontrollableVar() {
    open(UNCONTROLLABLE + ": {");
    add(LAMBDA + ",");
    for (String label : uncontrollable) add(action(label) + ",");
    removeLast(",");
    close("};");
  }
  ...
  protected void genInit() {
    open("INIT (");
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) add(set(at(lts), 0) + "&");
    removeLast("&");
    close(")");
  }

  /**
   * Generates the SMV TRANS definition, containing the transition system emerging from the
   * translation.
   */
  protected void genTrans() {
    ...
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      ...
      for (String label : alphabet) {
        if (lts.getValue().getActions().contains(label)) genReady(label, lts);
        genAction(label, lts);
      }
      removeLast("&");
      close("");
  ...
  protected String getReady(String label, Entry<String, LTS<Long, String>> lts) {
    ...
    for (Long s : lts.getValue().getStates()) {
      for (Pair<String, Long> transition : lts.getValue().getTransitions(s)) {
        if (transition.getFirst().equals(label)) ready += at(lts, s) + " | ";
      }
    }
  ...
  protected void genAction(String label, Entry<String, LTS<Long, String>> lts) {
    String unc = "";
    if (uncontrollable.contains(label)) unc = "(" + UNCONTROLLABLE + "=" + action(label) + ") | ";
    open(
        "(("
            + unc
            + "("
  ...
  protected void genStep(String label, Entry<String, LTS<Long, String>> lts) {
    if (lts.getValue().getActions().contains(label)) {
      ...
      if (effect) removeLast("&");
      else add(next(at(lts), at(lts)));
    } else {
  ...
  protected void genGoal() {
    ...
    switch (goalType) {
      ...
      case 2: // blocking + reachability
        open(
            "FULL_OBS_STRONG_GOAL"); // I could express these in CTL, but MBP may behave better if I
        // use the specific goal specification
        break;
    ...
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      ...
      for (String mark : marking) {
        if (lts.getValue().getActions().contains(mark)) {
          for (Long s : lts.getValue().getStates()) {
            for (Long t : lts.getValue().getTransitions(s).getImage(mark)) marked.add(t);
          }
        }
      }
      if (!marked.isEmpty()) {
        buffer.append("(");
        for (Long m : marked) buffer.append(at(lts, m) + " | ");
        removeLast(" | ");
        add(") &");
      } else {
        //				add("!(" + at(lts,-1l) + ") &");
