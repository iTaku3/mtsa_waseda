...
public abstract class TriggeredScenarioDefinition {
  ...
  private static Hashtable<String, TriggeredScenarioDefinition> definitions =
      new Hashtable<String, TriggeredScenarioDefinition>();
  ...
  /**
   * Transforms this triggered scenario to a triggered scenario that can be an input for the
   * synthesis algorithm
   *
   ...
   */
  public abstract TriggeredScenario adapt(LocationNamingStrategyImpl locationNamingStrategyImpl)
      throws TriggeredScenarioTransformationException;

  /**
   * Returns the definition with that name. An IllegalArgumentException is thrown if not found.
   *
  ...
  public static TriggeredScenarioDefinition getDefinition(Symbol definitionName) {
    ...
    if (definition == null) {
      throw new IllegalArgumentException(
          "Triggered scenario definition not found: " + definitionName.toString());
    }
    return definition;
  }
  ...
  public void addConditionDefinition(ConditionDefinition conditionDefinition) {
    if (this.conditionDefinitions.contains(conditionDefinition)) {
      Diagnostics.fatal(
          "duplicate Condition definition: " + conditionDefinition.getName(),
  ...
  public static Collection<CompactState> synthesiseAll(LTSOutput output)
    ...
    while (lscDefinitions.hasMoreElements()) {
      TriggeredScenarioDefinition definition =
          (TriggeredScenarioDefinition) lscDefinitions.nextElement();
  ...
  public CompactState synthesise(LTSOutput output) throws TriggeredScenarioTransformationException {
    ...
    GenericMTSToLongStringMTSConverter<
            SynthesizedState, MTSSynthesis.ar.dc.uba.model.language.Symbol>
        toLongStringMTSConverter =
    ...
    return MTSToAutomataConverter.getInstance()
        .convert(toLongStringMTSConverter.transform(synthesisedMTS), this.getName().toString());
  }
