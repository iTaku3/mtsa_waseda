...
/**
 * Singleton. Takes a TriggeredScenarioDefinition from MTSA and creates an TriggeredScenario to be
...
public class TriggeredScenarioDefinitionToTriggeredScenario {
  ...
  private static TriggeredScenarioDefinitionToTriggeredScenario instance =
      new TriggeredScenarioDefinitionToTriggeredScenario();

  public static TriggeredScenarioDefinitionToTriggeredScenario getInstance() {
    return instance;
  }
  ...
  public TriggeredScenario transformExistentialTriggeredScenario(
      TriggeredScenarioDefinition triggeredScenario, LocationNamingStrategy namingStrategy)
      ...
      prechart =
          new Chart(
              this.transformLocations(
                  triggeredScenario.getPrechart().getLocations().iterator(),
                  triggeredScenario,
                  involvedFluents),
              namingStrategy);
      mainchart =
          new Chart(
              this.transformLocations(
                  triggeredScenario.getMainchart().getLocations().iterator(),
                  triggeredScenario,
                  involvedFluents),
              namingStrategy);
    ...
    Set<Symbol> restricted =
        this.transformRestricted(triggeredScenario.getRestricted(), namingStrategy);
  ...
  public TriggeredScenario transformUniversalTriggeredScenario(
      TriggeredScenarioDefinition uTS, LocationNamingStrategy namingStrategy)
      throws TriggeredScenarioTransformationException {
    // Fluents involved in the TriggeredScenario
    Set<Fluent> involvedFluents = new HashSet<Fluent>();
    ...
    try {
      prechart =
          new Chart(
              this.transformLocations(
                  uTS.getPrechart().getLocations().iterator(), uTS, involvedFluents),
              namingStrategy);
      mainchart =
          new Chart(
              this.transformLocations(
                  uTS.getMainchart().getLocations().iterator(), uTS, involvedFluents),
              namingStrategy);
   ...
   * @return
   */
  private Set<Symbol> transformRestricted(
      Set<Interaction> restricteds, LocationNamingStrategy namingStrategy) {
    Set<Symbol> restrictedSymbols = new HashSet<Symbol>();

    for (Interaction restricted : restricteds) {
      MTSSynthesis.ar.dc.uba.model.lsc.Interaction interaction =
          new MTSSynthesis.ar.dc.uba.model.lsc.Interaction(
              restricted.getSource(), restricted.getMessage(), restricted.getTarget());
  ...
  /**
   * Transforms each Location and returns the transformed List in the order returned by the Iterator
   *
   * @param locations
   * @param lscDefinition
  ...
  private List<MTSSynthesis.ar.dc.uba.model.lsc.Location> transformLocations(
    ...
    List<MTSSynthesis.ar.dc.uba.model.lsc.Location> result =
        new LinkedList<MTSSynthesis.ar.dc.uba.model.lsc.Location>();
  ...
  private MTSSynthesis.ar.dc.uba.model.lsc.Location transformLocation(
    ...
    if (Location.isConditionPredicate.evaluate(location)) {
      ...
      ConditionDefinition conditionDefinition =
          lscDefinition.getConditionDefinition(conditionLocation.getId());
      Formula formula =
          FormulaUtils.adaptFormulaAndCreateFluents(
              conditionDefinition
                  .getFplFormula()
                  .expand(new FormulaFactory(), new Hashtable(), new Hashtable()),
              involvedFluents);
      return new MTSSynthesis.ar.dc.uba.model.lsc.ConditionLocation(
          conditionLocation.getId(), conditionLocation.getInstances(), formula);
    } else if (!Location.isConditionPredicate.evaluate(location)) {
      Interaction interaction = (Interaction) location;
      return new MTSSynthesis.ar.dc.uba.model.lsc.Interaction(
          interaction.getSource(), interaction.getMessage(), interaction.getTarget());
    } else {
