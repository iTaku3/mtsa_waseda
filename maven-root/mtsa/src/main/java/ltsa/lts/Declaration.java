...
class ProcessSpec extends Declaration {
  ...
  public String getname() {
    ...
    name = s.name;
    if (s.range != null) Diagnostics.fatal("process name cannot be indexed", name);
  ...
  public void addAlphabet(StateMachine m) {
    if (alphaAdditions != null) {
      ...
      while (e.hasMoreElements()) {
        ...
        if (!m.alphabet.containsKey(s)) m.alphabet.put(s, m.eventLabel.label());
      }
    }
  }
...
class StateDefn extends Declaration {
  ...
  private void check_put(String s, StateMachine m) {
    if (m.explicit_states.containsKey(s)) Diagnostics.fatal("duplicate definition -" + name, name);
    else m.explicit_states.put(s, m.stateLabel.label());
  }
  ...
  private void crunchAlias(StateExpr st, String n, Hashtable locals, StateMachine m) {
    ...
    if (i == null) {
      if (s.equals("STOP")) {
        ...
        m.explicit_states.put("ERROR", i = new Integer(Declaration.ERROR));
        Diagnostics.warning(s + " defined to be ERROR", "definition not found- " + s, st.name);
      }
    }
    CompactState mach = null;
    if (st.processes != null) mach = st.makeInserts(locals, m);
    if (mach != null) m.preAddSequential((Integer) m.explicit_states.get(n), i, mach);
    else m.aliases.put(m.explicit_states.get(n), i);
  }

  public void crunch(StateMachine m) {
    if (stateExpr.name == null && stateExpr.boolexpr == null) return;
    Hashtable locals = new Hashtable();
    if (range == null) crunchit(m, locals, stateExpr, name.toString());
    else {
      range.initContext(locals, m.constants);
      while (range.hasMoreNames()) {
  ...
  private void crunchit(StateMachine m, Hashtable locals, StateExpr st, String s) {
    if (st.name != null) crunchAlias(st, s, locals, m);
    else if (st.boolexpr != null) {
      if (Expression.evaluate(st.boolexpr, locals, m.constants).intValue() != 0) st = st.thenpart;
      else st = st.elsepart;
  ...
  public void transition(StateMachine m) {
    ...
    if (range == null) {
      ...
      if (accept) {
        if (!m.alphabet.containsKey("@")) m.alphabet.put("@", m.eventLabel.label());
  ...
  public StateDefn myclone() {
    ...
    sd.name = name;
    sd.accept = accept;
    if (range != null) sd.range = range.myclone();
    if (stateExpr != null) sd.stateExpr = stateExpr.myclone();
    return sd;
  }
}
  ...
  CompactState instantiate(Hashtable locals, Hashtable constants) {
    ...
    if (mach == null) {
      CompositionExpression ce =
          (CompositionExpression) LTSCompiler.composites.get(name.toString());
      if (ce != null) {
    ...
    if (mach != null) {
      ...
      if (!mach.isSequential()) Diagnostics.fatal("process is not sequential - " + name, name);
      return mach.myclone();
    }
...
class StateExpr extends Declaration {
  ...
  public CompactState makeInserts(Hashtable locals, StateMachine m) {
    ...
    if (seqs.size() > 0) return CompactState.sequentialCompose(seqs);
    return null;
  }
  ...
  public void firstTransition(int from, Hashtable locals, StateMachine m) {
    if (boolexpr != null) {
      if (Expression.evaluate(boolexpr, locals, m.constants).intValue() != 0) {
        if (thenpart.name == null) thenpart.firstTransition(from, locals, m);
      } else {
        if (elsepart.name == null) elsepart.firstTransition(from, locals, m);
      }
    } else {
  ...
  public void endProbabilisticTransition(
      int from, Symbol event, Hashtable locals, StateMachine m, BigDecimal prob, int bundle) {
    ...
    if (boolexpr != null) {
      ...
      else elsepart.endProbabilisticTransition(from, event, locals, m, prob, bundle);
    } else {
      Integer to;
      if (name != null) {
        ...
        if (to == null) {
          if (evalName(locals, m).equals("STOP")) {
            ...
            Diagnostics.warning(
                evalName(locals, m) + " defined to be ERROR",
        ...
        m.transitions.addElement(
            new ProbabilisticTransition(from, event, to.intValue(), prob, bundle));
      } else {
  ...
  public void endTransition(int from, Symbol event, Hashtable locals, StateMachine m) {
    if (boolexpr != null) {
      ...
      else elsepart.endTransition(from, event, locals, m);
    } else {
      Integer to;
      if (name != null) {
        to = (Integer) m.explicit_states.get(evalName(locals, m));
  ...
  public String evalName(Hashtable locals, StateMachine m) {
    if (expr == null) return name.toString();
    else {
  ...
  public StateExpr myclone() {
    ...
    if (choices != null) {
      ...
      Enumeration<ChoiceElement> e = choices.elements();
      while (e.hasMoreElements()) se.choices.addElement(e.nextElement().myclone());
    }
...
class ChoiceElement extends Declaration {
  ...
  public ChoiceElement myclone() {
    ...
    ce.guard = guard;
    if (action != null) ce.action = action.myclone();
    if (stateExpr != null) ce.stateExpr = stateExpr.myclone();
    return ce;
  }
}
...
class ProbabilisticChoiceElement extends ChoiceElement {
  Map<BigDecimal, List<StateExpr>> probabilisticChoices =
      new HashMap<BigDecimal, List<StateExpr>>(); // Map<BigDecimal, StateExpr>
  ...
  private void endProbabilisticTransition(int from, Symbol e, Hashtable locals, StateMachine m) {
    for (Object o1 : probabilisticChoices.entrySet()) {
      ...
      for (Object o2 : stExCol) {
        ...
        stEx.endProbabilisticTransition(from, e, locals, m, (BigDecimal) entry.getKey(), bundle);
      }
    }
  }
