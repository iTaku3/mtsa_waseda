...
/**
 * This class includes the logic to export a directed control problem to a CTL formula in python
 * format as supported by party-elli.
 */
public class CTLPYTranslator extends AbstractTranslator {
  ...
  /**
   * Performs the translation to LTL storing the representation in an internal buffer. This
   * completes the template method <code>translate</code> of the <code>AbstractTranslator</code>
  ...
  protected void genStateSignals() {
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      ...
      for (int i = 0; i < bits; ++i) addSignal(bit(machine(lts), i), output(), outputs);
    }
  }
  ...
  protected void genControllableSignals() {
    ...
    int c = 0;
    for (String label : alphabet) controlId.put(label, c++);
    for (int i = 0; i < controlBits; ++i) addSignal(bit(control(), i), output(), outputs);
  }
  ...
  protected void genUncontrollableSignals() {
    ...
    int u = 0;
    for (String label : uncontrollable) ucontrolId.put(label, u++);
    for (int i = 0; i < ucontrolBits; ++i) addSignal(bit(ucontrol(), i), input(), inputs);
  }
  ...
  /**
   * Generates assumption violation constraints (i.e., once a violation is detected it can never be
   * turned off and the controller may not act).
   */
  ...
  protected void genExtraConstraints() {
    ...
    for (int u = uncontrollable.size(); u < (1 << ucontrolBits); ++u)
      add(
          always()
              + "("
              + codeBits(ucontrol(), ucontrolBits, u)
              + then()
              + next(violation())
              + ")"
              + and()
  ...
  protected void genTransitions(Entry<String, LTS<Long, String>> lts) {
    for (String label : alphabet) {
      if (!label.equals(lambda())) {
        String uchoice = "";
        if (uncontrollable.contains(label)) uchoice = uncontrollable(label) + or();
        add(
            always()
                + "(("
                + uchoice
  ...
  protected void genMarked() {
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      ...
      for (String label : marking) {
        if (lts.getValue().getActions().contains(label)) {
          for (Long s : lts.getValue().getStates()) {
            for (Long d : lts.getValue().getTransitions(s).getImage(label)) marked.add(d);
          }
        }
      }
      String ltsGoal = "";
      if (marked.isEmpty()) {
        ...
        for (Long m : marked) ltsGoal += state(lts, m) + or();
        ltsGoal = removeLast(ltsGoal, or());
      }
      if (!ltsGoal.isEmpty()) add("(" + ltsGoal + ")" + and() + end());
    }
  }
  ...
  protected String removeLast(String s, String r) {
    ...
    String result = i == -1 ? s : s.substring(0, i) + s.substring(i + r.length());
    return result;
  }
}
