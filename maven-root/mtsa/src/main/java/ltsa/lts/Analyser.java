...
public class Analyser implements Animator, Automata {
  ...
  public Analyser(CompositeState cs, LTSOutput output, EventManager eman, boolean ia) {
    ...
    for (int i = 0; e.hasMoreElements(); i++) sm[i] = ((CompactState) e.nextElement()).myclone();
    Nmach = sm.length;
    ...
    for (int i = 0; i < sm.length; i++) {
      for (int j = 0; j < sm[i].alphabet.length; j++) {
        if (!sm[i].alphabet[j].contains("?")) { // omit the maybe actions
          ...
          // processes
          if (sm[i].endseq > 0) terminating.add(sm[i].alphabet[j]);
          else nonterminating.add(sm[i].alphabet[j]);
    ...
    while (e.hasMoreElements()) {
      ...
      if (highAction != null) {
        ...
        } else if (!CompactState.contains(s, priorLabels)) highAction.set(index);
      }
    }
    ...
    if (highAction != null) {
      ...
      if (acceptEvent > 0) highAction.clear(acceptEvent); // accept labels are always low
      // priority
    }
    actionCount[0] = 0; // tau
  ...
  private CompactState private_compose(boolean dohiding) {
    ...
    CompactState c =
        new CompactState(
            ...
            cs.name,
            compositionEngine.getExploredStates(),
            compTrans,
            actionName,
    ...
    if (dohiding) {
      if (cs.hidden != null) {
        if (!cs.exposeNotHide) c.conceal(cs.hidden);
        else c.expose(cs.hidden);
      }
    }
  ...
  private int countSet(BitSet b) {
    int count = 0;
    for (int i = 0; i < b.size(); i++) if (b.get(i)) count++;
    return count;
  }
  ...
  private boolean isEND(int[] state) {
    ...
    for (int i = 0; i < Nmach; i++) {
      if (sm[i].endseq < 0)
        ; // skip
      else if (sm[i].endseq != state[i]) return false;
    }
    return true;
  }
  ...
  List eligibleTransitions(int[] state) {
    ...
    if (partial != null) {
      if (asteriskEvent > 0
          && EventState.hasEvent(sm[Nmach - 1].states[state[Nmach - 1]], asteriskEvent)) {
        // do nothing
      } else {
    ...
    if (trs[0] != null) {
      ...
      if (highTau || highs == 0) computeTauTransitions(trs[0], state, transitions);
      if (highTau) ++highs;
    }
    while (nsucc > 0) { // do this loop once per successor state
      ...
      if (highs > 0 && !highAction.get(actionNo) && actionNo != acceptEvent)
        ...
        if (!nonDeterministic) {
          ...
          next[Nmach] =
              actionNo; // next[Nmach] is not a state (like the rest of the array but a transition.

          while (tr != null) {
            next[tr.machine] = tr.next;
            tr = tr.path;
          }
          if (actionNo != asteriskEvent) transitions.add(next);
          else {
            asteriskTransitions = new ArrayList(1);
            asteriskTransitions.add(next);
          }
        } else if (actionNo != asteriskEvent) computeNonDetTransitions(tr, state, transitions);
        else computeNonDetTransitions(tr, state, asteriskTransitions = new ArrayList(4));
      }
      ++ac[actionNo];
    }
    if (asteriskEvent < 0) return transitions;
    else return mergeAsterisk(transitions, asteriskTransitions);
  }
  ...
  private void computeNonDetTransitions(EventState first, int[] state, List v) {
    ...
    while (tr != null) {
      int[] next = LTSUtils.myclone(state);
      next[tr.machine] = tr.next;
      if (first.path != null) computeNonDetTransitions(first.path, next, v);
      else {
  ...
  private void outStatistics(int states, int transitions) {
    ...
    output.outln(
        "-- States: "
            ...
            + transitions
            + " Memory used: "
            + (r.totalMemory() - r.freeMemory()) / 1000
            + "K");
  }
  ...
  private void printPath(LinkedList v) {
    ...
    while (t.hasNext()) output.outln("\t" + (String) t.next());
  }
  ...
  private int newState_analyse(
      boolean checkDeadlocks, boolean callFromTracer, byte[] fromState, byte[] target) {
    stateCount = 0;
    explorerContext.stateCount = 0;
        ...
        while (e.hasNext()) {
          ...
          if (code == null || StateCodec.equals(code, target)) {
            ...
            if (!checkDeadlocks || callFromTracer) {
              if (code == null) return LTSConstants.ERROR;
              else return LTSConstants.FOUND;
            }
  ...
  private volatile List choices; // set of eligible choices
  ...
  public BitSet initialise(Vector menu) {
    ...
    // to
    // 0
    if (eman != null) eman.post(new LTSEvent(LTSEvent.NEWSTATE, currentA)); // initialise
    // animation
    // to 0
    getMenu(menu);
    ...
    if (cs.getErrorTrace() != null) {
      ...
      if (_replay.hasMoreElements()) _replayAction = (String) _replay.nextElement();
    }
    return menuActions();
  }
  ...
  int[] nonDetSelect(int[] x) {
    ...
    int last = start + 1;
    while (last < choices.size() && x[Nmach] == ((int[]) choices.get(last))[Nmach]) last++;
  ...
  public BitSet getPriorityActions() {
    ...
    for (int i = 1; i < actionName.length; i++) {
      ...
      if (ix != null
          && ((lowpriority && !highAction.get(i)) || (!lowpriority && highAction.get(i))))
        b.set(ix.intValue());
    }
    return b;
  }
  ...
  private Set getAndRemoveSameParentEntries(Iterator iter, ExploredTransitionsEntry entry) {
    ...
    while (iter.hasNext() && !finished) {
      ...
      if (newEntry.next == null) finished = true;
    }
