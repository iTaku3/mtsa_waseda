...
class CompositeBody {
  ...
  /**
   * This method fills the <i>machines</i> with the compiled version of the processes referenced in
   * the composition expression <i>c</i>. In addition to handling conditionals and applying
   * accessors it also applies relabelling if necessary.
   */
  void compose(CompositionExpression c, Vector machines, Hashtable<String, Value> locals) {
    ...
    if (boolexpr != null) {
      ...
      else if (elsepart != null) elsepart.compose(c, machines, locals);
    } else if (range != null) {
...
public class CompositionExpression {
  ...
  /**
   * If the isComponent flag is true, then this ProcessSpec represents a composition of several
   * component processes. The component process can be built with the componentAlphabet.
   */
  private boolean makeComponent = false;

  /** this alphabet is one of the component. It must be a subset of the process alphabet. */
  ...
  protected CompositeState compose(Vector<Value> actuals) {
    ...
    for (Object o : machines) { // machines contains a mixture of two
      // unrelated classes!
      if (o instanceof CompactState) flatmachines.addElement((CompactState) o);
      else {
        CompositeState cs = (CompositeState) o;
        if (isHeuristic
            || isPartialOrderReduction) { // avoid computing the parallel composition in heuristic
          // analysis or when using partial order reduction
    ...
    String refname =
        (actuals == null || makeControlStack)
            ? name.toString()
            : name.toString() + StateMachine.paramString(actuals);
    ...
    if (MTSUtils.isMTSRepresentation(c)
        && c.priorityLabels != null
      ...
      if (actuals != null) {
        if (actuals.size() != 1)
          throw new RuntimeException(
              "Control stack references may take only one integer parameter indicating a selected"
                  + " tier.");
        try {
    ...
    if (makeEnactment) {
      ...
      if (labelSet == null)
        Diagnostics.fatal(
            "Controllable actions set '" + enactmentControlled.toString() + "' not defined.");
    ...
    if (c.makeController
        ...
        || c.isControlledDet
        || c.makeRTCController
        || c.makeRTCAnalysisController
        || c.isHeuristic
        || c.isMonolithicDirector
        || c.isPartialOrderReduction) {
  ...
  public static Collection<CompactState> preProcessSafetyReqs(
    ...
    for (Symbol safetyDef : pendingGoal.getSafetyDefinitions()) {
      ...
      if (p != null) {
        ...
        MTS<Long, String> convert =
            AutomataToMTSConverter.getInstance().convert(compile.composition);
        convert.removeAction("@" + compile.name); // get rid of those
        // horrible @s

        CompactState convert2 =
            MTSToAutomataConverter.getInstance().convert(convert, safetyDef.getName(), false);
...
class ProcessRef {
  ...
  public void instantiate(
      CompositionExpression c, Vector machines, LTSOutput output, Hashtable<String, Value> locals) {
    // compute parameters
    Vector<Value> actuals = paramValues(locals, c);
    String refname =
        (actuals == null) ? name.toString() : name.toString() + StateMachine.paramString(actuals);
  ...
  private void isAParallelComposition(
      CompositionExpression c, Vector machines, LTSOutput output, Vector<Value> actuals) {
    CompactState mach;
  ...
  private boolean isDistributionDefinition(
      CompositionExpression c, Vector machines, LTSOutput output, Hashtable<String, Value> locals) {
    if (DistributionDefinition.contains(name)) {

      DistributionDefinition distributionDefinition =
          DistributionDefinition.getDistributionDefinitionContainingComponent(name);
      ...
      boolean isDistributionSuccessful =
          TransitionSystemDispatcher.tryDistribution(
              systemModel, distributionDefinition, output, distributedComponents);
  ...
  private boolean isAProcess(CompositionExpression c, Vector machines, Vector<Value> actuals) {
    ...
    if (p != null) {
      ...
      c.compiledProcesses.put(mach.name, mach); // add to compiled
      // processes
      if (!p.imported()) c.output.outln("Compiled: " + mach.name);
      else c.output.outln("Imported: " + mach.name);
      return true;
    }
    return false;
  }
  ...
  private boolean isAConstraint(
      ...
      String refname) {
    CompactState mach;
    mach = AssertDefinition.compileConstraint(output, name, refname, actuals);
    if (mach != null) {
      if (this.passBackClone) {
        machines.addElement(mach.myclone()); // pass back clone
      }
  ...
  private boolean isATriggeeredScenario(
      CompositionExpression c, Vector machines, LTSOutput output) {
    CompactState mach;
