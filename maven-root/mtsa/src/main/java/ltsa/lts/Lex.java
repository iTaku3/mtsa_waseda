...
public class Lex {
    ...
    while (isbase(ch, base)) {
      ...
      switch (base) {
        ...
        case 10:
          ...
          break;
        case 16:
          if (Character.isUpperCase(ch)) digit = (ch - 'A') + 10;
          else if (Character.isLowerCase(ch)) digit = (ch - 'a') + 10;
          else digit = ch - '0';
      }
    ...
    while (ch == '\\') {
      ...
      switch (ch) {
        ...
        case 'X':
          n = 0;
          next_ch();
          if (!isxdigit(ch)) error("hex digit expected after \\x");
          else {
            int hex_digits = 0;
            while (isxdigit(ch) && hex_digits < 2) { // max of two hex
              ...
              if (Character.isDigit(ch)) n = n * 16 + ch - '0';
              else if (Character.isUpperCase(ch)) n = n * 16 + ch - 'A';
              else n = n * 16 + ch - 'a';
              next_ch();
            }
          }
  ...
  private void in_string() {
    ...
    do {
      ...
      if (more = (ch != quote && eoln == false)) buf.append(ch);
    } while (more);
    symbol.setString(buf.toString());
    if (eoln == true) error("No closing character for string constant");
  ...
  private void in_identifier() {
    ...
    if (kind == null) {
      if (Character.isUpperCase(s.charAt(0))) symbol.kind = Symbol.UPPERIDENT;
      else symbol.kind = Symbol.IDENTIFIER;
    } else {
  ...
  public Symbol in_sym() {
    ...
    while (DoOnce) {
      ...
      switch (ch) {
        ...
        // case '\v':
        case '\f':
          while (Character.isWhitespace(ch)) next_ch();
          DoOnce = true;
          break;
        ...
        case '[':
          next_ch();
          if (ch == ']') symbol.kind = Symbol.ALWAYS;
          else {
        ...
        case '!':
          next_ch();
          if (ch == '=') symbol.kind = Symbol.NOT_EQUAL;
          else {
        ...
        case '<':
          next_ch();
          if (ch == '=') symbol.kind = Symbol.LESS_THAN_EQUAL;
          else if (ch == '<') symbol.kind = Symbol.SHIFT_LEFT;
          else if (ch == '>') symbol.kind = Symbol.EVENTUALLY;
          else if (ch == '-') {
            next_ch();
            if (ch == '>') symbol.kind = Symbol.EQUIVALENT;
            else {
        ...
        case '>':
          next_ch();
          if (ch == '=') symbol.kind = Symbol.GREATER_THAN_EQUAL;
          else if (ch == '>') symbol.kind = Symbol.SHIFT_RIGHT;
          else {
        ...
        case '=':
          next_ch();
          if (ch == '=') symbol.kind = Symbol.EQUALS;
          else {
        ...
        case '.':
          next_ch();
          if (ch == '.') symbol.kind = Symbol.DOT_DOT;
          else {
        ...
        case '-':
          next_ch();
          if (ch == '>') symbol.kind = Symbol.ARROW;
          else {
        ...
        case ':':
          next_ch();
          if (ch == ':') symbol.kind = Symbol.COLON_COLON;
          else {
