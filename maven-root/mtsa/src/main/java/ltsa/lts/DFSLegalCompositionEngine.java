...
public class DFSLegalCompositionEngine extends DFSCompositionEngine implements CompositionEngine {
  ...
  public DFSLegalCompositionEngine(
      StateCodec coder, int sourceMachineIndex, int targetMachineIndex, List<String> actionSet) {
    super(coder);
  ...
  public void processNextState() {
    ...
    if (transitions == null) {
      if (!ModelExplorer.isEND(ctx, state)) {
        ...
        if (ctx.endSequence < 0) ctx.endSequence = ctx.stateCount - 1;
        else {
  ...
  private void doStateChecks(int[] state, List<int[]> transitions) {
    ...
    ArrayList<String> sourceTransitions =
        getTransitionsForState(ctx.sm[sourceMachineIndex], sourceState);
    List<String> sourceAlphabet =
        new ArrayList<>(Arrays.asList(ctx.sm[sourceMachineIndex].alphabet));
    ...
    ArrayList<String> targetTransitions =
        getTransitionsForState(ctx.sm[targetMachineIndex], targetState);

    // si targetState tiene una transicion saliente que no esta en controlablesDeA entonces
    // sourceState tambien la tiene
    for (String transition : targetTransitions) {
      if (!sourceControlledTransitions.contains(transition)) {
        if (!sourceTransitions.contains(transition) && sourceAlphabet.contains(transition)) {
          // this.output.outln(String.format("Illegal: %s; action: %s", Arrays.toString(state),
          // transition));
