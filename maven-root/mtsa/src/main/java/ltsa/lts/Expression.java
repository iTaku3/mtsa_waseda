...
public class Expression {
  ...
  private static String labelVar(Stack s, Hashtable locals, Hashtable globals) {
    ...
    if (token.kind == Symbol.IDENTIFIER) {
      ...
      if (el.hasMultipleValues()) Diagnostics.fatal("label constants cannot be sets", token);
  ...
  protected static int countSet(Symbol token, Hashtable locals, Hashtable globals) {
    if (token.kind != Symbol.LABELCONST) Diagnostics.fatal("label set expected", token);
    ActionLabels el = (ActionLabels) token.getAny();
    el.initContext(locals, globals);
    int count = 0;
  ...
  protected static String indexSet(Stack s, Hashtable locals, Hashtable globals) {
    ...
    el.clearContext();
    if (count != index) Diagnostics.fatal("label set index expression out of range", token);
    return label;
  }
  ...
  private static BigDecimal eval(Stack s, Hashtable locals, Hashtable globals) {
    ...
    switch (token.kind) {
      ...
      case Symbol.IDENTIFIER:
        if (locals == null) Diagnostics.fatal("no variables defined", token);
        Value variable = (Value) locals.get(token.toString());
        if (variable == null) Diagnostics.fatal("variable not defined- " + token, token);
        if (variable.isLabel()) Diagnostics.fatal("not integer variable- " + token, token);
        return variable.doubleValue();
      case Symbol.UPPERIDENT:
        Value constant = null;
        if (globals != null) constant = (Value) globals.get(token.toString());
        if (constant == null) constant = (Value) constants.get(token.toString());
        if (constant == null)
      ...
      case Symbol.POWER:
        ...
        if (right.isNumeric() && left.isNumeric()) {
          ...
          else return left.toString().equals(right.toString()) ? BigDecimal.ZERO : BigDecimal.ONE;
        } else Diagnostics.fatal("invalid expression", token);
      case Symbol.QUESTION:
        ...
        return (condition.doubleValue().compareTo(BigDecimal.ZERO) != 0 ? thenBranch : elseBranch)
            .doubleValue();
      ...
      case Symbol.PLING:
        return eval(s, locals, globals).compareTo(BigDecimal.ZERO) == 1
            ? BigDecimal.ZERO
            : BigDecimal.ONE;
      case Symbol.SINE:
  ...
  private static BigDecimal exec_op(int op, BigDecimal left, BigDecimal right) {
    ...
    if (op == Symbol.CIRCUMFLEX
        || op == Symbol.BITWISE_AND
        || op == Symbol.BITWISE_OR
        || op == Symbol.SHIFT_LEFT
        || op == Symbol.SHIFT_RIGHT
        || op == Symbol.AND
        || op == Symbol.OR) {
    ...
    switch (op) {
      ...
      case Symbol.POWER:
        ...
        if (right.intValue() >= 0) return left.pow(right.intValue());
        else {
      ...
      case Symbol.BACKSLASH:
        return left.divideToIntegralValue(right, mathCtx);
      case Symbol.DIVIDE:
      ...
      case Symbol.SHIFT_LEFT:
        return new BigDecimal(left.intValue() << right.intValue());
      case Symbol.LESS_THAN:
        return left.compareTo(right) == -1 ? BigDecimal.ONE : BigDecimal.ZERO;
      case Symbol.LESS_THAN_EQUAL:
        return left.compareTo(right) != 1 ? BigDecimal.ONE : BigDecimal.ZERO;
      case Symbol.GREATER_THAN:
        return left.compareTo(right) == 1 ? BigDecimal.ONE : BigDecimal.ZERO;
      ...
      case Symbol.NOT_EQUAL:
        return left.compareTo(right) != 0 ? BigDecimal.ONE : BigDecimal.ZERO;
      case Symbol.AND:
        return left.compareTo(BigDecimal.ZERO) != 0 && right.compareTo(BigDecimal.ZERO) != 0
            ? BigDecimal.ONE
            : BigDecimal.ZERO;
      case Symbol.OR:
        return left.compareTo(BigDecimal.ZERO) != 0 || right.compareTo(BigDecimal.ZERO) != 0
