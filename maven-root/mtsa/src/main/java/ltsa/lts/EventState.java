...
public class EventState {
  ...
  public int getNext(int event) {
    if (this.event == event) {
      ...
      if (this.list == null) throw new IllegalStateException("Invalid action execution");
  ...
  public void updateEventAndNext(int oldEvent, int newEvent, int newNext) {
    if (this.event == oldEvent) {
      ...
      if (this.list != null) this.list.updateEventAndNext(oldEvent, newEvent, newNext);
    }
  }
  ...
  public void swapNext(int next1, int next2) {
    if (this.next == next1) {
      ...
      if (this.next == next2) this.next = next1;
    }

    if (this.list != null) this.list.swapNext(next1, next2);
  }
  ...
  public static EventState remove(EventState head, EventState tr) {
    ...
    if (head.event == tr.event && head.next == tr.next) {
      if (head.nondet == null) return head.list;
      else {
  ...
  public static EventState removeEvent(EventState anEventState, int anEvent) {
    if (anEventState == null) return null;

    EventState cleanEvent = anEventState;
    if (cleanEvent.event == anEvent) cleanEvent = cleanEvent.list;
    else cleanEvent.list = EventState.removeEvent(cleanEvent.list, anEvent);
    return cleanEvent;
  }
  ...
  public static EventState copy(EventState anEventState) {
    ...
    if (anEventState.list == null) aCopy.list = null;
    else aCopy.list = EventState.copy(anEventState.list);

    if (anEventState.nondet == null) aCopy.nondet = null;
    else aCopy.nondet = EventState.copy(anEventState.nondet);

    if (anEventState.path == null) aCopy.path = null;
    else aCopy.path = EventState.copy(anEventState.path);
  ...
  public static EventState replaceNext(EventState anEventState, int oldNext, int newNext) {
    ...
    if (anEventState.next == oldNext) anEventState.next = newNext;
  ...
  public static EventState addNonDetTau(EventState head, EventState states[], BitSet tauOnly) {
    ...
    if (toAdd == null) return head;
    else return EventStateUtils.union(head, toAdd);
  }
      ...
      while (q != null) {
        ...
        if (q instanceof ProbabilisticEventState) {
          ...
          while (r != null) {
            ...
            ProbabilisticEventState child2 =
                new ProbabilisticEventState(
                    oldtonew.get(r.event), r.next, r.getProbability(), r.getBundle());
      ...
      while (q != null) {
        ...
        if (o != null) {
          if (o instanceof Integer) {
            newhead =
                EventStateUtils.add(newhead, new EventState(((Integer) o).intValue(), q.next));
          } else {
            Vector<Integer> v = (Vector<Integer>) o;
            for (Enumeration<Integer> e = v.elements(); e.hasMoreElements(); ) {
              newhead =
                  EventStateUtils.add(
                      newhead, new EventState(((Integer) e.nextElement()).intValue(), q.next));
            }
          }
        }
        q = q.nondet;
  ...
  public static String[] eventsToNextNoAccept(EventState from, String[] alphabet) {
    ...
    String s[] = new String[size];
    for (int i = 0; i < s.length; ++i) {
      if (alphabet[q.event].charAt(0) != '@') s[i] = alphabet[q.event];
      else --i;
      q = q.nondet;
    }
    return s;
  ...
  public static EventState reachableTau(EventState[] states, int k) {
    ...
    if (head == null || head.event != Declaration.TAU) return null;
    ...
    while (stack != null) {
      ...
      if (j != Declaration.ERROR) {
        ...
        if (t != null && t.event == Declaration.TAU)
          while (t != null) {
            if (!inStack(t)) {
              if (t.next < 0 || !visited.get(t.next)) stack = push(stack, t);
            }
            t = t.nondet;
          }
      }
  ...
  public static int search(
      EventState trace, EventState[] states, int fromState, int findState, int ignoreState) {
...
final class EventStateEnumerator implements Enumeration<EventState> {
  ...
  public EventState nextElement() {
    if (es != null) {
      ...
      if (es.nondet != null) es = es.nondet;
      else {
        es = list;
    ...
    while (p != null) {
      ...
      while (q != null) {
        ...
        if (q instanceof ProbabilisticEventState) {
          ...
          evSt =
              new ProbabilisticEventState(
                  probQ.event, probQ.next, probQ.getProbability(), probQ.getBundle());
          ...
          while (probSt != null) {
            evSt =
                new ProbabilisticEventState(
                    probSt.event, probSt.next, probSt.getProbability(), probSt.getBundle());
            res = EventStateUtils.add(res, evSt);
