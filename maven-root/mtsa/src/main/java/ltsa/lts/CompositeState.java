...
public class CompositeState {
  ...
  private boolean isProbabilisticMachine(CompactState cs) {
    for (EventState evSt : cs.states) {
      if (evSt instanceof ProbabilisticEventState) return true;
    }
  ...
  private void applyHiding() {
    ...
    if (hidden != null) {
      ...
      if (!exposeNotHide) composition.conceal(hidden);
      else composition.expose(hidden);
    }
  }
  ...
  public void determinise(LTSOutput output) {
    if (composition != null) {
      ...
      if (isProperty) composition.makeProperty(this.actionsToErrorSet);
    }
  }
  ...
  protected void initAlphaStop() {
    ...
    for (CompactState m : machines) {
      for (int i = 1; i < m.alphabet.length; ++i) alpha.put(m.alphabet[i], m.alphabet[i]);
    }
