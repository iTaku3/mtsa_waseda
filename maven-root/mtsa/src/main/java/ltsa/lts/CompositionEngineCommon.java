...
public class CompositionEngineCommon {
  ...
  public static void processTransitions(
      StateCodec coder, ModelExplorerContext ctx, List transitions, StateMap analysed) {
    ...
    while (e.hasNext()) {
      ...
      if (next instanceof int[]) {
        ...
        ctx.compTrans.add(
            ...
            code,
            nextState[ctx.Nmach],
            ProbabilisticTransition.composeBundles(
                nextProbTr.sourceStates, nextProbTr.sourceBundles),
            ProbabilisticTransition.composeProbs(nextProbTr.sourceProbs));
      }

      if (code == null) {
        int i = 0;
        while (nextState[i] >= 0) i++;
        if (!ctx.violated[i]) {
