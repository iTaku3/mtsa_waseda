...
public class LTSCompiler {
  ...
  public static LTSCompiler getCompiler(LTSInput input) throws IOException {
    LTSCompiler compiler =
        new LTSCompiler(input, new StandardOutput(), (new File(".")).getCanonicalPath());
    return compiler;
  }
  ...
  private void current_is(int kind, String errorMsg) {
    if (current.kind != kind) error(errorMsg);
  }

  private void current_is(Collection<Integer> possibleKind, String errorMsg) {

    if (!CollectionUtils.exists(
        possibleKind, PredicateUtils.equalPredicate(new Integer(current.kind)))) {
      error(errorMsg);
    }
  }
  ...
  public CompositeState continueCompilation(String name) {
    ...
    if (ce != null) {
      ...
      // try to distribute
      Set<DistributionDefinition> allDistributionDefinitions =
          DistributionDefinition.getAllDistributionDefinitions();
      for (DistributionDefinition aDistributionDefinition : allDistributionDefinitions) {
        ...
        if (systemModel == null) {
          ...
          systemModel =
              this.compileSingleProcess((ProcessSpec) processes.get(systemModelId.getName()));
        }
        Collection<CompactState> distributedComponents = new HashSet<CompactState>();
        boolean isDistributionSuccessful =
            TransitionSystemDispatcher.tryDistribution(
                systemModel, aDistributionDefinition, output, distributedComponents);
        ...
        if (!isDistributionSuccessful) {
          Diagnostics.fatal(
              "Model " + systemModelId.getName() + " could not be distributed.", systemModelId);
        }
      }
  ...
  public static void makeFluents(Symbol symbol, Set<Fluent> involvedFluents) {
    ...
    if (def != null
        && !symbol.toString().equals(GeneralConstants.FALSE)
  ...
  /**
   * Returns a CompactState representation of a process given its name or null if no such process is
   * present
   *
  ...
  public CompactState getProcessCompactStateByName(String processName) {
    if (!processes.containsKey(processName)) return null;
  ...
  /**
   * Add all the CompactState in compiledToBeAdded to the compiled table with the name of the
   * CompactState as key.
   *
  ...
  private void compileProcesses(
      Hashtable<String, ProcessSpec> h, Hashtable<String, CompactState> compiled) {
    ...
    try {
      while (current.kind != Symbol.EOFSYM) {
        if (current.kind == Symbol.CONSTANT) {
          ...
          UpdatingControllersAnimatorUtils.updateDefinitions.put(
              cuDefinition.getName().getName(), cuDefinition);

          if (composites.put(cuDefinition.getName().getName(), cuDefinition) != null) {
            Diagnostics.fatal(
                "duplicate composite definition: " + cuDefinition.getName(),
          ...
          // identifier
          TriggeredScenarioDefinition eTSDefinition =
              new ExistentialTriggeredScenarioDefinition(current);
          ...
          // identifier
          TriggeredScenarioDefinition uTSDefinition =
              new UniversalTriggeredScenarioDefinition(current);
  ...
  // TODO reutilizar codigo,los metodos son iguales. Pueden hacer una interfaz hasGoal que tenga el
  // metodo setGoal()
  ...
  private void progressDefinition() {
    ...
    p.name = current;
    next_symbol();
    if (current.kind == Symbol.LSQUARE) p.range = forallRanges();
    current_is(Symbol.BECOMES, "= expected");
    next_symbol();
    if (current.kind == Symbol.IF) {
      next_symbol();
  ...
  private LabelSet labelSet(boolean consumedLCurly) {
    if (consumedLCurly) return new LabelSet(setValueHavingConsumedLCURLY());
    else if (current.kind == Symbol.LCURLY) return new LabelSet(setValue());
  ...
  private ActionLabels labelElement(boolean allredyConsumedLCurly) {
    ...
    if (!allredyConsumedLCurly
        && current.kind != Symbol.IDENTIFIER
        && !isLabelSet()
    ...
    if (!allredyConsumedLCurly && current.kind == Symbol.IDENTIFIER) {
      ...
      LabelSet left;
      if (allredyConsumedLCurly) left = labelSet(true);
      else left = labelSet();
  ...
  private void defDefinition() {
    ...
    while (current.kind != Symbol.RROUND) {
      ...
      next_symbol();
      if (current.kind == Symbol.COMMA) next_symbol();
      else current_is(Symbol.RROUND, ") expected");
    }
    expectBecomes();
    next_symbol();
    expression(d.getExpressionStack());
    if (Def.put(d)) Diagnostics.fatal("duplicate def definition: " + nameSymbol, nameSymbol);
    push_symbol();
  }
  ...
  private StateExpr stateExpr() {
    ...
    if (current.kind == Symbol.UPPERIDENT) stateRef(s);
      ...
      current_is(Symbol.RROUND, ") expected");
      next_symbol();
    } else error(" (, if or process identifier expected");
  ...
  private ChoiceElement choiceElement() {
    ...
    next_symbol();
    // TODO EPAVESE for now I only restrict to ONE transition per transformation. This really asks
    // for a yacc approach
    // we expect targets of the form {<float constant> : UPPERIDENT (+ <float constant> :
    // UPPERIDENT)* }
    // TODO EPAVESE this is not ok, isProbabilistic should be globally defined as a machine, not a
    // transition
    if (current.kind == Symbol.LCURLY) {
    ...
    if (isProbabilistic
      ...
      while (current.kind == Symbol.DOUBLE_VALUE || current.kind == Symbol.UPPERIDENT) {
        BigDecimal nextProb;
        if (current.kind == Symbol.DOUBLE_VALUE) nextProb = current.doubleValue();
        else {
  ...
  private ActionLabels labelConstant() {
    ...
    if (el != null) {
      return el;
    } else error("label definition expected");
    return null;
  }
  ...
  private void unary(Stack<Symbol> expr) { // +, -, identifier,
    ...
    if (unary_operator != null) expr.push(unary_operator);
  }
  ...
  /**
   * Validates that there is no process or composite process with name designated by <code>
   * processName</code> If so it reports the fatal error
   */
  private void validateUniqueProcessName(Symbol processName) {
    if (processes != null && processes.get(processName.toString()) != null
        ...
        || DistributionDefinition.contains(
            processName)) { // XXX: shouldn't controller definitions be here too?
  ...
  private void predicateDefinition() {
    ...
    next_symbol();
    if (current.kind == Symbol.LSQUARE) range = forallRanges();
    current_is(Symbol.BECOMES, "= expected");
    next_symbol();
  ...
  private void processActionSet(ActionSet actionLabels, Set<ActionName> addLabels) {
    ...
    for (Iterator<String> it = actionSet.getActions(null, null).iterator(); it.hasNext(); ) {
      ...
      addLabels.add(new ActionName(new Symbol(Symbol.STRING_VALUE, actionName)));
    }
  }

  /**
   * Computes the set of actions defined by <i>actionsName</i> and add for each of them the action
   * with opposite maybe condition.
   *
   * <p>For example, if <i>actionsName</i> is the label "a", it returns "a?".
   */
  ...
  private void goalDefinition(ControllerGoalDefinition goal) {
    ...
    while ((current.kind != Symbol.RCURLY)) {
      ...
      } else error("Controller symbol expected");
    }
  ...
  private List<List<Symbol>> listsOfComponentsNotEmpty() {
    ...
    while (current.kind == Symbol.LCURLY || current.kind == Symbol.COMMA) {
      ...
      next_symbol();
      if (current.kind != Symbol.COMMA) finish = true;
      else next_symbol();

      while (current.kind == Symbol.IDENTIFIER && !finish) {
        definitions.add(current);
        next_symbol();
        if (current.kind != Symbol.COMMA) break;
        next_symbol();
      }
      current_is(Symbol.RCURLY, "} expected");
      next_symbol();
  ...
  private List<Symbol> componentsByCount(int count) {
    ...
    if (definitions.size() != count) error("View and Model haven't the same number of components");
  ...
  private void distributionDefinition() {
    ...
    if (alphabets.size() != componentsName.size()) {
      error(
          "There should be one and only one alphabet for each component. Components: "
              + componentsName
              + " . Alphabets: "
              + alphabets);
    }
    ...
    if (outputFileName != null) {
      distributionDefinition =
          new DistributionDefinition(systemModel, componentsName, alphabets, outputFileName);
    } else {
  ...
  private void triggeredScenarioDefinition(TriggeredScenarioDefinition triggeredScenarioDefinition)
    ...
    while (current.kind == Symbol.CONDITION) {
      next_symbol();
      triggeredScenarioDefinition.addConditionDefinition(this.conditionDefinition());
    }

    // Prechart must come next
    current_is(Symbol.PRECHART, "prechart expected");
    triggeredScenarioDefinition.setPrechart(
        this.basicChartDefinition(true, triggeredScenarioDefinition));

    // Mainchart must come next
    next_symbol();
    current_is(Symbol.MAINCHART, "mainchart expected");
    triggeredScenarioDefinition.setMainchart(
        this.basicChartDefinition(false, triggeredScenarioDefinition));

    triggeredScenarioDefinition.setRestricted(
        this.restrictsDefinition(triggeredScenarioDefinition));
  ...
  private ConditionDefinition conditionDefinition() {
    ...
    ConditionDefinition conditionDefinition = new ConditionDefinition(name.getName(), formula);
  ...
  private BasicChartDefinition basicChartDefinition(
      boolean isPrechartDefinition, TriggeredScenarioDefinition tsDefinition) {
    expectLeftCurly();
    next_symbol();
  ...
  private Location locationValue(
    ...
    // Conditions are like: UPPERIDENT [UPPERIDENT UPPERIDENT...]. The first ident is the name of
  ...
  static Symbol saveControllableSet(Set<String> controllableSet, String name) {
    Symbol updateControllableSetSymbol =
        new Symbol(Symbol.SET, "controller_update_" + name + "_controllable_set");
