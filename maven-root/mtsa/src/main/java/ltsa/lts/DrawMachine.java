...
public class DrawMachine {
  ...
  protected Dimension computeDimension(CompactState m) {
    ...
    } else largestEndLabel = labels[m.maxStates][m.maxStates];
    int endWidth = 10;
    if (largestEndLabel != null) {
      Graphics g = parent.getGraphics();
      if (g != null) {
  ...
  public void draw(Graphics g) {
    ...
    if (m.maxStates > MAXDRAWSTATES) {
          ...
          if (event.charAt(0) != '@')
            while (tr != null) {
              ...
              drawTransition(
                  ...
                  event,
                  ntrans[tr.next + 1],
                  i == lastselected && tr.next == selected && lastaction != null,
                  false);
              if (tr instanceof ProbabilisticEventState) {
                ProbabilisticEventState probTr =
                    (ProbabilisticEventState) ((ProbabilisticEventState) tr).probTr;
                while (probTr != null) {
                  ...
                  drawTransition(
                      ...
                      event,
                      ntrans[probTr.next + 1],
                      i == lastselected && probTr.next == selected && lastaction != null,
                      false);
                ...
                if (tr instanceof ProbabilisticEventState) {
                  ...
                  while (probTr != null) {
                    ...
                    drawTransition(
                        ...
                        event,
                        ntrans[probTr.next + 1],
                        i == lastselected && probTr.next == selected,
                        true);
                    probTr = (ProbabilisticEventState) probTr.probTr;
                  }
                }
      ...
      for (int i = -errorState; i < m.maxStates; i++) drawState(g, i, i == selected);
    }
    if (selectedMachine) {
  ...
  private void drawState(Graphics g, int id, boolean highlight) {
    ...
    if (id >= 0 && accepting.get(id)) g.fillArc(x - 3, y - 3, STATESIZE + 6, STATESIZE + 6, 0, 360);
    else g.fillArc(x, y, STATESIZE, STATESIZE, 0, 360);
    g.setColor(Color.black);
    g.setFont(stateFont);
    if (id >= 0 && accepting.get(id)) g.drawArc(x - 3, y - 3, STATESIZE + 6, STATESIZE + 6, 0, 360);
  ...
  //     private void drawTransition(Graphics g,int from, int to, String s, int n, boolean
  // highlight, boolean dotext) {
  private void drawTransition(
    ...
    if (n == 1 && !dotext) { // only draw arc for first transition to->from
      if (from != to) {
        ...
        if (sign > 0) drawArrow(g, x + w / 2, y, arrowForward);
        else drawArrow(g, x + w / 2, y + h - 1, arrowBackward);
      } else {
