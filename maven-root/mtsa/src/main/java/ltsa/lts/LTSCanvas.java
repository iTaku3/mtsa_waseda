...
public class LTSCanvas extends JPanel implements Scrollable {
  ...
  public void setMachines(int n) {
    focus = null;
    if (n > 0) drawing = new DrawMachine[n];
    else drawing = null;
    setPreferredSize(initial);
    revalidate();
    repaint();
  ...
  public void draw(int id, CompactState m, int last, int current, String name) {
    ...
    setPreferredSize(new Dimension(Math.max(e.width, d.width), Math.max(e.height, d.height)));
    revalidate();
    repaint();
  }
  ...
  public void stretchHorizontal(int d) {
    if (focus != null) {
      ...
      setPreferredSize(
          new Dimension(Math.max(p.width, rr.x + rr.width), Math.max(p.height, rr.y + rr.height)));
      revalidate();
      repaint();
    }
  }
  ...
  public int getScrollableBlockIncrement(Rectangle visibleRect, int orientation, int direction) {
    if (orientation == SwingConstants.HORIZONTAL) return visibleRect.width - SEPARATION;
    else return visibleRect.height - ARCINC;
  }
