...
public class MyHashQueue implements StackCheck, Iterable, StateMap {
  ...
  class MyHashQueueIterator implements Iterator {
    ...
    public boolean hasNext() {
      ...
      while (iter == null && tableIter < table.length) {
        if (table[tableIter] != null) return true;
        else tableIter++;
      }
  ...
  private MyHashQueueEntry getEntry(byte[] key) throws IllegalArgumentException {
    int hash = StateCodec.hash(key) % table.length;
    ...
    if (tableEntry == null)
      throw new IllegalArgumentException("key passed to getEntry does not exist");

    return tableEntry;
  }

  public void addPut(MyHashQueueEntry qe) {
    if (qe.parent != null) qe.level = qe.parent.level + 1;
  ...
  public LinkedList<String> getPath(MyHashQueueEntry end, String[] actionNames) {
    ...
    while (end != null) {
      if (end.parent != null) trace.addFirst(actionNames[end.action]);
      end = end.parent;
    }
    return trace;
  }
