...
class StateMachine {
  ...
  public CompactState makeCompactState() {
    ...
    if (sequentialInserts != null) c.expandSequential(sequentialInserts);
    if (relabels != null) c.relabel(relabels);
    applyHiding(c);
    if (isProperty) {
  ...
  private void compactStateTransitions(CompactState c, boolean isProbabilistic) {
    ...
    while (e.hasMoreElements()) {
      ...
      //                } else {
      //                    int max = CompactStateUtils.getMaxBundle(c);//Esto no me sirve porque la
      // maqina la estoy generando
      //                    //entonces aun no tengo asignados todos los bundles. Necesito tener el
      // max de StateMachine.
  ...
  public void applyHiding(CompactState c) {
    if (hidden != null) {
      ...
      if (!exposeNotHide) c.conceal(hidden);
      else c.expose(hidden);
    }
  }
  ...
  void check_for_ERROR(CompactState c) {
    ...
    if (I.intValue() == Declaration.ERROR) {
      ...
      c.maxStates = 1;
      c.states[0] =
          EventStateUtils.add(c.states[0], new EventState(Declaration.TAU, Declaration.ERROR));
    }
  }
  ...
  private void renumber() { // relabel states
    ...
    // crunch aliases
    for (int i = 0; i < map.length; ++i) crunch(i, map);
    // renumber
    ...
    while (e.hasMoreElements()) {
      ...
      if (ii.intValue() >= 0) explicit_states.put(s, new Integer(map[ii.intValue()]));
    }
    stateLabel = newLabel;
  }
