...
/**
 * This class includes the logic to export an directed control problem to a PDDL model as supported
 * by PRP.
 */
public class PDDLTranslator extends AbstractTranslator {
  ...
  /**
   * Performs the PDDL translation storing the representation in an internal buffer. This completes
   * the template method <code>translate</code> of the <code>AbstractTranslator</code>, which
   * flushes the internal buffer into an output stream.
   */
  @Override
  ...
  protected void genDomainDefinition() {
    ...
    genSetEnabled();
    if (!uncontrollable
        .isEmpty()) { // if there are uncontrollable actions add the environment's non-deterministic
      // choice
      genPick();
      genDefault();
    }
  ...
  protected void genProblemDefinition() {
    ...
    open("(:" + goal());
    open("(and");
    if (!reachability) { // if we want liveness the goal is to close a loop over a completed task
      // (i.e., hoop)
      add("(status event)");
      for (Entry<String, LTS<Long, String>> lts : ltss.entrySet())
    ...
    if (excludeProblemPDDL) buffer.delete(current, end);
  }
  ...
  private void genStatus() {
    String liveness = "";
    if (!reachability) liveness = "looping event";
  ...
  private void genReset() {
    ...
    for (String label : alphabet) add("(not (enabled " + action(label) + "))");
    for (String label : uncontrollable) add("(not (inprogress " + action(label) + "))");
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      for (String label : lts.getValue().getActions())
        if (validAction(label)) add("(not (ready " + action(label) + " " + machine(lts) + "))");
    }
    if (!reachability) {
      for (Entry<String, LTS<Long, String>> lts : ltss.entrySet())
  ...
  private void genSetEnabled() {
    ...
    add("(not (status setup))");
    add("(not (status busy))");
    if (!reachability) add("(not (status event))");
    add("(status idle)");
    for (String label : alphabet) {
      open("(when");
      open("(and");
      for (Entry<String, LTS<Long, String>> lts : ltss.entrySet())
        if (lts.getValue()
            .getActions()
            .contains(
                label)) // every machine with the label in its alphabet must be ready in order to
          // enable the action
      ...
      add(
          "(enabled "
              + action(label)
              + ")"); // XXX I am changing (status uncontrollable) to indicate whether an
      // uncontrollable event was actually picked non-deterministcally
      //			if (uncontrollable.contains(label)) { // if an uncontrollable action is enabled deal with
      // events
      //				open("(and");
      //				add("(enabled " + action(label) + ")");
      //				add("(status uncontrollable)");
      ////				if (avoid.contains(label)) // enabling an uncontrollable avoid action is an error
      // already
      ////					add("(status error)");
      //				close(")");
      //			} else { // controllable
  ...
  /**
   * Generates the default action, which allows the planner to choose an enabled uncontrollable
   * action after a non-deterministic selection step selected an invalid action.
   */
  private void genDefault() {
    ...
    open(":effect");
    open("(and");
    for (String label : uncontrollable) add("(inprogress " + action(label) + ")");
    close(")");
    close("");
  ...
  /** For each action in the alphabet generates a PDDL action that updates the LTSs local states. */
  private void genActions() {
    for (String label : alphabet) {
      ...
      add("(status busy)");
      add("(enabled " + action(label) + ")");
      if (uncontrollable.contains(label)) add("(inprogress " + action(label) + ")");
      else add("(not (status uncontrollable))");
      close(")");
      close("");
      ...
      if (!reachability) add("(status event)");

      add("(not (status busy))");

      if (marking.contains(label)) add("(status complete)");
