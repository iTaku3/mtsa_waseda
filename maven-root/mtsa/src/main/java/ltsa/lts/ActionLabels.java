...
public abstract class ActionLabels {
  ...
  public boolean hasMultipleValues() {
    if (this instanceof ActionRange
        || this instanceof ActionSet
        || this instanceof ActionVarRange
        || this instanceof ActionVarSet) return true;
    else if (follower != null) return follower.hasMultipleValues();
    return false;
  }
  ...
  public ActionLabels myclone() {
    ...
    if (follower != null) an.follower = follower.myclone();
    return an;
  }
...
class ActionVarRange extends ActionRange {
  ...
  protected String computeName() {
    if (locals != null) locals.put(var.toString(), new Value(current));
    return String.valueOf(current);
  }
  ...
  protected void removeVarDefn() {
    if (locals != null) locals.remove(var.toString());
  }
...
class ActionVarSet extends ActionSet {
  ...
  protected String computeName() {
    ...
    if (locals != null) locals.put(var.toString(), new Value(s));
    return s;
  }
