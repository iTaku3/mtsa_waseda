...
/**
 * This abstract class contains the basic logic and data structures shared by translators from LTSs
...
public abstract class AbstractTranslator {
  ...
  /**
   * Template method that populates internal data structures shared by Translators and then invokes
   * the actual translation through the <code>doTranslate</code> method. Finally printing the
   * internal buffer with the result of the translation to the output stream.
   */
  public void translate(CompositeState composite, PrintStream output) {
    ...
    for (CompactState machine : machines) {
      ...
      for (String label : machine.getAlphabet()) if (validAction(label)) alphabet.add(label);
    }
    if (controllableActions
        != null) { // when no controllable actions are declared assume everything controllable
      Set<String> controllable = new HashSet<>(controllableActions);
      for (String action : alphabet) if (!controllable.contains(action)) uncontrollable.add(action);
    }
    goal = goalDef.getNameString();
    environment = composite.env.getName();
    int i = environment.indexOf(".");
    if (i > 0) environment = environment.substring(0, i);
  ...
  /**
   * Abstract translation method. Override this to take advantage of the template method <code>
   * translate</code>.
   */
  ...
  /**
   * Returns the goal type codified: 0) blocking + liveness 1) non-blocking + liveness 2) blocking +
   * reachability 3) non-blocking + reachability
   */
