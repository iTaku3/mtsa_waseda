...
public class GeneralizedBuchiAutomata {
  ...
  public int indexEquivalence(Node n) {
    int i;
    for (i = 0; i < maxId; i++) {
      if (equivClasses[i] == null) break;
      if (equivClasses[i].next.equals(n.next)) return equivClasses[i].id;
    }
    if (i == maxId) Diagnostics.fatal("size of equivalence classes array was incorrect");
    equivClasses[i] = n;
  ...
  Graph Gmake() {
    ...
    for (int k = 0; k < i; k++)
      if (states[k] != null && k == states[k].getId()) states[k].Gmake(anode, anode[k], naccept);

    if (naccept == 0) graph.setIntAttribute("nsets", 1);
    else graph.setIntAttribute("nsets", naccept);
    return graph;
  }
}
...
class State implements Comparable {
  ...
  void print(LTSOutput out, int nacc) {
    ...
    while (i.hasNext()) ((Transition) i.next()).print(out, nacc);
  }
...
class Transition {
  ...
  BitSet computeAccepting(int nacc) {
    ...
    for (int i = 0; i < nacc; ++i) if (!accepting.get(i)) b.set(i);
    return b;
  }

  void print(LTSOutput out, int nacc) {
    if (propositions.isEmpty()) out.out("LABEL True");
    else Node.printFormulaSet(out, "LABEL", propositions);
    out.out(" T0 " + goesTo());
    if (nacc > 0) out.outln(" Acc " + computeAccepting(nacc));
    else if (safe_acc) out.outln(" Acc {0}");
    else out.outln("");
  }
  ...
  void Gmake(gov.nasa.ltl.graph.Node anode[], gov.nasa.ltl.graph.Node node, int nacc) {
    ...
    if (nacc == 0) {
      ...
      for (int i = 0; i < nacc; i++)
        if (!accepting.get(i)) edge.setBooleanAttribute("acc" + i, true);
    }
  }
}
