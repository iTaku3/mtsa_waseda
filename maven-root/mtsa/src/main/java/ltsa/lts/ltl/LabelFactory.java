...
public class LabelFactory {
  ...
  public PredicateDefinition[] getFluents() {
    ...
    for (int i = 0; i < pds.length; ++i) pds[i] = (PredicateDefinition) fluents.get(i);
    return pds;
  }
  ...
  protected void compileProps() {
    ...
    while (ii.hasNext()) {
      ...
      } else Diagnostics.fatal("Proposition " + f + " not found", f.sym);
      ++m;
    }
      ...
      if (p != null) {
        ...
        p =
            new PredicateDefinition(
                new Symbol(Symbol.UPPERIDENT, f.toString()), trueActions, falseActions);
