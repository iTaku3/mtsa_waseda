...
public class Node implements Comparable {
  ...
  public Node(Formula f) {
    this();
    collapsed = false;
    if (!(f instanceof True)) decomposeAndforNext(f);
  }
  ...
  public void decomposeAndforNext(Formula f) {
    ...
    } else if (!isRedundant(next, null, f)) next.add(f);
  }
  ...
  private Node split(Formula f) {
    ...
    if (!oldf.contains(nf)) node.newf.add(nf);
    if (f instanceof Release) {
      nf = f.getSub1();
      if (!oldf.contains(nf)) node.newf.add(nf);
    }
    nf = f.getSub1();
    if (!oldf.contains(nf)) newf.add(nf);
    nf = ((f instanceof Until) || (f instanceof Release)) ? f : null;
    if (nf != null) decomposeAndforNext(nf);
  ...
  public List expand(List aut) {
    ...
    newf.remove(nf);
    if (contradiction(nf)) return aut;
    SortedSet oldUnew = new TreeSet();
    oldUnew.addAll(oldf);
    oldUnew.addAll(newf);
    if (isRedundant(oldUnew, next, nf)) return expand(aut);
    if (!nf.isLiteral()) {
      ...
      if (nf instanceof And) {
        ...
        if (!oldf.contains(f)) newf.add(f);
        f = nf.getSub2();
        if (!oldf.contains(f)) newf.add(f);
        if (nf.isRightOfUntil()) rightOfU.or(nf.getRofWU());
        return expand(aut);
      }
    ...
    if (!(nf instanceof True)) oldf.add(nf);
    if (nf.isRightOfUntil()) rightOfU.or(nf.getRofWU());
    return expand(aut);
  }
  ...
  private boolean compareAccepting(Node n) {
    if (id == 0 && !collapsed) return true;
    else return accepting.equals(n.accepting);
  }
  ...
  private boolean isSafetyAcc() {
    ...
    while (i.hasNext()) {
      Formula f = (Formula) i.next();
      if (!(f instanceof Release)) return false;
    }
    return true;
  }
  ...
  public void makeTransitions(State astate[]) {
    boolean flag = false;
    if (astate[id] == null) astate[id] = new State(equivId);
    else astate[id].setId(equivId);
