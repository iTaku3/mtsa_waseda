...
/**
 * Visits a formula and stores the resulting transformed Formula and the Fluents occurring in the
 * Formula
 *
 * @author gsibay
 */
public class FormulaTransformerVisitor implements FormulaVisitor {
  ...
  private Fluent createFluent(Proposition proposition) {
    ...
    Set<Symbol> initiatingActions =
        this.transformFluentActions(predicateDefinition.getInitiatingActions());
    Set<Symbol> terminatingActions =
        this.transformFluentActions(predicateDefinition.getTerminatingActions());
