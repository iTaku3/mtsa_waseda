    ...
    if (proposition != null) {
      if (range == null) {
        if (PredicateDefinition.definitions != null
          ...
          AssertDefinition p =
              (AssertDefinition) AssertDefinition.definitions.get(proposition.toString());
          if (p == null)
            Diagnostics.fatal("LTL fluent or assertion not defined: " + proposition, proposition);
          if (parameters == null) return p.ltl_formula.expand(fac, locals, p.init_params);
          else {
            if (parameters.size() != p.params.size())
              Diagnostics.fatal(
                  "Actual parameters do not match formals: " + proposition, proposition);
      ...
      if (left == null) {
        ...
        return fac.make(
            left.expand(fac, locals, globals), operator, right.expand(fac, locals, globals));
      }
      ...
      while (range.hasMoreNames()) {
        range.nextName();
        if (f == null) f = right.expand(fac, locals, globals);
        else {
          if (operator.kind == Symbol.AND) f = fac.makeAnd(f, right.expand(fac, locals, globals));
          else f = fac.makeOr(f, right.expand(fac, locals, globals));
        }
      }
      range.clearContext();
      return f;
  ...
  public FormulaSyntax removeLeftTemporalOperators() {
    ...
    if (this.operator != null) {
      if (this.operator.kind == Symbol.ALWAYS) {
        ...
        answer =
            new FormulaSyntax(
                this.left.removeLeftTemporalOperators(),
                this.operator,
                this.right.removeLeftTemporalOperators(),
                this.proposition,
                this.range,
                this.action,
                this.parameters);
