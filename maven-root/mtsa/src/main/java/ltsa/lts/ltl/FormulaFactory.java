...
public class FormulaFactory {
  ...
  public Formula make(Symbol sym, ActionLabels range, Hashtable locals, Hashtable globals) {
    ...
    while (range.hasMoreNames()) {
      ...
      if (f == null) f = make(newSym);
      else f = makeOr(f, make(newSym));
    }
    range.clearContext();
    return f;
  }
  ...
  public Formula make(Stack expr, Hashtable locals, Hashtable globals) {
    if (Expression.evaluate(expr, locals, globals).intValue() > 0) return True.make();
  ...
  public Formula make(ActionLabels act, Hashtable locals, Hashtable globals) {
    ...
    if (!actionPredicates.containsKey(name)) actionPredicates.put(name, av);
  ...
  public Formula make(Formula left, Symbol op, Formula right) {
    switch (op.kind) {
      ...
      case Symbol.NEXTTIME:
        if (normalLTL) return makeNext(right);
        else return makeNext(makeWeakUntil(makeNot(makeTick()), makeAnd(makeTick(), right)));
      case Symbol.EVENTUALLY:
        if (normalLTL) return makeEventually(right);
        else return makeEventually(makeAnd(makeTick(), right));
      case Symbol.ALWAYS:
        if (normalLTL) return makeAlways(right);
        else return makeAlways(makeImplies(makeTick(), right));
      case Symbol.AND:
      ...
      case Symbol.UNTIL:
        if (normalLTL) return makeUntil(left, right);
        else return makeUntil(makeImplies(makeTick(), left), makeAnd(makeTick(), right));
      case Symbol.WEAKUNTIL:
        if (normalLTL) return makeWeakUntil(left, right);
        else return makeWeakUntil(makeImplies(makeTick(), left), makeAnd(makeTick(), right));
  ...
  Formula makeAnd(Formula left, Formula right) {
    ...
    if (left.compareTo(right) < 0) return unique(new And(left, right));
    else return unique(new And(right, left));
  }
  ...
  Formula makeOr(Formula left, Formula right) {
    ...
    if (left.compareTo(right) < 0) return unique(new Or(left, right));
    else return unique(new Or(right, left));
  }
  ...
  boolean syntaxImplied(Formula f, SortedSet one, SortedSet two) {
    ...
    boolean cf;
    if (c != null) {
      if (two != null) cf = two.contains(c);
      else cf = false;
    } else cf = true;
    if ((f instanceof Until) || (f instanceof Or)) return bf || af && cf;
    if (f instanceof Release) return af && bf || af && cf;
    if (f instanceof And) return af && bf;
    if (f instanceof Next) {
      if (a != null) {
        if (two != null) return two.contains(a);
        else return false;
      } else {
        return true;
      }
  ...
  private Formula unique(Formula f) {
    String s = f.toString();
    if (subf.containsKey(s)) return (Formula) subf.get(s);
    else {
      f.setId(newId());
      subf.put(s, f);
