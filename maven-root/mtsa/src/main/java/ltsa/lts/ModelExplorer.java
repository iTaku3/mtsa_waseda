...
public class ModelExplorer {
  public static boolean isEND(ModelExplorerContext ctx, int[] state) {
    if (!ctx.canTerminate) return false;
    for (int i = 0; i < ctx.Nmach; i++) {
      // if (sm[i].endseq < 0)
      // ; // skip
      /*else*/
      if (ctx.sm[i].endseq != state[i]) return false;
    }
    return true;
  }
  ...
  public static List eligibleTransitions(ModelExplorerContext ctx, int[] state) {
    ...
    if (ctx.partial != null) {
      if (ctx.asteriskEvent > 0
          && EventState.hasEvent(
              ctx.sm[ctx.Nmach - 1].states[state[ctx.Nmach - 1]], ctx.asteriskEvent)) {
        // do nothing
      } else {
      ...
      while (p != null) { // foreach transition
        ...
        if (tr.event != 0 && ac[tr.event] == 0) {
          ...
          if (ctx.highAction != null
              && ctx.highAction.get(tr.event)
              && tr.event != ctx.asteriskEvent) ++highs;
        }
    ...
    while (nsucc > 0) { // do this loop once per successor state
      ...
      if (highs <= 0 || ctx.highAction.get(actionNo) || actionNo == ctx.acceptEvent) {
        ...
        while (tr != null) { // test for non determinism or probabilistic transitions
          ...
          if (nonDeterministic || probabilistic) break;
        ...
        if (!nonDeterministic && !probabilistic) {
          ...
          if (actionNo != ctx.asteriskEvent) transitions.add(next);
          else {
          ...
          if (actionNo != ctx.asteriskEvent) {
            ...
            computeNonDetTransitions(ctx, tr, state, asteriskTransitions = new ArrayList(4));
          }
        } else {
    ...
    if (ctx.asteriskEvent < 0) return transitions;
    else return mergeAsterisk(ctx, transitions, asteriskTransitions);
  }
  ...
  private static void computeTauTransitions(
      ModelExplorerContext ctx, EventState first, int[] state, List v) {
  ...
  private static void computeNonDetTransitions(
      ModelExplorerContext ctx, EventState first, int[] state, List v) {
    EventState tr = first;
    while (tr != null) {
      int[] next = LTSUtils.myclone(state);
    ...
    while (tr != null) {
      ...
      while (probTr != null) {
        if (bundles == null) {
          ...
          for (int i = 0; i < ctx.Nmach; i++) bundles[i] = ProbabilisticTransition.NO_BUNDLE;
        }

        if (probs == null) probs = new BigDecimal[ctx.Nmach];
  ...
  private static List mergeAsterisk(
      ModelExplorerContext ctx, List transitions, List asteriskTransitions) {
