...
/**
 * This class contains the logic to export a directed control problem to an LTL formula with a GR(1)
 * goal in the structuredslugs format as supported by slugs.
 */
public class SlugsTranslator extends CTLPYTranslator {
  ...
  protected void genFormula() {
    add("[ENV_INIT]");
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) genInit(lts);
    newLine();
    ...
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) genControllableConstraints(lts);
    newLine();
  ...
  protected void genGoal() {
    ...
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) {
      ...
      if (!ltsGoal.isEmpty()) buffer.append("(" + ltsGoal + ")" + and());
    }
    removeLast(and());
    add(")");
  }
  ...
  /**
   * Returns an empty always operator (slug does not use this operator and hence we remove it from
   * super's behavior).
   */
  @Override
