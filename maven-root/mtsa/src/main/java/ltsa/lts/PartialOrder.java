...
public class PartialOrder {
  ...
  private int
      candidates[][]; // 0=not a candidate, 1= all independent actions, 2= max shared by two etc.
  ...
  private int countSet(BitSet b) {
    int count = 0;
    int len = b.length();
    for (int i = 0; i < len; i++) if (b.get(i)) count++;
    return count;
  }
  ...
  private int candidateNumber(int mach, int state, int[] actions) {
    ...
    for (int i = 0; i < actions.length; ++i) {
      ...
      if (high != null && !high.get(a))
        return 0; // eliminate if contains low priority action (if priority)
      if (a == 0) shared = 1; // tau
      else shared = actionSharedBy[a].length;
      if (shared == 1) ++singles;
  ...
  private int getPartner(int mach, int event) {
    if (actionSharedBy[event][0] == mach) return actionSharedBy[event][1];
    else return actionSharedBy[event][0];
  }
}
