...
public class Minimiser {
  ...
  private CompactState machTau(CompactState m) {
    ...
    for (int i = 0; i < m.states.length; i++) m.states[i] = EventState.tauAdd(m.states[i], T);
  ...
  private CompactState removeTau(CompactState m) {
    for (int i = 0; i < m.states.length; i++) m.states[i] = EventState.removeTau(m.states[i]);
    return m;
  }
  ...
  private void initialise() {
    ...
    for (int i = 0; i < E.length; i++) E[i] = new BitSet(E.length);
  ...
  public CompactState trace_minimise() {
    ...
    // now minimise
    if (must_minimize) return minimise();
    else return machine;
  }
  ...
  private CompactState makeNewMachine() {
    ...
    if (machine.endseq < 0) m.endseq = machine.endseq;
    else {
  ...
  private CompactState handleMarkedCompactState(
      CompactState m, CompactState machine, Map<Integer, Integer> oldToNew) {
