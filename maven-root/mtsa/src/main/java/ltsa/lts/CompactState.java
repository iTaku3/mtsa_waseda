...
public class CompactState implements Automata {
    ...
    while (!transitions.empty()) {
      ...
      if (transitions.peek() instanceof MyProbListEntry) {
        ...
        states[fromState] =
            EventStateUtils.add(
                states[fromState],
                new ProbabilisticEventState(transitions.getAction(), toState, prob, bundle));
      } else {
        states[fromState] =
            EventStateUtils.add(
                states[fromState], new EventState(transitions.getAction(), toState));
      }
      transitions.next();
    }
    endseq = endSequence;
  ...
  public void removeAtActions() {
    ...
    assertEquals(
        "there should only be one accepting action, since cs.composition.isSafetyOnly()",
        1,
        amountOfAccepting);

    for (int i = 0; i < states.length; i++) {
      assertFalse(
          "We tried removing accepting (\"@\" starting) actions from CompactState alphabet, "
  ...
  public String getMtsControlProblemAnswer() {
    if (this.mtsControlProblemAnswer.equals("")) return "NONE";
    else return this.mtsControlProblemAnswer;
  }
  ...
  public void removeNonDetTau() {
    ...
    while (true) {
      ...
      for (int i = 0; i < maxStates; ++i) {
        if (!tauOnly.get(i)) states[i] = EventState.addNonDetTau(states[i], states, tauOnly);
      }
  ...
  public boolean isSafetyOnly() {
    ...
    for (int i = 0; i < maxStates; i++) {
      if (EventState.isAccepting(states[i], alphabet)) {
        ++acceptStates;
        if (EventState.isTerminal(i, states[i])) ++terminalAcceptStates;
      }
    }
  ...
  public boolean hasERROR() {
    for (int i = 0; i < maxStates; i++)
      if (EventState.hasState(states[i], Declaration.ERROR)) return true;
    return false;
  }
  ...
  public void relabel(Relation oldtonew) {
    hasduplicates = false;
    if (oldtonew.isRelation()) relational_relabel(oldtonew);
    else functional_relabel(oldtonew);
  }
  ...
  private void functional_relabel(Hashtable<?, ?> oldtonew) {
    for (int i = 1; i < alphabet.length; i++) { // don't relabel tau
      ...
      if (newlabel != null) alphabet[i] = newlabel;
      else alphabet[i] = prefixLabelReplace(i, oldtonew);
    }
    checkDuplicates();
  }
  ...
  public Vector<String> hide(Vector<String> toShow) {
    ...
    for (int i = 1; i < alphabet.length; i++) {
      if (!contains(alphabet[i], toShow)) toHide.addElement(alphabet[i]);
    }
    return toHide;
  }
  ...
  public void unMakeProperty() {
    endseq = -9999;
    prop = false;
    for (int i = 0; i < maxStates; i++) states[i] = EventState.removeTransToError(states[i]);
  }

  public boolean isNonDeterministic() {
    for (int i = 0; i < maxStates; i++) if (EventState.hasNonDet(states[i])) return true;
    return false;
  }
  ...
  public void printAUT(PrintStream out) {
    ...
    if (states[0] instanceof ProbabilisticEventState) out.print("// probabilistic\n");

    out.print("des(0," + ntransitions() + "," + maxStates + ")\n");
    for (int i = 0; i < states.length; i++) EventStateUtils.printAUT(states[i], i, alphabet, out);
  }
  ...
  public CompactState myclone() {
    ...
    //            }
    //        }
    for (int i = 0; i < maxStates; i++) m.states[i] = EventStateUtils.union(m.states[i], states[i]);
  ...
  public int ntransitions() {
    int count = 0;
    for (int i = 0; i < states.length; i++) count += EventStateUtils.count(states[i]);
    return count;
  }
  ...
  private int maximalPrefix(String s, Hashtable<?, ?> oldtonew) {
    ...
    if (oldtonew.containsKey(s.substring(0, prefix_end))) return prefix_end;
    else return maximalPrefix(s.substring(0, prefix_end), oldtonew);
  }

  private static boolean isPrefix(String prefix, String s) {
    int prefix_end = s.lastIndexOf('.');
    if (prefix_end < 0) return false;
    if (prefix.equals(s.substring(0, prefix_end))) return true;
    else return isPrefix(prefix, s.substring(0, prefix_end));
  }
  ...
  public boolean isErrorTrace(Vector<?> trace) {
    ...
    for (int i = 0; i < maxStates && !hasError; i++)
      if (EventState.hasState(states[i], Declaration.ERROR)) hasError = true;
  ...
  private boolean isTrace(Vector<?> v, int index, int start) {
    ...
    } else return (start == Declaration.ERROR);
  }
  ...
  */
  ...
  private void addtransitions(Relation oni) {
    for (int i = 0; i < states.length; i++) {
      ...
      if (ns != null) states[i] = EventStateUtils.union(states[i], ns);
    }
  }
  ...
  public boolean hasLabel(String label) {
    for (int i = 0; i < alphabet.length; ++i) if (label.equals(alphabet[i])) return true;
    return false;
  }
  ...
  private static int seqSize(CompactState[] sm) {
    int length = 0;
    for (int i = 0; i < sm.length; i++) length += sm[i].states.length;
    return length;
  }
  ...
  private static void copyOffset(int offset, EventState[] dest, CompactState m, boolean last) {
    for (int i = 0; i < m.states.length; i++) {
      if (!last)
        dest[i + offset] =
            EventState.offsetSeq(offset, m.endseq, m.maxStates + offset, m.states[i]);
      else
  ...
  public Vector<String> getAlphabetV() {
    ...
    for (int i = 1; i < alphabet.length; ++i) v.add(alphabet[i]);
    return v;
  }
  ...
  public MyList getTransitions(byte[] fromState) {
    ...
    int state;
    if (fromState == null) state = Declaration.ERROR;
    else state = decode(fromState);
  ...
  public BitSet accepting() {
    BitSet b = new BitSet();
    for (int i = 0; i < maxStates; ++i) if (isAccepting(i)) b.set(i);
    return b;
  }
  ...
  Map<String, Integer> alphabetMap() {
    ...
    for (int i = 0; i < alphabet.length; i++) alphaMap.put(alphabet[i], i);
    return alphaMap;
  }

  public int getEvent(String action) {
    for (int i = 0; i < this.alphabet.length; i++) if (this.alphabet[i].equals(action)) return i;
  ...
  public void swapStates(int i, int j) {
    ...
    for (EventState state : this.states) if (state != null) state.swapNext(i, j);
  }
  ...
  /**
   * Simulates a run of this nondeterministic automaton on the given word. If stopOnError is set,
   * and the error state is encountered during simulation, the ErrorStateReachedException is thrown;
   * otherwise the exception is never thrown. The set of possible current states as a result of
  ...
  public List<Integer> simulate(List<String> word, List<String> wordAlphabet, boolean stopOnError)
    ...
    for (int s = 0; s < state.length(); s++) if (state.get(s)) v.add(s);
    return v;
  }
  ...
  public List<Integer> closure(List<Integer> state, List<String> closeActions, boolean stopOnError)
    ...
    for (int s : state) stateBS.set(s);

    Map<String, Integer> alphaMap = alphabetMap();
    Set<Integer> closeEvents = new HashSet<Integer>();
    for (String a : closeActions) closeEvents.add(alphaMap.get(a));

    BitSet nextStateBS = nfaClosure(stateBS, closeEvents, stopOnError);

    List<Integer> v = new Vector<Integer>();
    for (int s = 0; s < nextStateBS.length(); s++) if (nextStateBS.get(s)) v.add(s);
    return v;
  }

  /**
   * Computes the possible set of states that could result from the given sequence of events
   * occurring from the given set of states. If stopOnError is set, then the
   ...
   * @throws ErrorStateReachedException
   */
  BitSet nfaSimulation(List<String> word, List<String> wordAlphabet, boolean stopOnError)
      throws NoStateReachedException, ErrorStateReachedException {
    ...
    Map<String, Integer> alphaMap = alphabetMap();
    Set<Integer> wordEvents = new HashSet<Integer>(),
        skipEvents = new HashSet<Integer>(alphaMap.values());

    // Assuption: tau is represented with 0.
    skipEvents.add(0);

    for (String a : wordAlphabet) wordEvents.add(alphaMap.get(a));
    ...
    for (String event : word) {
      ...
      if (!alphaMap.containsKey(event)) continue;
      else
        try {
          state =
              nfaClosure(
                  nfaNextState(state, alphaMap.get(event), stopOnError), skipEvents, stopOnError);

          if (state == null || state.isEmpty()) throw new NoStateReachedException(prefixTrace);
  ...
  /**
   * Computes the possible set of states that could result from the given event occurring once at
   * the given set of states. If stopOnError is set, then the ErrorStateReachedException is thrown
   * upon encountering the error state.
   *
  ...
  BitSet nfaNextState(BitSet state, int event, boolean stopOnError)
    ...
    for (int s = 0; s < state.length(); s++) {
      if (state.get(s)) {
        if (s == Declaration.ERROR && stopOnError) throw new ErrorStateReachedException();

        int[] next = EventState.nextState(this.states[s], event);
        if (next != null)
          for (int t : next)
            if (t == Declaration.ERROR && stopOnError) throw new ErrorStateReachedException();
            else nextState.set(t);
      }
    }
    return nextState;
  }
  ...
  /**
   * Computes the set of states reachable from the given set under zero or more repeated occurrences
   * of the null action (tau). If stopOnError is set, then the ErrorStateReachedException is thrown
   * upon encountering the error state.
   *
   * @param state The initial set of states
   * @param stopOnError Whether to stop when the error state is reached
   ...
   * @throws ErrorStateReachedException
   */
  BitSet nfaTauClosure(BitSet state, boolean stopOnError) throws ErrorStateReachedException {
  ...
  /**
   * Computes the set of states reachable from the given set under zero or more repeated occurrences
   * of the given events. If stopOnError is set, then the ErrorStateReachedException is thrown upon
   * encountering the error state.
   *
   * @param state The initial set of states
