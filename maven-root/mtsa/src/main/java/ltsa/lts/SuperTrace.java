...
public class SuperTrace {
  ...
  public void analyse() {
    output.outln(
        "Analysing using Supertrace (Depth bound "
            + DEPTHBOUND
            + " Hashtable size "
            + HASHSIZE
            + "K )...");
    ...
    if (ret == DEADLOCK) {
      ...
      if (errorTrace.size() <= 100) printPath(errorTrace);
      else output.outln("Trace length " + errorTrace.size() + ", replay using Check/Run");
    } else if (ret == ERROR) {
  ...
  private int search() {
    ...
    while (!stack.empty()) {
      if (stack.marked()) {
        ...
        ++nstate;
        if (nstate % 10000 == 0) outStatistics(stack.getDepth(), nstate, nTrans);
