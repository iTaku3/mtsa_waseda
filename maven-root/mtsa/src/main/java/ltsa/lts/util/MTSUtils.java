...
/**
 * This class consists exclusively of static methods that operate on or return CompactState and
 * CompositeState.
 */
public class MTSUtils {
  ...
  /**
   * Returns true in tow cases: if the composition field of <code>compositeState</code> it is not
   * null and it's an MTS, or if there is at least one MTS in the machines field of <code>
   * compositeState</code> otherwise returns false
   *
  ...
  /**
   * Returns true if there is at least one maybe transition in <code>compactState</code>
   *
   * @param automata
   */
  public static boolean isMTSRepresentation(CompactState compactState) {
    for (int i = 0; i < compactState.states.length; i++) {
      MyList transitions = compactState.getTransitions(MTSUtils.encode(i));
  ...
  /**
   * Build a new String array with the original Strings plus the same ones with maybe mark.
   *
  ...
  /**
   * Returns an array with the same elements but with "tau?" label added in the second place.
   *
   * @param oldAlphabet
   * @return the new alphabet
   */
  ...
  protected <State, Action> boolean orphanState(
      State to, MTS<State, Action> mts, TransitionType type) {
    for (State state : mts.getStates()) {
      for (Pair<Action, State> transition : mts.getTransitions(state, type)) {
  ...
  /** Remove all actions in <code>actionNames</code> from the <code>mts</code> alphabet. */
  public static <State, Action> void removeActionsFromAlphabet(
    ...
    for (Action actionToRemove : new ArrayList<Action>(toDelete)) {
      for (State stateFrom : mts.getStates()) {
        for (State stateTo :
            mts.getTransitions(stateFrom, TransitionType.POSSIBLE).getImage(actionToRemove)) {
  ...
  public static boolean isEmptyMTS(CompactState compactState) {
    MTS<Long, String> mts =
        AutomataToMTSConverter.getInstance().convert((CompactState) compactState);
    return mts.getTransitions(mts.getInitialState(), TransitionType.POSSIBLE).size() == 0;
  }
