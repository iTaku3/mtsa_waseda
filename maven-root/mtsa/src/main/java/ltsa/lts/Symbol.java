...
public class Symbol {
  ...
  public Color getColor() {
    if (kind > 0 && kind <= INIT || blueSymbols.contains(kind)) return Color.blue;
    else if (kind == COMMENT) return commentColor;
    else if (kind == INT_VALUE || kind == STRING_VALUE || kind == DOUBLE_VALUE) return Color.red;
    else if (kind == UPPERIDENT) return upperColor;
  ...
  public static final int UPDATING_CONTROLLER =
      3200; // 3200 series are for updating controller problem
