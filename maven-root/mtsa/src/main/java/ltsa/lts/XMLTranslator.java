...
/**
 * This class includes the logic to export a directed control problem to an XML model as supported
 * by Supremica.
 */
public class XMLTranslator extends AbstractTranslator {
  ...
  /**
   * Performs the XML translation storing the representation in an internal buffer. This completes
  ...
  protected void genPlant() {
    for (Entry<String, LTS<Long, String>> lts : ltss.entrySet()) genAutomaton(lts);
  }
    ...
    for (Long state : lts.getValue().getStates()) {
      ...
      add(
          "<State id="
              + state(state)
              + " name="
              + quote(machine(lts) + "[" + state + "]")
              + // names should be optional
              " accepting="
              + (state.equals(-1l) ? "\"false\"" : "\"true\"")
  ...
  protected void genTransitions(Entry<String, LTS<Long, String>> lts) {
    open("<Transitions>");
    for (Entry<Long, BinaryRelation<String, Long>> transition :
        lts.getValue().getTransitions().entrySet()) {
  ...
  protected String id(String label) {
    ...
    if (result == null) ids.put(label, result = ids.size());
