...
public class MTSAEnactmentSimulation<State, Action> implements Runnable {
  ...
  public void runSimulation(
      CompositeState currentComposition,
    ...
    MTS<Long, String> mts =
        AutomataToMTSConverter.getInstance().convert(currentComposition.composition);
    LTSAdapter<Long, String> ltsAdapter =
        new LTSAdapter<Long, String>(mts, TransitionType.REQUIRED);
    Set<String> controllableActions;

    controllableActions = currentComposition.goal.getControllableActions();

    SchedulerFactory<Long, String> schedulerFactory =
        applicationContext.getBean(SchedulerFactory.class);
    ...
    try {
      controllerScheduler =
          schedulerFactory.getScheduler(
              enactmentOptions.scheduler, ltsAdapter, controllableActions);
    } catch (Exception e1) {
      e1.printStackTrace();
      return;
    }
    ...
    runEnactorsWindow.addWindowListener(
        new WindowListener() {
          @Override
    ...
    while (this.isRunning()) {
      try {
        if (this.isPauseSimulation()) {
          if (!controllerScheduler.isStop()) controllerScheduler.stop();
        } else if (controllerScheduler.isStop()) controllerScheduler.start();
