...
public class TreeLikeGraphLayout<V, E> implements Layout<V, E> {
  ...
  public TreeLikeGraphLayout(Graph<V, E> g) {
    if (g == null) throw new IllegalArgumentException("Graph must be non-null");
  ...
  /**
   * Computes the depths of every node by a BFS on the graph. Stores the depth in {depths} and the
   * maximal depth in {maxDepth}.
   */
  ...
  /**
   * Set locations of nodes for subtree from {v} within rectangle such that top-left corner is ({x},
   * {y}). Ignore nodes in {alreadyDone}.
   */
  ...
  /**
   * Calculates the total width (nbr nodes - 1) of the subtree rooted at node {v}. Stores it in
   * {treeWidth}. {depths} must have been previously computed.
   *
  ...
  private int computeTreeWidth(V v, Set<V> frontier) {
    ...
    for (V element : graph.getSuccessors(v)) {
      if (depths.get(element) == depth + 1 && !frontier.contains(element)) {
