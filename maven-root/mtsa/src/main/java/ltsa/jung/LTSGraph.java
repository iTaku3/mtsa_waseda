...
public class LTSGraph extends DirectedSparseMultigraph<StateVertex, TransitionEdge> {
  ...
  public LTSGraph(CompactState lts) {
    ...
    for (int currentState = 0; currentState < lts.maxStates; currentState++) {
      ...
      try {
        ...
        final Enumeration<EventState> enumEvent =
            (Enumeration<EventState>) lts.states[currentState].elements();
    ...
    while (enumTrans.hasNext()) {
      ...
      if (from != null && to != null) {
        ...
        if (!added) addEdge(trans, from, to);
      } else
        System.out.println(
            trans
                + String.valueOf(trans.getOriginState())
                + String.valueOf(trans.getDestinationState()));
    }
  ...
  public int numberSCC(StateVertex v) {
    ...
    for (int set = 0; set < scc.size(); set++) {
      if (scc.get(set).contains(v)) return set + 1;
    }
    return 0;
  }

  private void computeSCC(LTSGraph lts) {
    StronglyConnectedComponentClusterer<StateVertex, TransitionEdge> clusterer =
        new StronglyConnectedComponentClusterer<StateVertex, TransitionEdge>();
