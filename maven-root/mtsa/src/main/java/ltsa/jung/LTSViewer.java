...
public class LTSViewer extends VisualizationViewer<StateVertex, TransitionEdge> {
  ...
  public LTSViewer(
      Layout<StateVertex, TransitionEdge> layout, Dimension preferredSize, LTSJUNGCanvas canvas) {
  ...
  private void select(Set<StateVertex> vertices) {
    ...
    if (interaction == EnumMode.Activate) {
      navigator =
          ((LTSGraph) this.getGraphLayout().getGraph())
              .getNavigator(new HashSet<StateVertex>(this.getPickedVertexState().getPicked()));
      refresh();
    }
  }
  ...
  private void paintEdgeActivate() {
    final Transformer<TransitionEdge, String> stringer =
        new Transformer<TransitionEdge, String>() {
    ...
    final QuadCurve<StateVertex, TransitionEdge> shaper =
        new EdgeShape.QuadCurve<StateVertex, TransitionEdge>();
  ...
  private void paintEdgeEdit() {
    ...
    class EdgeLabelColorRenderer extends DefaultEdgeLabelRenderer {
      ...
      public <E> Component getEdgeLabelRendererComponent(
        ...
        Component out = super.getEdgeLabelRendererComponent(vv, value, font, isSelected, edge);
        super.setForeground(
            canvas.getSelectedTransitions().contains(edge)
                ? EditColors.Edge.animated.darker().darker()
                : EditColors.Edge.label);
        return out;
      }
    }
    final Transformer<TransitionEdge, Paint> colorer =
        new EdgePaintTransformer(this.getPickedEdgeState());
    final QuadCurve<StateVertex, TransitionEdge> shaper =
        new EdgeShape.QuadCurve<StateVertex, TransitionEdge>();
    shaper.setControlOffsetIncrement(canvas.getCurve());
    ...
    getRenderContext().getEdgeLabelRenderer().setRotateEdgeLabels(false);
    getRenderContext()
        .setArrowDrawPaintTransformer(new EdgePaintTransformer(this.getPickedEdgeState()));
    getRenderContext()
        .setArrowFillPaintTransformer(new EdgePaintTransformer(this.getPickedEdgeState()));
    getRenderContext().setEdgeShapeTransformer(shaper);
  }
  ...
  private void paintVertexActivate() {
    final Transformer<StateVertex, String> stringer =
        new Transformer<StateVertex, String>() {
    ...
    class VertexLabelColorRenderer extends DefaultVertexLabelRenderer {
      ...
      public <E> Component getVertexLabelRendererComponent(
          JComponent vv, Object value, Font font, boolean isSelected, E edge) {
    ...
    DefaultVertexLabelRenderer labeler =
        new VertexLabelColorRenderer(NavigationColors.Vertex.label, NavigationColors.Vertex.label);
  ...
  private void paintVertexEdit() {
    ...
    class VertexPaintTransformer implements Transformer<StateVertex, Paint> {
      ...
      public Paint transform(StateVertex v) {
        Color color;
        return p.isPicked(v)
            ? EditColors.Vertex.picked
            : ((canvas.shouldColorSCC() && (color = isInSCC(v)) != null)
                ? color
                : canvas.getSelectedVertices().contains(v)
                    ? EditColors.Vertex.animated
                    : (v.toString().equals("0") && canvas.getSelectedVertices().size() == 0
                        ? EditColors.Vertex.animated
                        : (v.toString().equals("-1")
                            ? EditColors.Vertex.error
    ...
    final Transformer<StateVertex, Paint> colorer =
        new VertexPaintTransformer(getPickedVertexState());
    final VertexLabelColorRenderer labeler =
        new VertexLabelColorRenderer(EditColors.Vertex.label, EditColors.Vertex.label);
