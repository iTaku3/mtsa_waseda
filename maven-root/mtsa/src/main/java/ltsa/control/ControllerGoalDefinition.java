...
public class ControllerGoalDefinition {
  ...
  private static Hashtable<String, ControllerGoalDefinition> definitions =
      new Hashtable<String, ControllerGoalDefinition>();
  ...
  public static void put(ControllerGoalDefinition goal) {
    if (definitions.put(goal.getNameString(), goal) != null) {
      Diagnostics.fatal("duplicate Goal definition: " + goal.getName(), goal.getName());
    }
  }
  ...
  public static ControllerGoalDefinition getDefinition(Symbol definitionName) {
    ...
    ControllerGoalDefinition definition = definitions.get(definitionName.getName());
    if (definition == null) {
  ...
  public boolean addGuaranteeDefinition(Symbol guaranteeDefinition) {
    if (!this.guaranteeDefinitions.add(guaranteeDefinition)) {
      Diagnostics.fatal(
          "Duplicate Guarantee : " + guaranteeDefinition.getName(), guaranteeDefinition.getName());
      return false;
    } else {
      return true;
    }
  ...
  public boolean addFaultDefinition(Symbol faultDefinition) {
    if (!this.faultsDefinitions.add(faultDefinition)) {
      Diagnostics.fatal(
          "Duplicate Fault : " + faultDefinition.getName(), faultDefinition.getName());
      return false;
    } else {
      return true;
    }
  ...
  public boolean addAssumeDefinition(Symbol assumeDefinition) {
    if (!this.assumeDefinitions.add(assumeDefinition)) {
      Diagnostics.fatal(
          "Duplicate Assumption : " + assumeDefinition.getName(), assumeDefinition.getName());
      return false;
    } else {
      return true;
    }
  ...
  public boolean addSafetyDefinition(Symbol safetyDefinition) {
    if (!this.safetyDefinitions.add(safetyDefinition)) {
      Diagnostics.fatal(
          "duplicate Safety requirement: " + safetyDefinition.getName(),
  ...
  public boolean addBuchiDefinition(Symbol buchiDefinition) {
    if (!this.buchiDefinitions.add(buchiDefinition)) {
      Diagnostics.fatal(
          "duplicate Buchi requirement: " + buchiDefinition.getName(), buchiDefinition.getName());
      return false;
    } else {
      return true;
    }
  ...
  public static void setDefinitions(Hashtable<String, ControllerGoalDefinition> definitions) {
    ControllerGoalDefinition.definitions = definitions;
  }

  public void setLazyness(Integer lazyness) {
    Validate.isTrue(
        lazyness >= 0, "Lazyness value must be greater than zero. Where zero means no lazyness.");
  ...
  /**
   * @param maxControllers a number greater than zero. Use for test_latency = {int, int}
   */
  public void setMaxControllers(Integer maxControllers) {
    Validate.isTrue(
        maxControllers > 0, "The number of controllers value must be greater than zero.");
    this.maxControllers = maxControllers;
  }

  /**
   * @param maxSchedulers a number greater than zero. Use for test_latency = {int, int}
   */
