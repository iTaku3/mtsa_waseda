...
public class ControllerUtils {
  ...
  public static MTS<Long, String> removeTopStates(
      MTS<Long, String> mts, Collection<Fluent> fluents) {
        ...
        for (String actionInFluent : fluentActions) {
          if (!mtsActions.contains(actionInFluent)) {
            Logger.getAnonymousLogger()
                .warning(
                    "While doing 'removeTopStates'. Action: "
                        + actionInFluent
                        + " is not in MTS alphabet");
            if (actionInFluent.equals("*")) {
              throw (new Error(
                  "Liveness goals and assumptions may not use events. Please define a fluent"
                      + " instead. "));
            }
          }
        }
    ...
    //		MTS<Long, String> result = new MTSMultipleComposer<Long,
    // String>(compositionRuleApplier).compose(toCompose);
  ...
  /**
   * Generates, a domain model as described in [FSE2010]. Uses <i>output</i> to log the process.
   *
  ...
  public static MTS<Long, String> embedFluents(
      MTS<Long, String> mts, ControllerGoal<String> goal, LTSOutput output) {
  ...
  public static MTS<Long, String> embedFluentsNoText(
      MTS<Long, String> mts, ControllerGoal<String> goal, LTSOutput output) {
    MTS<Long, String> result = ControllerUtils.removeTopStates(mts, goal.getFluents());
    return result;
  }

  // to be refactored. It needs to be generalised as a strategy together with the subset
  // construction for MTS Control.
  public static MTS<Pair<Long, Set<String>>, String> getSubSetConstructionFor(
      MTS<Long, String> mts, Set<String> controllable, LTSOutput output) {
    MTS<Pair<Long, Set<String>>, String> result =
        new MTSImpl<Pair<Long, Set<String>>, String>(
            Pair.create(mts.getInitialState(), (Set<String>) null));
    result.addActions(mts.getActions());

    Set<String> uncontrollable =
        new HashSet<String>(Sets.difference(mts.getActions(), controllable));
        ...
        for (Set<String> enabled : ps) {
          ...
          for (Pair<String, Long> tr : mts.getTransitions(next, POSSIBLE)) {
            if (enabled.contains(tr.getFirst()) || uncontrollable.contains(tr.getFirst())) {
              result.addRequired(
                  to, tr.getFirst(), Pair.create(tr.getSecond(), (Set<String>) null));
            }
          }
        }
      }
  ...
  public static MTS<Pair<Long, Set<String>>, String> generateStarredEnvModel(
      MTS<Long, String> mts) {
  ...
  // to be refactored. It needs to be generalised as a strategy together with the subset
  // construction for MTS Control.
  public static MTS<Pair<Set<Long>, String>, String> getControllableDeterminisationFor(
      MTS<Long, String> mts, Set<String> controllable, LTSOutput output) {
    Pair<Set<Long>, String> initialSet =
        Pair.create(Collections.singleton(mts.getInitialState()), (String) null);
    MTS<Pair<Set<Long>, String>, String> result =
        new MTSImpl<Pair<Set<Long>, String>, String>(initialSet);
  ...
  private static void initUncontrolledStates(
      MTS<Long, String> c, Set<Long> uncontrolledStates, Set<String> uncontrolledActions) {
