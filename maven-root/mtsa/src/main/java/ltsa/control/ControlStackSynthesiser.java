...
public class ControlStackSynthesiser {
  ...
  */
  public static CompactState synthesiseControlStack(CompositeState cStackDef, LTSOutput output) {
    ...
    if (cStackDef.controlStackSpecificTier < -1
        || cStackDef.controlStackSpecificTier > stackDef.tierCount()
        || cStackDef.controlStackSpecificTier == 0) {
      output.outln(
          "Control stack synthesis FAILED: Parameter "
              + cStackDef.controlStackSpecificTier
              + " is not a valid tier number.");
      return null;
    }

    // sanity check simulation first
    if (!checkStackSimulation(cStackDef, output)) return null;
    ...
    CompactState envWithInitialTrace =
        null; // environment where initial states are specified (with a trace)
    ...
    for (int i = tiers.size() - 1; i >= 0; i--) {
      ...
      CompactState envModel =
          getControlStackEnvironment(cStackDef, tier.getEnvModel().toString(), output);
      if (envModel == null)
        throw new RuntimeException(
            "Failed to get environment model '"
                + tier.getEnvModel().toString()
                + "' from "
                + cStackDef.controlStackEnvironments);
      ...
      if (initialTrace != null && initialTrace.size() > 0) {
        if (envWithInitialTrace != null) {
          output.outln(
              "ERROR: an initial trace has already been specified in another tier ("
                  + envWithInitialTrace.getName()
                  + ")");
          return null;
        }
        ...
        if (initialStates.size() > 0) {
          ...
          envModel =
              MTSToAutomataConverter.getInstance().convert(env, envWithInitialTrace.getName());
        } else {
      ...
      if (envWithInitialTrace != null && initialStates != null) {
        //        System.out.println("setting initial states to those simulating initial states of
        // lower tier");

        // find the matching initial states
        MTS<Long, String> lowerEnv =
            AutomataToMTSConverter.getInstance().convert(envWithInitialTrace);
        MTS<Long, String> upperEnv = AutomataToMTSConverter.getInstance().convert(envModel);
        BinaryRelation<Long, Long> stateMapping =
            new LTSSimulationSemantics().getLargestRelation(lowerEnv, upperEnv);
        List<Long> upperInitialStates = new Vector<Long>();
        for (Long s : initialStates) upperInitialStates.addAll(stateMapping.getImage(s));
      ...
      Collection<CompactState> safetyMachines =
          CompositionExpression.preProcessSafetyReqs(tierGoalDef, output);
      ControllerGoal<String> tierGoal =
          GoalDefToControllerGoal.getInstance().buildControllerGoal(tierGoalDef);
      ...
      //      //System.out.println("ENV-Alpha "+tierNum+"
      // "+envComposite.composition.getAlphabetV());

      CompactState controller =
          TransitionSystemDispatcher.synthesise(envComposite, tierGoal, output);
      if (controller != null) {
        ...
        if (cStackDef.controlStackSpecificTier
            == tierNum) // return this tier, not the whole shebang
        {
        ...
        CompactState excController =
            addControllerExceptions(tierNum, controller, stackDef.getControllableActions());
    ...
    CompositeState controlStack =
        new CompositeState(stackDef.getName().toString(), exceptionControllers);
  ...
  public static boolean checkStackSimulation(CompositeState cStackDef, LTSOutput output) {
    List<ControlTierDefinition> tiers =
        ControlStackDefinition.getDefinition(cStackDef.name).getTiers();
    for (int i = tiers.size() - 1; i >= 1; i--) {
      ControlTierDefinition tierLower = tiers.get(i);
      ControlTierDefinition tierHigher = tiers.get(i - 1);
      int tierNum = tiers.size() - i;

      CompactState lowerEnv =
          getControlStackEnvironment(cStackDef, tierLower.getEnvModel().toString(), output);
      CompactState higherEnv =
          getControlStackEnvironment(cStackDef, tierHigher.getEnvModel().toString(), output);
      ...
      if (simulates) output.outln("'" + lowerEnv.name + "' simulates '" + higherEnv.name + "'.");
      else {
        output.outln(
            "Control stack synthesis FAILED: Tier "
                + tierNum
                + " environment does not simulate tier "
                + (tierNum + 1)
                + " environment.");
        return false;
      }
  ...
  private static void setInitialStates(
      MTS<Long, String> env, String envName, List<Long> initialStates) {
  ...
  private static CompactState cleanUpAlphabet(CompactState machine) {
    ...
    for (String a : m2.getActions())
      ...
      toRemove.add(a);
    for (String a : toRemove) m2.removeAction(a);
    return MTSToAutomataConverter.getInstance().convert(m2, machine.name, false);
  }

  private static CompactState solveSafety(
      CompactState envModel, Collection<CompactState> safetyMachines, LTSOutput output) {
  ...
  public static CompactState addControllerExceptions(
      int tier, CompactState controller, List<String> controlledActions) {
    ...
    controller2 =
        addControllerExceptions(
            tier,
            controller2,
            controlledActions); // was called with controller not controller2 -> inf loop, why never
    // happened before?
  ...
  public static MTS<Long, String> addControllerExceptions(
      int tier, MTS<Long, String> controller, List<String> controlledActions) {
    ...
    for (String action : alphabet)
      controller.addTransition(
          SPECIAL_EXCEP_STATE2, action, SPECIAL_EXCEP_STATE2, TransitionType.REQUIRED);
    String disabledAction = "tier_disabled" + tier + "";
    controller.addAction(disabledAction);
    controller.addTransition(
        SPECIAL_EXCEP_STATE1, disabledAction, SPECIAL_EXCEP_STATE2, TransitionType.REQUIRED);
    ...
    for (long state : controller.getStates()) {
      if (state != SPECIAL_EXCEP_STATE1 && state != SPECIAL_EXCEP_STATE2) {
        ...
        for (Pair<String, Long> transition :
            controller.getTransitions(state, TransitionType.REQUIRED))
  ...
  private static CompactState getControlStackEnvironment(
      CompositeState cStackDef, String name, LTSOutput output) {
    ...
    } else return (CompactState) compactOrComposite;
  }
  ...
  public static void generatePrismMDP(CompactState lts, List<String> controlledActions) {
    ...
    try {
      ...
      final String stateVar =
          "state_"
              + lts.name.substring(0, 2); // try to make them unique when combined with other LTSs
      ...
      for (int r = 0; r < uncontrolledActions.size(); r++)
        writer.println(
            "["
                + prismifyLabel(uncontrolledActions.get(r))
                + "] "
                + responseVar
                + "="
                + (r + 1)
      ...
      for (long state : lts2.getStates()) {
        ...
        for (Pair<String, Long> transition : lts2.getTransitions(state, TransitionType.REQUIRED)) {
          if (uncontrolledActions.contains(transition.getFirst())) {
            ...
            writer.println(
                "["
                    + prismifyLabel(transition.getFirst())
                    + "] "
                    + stateVar
                    + "="
                    + state
        ...
        if (envTrans.size() > 0) // split env actions to unlabelled choice
        {
          ...
          for (Pair<String, Long> transition : envTrans)
            if (transition.getSecond() == exceptionState) excCount++;
          ...
          for (int i = 0; i < envTrans.size(); i++) { // split prob between exp and non-exc
            Pair<String, Long> transition = envTrans.get(i);
            envChoice +=
                (transition.getSecond() == exceptionState ? excProb : normalProb)
                    + ":("
                    + responseVar
                    + "'="
                    + (uncontrolledActions.indexOf(transition.getFirst()) + 1)
                    + ")&("
                    + stateVar
                    + "'="
                    + transition.getSecond()
