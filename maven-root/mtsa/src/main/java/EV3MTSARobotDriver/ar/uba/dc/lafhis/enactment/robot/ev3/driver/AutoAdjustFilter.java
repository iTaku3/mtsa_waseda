...
/**
 * This filter dynamicaly adjust the samples value to a range of 0-1. The purpose of this filter is
 * to autocalibrate a light Sensor to return values between 0 and 1 no matter what the light
 * conditions. Once the light sensor has "seen" both white and black it is calibrated and ready for
 * use.
 *
 * <p>The filter could be used in a line following robot. The robot could rotate to calibrate the
 * sensor.
 *
 * @author Aswin
 */
public class AutoAdjustFilter extends AbstractFilter {
  ...
  public void fetchSample(float[] sample, int offset) {
    ...
    for (int i = 0; i < sampleSize; i++) {
      if (minimum[i] > sample[offset + i]) minimum[i] = sample[offset + i];
      if (maximum[i] < sample[offset + i]) maximum[i] = sample[offset + i];
