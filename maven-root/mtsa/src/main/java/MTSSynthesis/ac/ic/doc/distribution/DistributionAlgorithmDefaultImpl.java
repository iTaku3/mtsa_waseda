...
public class DistributionAlgorithmDefaultImpl<S, A> implements DistributionAlgorithm<S, A> {
  ...
  @Override
  public DistributionResult<S, A> tryDistribute(
    ...
    boolean validPreconditions =
        this.validatePreconditionsAndAddFeedback(
            monolithicModel, alphabetDistribution, distributionResult, tauAction);
    if (validPreconditions) {
      isDistributable =
          this.doTryDistribute(monolithicModel, alphabetDistribution, distributionResult);
    }
  ...
  private boolean doTryDistribute(
      MTS<S, A> systemModel,
      AlphabetDistribution<A> alphabetDistribution,
    ...
    Map<Set<A>, Set<DeterminisationModalityMismatch<S, A>>> mismatchesByComponentAlphabet =
        new HashMap<Set<A>, Set<DeterminisationModalityMismatch<S, A>>>();
    Map<Set<A>, MTS<Set<S>, A>> componentByComponentAlphabet =
        new HashMap<Set<A>, MTS<Set<S>, A>>();
    for (Set<A> componentAlphabet : componentsAlphabet) {
      ComponentBuiltFromDistribution<S, A> builtComponentResult =
          componentBuilder.buildComponent(systemModel, componentAlphabet, fullAlphabet);
      componentByComponentAlphabet.put(componentAlphabet, builtComponentResult.getComponent());
      mismatchesByComponentAlphabet.put(
          componentAlphabet, builtComponentResult.getDeterminisationModalityMismatches());
    ...
    boolean isModalConsistent =
        this.checkModalConsistencyAndAddFeedback(
            systemModel,
            mismatchesByComponentAlphabet,
            alphabetDistribution,
            componentByComponentAlphabet,
            distributionResult);

    // Check if there is the language of the composition is bigger than the language of the
    // monolithic model. If so add to feedback.
    boolean doesCompositionOfComponentAddMoreBehaviour =
        this.checkIfComponentsAddNewTracesAndAddFeedback(
            systemModel, componentByComponentAlphabet, distributionResult);
  ...
  private boolean checkIfComponentsAddNewTracesAndAddFeedback(
      MTS<S, A> systemModel,
      Map<Set<A>, MTS<Set<S>, A>> componentByComponentAlphabet,
      DistributionResult<S, A> distributionResult) {
    ...
    CollectionUtils.collect(
        componentByComponentAlphabet.values(),
        new GenericMTSToLongStringMTSConverter<Set<S>, A>(),
        componentsAsMTSLongString);

    MTS<Long, String> compositionResult =
        new MTSMultipleComposer<S, String>(compositionRuleApplier)
            .compose(componentsAsMTSLongString);

    // XXX the system Model is transformed because the composer is not generic...
    MTS<Long, String> transformedSystemModel =
        new GenericMTSToLongStringMTSConverter<S, A>().transform(systemModel);

    Set<MTSTrace<String, Long>> tracesAddedByComposition =
        this.getTracesAddedByComposition(
            compositionResult.getInitialState(),
            compositionResult,
            transformedSystemModel.getInitialState(),
            transformedSystemModel,
            new HashSet<Pair<Long, Long>>());
    if (tracesAddedByComposition.size() != 0) {
      distributionResult.addFeedbackItem(
          new DistributionFeedbackOnTracesAddedByComposition<Long, String>(
              tracesAddedByComposition));
   ...
   * @return
   */
  private Set<MTSTrace<String, Long>> getTracesAddedByComposition(
      Long currentStateInComposition,
      MTS<Long, String> compositionMTS,
      Long currentStateInSystemModel,
      MTS<Long, String> systemModel,
      Set<Pair<Long, Long>> visited) {
    // XXX This method should and could be easily made generic but that depends on
    // MTSMultipleComposer be made generic.
    ...
    Iterator<Pair<String, Long>> transitionsFromCurrentStateInCompositionIt =
        compositionMTS
            .getTransitions(currentStateInComposition, TransitionType.POSSIBLE)
            .iterator();
    while (transitionsFromCurrentStateInCompositionIt.hasNext()) {
      Pair<String, Long> transitionFromCurrentStateInComposition =
          transitionsFromCurrentStateInCompositionIt.next();
      ...
      Set<Long> transitionsFromCurrentStateInSystemModel =
          systemModel
              .getTransitions(currentStateInSystemModel, TransitionType.POSSIBLE)
              .getImage(transitionFromCurrentStateInComposition.getFirst());

      boolean isMaybeTransition;
      if (compositionMTS
          .getTransitions(currentStateInComposition, TransitionType.MAYBE)
          .contains(transitionFromCurrentStateInComposition)) {
        isMaybeTransition = true;
      } else {
        isMaybeTransition = false;
      }
      MTSTransition<String, Long> transitionFromCurrentToNextStateInComposition =
          new MTSTransitionWithModalityInfo<String, Long>(
              currentStateInComposition,
              transitionFromCurrentStateInComposition.getFirst(),
              nextStateInComposition,
              isMaybeTransition);

      if (transitionsFromCurrentStateInSystemModel.isEmpty()) {
        // this is a transition not present in the original model, add it to the result as a single
        // transition trace.
        LinkedListMTSTrace<String, Long> traceNotPresentInSystemModel =
            new LinkedListMTSTrace<String, Long>();
          ...
          if (!visited.contains(Pair.create(nextStateInComposition, nextStateInSystemModel))) {
            Set<MTSTrace<String, Long>> tracesAddedByCompositionFromNextState =
                this.getTracesAddedByComposition(
                    nextStateInComposition,
                    compositionMTS,
                    nextStateInSystemModel,
                    systemModel,
                    visited);
            // add the transition to the first position of each of the
            // tracesToStateSatisfyingPredicateFromNextState and add it to the final result
            this.appendTransitionToTracesAndAddToResult(
                transitionFromCurrentToNextStateInComposition,
                tracesAddedByCompositionFromNextState,
                result);
          }
        }
      }
  ...
  private void appendTransitionToTracesAndAddToResult(
      MTSTransition<String, Long> transitionFromCurrentToNextState,
      Set<MTSTrace<String, Long>> tracesNotPresentInAFromNextState,
      Set<MTSTrace<String, Long>> result) {

    for (MTSTrace<String, Long> traceNotPresentInAFromNextState :
        tracesNotPresentInAFromNextState) {
  ...
  /**
   * Not every mismatch is a problem. If there is a component that can differentiate the candidate
   * mismatch then it is not a real mismatch and there is no problem
   *
   ...
   */
  private boolean checkModalConsistencyAndAddFeedback(
      MTS<S, A> systemModel,
      Map<Set<A>, Set<DeterminisationModalityMismatch<S, A>>> mismatchesByComponent,
      AlphabetDistribution<A> alphabetDistribution,
      Map<Set<A>, MTS<Set<S>, A>> componentByComponentAlphabet,
      DistributionResult<S, A> distributionResult) {

    boolean isModalConsistent = true;
    Set<Entry<Set<A>, Set<DeterminisationModalityMismatch<S, A>>>> entrySet =
        mismatchesByComponent.entrySet();
    ...
    for (Entry<Set<A>, Set<DeterminisationModalityMismatch<S, A>>> entry : entrySet) {
      ...
      for (DeterminisationModalityMismatch<S, A> aMismatch : mismatchesForComponent) {
        if (this.isRealConflict(
            aMismatch, alphabetDistribution, componentAlphabet, componentByComponentAlphabet)) {
          DistributionFeedbackOnModalityMismatch<S, A> feedbackOnModalityMismatch =
              new DistributionFeedbackOnModalityMismatch<S, A>(aMismatch);

          feedbackOnModalityMismatch.setTraceToStateWithReqTransition(
              this.getTraceToState(
                  aMismatch.getStateByModality(TransitionType.REQUIRED), systemModel));

          feedbackOnModalityMismatch.setTraceToStateWithMayTransition(
              this.getTraceToState(
                  aMismatch.getStateByModality(TransitionType.MAYBE), systemModel));
  ...
  private MTSTrace<A, S> getTraceToState(S state, MTS<S, A> systemModel) {
    ...
    return MTSUtils.getAlltracesToStatesSatisfyingPredicate(
            systemModel, PredicateUtils.equalPredicate(state), TransitionType.POSSIBLE)
        .iterator()
        .next();
  }

  private boolean isRealConflict(
      DeterminisationModalityMismatch<S, A> aMismatch,
      AlphabetDistribution<A> alphabetDistribution,
      Set<A> componentAlphabet,
      Map<Set<A>, MTS<Set<S>, A>> componentByComponentAlphabet) {
    boolean isRealConflict = true;
    A action = aMismatch.getAction();

    // get the alphabets from components sharing this action.
    Set<Set<A>> alphabetsContainingAction =
        alphabetDistribution.getAlphabetsContainingAction(action);
    alphabetsContainingAction.remove(componentAlphabet);

    // for each of those component see if there is at least one that can differentiate the
    // conflicting states
   ...
   * @return
   */
  private boolean validatePreconditionsAndAddFeedback(
      MTS<S, A> monolithicModel,
      AlphabetDistribution<A> alphabetDistribution,
      DistributionResult<S, A> distributionResult,
      A tauAction) {
    ...
    if (!monolithicModelActionsWithoutTau.equals(alphabetDistribution.getFullAlphabet())) {
      distributionResult.addFeedbackItem(
          new DistributionFeedbackOnFullAlphabet<A>(
              alphabetDistribution, monolithicModelActionsWithoutTau));
      distributionResult.setDistributable(false);
      result = false;
    }

    // clone and apply closure to the clone with the tau action. Tau actions can cause non
    // determinism.
    MTS<S, A> closuredMonolithicModel = MTSUtils.cloneMTS(monolithicModel);
    MTSAFacade.applyClosure(
        closuredMonolithicModel, new HashSet<A>(Collections.singleton(tauAction)));

    // Check whether the monolithic model is deterministic
    Predicate<S> hasNonDetTransitionsFromState =
        new HasNonDeterministicTransitionsStatePredicate<S, A>(
            closuredMonolithicModel, TransitionType.POSSIBLE);
    Set<MTSTrace<A, S>> tracesToNonDetStates =
        MTSUtils.getAlltracesToStatesSatisfyingPredicate(
            closuredMonolithicModel, hasNonDetTransitionsFromState, TransitionType.POSSIBLE);

    if (!tracesToNonDetStates.isEmpty()) {
      distributionResult.addFeedbackItem(
          new DistributionFeedbackOnNonDeterminism<S, A>(tracesToNonDetStates));
      distributionResult.setDistributable(false);
      result = false;
    }
