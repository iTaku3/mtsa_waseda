 ...
 * @author gsibay
 */
public class DistributionFeedbackOnTracesAddedByComposition<S, A>
    implements DistributionFeedbackItem {
  ...
  public String getMessage() {
    ...
    message.append(
        "The LTS view of the model being distributed is not distributable. The composition of the"
            + " determinised and minimised projection of the LTS onto the distributed alphabet"
            + " produced new traces:\n");
  ...
  private List<String> getTraceAsListOfActions(MTSTrace<A, S> trace) {
    ...
    for (MTSTransition<A, S> transition : trace) {
      ...
      MTSTransitionWithModalityInfo<A, S> transitionWithModality =
          (MTSTransitionWithModalityInfo<A, S>) transition;
