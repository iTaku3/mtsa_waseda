...
/** This class allows translating boolean formulas over fluents into an LTS with marked states. */
public class FormulaToMarkedLTS {
  ...
  /**
   * Returns the LTS representing the formula A => G, where A is a conjunction of assumptions and G
   * is the conjunction of guarantees.
   */
  public MarkedLTSImpl<Long, String> translate(
      List<Formula> assumptions, List<Formula> guarantees) {
  ...
  private Formula conjunct(List<Formula> formulas) {
    ...
    for (int i = 1; i < formulas.size(); i++) result = new AndFormula(result, formulas.get(i));
    return result;
  }
  ...
  public MarkedLTSImpl<Long, String> translate(Formula formula) {
    ...
    if (formula instanceof FluentPropositionalVariable) {
      ...
      result =
          binary(BinaryOperation.AND, andFormula.getLeftFormula(), andFormula.getRightFormula());
  ...
  private MarkedLTSImpl<Long, String> binary(
      BinaryOperation op, Formula leftFormula, Formula rightFormula) {
    ...
    Map<Pair<Long, Long>, Long> states =
        new InitMap<>(
            new Factory<Long>() {
              long next = 0L;
    ...
    if (isResultMarked(
        op, left.isMarked(initial.getFirst()), right.isMarked(initial.getSecond()))) {
  ...
  public void populateAsterisk(MarkedLTSImpl<Long, String> host, Set<String> newActions) {
    if (host.getActions().contains("*")) {
      ...
      for (Map.Entry<Long, BinaryRelation<String, Long>> Asterisk :
          host.getTransitions().entrySet()) {
