...
public class FluentUtils {
  ...
  /**
   * Returns all the symbols affecting the valuation of the fluents; i.e all the initiating an
   * terminating symbols of each fluent.
   *
   * @param fluents
   * @return
   */
  ...
  public <State, Action> FluentStateValuation<State> buildValuation(
      MTS<State, Action> mts, Collection<Fluent> fluents) {
    ...
    while (!toVisit.isEmpty()) {
      ...
      for (Pair<Action, State> transition : mts.getTransitions(actual, TransitionType.REQUIRED)) {
        ...
        for (Fluent fluent : fluents) {
          ...
          boolean actionInTerminatingSymbols =
              isActionInTerminatingSymbols(transitionLabel, fluent);
          ...
          if ((valuation.isTrue(actual, fluent) && !actionInTerminatingSymbols)
            ...
            //						Validate.isTrue(!mts.getInitialState().equals(to) || valuation.isTrue(to,
            // fluent)
  ...
  private <State, Action> void initialiseInitialState(
      Collection<Fluent> fluents, MTS<State, Action> mts, FluentStateValuation<State> relation) {
