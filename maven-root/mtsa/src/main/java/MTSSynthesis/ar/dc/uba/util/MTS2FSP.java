...
public class MTS2FSP<State, Action> {
  ...
  /**
   * Writes as a comment the mapping between a FSP's Proccess and a MTS' State.
   *
   * @param fsp
   */
  ...
  /**
   * Writes the state as a Process in FSP. Example: E_1 = (k? -> E_1 |t -> E_2 |t? -> E_3 |k -> E_4
   * ), E_2 = STOP, E_3 = STOP, E_4 = STOP.
   *
   * @param state
   * @param mts
   * @param fsp
  ...
  private void writeProccessForState(State state, MTS<State, Action> mts, StringBuffer fsp) {
    ...
    BinaryRelation<Action, State> reqTransitions =
    ...
    if (reqTransitions.isEmpty()
        && mayTransitions.isEmpty()) { // There are no transitions from this state
      // The process is END
      fsp.append("END");
    } else {
      fsp.append("(");
  ...
  /**
   * Assigns id to the states. The initial state has the mtsName, and then they are named E_ with a
   * number starting from 1. The states have consecutives numbers (except the first one). The state
   * is then identified as E_ and then the number. For instance, if there are 8 different states,
