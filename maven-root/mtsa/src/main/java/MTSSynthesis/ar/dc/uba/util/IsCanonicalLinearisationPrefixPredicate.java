...
public class IsCanonicalLinearisationPrefixPredicate implements Predicate<Word> {
  ...
  /**
   * Each bounded canonical linearisation prefix stands for a set of canonical linearisation prefix.
   * TODO: GS rename
   */
  ...
  public IsCanonicalLinearisationPrefixPredicate(Chart prechart) {
    Set<Word> prefixes =
        WordUtils.getInstance().preffixes(prechart.calculateOperationalLanguageWords());
  ...
  public boolean evaluate(Word word) {
    ...
    Iterator<Word> boundedLinearisationsPrefixIt =
        this.boundedCanonicalLinearisationPrefixes.iterator();
  ...
  private boolean isIncluded(Word word, Word boundedLinearisationPrefix) {
    ...
    if (word.length() != boundedLinearisationPrefix.length()) {
      ...
      while (boundedLinearisationSymbolsIt.hasNext() && included) {
        ...
        CanonicalSymbol currentBoundedLinearisationSymbol =
            (CanonicalSymbol) boundedLinearisationSymbolsIt.next();
        ...
        included =
            included
                && (new EqualsBuilder()
                    .append(
                        currentWordSymbol.getSymbol(),
                        currentBoundedLinearisationSymbol.getSymbol())
                    .isEquals())
                && (CollectionUtils.isSubCollection(
                    currentBoundedLinearisationSymbol.getConditionsNames(),
