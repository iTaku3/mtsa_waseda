...
public class GamaComparator<A, S> {
  ...
  public GamaComparator(
      ...
      Set<S> states,
      String END_TO_END_NAME1,
      String END_TO_END_NAME2) {
    this();
  ...
  private void boot(
      ...
      String END_TO_END_NAME2) {
    BoolExpr[] and =
        new BoolExpr[(states.size() + activityDefinitions.getActivities().size() + 2) * 2];
    int i = 0;
  ...
  public Result compareControllers(
      BoolExpr gama1, BoolExpr gama2, String END_TO_END_NAME_1, String END_TO_END_NAME_2) {
    try {
      if (uses > limit) {
        this.disposeContext();
  ...
  public BoolExpr calculateGama(
      MTS<S, A> scheduled,
      String END_TO_END_NAME,
      MTS<S, A> original,
    ...
    for (S st : finalStates) {
      if (scheduled.getStates().contains(st)) winningStates.add(st);
    }
    ...
    for (Pair<A, S> transState :
        scheduled.getTransitions(scheduled.getInitialState(), TransitionType.REQUIRED)) {
      s.push(transState);
      translateTransitionToPTA(
          timedStates,
          actTimers,
          second,
          transState,
    ...
    for (S winning : winningStates) {
      if (timedStates.containsKey(winning)) {
        BoolExpr entranceCondition =
            z3.MkEq(getConst(END_TO_END_STOPWATCH), getConst(END_TO_END_TIME));
        Set<RealExpr> resetedClocks = new HashSet<RealExpr>();
        resetedClocks.add(getConst(URGENT_VAR));
        timedStates
            .get(winning)
            .addSuccessor(entranceCondition, resetedClocks, END_TO_END_END, ending);
      } else {
        //				System.out.println("?");
      }
    }
  ...
  private BoolExpr pre(
      TimedState s1, TimedTrace e, TimedState s2, BoolExpr phi, Set<String> allTimers) {
  ...
  private void translateTransitionToPTA(
      ...
      MTS<S, A> original,
      Set<A> controllableActions,
      Set<String> allTimers,
      Map<TimedState, String> stateTimers)
      throws Z3Exception {
    ...
    if (TimeUtils.isFinal(original, state)
        || TimeUtils.isTransient(state, original, controllableActions)) {
      condition = URG_COND;
      actualValuations.add(getConst(URGENT_VAR));
    } else {
    ...
    if (stateTimers.containsKey(previous)) {
      ...
      BoolExpr soujournCondition =
          z3.MkEq(getConst(stateTimer), getConst(TimeUtils.getTimeFromTimer(stateTimer)));
    ...
    if (next == null) {
      ...
      if (!condition.equals(next.getCondition())) {
        throw new RuntimeException(
            "The condition of the time state is different depending of the path.");
      }
    }

    actTimers.put(next, actualTimers);
    if (nextStateTimer != null) {
      stateTimers.put(next, nextStateTimer);
    }
  ...
  private static <A, S> boolean isSuccessor(
      Pair<A, S> lastElement, Pair<A, S> next, MTS<S, A> result) {
    for (Pair<A, S> pair :
  ...
  private void addConst(String E2EndClock) throws Z3Exception {
    if (!consts.keySet().contains(TimeUtils.getTimerFromTime(E2EndClock)))
      consts.put(
          TimeUtils.getTimerFromTime(E2EndClock),
          z3.MkRealConst(TimeUtils.getTimerFromTime(E2EndClock)));
    if (!consts.keySet().contains(E2EndClock)) consts.put(E2EndClock, z3.MkRealConst(E2EndClock));
  }
