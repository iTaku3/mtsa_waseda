...
public class LatencyNotPresetEvaluator<S, A> extends LatencyEvaluator<Pair<S, S>, A, S> {
  ...
  public LatencyNotPresetEvaluator(
      MTS<Pair<S, S>, A> heuristic,
      MTS<Pair<S, S>, A> maximalControllerUsingGR1,
      Set<Pair<S, S>> heuristicFinalStates,
      Set<Pair<S, S>> perfectFinalStates,
      ActivityDefinitions<A> activityDefinitions,
      Translator<S, Pair<S, S>> translator,
      Set<A> controllableAction,
  ...
  private Set<GenericChooser<S, A, Pair<S, S>>> generateSchedulers(
    ...
    SchedulerGenerator<S, A> schedulerGenerator =
        new SchedulerGenerator<S, A>(environment, controllableActions, activityDefinitions);
  ...
  @Override
  protected Set<GenericChooser<S, A, Pair<S, S>>> getSchedulers(
      MTS<Pair<S, S>, A> result1, MTS<Pair<S, S>, A> result2) {
    LTS<S, A> prunedEnvironment =
        utils.pruneRealEnvironment(
            new LTSAdapter<Pair<S, S>, A>(result1, TransitionType.REQUIRED),
            new LTSAdapter<Pair<S, S>, A>(result2, TransitionType.REQUIRED),
            realEnvironment);
    return generateSchedulers(
        maxSchedulers, prunedEnvironment, controllableActions, activityDefinitions);
  }
  ...
  @Override
  protected ControllerComparator<S, A, Pair<S, S>> getControllerComparator(
      int i,
      Set<ControllerChooser<Pair<S, S>, A>> controllerChooser,
      ResultCounter stats,
      MTS<Pair<S, S>, A> maximalController,
      Set<Pair<S, S>> perfectFinalStates,
      MTS<Pair<S, S>, A> result2,
      Set<Pair<S, S>> heuristicFinalStates,
      Integer maxSchedulers,
      Set<A> controllableActions,
      ActivityDefinitions<A> activityDefinitions,
      LTS<S, A> realEnvironment,
      Translator<S, Pair<S, S>> translator,
      ComparatorPool<A, Pair<S, S>> comparator,
      Integer maxThreads) {
    return new ControllerPairsComparator<S, A>(
        i,
        stats,
        controllerChooser,
        maximalController,
