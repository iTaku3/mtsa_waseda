...
public abstract class RuleBasedCalculator<A, S> {
  ...
  private static RealExpr getConst(String ki) {
    String key = ki.trim();
    if (consts.containsKey(key)) return consts.get(key);
    try {
  ...
  public static <A, S> void calculateControllerExpression(
      GRGame<S> game, MTS<StrategyState<S, Integer>, A> result) {
    getControllerExpression(game, result, "C");
  }

  private static <A, S> List<String> getTraceString(
      Stack<Pair<A, StrategyState<S, Integer>>> trace) {
  ...
  private static <A, S> List<Set<String>> getNTTraceString(
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace) {
  ...
  public static <A, S> void compareControllers(
      GRGame<S> game, ArrayList<MTS<StrategyState<S, Integer>, A>> results) {
    try {
  ...
  public static <A, S> BoolExpr getControllerExpression(
      GRGame<S> game, MTS<StrategyState<S, Integer>, A> result, String controller) {
    Stack<Pair<A, StrategyState<S, Integer>>> s = new Stack<Pair<A, StrategyState<S, Integer>>>();
    Stack<Pair<A, StrategyState<S, Integer>>> trace =
        new Stack<Pair<A, StrategyState<S, Integer>>>();
    ...
    Set<Pair<Set<BoolExpr>, Set<BoolExpr>>> repes =
        new HashSet<Pair<Set<BoolExpr>, Set<BoolExpr>>>();
    // Add successors of initial state
    for (Pair<A, StrategyState<S, Integer>> pair :
        result.getTransitions(result.getInitialState(), TransitionType.REQUIRED)) {
      s.push(pair);
    }
    ...
    while (!s.isEmpty()) {
      ...
      if (game.getGoal().getGuarantee(1).contains(v.getSecond().getState())) {
        ...
        Pair<Set<BoolExpr>, Set<BoolExpr>> p =
            new Pair<Set<BoolExpr>, Set<BoolExpr>>(
                new HashSet<BoolExpr>(conditions), new HashSet<BoolExpr>(expression));
        if (!repes.contains(p)) {
        ...
        trace.add(v);
        for (Pair<A, StrategyState<S, Integer>> pair :
            result.getTransitions(v.getSecond(), TransitionType.REQUIRED)) {
          s.push(pair);
        }
      }
    }
  ...
  private static <A, S> void addRelations(
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace,
      List<BoolExpr> relations,
      HashMap<String, Pair<Integer, Integer>> positions) {
    for (String labelA : positions.keySet()) {
      for (String labelB : positions.keySet()) {
        if (labelA.compareTo(labelB) < 0 && isNotRedundantComparison(labelA, labelB)) {
          BoolExpr rel =
              getRelation(nTTrace, labelA, labelB, positions.get(labelA), positions.get(labelB));
  ...
  private static <A, S> void addTimeRelations(
      Stack<Pair<A, StrategyState<S, Integer>>> trace,
      List<BoolExpr> timeRelations,
      HashMap<String, Pair<Integer, Integer>> positions) {
    try {
  ...
  private static HashMap<String, Pair<Integer, Integer>> getMorePositions(
      HashMap<String, Pair<Integer, Integer>> positions, ArrayList<Set<String>> ends) {
    HashMap<String, Pair<Integer, Integer>> result = new HashMap<String, Pair<Integer, Integer>>();
    boolean[] visited = new boolean[ends.size()];
    ArrayList<HashMap<String, Pair<Integer, Integer>>> expresions =
        new ArrayList<HashMap<String, Pair<Integer, Integer>>>(ends.size());
  ...
  private static synchronized HashMap<String, Pair<Integer, Integer>> calculatePositionExp(
      int i,
      boolean[] visited,
      HashMap<String, Pair<Integer, Integer>> positions,
      ArrayList<Set<String>> ends,
      ArrayList<HashMap<String, Pair<Integer, Integer>>> expresions) {
    HashMap<String, Pair<Integer, Integer>> result = new HashMap<String, Pair<Integer, Integer>>();
    if (visited[i]) {
      ...
      for (String label : ends.get(i)) {
        ...
        HashMap<String, Pair<Integer, Integer>> subRes =
            calculatePositionExp(start, visited, positions, ends, expresions);
          ...
          for (String string : subRes.keySet()) {
            result.put(
                ...
                new Pair<Integer, Integer>(
                    subRes.get(string).getFirst(), positions.get(label).getSecond()));
          }
        }
      }
  ...
  private static <A, S> boolean startAnActionBefore(
      String timeLabel, Stack<Pair<A, StrategyState<S, Integer>>> trace) {
    boolean isAnStartBefore = false;
    for (Pair<A, StrategyState<S, Integer>> pair : trace) {
      String action = (String) pair.getFirst();
      String timeLabelAction = getTimeLabel(action);
      if (isEnding(action) && timeLabelAction.equals(timeLabel)) return isAnStartBefore;
      isAnStartBefore = isStarting(action) && !timeLabelAction.equals(timeLabel);
    }
    return false;
  }

  private static <A, S> BoolExpr getRelation(
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace,
      String labelA,
      String labelB,
      Pair<Integer, Integer> positionsA,
      Pair<Integer, Integer> positionsB) {
  ...
  private static <A, S> Set<String> getSetActionsFrom(
      Set<Pair<A, StrategyState<S, Integer>>> nTTraceState) {
  ...
  private static <A, S> BoolExpr compareOneDirection(
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace,
      Integer startA,
      Integer startB,
      Integer endA,
    ...
    try {
      if (startA.equals(startB) && endA.equals(endB)) {
        ...
        // R2,R6?
      } else if ((startA.equals(startB)
              && endInNextAlone(endA, endB, originalEndB, nTTrace, labelA, labelB))
          || (endA.equals(endB) && startA < startB && hasRealEnds(nTTrace, originalStartB))) {
        return z3.MkLe(getConst(labelB), getConst(labelA));
        //			 return "<=";
      } else if ((startA.equals(startB) && endA - endB >= 1)
          || (startA < startB && endB < endA)
          || (endA.equals(endB) && startA < startB)) {
  ...
  private static Set<String> allTransitionsFrom(String labelA, Set<String> borders) {
    ...
    for (int i = 0; i < timeExpressions.length; i++) {
      ...
      if (!borders.contains(end)) result.add(end);
      if (!borders.contains(start)) result.add(start);
    }
    return result;
  }

  private static <A, S> boolean hasRealEnds(
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace, Integer startB) {
    return getEndings(nTTrace.get(startB)).size() > 0;
  }

  private static <A, S> boolean endInNextAlone(
      Integer endA,
      Integer endB,
      Integer realEndB,
      List<Set<Pair<A, StrategyState<S, Integer>>>> nTTrace,
  ...
  private static Map<String, Integer> getMultiSet(String string2) {
    ...
    for (String string3 : timeExpressions2) {
      if (timeExpM.containsKey(string3)) timeExpM.put(string3, timeExpM.get(string3) + 1);
      else timeExpM.put(string3, +1);
    }
    return timeExpM;
  }

  private static <A, S> String getTimeExpression(
      List<Set<Pair<A, StrategyState<S, Integer>>>> ntTrace) {
    return g(ntTrace, ntTrace.size() - 1);
  }

  private static <A, S> List<BoolExpr> getTimeExpressionList(
      List<Set<Pair<A, StrategyState<S, Integer>>>> ntTrace) {
  ...
  private static <A, S> List<RealExpr> g2(
      List<Set<Pair<A, StrategyState<S, Integer>>>> ntTrace, int i) {
  ...
  private static <A, S> Integer h(
      String action, List<Set<Pair<A, StrategyState<S, Integer>>>> ntTrace, int i2) {
  ...
  private static <A, S> Set<Pair<A, StrategyState<S, Integer>>> getEndings(
      Set<Pair<A, StrategyState<S, Integer>>> actual) {
    Set<Pair<A, StrategyState<S, Integer>>> result =
        new HashSet<Pair<A, StrategyState<S, Integer>>>();
  ...
  private static <A, S> Set<Pair<A, StrategyState<S, Integer>>> getInstantEndings(
      Set<Pair<A, StrategyState<S, Integer>>> actual) {
    Set<Pair<A, StrategyState<S, Integer>>> result =
        new HashSet<Pair<A, StrategyState<S, Integer>>>();
    for (Pair<A, StrategyState<S, Integer>> pair : actual) {
  ...
  private static String getTimeLabel(String action) {
    if (isEnding(action)) return "T" + action.replaceFirst("e", "").replace(".", "");
    else return "T" + action.replaceFirst("s", "").replace(".", "");
  }

  private static <A, S> boolean isTransition(
      String action, Set<Pair<A, StrategyState<S, Integer>>> actual) {
    for (Pair<A, StrategyState<S, Integer>> pair : actual) {
      if (pair.getFirst().equals(action)) return true;
    }
    return false;
  }

  private static <A, S> List<Set<Pair<A, StrategyState<S, Integer>>>> getNTTrace(
      Stack<Pair<A, StrategyState<S, Integer>>> trace, MTS<StrategyState<S, Integer>, A> result) {
    List<Set<Pair<A, StrategyState<S, Integer>>>> tms =
        new LinkedList<Set<Pair<A, StrategyState<S, Integer>>>>();
    ...
    if (!seq.isEmpty()) tms.add(seq);
    return tms;
  }

  private static <A, S> boolean isNotTransient(
      Pair<A, StrategyState<S, Integer>> a, MTS<StrategyState<S, Integer>, A> result) {
    for (Pair<A, StrategyState<S, Integer>> pair :
        result.getTransitions(a.getSecond(), TransitionType.REQUIRED)) {
      String action = ((String) pair.getFirst());
      if (!isEnding(action)) return false;
    }
    return true;
  }
  ...
  private static <A, S> boolean isSuccessor(
      Pair<A, StrategyState<S, Integer>> lastElement,
      Pair<A, StrategyState<S, Integer>> next,
      MTS<StrategyState<S, Integer>, A> result) {
    for (Pair<A, StrategyState<S, Integer>> pair :
        result.getTransitions(lastElement.getSecond(), TransitionType.REQUIRED)) {
      if (pair.equals(next)) return true;
    }
    return false;
  }
