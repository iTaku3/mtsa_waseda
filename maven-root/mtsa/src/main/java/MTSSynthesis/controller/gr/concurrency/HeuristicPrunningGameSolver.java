...
public abstract class HeuristicPrunningGameSolver<S, A> extends RankBasedGameSolver<S, Integer> {
  ...
  public HeuristicPrunningGameSolver(
      Game<S> game, LTS<S, A> lts, DoubleRankFunction<S> function, Set<S> finalStates) {
  ...
  //			return compare > 0;
  //		}else{
  //			throw new RuntimeException("Better condition not defined for the Prunning Strategy " +
  // strategy);
  //		}
  //	}
  ...
  protected void addPredecessorsTo(
    ...
    for (S pred : lts.getStates()) {
      ...
      if (isSuccessor(strategyState.getState(), pred)
          && isWinning(strategyState.getState())
          && needsToBeUpdated(strategyPred)
          && !isTheLastGoal(strategyState.getState(), strategyPred.getState())) {
  ...
  protected Rank getBestFromSuccessors(S strategyState) {
    ...
    if (strategySuccesors.isEmpty()) return this.function.getRank(strategyState);
    ...
    if (getGame().isUncontrollable(strategyState)) {
      ...
      else return this.function.getAverage(strategySuccesors);
    }
