...
public abstract class LatencyEvaluator<P, A, S> {
  ...
  public void evaluateLatency(Integer maxControllers) {
    ...
    HeuristicSolutionsSchedulerIterator<P, A> heuristicIterator =
        new HeuristicSolutionsSchedulerIterator<P, A>(
            heuristic, controllableActions, heuristicFinalStates);
    //		System.out.println("#Heuristic Solutions:" + heuristicIterator.getSize());
    boolean doo = true;
    this.comparatorPool =
        new ComparatorPool<A, P>(
            maxThreadsByScheduler * numberOfThreadsByControllers,
            activityDefinitions,
            maximalControllerUsingGR1.getStates(),
            "f1",
            "f2");
    ...
    while (heuristicIterator.hasNext() && doo) {
      ...
      try {
        ...
        ControllerGenerator<P, A> controllerGenerator =
            new ControllerGenerator<P, A>(
                new LTSAdapter<P, A>(maximalControllerUsingGR1, TransitionType.REQUIRED),
                controllableActions,
                perfectFinalStates);
        ...
        // Executors.newFixedThreadPool(numberOfThreadsByControllers);
        while (!controllerGenerator.next(size).isEmpty() && i < maxControllers) {
          ControllerComparator<S, A, P> controllerTask =
              getControllerComparator(
                  i,
                  controllerGenerator.getLasts(),
                  stats,
        ...
        //				System.out.println("Warnings: Eq " + warningEq + " - Better" + warningBetter + " -
        // Worse "+ warningWorse);
  ...
  protected abstract ControllerComparator<S, A, P> getControllerComparator(
      int i,
      Set<ControllerChooser<P, A>> controllerChooser,
      ResultCounter stats,
      MTS<P, A> result1,
      Set<P> perfectFinalStates2,
      MTS<P, A> result2,
      Set<P> heuristicFinalStates2,
      Integer maxSchedulers2,
      Set<A> controllableActions2,
      ActivityDefinitions<A> activityDefinitions2,
      LTS<S, A> realEnvironment2,
      Translator<S, P> translator2,
      ComparatorPool<A, P> comparator2,
      Integer maxThreads);

  protected abstract Set<GenericChooser<S, A, P>> getSchedulers(
      MTS<P, A> result1, MTS<P, A> result2);
