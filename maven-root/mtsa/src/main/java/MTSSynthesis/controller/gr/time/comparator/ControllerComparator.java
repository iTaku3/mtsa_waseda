...
public abstract class ControllerComparator<S, A, P> implements Runnable {
  ...
  public void run() {
    ...
    try {
      ...
      for (ControllerChooser<P, A> controllerScheduler : this.controllerChooser) {
        this.result1 =
            new MTSAdapter<P, A>(
                controllerScheduler.applyTo(
                    new LTSAdapter<P, A>(maximalController, TransitionType.REQUIRED),
                    trivialTranslator));
  ...
  protected Result compare() throws Z3Exception {
    ...
    // First we are going to try to see if this is a case of Incomparable to avoid exploring all the
    // schedulers.
    int exploringMax = Math.min(25, maxSchedulers);
    SchedulerByComparator<S, A, P> singleComparator =
        new SchedulerByComparator<S, A, P>(
            END_TO_END_NAME_1,
            END_TO_END_NAME_2,
            ctr_stats,
            ctr_res,
    ...
    while (scheduler != null
        && sused < exploringMax
        && !(ctr_stats.contains(Result.INCOMPARABLES))) {
    ...
    if (!(ctr_stats.contains(Result.INCOMPARABLES))) {
      ...
      //			int sizeOf = 2000;
      //			while(!getSchedulers(sizeOf).isEmpty() && sused < maxSchedulers &&
      // !(ctr_stats.contains(Result.INCOMPARABLES))){
      //				System.out.println("Start Comparison" + Calendar.getInstance().getTime().toString());
      SchedulerByComparator<S, A, P> schedulerByTask =
          new SchedulerByComparator<S, A, P>(
              END_TO_END_NAME_1,
              END_TO_END_NAME_2,
              ctr_stats,
    ...
    if (ctr_res == null) {
      if (!ctr_stats.contains(Result.INCOMPARABLES) && !ctr_stats.contains(Result.WORSE)) {
        if (ctr_stats.contains(Result.BETTER)) {
          if (sused >= maxSchedulers) ctr_res = Result.BETTER_WARNING;
          else ctr_res = Result.BETTER;
        } else {
          if (sused >= maxSchedulers) ctr_res = Result.EQUALLYGOOD_WARNING;
          else ctr_res = Result.EQUALLYGOOD;
        }
        ...
        if (sused >= maxSchedulers) ctr_res = Result.WORSE_WARNING;
        else ctr_res = Result.WORSE;
      }
    }
