...
public class GamaCalculator2<A, S> {
  ...
  public void evaluateLatency(
    ...
    HeuristicSolutionsSchedulerIterator<StrategyState<S, Integer>, A> heuristicIterator =
        new HeuristicSolutionsSchedulerIterator<StrategyState<S, Integer>, A>(
            heuristic, goal.getControllableActions(), finalStates);
    ...
    HashMap<Integer, Scheduler<StrategyState<S, Integer>, A>> schedulerChoosed =
        new HashMap<Integer, Scheduler<StrategyState<S, Integer>, A>>();

    GamaComparator<A, StrategyState<S, Integer>> comparator =
        new GamaComparator<A, StrategyState<S, Integer>>();
    ...
    while (heuristicIterator.hasNext() && doo) {
      ...
      try {
        ...
        preCalculateGamaForHeuristic(
            ...
            comparator,
            finalStates);
        ControllableSchedulerIterator<StrategyState<S, Integer>, A> controllerIterator =
            new ControllableSchedulerIterator<StrategyState<S, Integer>, A>(
                maximalControllerUsingGR1, goal.getControllableActions(), finalStates);
        ...
        while (controllerIterator.hasNext()) {
          ...
          for (Integer sched_id : choosed) {
            MTS<StrategyState<S, Integer>, A> result1 =
                controllerScheduler.applyTo(maximalControllerUsingGR1);
            ...
            BoolExpr gama1 =
                comparator.calculateGama(
                    ...
                    finalStates);
            Result res =
                comparator.compareControllers(
                    gama1, heursticGamas.get(sched_id), END_TO_END_NAME_1, END_TO_END_NAME_2);
            addStat(ctr_stats, res);
            if (ctr_stats.keySet().contains(Result.INCOMPARABLES)
                || (ctr_stats.keySet().contains(Result.WORSE)
                    && ctr_stats.keySet().contains(Result.BETTER))) {
          ...
          if (ctr_res == null) {
            if (!ctr_stats.containsKey(Result.INCOMPARABLES)
                && !ctr_stats.containsKey(Result.WORSE)) {
  ...
  private Set<Integer> chooseSchedulers(
      GRGame<S> g,
      ControllerGoal<A> goal,
      Set<StrategyState<S, Integer>> finalStates,
      MTS<StrategyState<S, Integer>, A> maximalControllerUsingGR1,
      String END_TO_END_NAME_2,
      ArrayList<Set<A>> relatedActions,
      HashMap<Integer, BoolExpr> heursticGamas,
      HashMap<Integer, Scheduler<StrategyState<S, Integer>, A>> schedulerChoosed,
      MTS<StrategyState<S, Integer>, A> result2)
      throws Z3Exception {
    SchedulerIterator<StrategyState<S, Integer>, A> schedulerIterator =
        new SchedulerIterator<StrategyState<S, Integer>, A>(
            maximalControllerUsingGR1, goal.getControllableActions(), finalStates, relatedActions);
  ...
  private void preCalculateGamaForHeuristic(
      ControllerGoal<A> goal,
      String END_TO_END_NAME_2,
      HashMap<Integer, BoolExpr> heursticGamas,
      HashMap<Integer, Scheduler<StrategyState<S, Integer>, A>> schedulerChoosed,
