...
public class SchedulerByComparator<S, A, P> implements Runnable {
  ...
  protected Result compare(GenericChooser<S, A, P> scheduler, GamaComparator<A, P> comparator)
      throws Z3Exception {
    ...
    if (ctr_stats.contains(Result.INCOMPARABLES)
      ...
      //			MTS<P, A> result_1_schedulled = new MTSAdapter<P, A>(scheduler.applyTo(new
      // LTSAdapter<P,A>(result1, TransitionType.REQUIRED),translator));
      //			System.out.println(result_1_schedulled.toString());
      //			MTS<P, A> heuristic_schedulled = new MTSAdapter<P, A>(scheduler.applyTo(new
  ...
  protected BoolExpr getGama(
      GenericChooser<S, A, P> scheduler,
      MTS<P, A> mts,
      Set<P> finalStates,
      String stopwatch,
    ...
    MTS<P, A> schedulled =
        new MTSAdapter<P, A>(
            scheduler.applyTo(new LTSAdapter<P, A>(mts, TransitionType.REQUIRED), translator));
