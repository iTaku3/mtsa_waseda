...
/**
 * The diference between OptimisticPerfectInfoGRGameSolver and PerfectInfoGRGameSolver is that when
 * rankMayIncrease (satisfies the actual goal) do not add all succesors, just the better candidates
 * in GRRank.
 *
 * @author ecastellano
 * @param <State>
...
public class OptimisticPerfectInfoGRGameSolver<State> extends PerfectInfoGRGameSolver<State> {
  ...
  protected void addControllableSuccesors(
    ...
    for (State succ : succs) {
      if (this.isBetterThan(
        ...
        if (rankMayIncrease) updateBestCandidates(target, candidates);
  ...
  private void updateBestCandidates(
      StrategyState<State, Integer> newCandidate,
      Set<StrategyState<State, Integer>> bestCandidates) {
    boolean better = bestCandidates.isEmpty();
    Set<StrategyState<State, Integer>> removableCandidates =
    ...
    if (better) bestCandidates.add(newCandidate);
  }

  private int compareCandidates(
      StrategyState<State, Integer> newCandidate, StrategyState<State, Integer> candidate) {
