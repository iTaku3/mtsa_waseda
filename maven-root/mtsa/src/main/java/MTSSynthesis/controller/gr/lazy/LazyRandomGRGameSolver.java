...
public class LazyRandomGRGameSolver<State> extends LazyGRGameSolver<State> {
  ...
  public LazyRandomGRGameSolver(
      GRGame<State> game, GRRankSystem<State> rankSystem, Integer maxLazyness) {
  ...
  protected void addControllableSuccesors(
    ...
    Set<StrategyState<State, Integer>> possibleSuccessors =
    ...
    for (State succ : this.getGame().getControllableSuccessors(state)) {
      if (this.isBetterThan(
        ...
        if (goalIsSatisfied)
          target =
              new StrategyState<State, Integer>(succ, nextMemoryToConsider, this.getMaxLazyness());
        else
          target =
    ...
    for (StrategyState<State, Integer> strategyState : possibleSuccessors) {
      if (RandomUtils.nextBoolean()) successors.add(strategyState);
    }
