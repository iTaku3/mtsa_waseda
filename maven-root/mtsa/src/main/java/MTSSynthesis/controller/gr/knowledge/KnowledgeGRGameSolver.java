...
public class KnowledgeGRGameSolver<S, A> extends RankBasedGameSolver<Set<S>, Integer> {
  ...
  private Set<Pair<StrategyState<Set<S>, Integer>, StrategyState<Set<S>, Integer>>> worseRank =
      new HashSet<Pair<StrategyState<Set<S>, Integer>, StrategyState<Set<S>, Integer>>>();
  ...
  private GRRank getBestFromSuccessors(Set<S> state, int guaranteeId) {
    ...
    //		SortedSet<Pair<A, Set<S>>> toSortActionSet = new TreeSet<Pair<A,
    // Set<S>>>(getRankComparator(nextGuarantee));
    ...
    for (Set<S> succ : getGame().getUncontrollableSuccessors(state)) {
      GRRank succRank =
          getRankSystem().getRank(new StrategyState<Set<S>, Integer>(succ, nextGuarantee));
    ...
    BinaryRelation<A, Set<S>> trs =
        getKnowdlegeGRGame().getDetMTS().getTransitions(state, POSSIBLE);
    for (Pair<A, Set<S>> tr : trs) {
      if (getKnowdlegeGRGame().getControllable().contains(tr.getFirst())) {
        GRRank rank =
            getRankSystem()
                .getRank(new StrategyState<Set<S>, Integer>(tr.getSecond(), nextGuarantee));
    ...
    if (getKnowdlegeGRGame().isEnabled(state, candidateActionSet)) {
      ...
      for (Pair<A, Set<S>> tr : toSortActionSet) {
        ...
        Set<Set<S>> succState =
            getKnowdlegeGRGame().getDetMTS().getTransitions(state, POSSIBLE).getImage(action);
        GRRank succRank =
            getRankSystem()
                .getRank(
                    new StrategyState<Set<S>, Integer>(succState.iterator().next(), nextGuarantee));
        bestRank.set(succRank);
    ...
    // This code is not reachable since reaching this point would mean that <i>state</i> is a dead
    // end.
    GRRankContext nextContext =
        getRankSystem().getContext(new StrategyState<Set<S>, Integer>(state, nextGuarantee));
    return GRRank.getInfinityFor(nextContext);
  }

  private Comparator<Pair<A, Set<S>>> getRankComparator(final int nextGuarantee) {
    Comparator<Pair<A, Set<S>>> rankComparator =
        new Comparator<Pair<A, Set<S>>>() {
          @Override
          public int compare(Pair<A, Set<S>> tr1, Pair<A, Set<S>> tr2) {
            StrategyState<Set<S>, Integer> pair1 =
                new StrategyState<Set<S>, Integer>(tr1.getSecond(), nextGuarantee);
            StrategyState<Set<S>, Integer> pair2 =
                new StrategyState<Set<S>, Integer>(tr2.getSecond(), nextGuarantee);
  ...
  protected void addIfNotIn(
      Collection<StrategyState<Set<S>, Integer>> pending, StrategyState<Set<S>, Integer> state) {
  ...
  protected void addPredecessorsTo(
      Queue<StrategyState<Set<S>, Integer>> pending,
      StrategyState<Set<S>, Integer> strategyState,
      Rank bestRank) {
    ...
    for (Set<S> pred : predecessors) {
      ...
      if (bestRank.isInfinity()) {
        ...
        StrategyState<Set<S>, Integer> predecessor =
            new StrategyState<Set<S>, Integer>(pred, guaranteeId);
        addIfNotIn(pending, predecessor);
      }
    }
  }
  ...
  protected void initialise(Queue<StrategyState<Set<S>, Integer>> pending) {
    ...
    for (Set<S> state : this.getGame().getStates()) {
      if (this.getGame().getControllableSuccessors(state).isEmpty()
        ...
        for (int guaranteeId = 1;
            guaranteeId <= this.getKnowdlegeGRGame().getGoal().getGuaranteesQuantity();
            guaranteeId++) {
          StrategyState<Set<S>, Integer> strategyState =
              new StrategyState<Set<S>, Integer>(state, guaranteeId);
        ...
        StrategyState<Set<S>, Integer> firstGuaranteeRank =
            new StrategyState<Set<S>, Integer>(state, 1);
  ...
  public Strategy<Set<S>, Integer> buildStrategy() {
    ...
    for (Set<S> state : winningStates) {
      for (int guaranteeId = 1; guaranteeId <= getGRGoal().getGuaranteesQuantity(); guaranteeId++) {
        StrategyState<Set<S>, Integer> source =
            new StrategyState<Set<S>, Integer>(state, guaranteeId);

        int nextGuaranteeId = this.getNextGuaranteeStrategy(guaranteeId, state);

        // If either a guarantee or a failure was just visited then it is ok for the successor of
        // state to have higher rank.
        boolean rankMayIncrease =
            getGRGoal().getGuarantee(guaranteeId).contains(state)
                || getGRGoal().getFailures().contains(state);

        Set<StrategyState<Set<S>, Integer>> successors =
            new HashSet<StrategyState<Set<S>, Integer>>();
        for (Set<S> succ : this.getGame().getUncontrollableSuccessors(state)) {
          ...
          Validate.isTrue(
              this.isBetterThan(
                  source,
                  new StrategyState<Set<S>, Integer>(succ, nextGuaranteeId),
                  rankMayIncrease),
              "Set<S>: " + succ + " must have a better rank than state: " + state);
          StrategyState<Set<S>, Integer> target =
              new StrategyState<Set<S>, Integer>(succ, nextGuaranteeId);
          successors.add(target);
        }

        for (Set<S> succ : this.getGame().getControllableSuccessors(state)) {
          if (this.isBetterThan(
              source, new StrategyState<Set<S>, Integer>(succ, nextGuaranteeId), rankMayIncrease)) {
            Validate.isTrue(this.isWinning(succ), "state: " + succ + " it's not winning.");
            StrategyState<Set<S>, Integer> target =
                new StrategyState<Set<S>, Integer>(succ, nextGuaranteeId);
            successors.add(target);
  ...
  protected void updateRank(StrategyState<Set<S>, Integer> strategyState, Rank bestRank) {
    ...
    if (bestRank.isInfinity()) {
      ...
      for (int i = 1; i <= getGRGoal().getGuaranteesQuantity(); i++) {
        GRRank infinityFor =
            GRRank.getInfinityFor(
                this.getRankSystem().getContext(new StrategyState<Set<S>, Integer>(state, i)));
  ...
  public boolean isBetterThan(
      ...
      StrategyState<Set<S>, Integer> succ,
      boolean mayIncrease) {

    Rank succRank = this.getRankSystem().getRank(succ);
    GRRank stateRank = this.getRankSystem().getRank(state);

    boolean isInAssumption =
        getGRGoal().getAssumption(stateRank.getAssume()).contains(state.getState());
