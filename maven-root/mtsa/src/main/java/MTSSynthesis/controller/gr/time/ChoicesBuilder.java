...
public abstract class ChoicesBuilder<S, A> {
  ...
  private void identifyTransitionType(
      BinaryRelation<A, S> stateTransitions, Set<A> cActions, S state) {
    ...
    for (Pair<A, S> transition : stateTransitions) {
      ...
      if (type.equals(ChoiceType.CONTROLLABLE)) controllable.add(label);
      else if (type.equals(ChoiceType.UNCONTROLLABLE)) {
        uncontrollable.add(label);
      } else if (type.equals(ChoiceType.ENDS)) ends.add(label);
