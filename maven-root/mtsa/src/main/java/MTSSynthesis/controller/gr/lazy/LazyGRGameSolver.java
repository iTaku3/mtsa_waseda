...
public class LazyGRGameSolver<State> extends PerfectInfoGRGameSolver<State> {
    ...
    for (State state : winningStates) {
      for (int guaranteeId = 1; guaranteeId <= getGRGoal().getGuaranteesQuantity(); guaranteeId++) {
        for (int lazyness = maxLazyness; lazyness >= 0; lazyness--) {
          StrategyState<State, Integer> source =
              new StrategyState<State, Integer>(state, guaranteeId, lazyness);
          ...
          // or a failure was just visited then it is ok for the successor of state to have higher
          // rank.
          boolean rankMayIncrease = goalIsSatisfied || getGRGoal().getFailures().contains(state);

          boolean lazynessCanDecrease = source.getLazyness() > 0;

          Set<StrategyState<State, Integer>> successors =
              new HashSet<StrategyState<State, Integer>>();

          this.addUncontrollableSuccesors(
              state,
              source,
              nextMemoryToConsider,
              rankMayIncrease,
              successors,
              lazynessCanDecrease,
              goalIsSatisfied);
  ...
  protected void addUncontrollableSuccesors(
      ...
      Set<StrategyState<State, Integer>> successors,
      boolean lazynessCanDecrease,
      boolean goalIsSatisfied) {
    for (State succ : this.getGame().getUncontrollableSuccessors(state)) {
      boolean isBetterThan =
          this.isBetterThan(
              source,
              new StrategyState<State, Integer>(succ, nextMemoryToConsider),
              rankMayIncrease);
      Validate.isTrue(
          isBetterThan, "State: " + succ + " must have a better rank than state: " + state);
      StrategyState<State, Integer> target;
      if (goalIsSatisfied)
        target = new StrategyState<State, Integer>(succ, nextMemoryToConsider, maxLazyness);
      else
        target =
            new StrategyState<State, Integer>(succ, nextMemoryToConsider, source.getLazyness());
  ...
  protected void addControllableSuccesors(
      ...
      int nextMemoryToConsider,
      boolean rankMayIncrease,
      Set<StrategyState<State, Integer>> successaors,
      boolean lazynessCanDecrease,
      boolean goalIsSatisfied,
      boolean base) {

    Set<StrategyState<State, Integer>> controllableSuccessors =
        new HashSet<StrategyState<State, Integer>>();

    for (State succ : this.getGame().getControllableSuccessors(state)) {
      if (this.isBetterThan(
          source, new StrategyState<State, Integer>(succ, nextMemoryToConsider), rankMayIncrease)) {
        ...
        Validate.isTrue(this.isWinning(succ), "state: " + succ + " it's not winning.");
        StrategyState<State, Integer> target =
            new StrategyState<State, Integer>(succ, nextMemoryToConsider, source.getLazyness() - 1);
        controllableSuccessors.add(target);
        ...
        this.getWorseRank()
            .add(
                new Pair<StrategyState<State, Integer>, StrategyState<State, Integer>>(
                    source, target));
      }
    }
