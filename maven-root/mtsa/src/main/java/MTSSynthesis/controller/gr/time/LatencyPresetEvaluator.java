...
public class LatencyPresetEvaluator<P, A, S> extends LatencyEvaluator<P, A, S> {
  ...
  protected void preCalculateGamaForHeuristic() throws Z3Exception {
    ...
    //			MTS<P, A> result_2_schedulled = new MTSAdapter<P,A>(scheduler.applyTo(new
    // LTSAdapter<P,A>(result2, TransitionType.REQUIRED),translator));
    //			BoolExpr gama2 = comparator.calculateGama(result_2_schedulled , END_TO_END_NAME_2, result2,
    // controllableActions, heuristicFinalStates);
    //			if(gama2 == null){
  ...
  @Override
  protected ControllerComparator<S, A, P> getControllerComparator(
      int i,
