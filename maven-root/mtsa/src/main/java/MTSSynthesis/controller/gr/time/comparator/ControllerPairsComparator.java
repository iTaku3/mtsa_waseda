...
public class ControllerPairsComparator<S, A> extends ControllerComparator<S, A, Pair<S, S>> {
  ...
  public ControllerPairsComparator(
      final Integer id,
      ResultCounter stats,
      Set<ControllerChooser<Pair<S, S>, A>> controllerChooser,
      final MTS<Pair<S, S>, A> maximalController,
      final Set<Pair<S, S>> perfectFinalStates,
      final MTS<Pair<S, S>, A> result2,
      final Set<Pair<S, S>> heuristicFinalStates,
      final Integer maxSchedulers,
      final Set<A> controllableActions,
      final ActivityDefinitions<A> activityDefinitions,
      final LTS<S, A> realEnvironment,
      final Translator<S, Pair<S, S>> translator,
      ComparatorPool<A, Pair<S, S>> comparatorPool,
      Integer maxThreads) {
    super(
        id,
        stats,
  ...
  protected Result compare() throws Z3Exception {
    LTS<S, A> prunedEnvironment =
        utils.pruneRealEnvironment(
            ...
            realEnvironment);
    this.schedulerGenerator =
        new SchedulerGeneratorLight<S, A>(
            prunedEnvironment, controllableActions, activityDefinitions);
  ...
  protected GenericChooser<S, A, Pair<S, S>> getScheduler() {
    ...
    if (tmp != null) return schedulerGenerator.build(tmp);
    return null;
  }
