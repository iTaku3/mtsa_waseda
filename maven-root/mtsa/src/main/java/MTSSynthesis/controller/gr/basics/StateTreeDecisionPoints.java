...
public class StateTreeDecisionPoints<State, Memory> extends DecisionPoints {
  ...
  public StateTreeDecisionPoints(
      StrategyState<State, Memory> state,
      Set<StrategyState<State, Memory>> contrSuccessors,
      boolean zero) {
    this(contrSuccessors.size(), zero);
    this.state = state;
    this.succesors =
        new HashMap<StrategyState<State, Memory>, Set<StateTreeDecisionPoints<State, Memory>>>(
            this.length);
  ...
  public Set<StrategyState<State, Memory>> getAvailableSucc() {
    ...
    for (int i = 0; i < length; i++) {
      if (decisions[i]) available.add(realSuccesors.get(i));
    }
    return available;
  }
  ...
  public Set<StateTreeDecisionPoints<State, Memory>> getNextDecisionPoints() {
    Set<StateTreeDecisionPoints<State, Memory>> available =
        new HashSet<StateTreeDecisionPoints<State, Memory>>();
    for (int i = 0; i < length; i++) {
      if (decisions[i]) available.addAll(succesors.get(realSuccesors.get(i)));
    }
    return available;
  }

  public void updateReal(
      StrategyState<State, Memory> succ, Set<StateTreeDecisionPoints<State, Memory>> next) {
