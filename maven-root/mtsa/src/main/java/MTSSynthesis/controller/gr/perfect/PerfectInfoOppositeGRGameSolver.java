...
public class PerfectInfoOppositeGRGameSolver<State> extends OppositeGRGameSolver<State> {
  ...
  protected void addPredecessorsTo(
      Queue<StrategyState<State, Integer>> pending,
      StrategyState<State, Integer> strategyState,
      Rank bestRank) {
    ...
    for (State pred : predecessors) {
      ...
      if (bestRank.isInfinity()) {
        ...
        StrategyState<State, Integer> predecessor =
            new StrategyState<State, Integer>(pred, guaranteeId);
        if (this.needsToBeUpdated(predecessor)) {
          addIfNotIn(pending, predecessor);
        }
      }
  ...
  protected void initializeStates(Queue<StrategyState<State, Integer>> pending) {
    ...
    for (State state : this.getGame().getStates()) {
      for (int i = 1; i <= this.getGame().getGoal().getGuaranteesQuantity(); i++) {
        if (!this.getGame().getGoal().getGuarantee(i).contains(state)
            && !this.getGame().getGoal().getFailures().contains(state)
    ...
    for (State state : this.getGame().getStates()) {
      if (this.getGame().getControllableSuccessors(state).isEmpty()
        ...
        for (int guaranteeId = 1;
            guaranteeId <= this.getGame().getGoal().getGuaranteesQuantity();
            guaranteeId++) {
          StrategyState<State, Integer> strategyState =
              new StrategyState<State, Integer>(state, guaranteeId);
          setInfinity(strategyState);
          deadEnds.add(state);
        }
        StrategyState<State, Integer> firstGuaranteeRank =
            new StrategyState<State, Integer>(state, 1);
  ...
  protected Rank best(StrategyState<State, Integer> strategyState) {
    ...
    if (getGRGoal().getAssumption(bestRank.getAssume()).contains(state)) {
      bestRank.increase();
    }
  ...
  private GRRank getBestFromSuccessors(State state, Integer guarantee) {
    ...
    if (getGame().isUncontrollable(state)) {
      ...
      return this.getRankSystem().getMax(
      		...
      		*/
      return this.getRankSystem()
          .getMax(GRUtils.getStrategyStatesFrom(nextGuarantee, controllableSuccesors));
    }
  }
