...
import MTSSynthesis.controller.model.RankBasedLabelledGameSolver;
...
/**
 * For the computations of successors and predecessors: A state s is uncontrollable if it has some
 * uncontrollable action enabled (at least one) A state s is controllable if ALL the enabled actions
 * are controllable Consider a transition s --a--> t t is a successor of s if s is uncontrollable
 * and a is uncontrollable or if s is controllable (and then obviously a is controllable) s is a
 * predecessor of t if s is uncontrollable and a is uncontrollable or if s is controllable (and then
 * obviously a is controllable)
 */
public abstract class OppositeGRLabelledGameSolver<S, A>
    extends RankBasedLabelledGameSolver<S, A, Integer> {

  private GRRankSystem<S> rankSystem;

  // PLEASE REMOVE!!!
  private Set<Pair<StrategyState<S, Integer>, StrategyState<S, Integer>>> worseRank =
      new HashSet<Pair<StrategyState<S, Integer>, StrategyState<S, Integer>>>();
      ...
      for (int i = 1; i <= getGRGoal().getGuaranteesQuantity(); i++) {
        ...
        GRRank infinityFor =
            GRRank.getInfinityFor(
                this.getRankSystem().getContext(new StrategyState<S, Integer>(state, i)));
  ...
  protected void addIfNotIn(
      Collection<StrategyState<S, Integer>> pending, StrategyState<S, Integer> state) {
  ...
  public boolean isBetterThan(
      StrategyState<S, Integer> state, StrategyState<S, Integer> succ, boolean mayIncrease) {
  ...
  public Strategy<S, Integer> buildStrategy() {
    if (!isGameSolved()) this.solveGame();
    ...
    for (S state : winningStates) {
      for (int guaranteeId = 1; guaranteeId <= getGRGoal().getGuaranteesQuantity(); guaranteeId++) {
        ...
        this.addUncontrollableSuccesors(
            state, source, nextMemoryToConsider, rankMayIncrease, successors);

        this.addControllableSuccesors(
            state, source, nextMemoryToConsider, rankMayIncrease, successors);
  ...
  protected void addUncontrollableSuccesors(
      S state,
      StrategyState<S, Integer> source,
      int nextMemoryToConsider,
      boolean rankMayIncrease,
      Set<StrategyState<S, Integer>> successors) {
    for (S succ : this.getGame().getUncontrollableSuccessors(state)) {
      Validate.isTrue(
          this.isBetterThan(
              source, new StrategyState<S, Integer>(succ, nextMemoryToConsider), rankMayIncrease),
  ...
  protected void addControllableSuccesors(
      S state,
      StrategyState<S, Integer> source,
      int nextMemoryToConsider,
      boolean rankMayIncrease,
    ...
    for (S succ : this.getGame().getControllableSuccessors(state)) {
      if (this.isBetterThan(
        ...
        StrategyState<S, Integer> target =
        ...
        successors.add(target);
        this.getWorseRank()
            .add(new Pair<StrategyState<S, Integer>, StrategyState<S, Integer>>(source, target));
      }
    }
  }
  ...
  private GRRank getBestFromSuccessors(S state, Integer guarantee) {
    ...
    if (getGame().isUncontrollable(state)) {
      ...
      return this.getRankSystem()
          .getMax(
              GRUtils.getStrategyStatesFrom(
                  nextGuarantee, this.getGame().getUncontrollableSuccessors(state)));
    } else {
      return this.getRankSystem()
          .getMin(
              GRUtils.getStrategyStatesFrom(
                  nextGuarantee, this.getGame().getControllableSuccessors(state)));
    }
  }
