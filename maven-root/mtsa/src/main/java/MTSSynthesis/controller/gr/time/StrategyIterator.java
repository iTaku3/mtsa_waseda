...
public abstract class StrategyIterator<S, A> {
  ...
  protected void init(MTS<S, A> mts, Set<A> controllableActions, Set<S> finalStates) {
    ...
    this.choicesBuilder =
        new ControllerChoicesBuilder<S, A>(
            new LTSAdapter(mts, MTS.TransitionType.REQUIRED), controllableActions, finalStates);
    this.choices = choicesBuilder.getAllChoices();
    this.uncontrollableActions = new HashSet<A>();
    this.finalStates = finalStates;
    for (A action : mts.getActions()) {
      if (!this.controllableActions.contains(action)) this.uncontrollableActions.add(action);
    }
        ...
        for (A l : c.getAvailableLabels()) {
          ...
          for (S succ : succs) {
            ...
            if (!st.equals(succ)) {
              ...
              if (!Ss.containsKey(l)) Ss.put(l, scheds);
              else Ss.put(l, mergeSchedullers(Ss.get(l), scheds));
            } else {
  ...
  private List<Scheduler<S, A>> cartesiano(Map<A, List<Scheduler<S, A>>> ss, Set<A> ks) {
    if (ks.isEmpty()) return new ArrayList<Scheduler<S, A>>();
    A k = ks.iterator().next();
    ks.remove(k);
    return mergeSchedullers(ss.get(k), cartesiano(ss, new HashSet<A>(ks)));
  }

  private List<Scheduler<S, A>> mergeSchedullers(
      List<Scheduler<S, A>> k_list, List<Scheduler<S, A>> c_list) {
    if (c_list.isEmpty()) return k_list;
    else {
      List<Scheduler<S, A>> res = new ArrayList<Scheduler<S, A>>();
      for (Scheduler<S, A> sl : k_list) {
        for (Scheduler<S, A> sc : c_list) {
          if (compatible(sl, sc)) res.add(merge(sl, sc));
        }
      }
      return res;
    }
