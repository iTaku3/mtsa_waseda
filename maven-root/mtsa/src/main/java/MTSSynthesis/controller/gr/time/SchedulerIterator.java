...
public class SchedulerIterator<S, A> extends RandomStrategyIterator<S, A> {
  ...
  public SchedulerIterator(
    ...
    choicesBuilder =
        new RestrictedSchedulerChoicesBuilder<S, A>(
            new LTSAdapter<S, A>(mts, MTS.TransitionType.REQUIRED),
            controllableActions,
            finalStates);
  }
  ...
  protected boolean compatible(Scheduler<S, A> sl, Scheduler<S, A> sc) {
    return (super.compatible(sl, sc)
        && consistentUncontrollable(sl.getUncontrollableChoices(), sc.getUncontrollableChoices()));
  }
