...
public class GBRankSystem<State> /* implements RankSystem<State>*/ {
  ...
  public GBRankSystem(
      Set<State> allStates, Guarantees<State> guarantees, Assumptions<State> assumptions) {
    ...
    for (int guaranteeId = 1; guaranteeId <= guarantees.size(); guaranteeId++) {
      ...
      this.system.put(
          guaranteeId,
          new GBRankFunction<State>(allStates, maxAssumptionSize + 1, assumptions.getSize()));
