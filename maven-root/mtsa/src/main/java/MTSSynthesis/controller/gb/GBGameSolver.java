...
public class GBGameSolver<S> implements GameSolver<S, Integer> {
   ...
   */
  private void addPredecessorsTo(
      Collection<StrategyState<S, Integer>> pending, S state, int guaranteeId, GBRank bestRank) {
  ...
  private void addIfNotIn(Collection<StrategyState<S, Integer>> pending, S state, int guaranteeId) {
    ...
    for (S state : this.game.getStates()) {
      if (this.game.getControllableSuccessors(state).isEmpty()
        ...
        for (int guaranteeId = 1;
            guaranteeId <= this.game.getGoal().getGuaranteesQuantity();
            guaranteeId++) {
      ...
      for (int guaranteeId = 1;
        ...
        if (this.game.isUncontrollable(state)) {
          for (S succ : this.game.getUncontrollableSuccessors(state)) {
            Validate.isTrue(
                this.isBetterThan(state, succ, guaranteeId),
          ...
          for (S succ : this.game.getControllableSuccessors(state)) {
            if (this.isBetterThan(state, succ, guaranteeId)) {
              StrategyState<S, Integer> target =
                  new StrategyState<S, Integer>(succ, nextGuaranteeId);
              successors.add(target);
  ...
  /** (Main ...) What is the best possible value that the rank of a state can be? */
  ...
  private GBRank getBestFromSuccessors(S state, int guaranteeId) {
    ...
    // In a controllable state, the best we can expect is our choice, so we are going to get the
    // min.
    ...
    if (game.isUncontrollable(state)) {
      ...
      bestRank =
          this.rankSystem.getMaximum(nextGuarantee, this.game.getUncontrollableSuccessors(state));
    } else {
      bestRank =
          this.rankSystem.getMinimum(nextGuarantee, this.game.getControllableSuccessors(state));
    }
    return bestRank;
  }
}
