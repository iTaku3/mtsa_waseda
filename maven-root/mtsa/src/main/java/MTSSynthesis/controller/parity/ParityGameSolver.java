...
public class ParityGameSolver<State> extends RankBasedGameSolver<State, Integer> {
  ...
  public Strategy<State, Integer> buildStrategy() {
    ...
    for (State state : this.getWinningStates()) {
      ...
      for (State succ : this.game.getSuccessors(state)) {
        StrategyState<State, Integer> succStrategyState =
            new StrategyState<State, Integer>(succ, 0);
  ...
  private Collection<StrategyState<State, Integer>> getStrategyStates(Set<State> predecessors) {
    Collection<StrategyState<State, Integer>> strategyPredeccessors =
  ...
  protected ParityRank getBestFromSuccessors(StrategyState<State, Integer> state) {
    ...
    if (this.game.isUncontrollable(state.getState())) {
      best =
          this.rankSystem.getMax(
              buildStrategyStatesSet(
                  state, this.game.getUncontrollableSuccessors(state.getState())));
    } else {
      best =
          this.rankSystem.getMin(
              buildStrategyStatesSet(state, this.game.getControllableSuccessors(state.getState())));
    }
    return best;
  }

  private Set<StrategyState<State, Integer>> buildStrategyStatesSet(
      StrategyState<State, Integer> state, Set<State> successors) {
