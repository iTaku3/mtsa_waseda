...
public class AbstractSchedulerGenerator<S, A> extends Generator<S, A> {
  ...
  public AbstractSchedulerGenerator(
      LTS<S, A> environment,
      Set<A> controllableActions,
      ActivityDefinitions<A> activityDefinitions) {
    super(environment, controllableActions);
    this.activityDefinitions = activityDefinitions;
    ChoicesBuilder<S, A> choicesBuilder =
        new SchedulerChoicesBuilder<S, A>(environment, actions.getControllableActions());
    this.choices = choicesBuilder.getAllChoices();
  }

  protected boolean compatibleLabel(Set<A> uncontrollableChoices, Set<A> labels) {
    for (A label : labels) {
      if (actions.getUncontrollableActions().contains(label)
          && activityDefinitions.hasRelatedActions(label)) {
