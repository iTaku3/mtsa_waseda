...
public interface LTSControllerSynthesiser<S, A> {
   ...
   * to the definition provided in FSE 2010 paper. Essentially, the compatibility check is done by
   * checking if there is a strategy for a controller to win a game in which the only way for he to
   * win is to violate the assumptions. This is, the game with all the assumptions, the safety
   * specification and no goals.
   *
   * @param mts
   * @param goal
   */
