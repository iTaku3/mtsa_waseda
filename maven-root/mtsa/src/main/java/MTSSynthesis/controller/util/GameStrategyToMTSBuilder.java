...
public class GameStrategyToMTSBuilder {
  ...
  public <State, Action> MTS<StrategyState<State, Integer>, Action> buildMTSFrom(
  ...
  public <State, Action> MTS<StrategyState<State, Integer>, Action> buildMTSFrom(
    ...
    StrategyState<State, Integer> initialState =
        new StrategyState<State, Integer>(mts.getInitialState(), 1, maxLazyness);
    MTS<StrategyState<State, Integer>, Action> result =
        new MTSImpl<StrategyState<State, Integer>, Action>(initialState);
    ...
    for (StrategyState<State, Integer> strategyState : strategy.getStates()) {
      for (Pair<Action, State> transition :
          mts.getTransitions(strategyState.getState(), TransitionType.REQUIRED)) {

        State to = transition.getSecond();

        StrategyState<State, Integer> rankedState =
            this.getRankedState(to, strategy.getSuccessors(strategyState));
        if (rankedState != null) {
  ...
  public <State> StrategyState<State, Integer> getRankedState(
      State state, Set<StrategyState<State, Integer>> successors) {
