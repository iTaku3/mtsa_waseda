...
public class FluentStateValuation<State> implements Valuation {
  ...
  public ArrayList<Boolean> getFluentsFromState(State state, List<Fluent> fluents) {
    ...
    for (Fluent fluent : fluents) {
      ...
      if (name.equals(GeneralConstants.FALSE)) result.add(false);
      else if (name.equals(GeneralConstants.TRUE)) result.add(true);
      else result.add(isTrue(state, fluent));
    }

    return result;
  }

  public Set<Fluent> getFluentsFromState(State state) {
    return statesFromFluents.get(state);
  }
