...
public class SubsetConstructionBuilder<S, A> {
  ...
  private void addSuccessors(
      Set<S> currentSet, MTS<Set<S>, A> result, Queue<Set<S>> toVisit, Set<Set<S>> visited) {
  ...
  /**
   * A set of states <i>S</i> is a deadlock state if there exists a state <i>s</i> in <i>S</i> such
   * that no action in <i>enabledActions</i> is enabled.
   */
