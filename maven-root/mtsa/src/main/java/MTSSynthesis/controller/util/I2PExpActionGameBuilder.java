...
public class I2PExpActionGameBuilder<State, Action>
    implements Imperfect2PerfectInfoGameBuilder<State, Action, Pair<Set<State>, Set<Action>>> {
  ...
  //
  //	public I2PExpActionGameBuilder(MTS<State, Action> nonDet, Set<Action> controllableSet, Action
  // newControllable) {
  //		this.nonDet = nonDet;
  ...
  //		Pair<Set<State>,Set<Action>> initial =
  // Pair.create(Collections.singleton(this.nonDet.getInitialState()),null);
  //		MTS<Pair<Set<State>, Set<Action>>, Action> result = new MTSImpl<Pair<Set<State>, Set<Action>>,
  // Action>(initial);
  ...
  //			Set<Action> intersection = Sets.intersection(enabledControllableActions,
  // this.controllableSet);
  //
  ...
  //				result.addTransition(currentState, this.newControllable, intermediateState
  ...
  //			BinaryRelation<Action, State> transitions = this.nonDet.getTransitions(state,
  // TransitionType.REQUIRED);
  ...
  //	}
  //
  //	private Set<State> computeSuccessorsSet(Pair<Set<State>, Set<Action>> intermediateState, Action
  // action) {
  ...
  //		BinaryRelation<Action, State> trs = this.nonDet.getTransitions(state,
