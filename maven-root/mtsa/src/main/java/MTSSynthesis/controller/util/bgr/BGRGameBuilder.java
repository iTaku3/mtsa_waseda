...
public class BGRGameBuilder<State, Action> extends GRGameBuilder<State, Action> {
  ...
  public BGRGameBuilder(MTS<State, Action> env, ControllerGoal<Action> goal) {
    this.env = env;
    this.goal = goal;
  }
  ...
  public BGRGame<State> buldBGRGame() {
    ...
    Validate.isTrue(
        this.goal.getBuchi().size() <= 1,
        "\n Synthesizing a BGR controller with more than one  " + "buchi set is not supported.");
  ...
  public BGRGame<State> buildBGRGameForRTC(
      Action envYields,
      Action contYields,
    ...
    // Fluents indicating the environment giving control back and the controller giving control
    // back.
    ...
    FluentStateValuation<State> yieldValuations =
        FluentUtils.getInstance().buildValuation(env, new HashSet<>(Arrays.asList(en_m, en_e)));

    // States which only had controllable actions on environment
    Set<State> controllableStates =
        this.env.getStates().stream()
            .filter(
                s ->
                    this.env.getTransitions(s, MTS.TransitionType.REQUIRED).stream()
                        .anyMatch((transition) -> transition.getFirst() == controllableState))
            .collect(Collectors.toSet());

    Set<State> uncontrollableStates =
        this.env.getStates().stream()
            .filter(
                s ->
                    this.env.getTransitions(s, MTS.TransitionType.REQUIRED).stream()
                        .anyMatch((transition) -> transition.getFirst() == uncontrollableState))
            .collect(Collectors.toSet());
    ...
    // States on which the controller has control due to the environment yielding or the state only
    // having
    // uncontrollable actions on the environment
    Assume<State> envYieldsAssumptions = new Assume<>();
    envYieldsAssumptions.addStates(
        this.env.getStates().stream()
            .filter((s) -> yieldValuations.isTrue(s, en_m))
            .collect(Collectors.toSet()));
    ...
    FluentStateValuation<State> actionValuations =
        FluentUtils.getInstance().buildValuation(env, Collections.singleton(actionFluent));
    A.addStates(
        this.env.getStates().stream()
    ...
    controllableStates.forEach(
        s1 -> {
          Set<State> image =
              this.env.getTransitions(s1, MTS.TransitionType.REQUIRED).getImage(controllableState);
          image.forEach(
              s2 ->
                  this.env.removeTransition(
                      s1, controllableState, s2, MTS.TransitionType.REQUIRED));
        });
    uncontrollableStates.forEach(
        s1 -> {
          Set<State> image =
              this.env
                  .getTransitions(s1, MTS.TransitionType.REQUIRED)
                  .getImage(uncontrollableState);
          image.forEach(
              s2 ->
                  this.env.removeTransition(
                      s1, uncontrollableState, s2, MTS.TransitionType.REQUIRED));
        });
