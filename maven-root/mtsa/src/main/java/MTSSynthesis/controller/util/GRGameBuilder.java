...
public class GRGameBuilder<State, Action> {
  ...
  /**
   * Builds a GRGame from an MTS. The MTS is assumed to be an LTS representation (i.e. only required
   * transitions).
   */
  public GRGame<State> buildGRGameFrom(MTS<State, Action> mts, ControllerGoal<Action> goal) {
    ...
    FluentStateValuation<State> valuation =
        buildGoalComponents(mts, goal, assumptions, guarantees, failures);

    GRGoal<State> grGoal =
        new GRGoal<State>(guarantees, assumptions, failures, goal.isPermissive());
  ...
  public GRCGame<State> buildGRCGameFrom(MTS<State, Action> mts, ControllerGoal<Action> goal) {
    ...
    FluentStateValuation<State> valuation =
        buildGoalComponents(mts, goal, assumptions, guarantees, failures, concurrencyFluents);

    GRCGoal<State> grGoal =
        new GRCGoal<State>(
            guarantees, assumptions, failures, goal.isPermissive(), concurrencyFluents);
    Set<State> initialStates = new HashSet<State>();
    initialStates.add(mts.getInitialState());
  ...
  protected FluentStateValuation<State> buildGoalComponents(
      MTS<State, Action> mts,
      ControllerGoal<Action> goal,
      Assumptions<State> assumptions,
      Guarantees<State> guarantees,
      Set<State> failures,
      ConcurrencyLevel<State> concurrencyFluents) {
    FluentStateValuation<State> valuation =
        buildGoalComponents(mts, goal, assumptions, guarantees, failures);
    this.formulasToConcurrency(
        concurrencyFluents, mts.getStates(), goal.getConcurrencyFluents(), valuation);
    return valuation;
  }
  ...
  protected FluentStateValuation<State> buildGoalComponents(
      ...
      Set<State> failures) {
    FluentStateValuation<State> valuation =
        FluentUtils.getInstance().buildValuation(mts, goal.getFluents());
    ...
    return valuation; // this contains the marked states, only fluents not a goal formula (even
    // propositional), need to output formula itself into prism and evaluate there
  }

  protected void formulaToStateSet(
      Set<State> toBuild,
      Set<State> allStates,
  ...
  protected void formulaToStateSet(
      Set<State> toBuild, Formula formula, State state, FluentStateValuation<State> valuation) {
  ...
  public void validateActions(MTS<State, Action> mts, ControllerGoal<Action> goal) {
    ...
    if (!actions.containsAll(goal.getControllableActions())) {
      ...
      Logger.getAnonymousLogger()
          .log(
              Level.WARNING,
              "The following actions in the controller "
  ...
  private void validateFluentSymbols(
      Fluent fluent, Set<Action> actions, Set<Symbol> initiatingActions) {
  ...
  private void initialiseSuccessors(
      MTS<State, Action> mts, ControllerGoal<Action> goal, GRGame<State> game) {
  ...
  private void formulasToAssumptions(
      Assumptions<State> assumptions,
      Set<State> states,
      List<Formula> formulas,
      FluentStateValuation<State> valuation) {
  ...
  private void formulasToGuarantees(
      Guarantees<State> guarantees,
      Set<State> states,
      List<Formula> formulas,
      FluentStateValuation<State> valuation) {
  ...
  private void formulasToConcurrency(
