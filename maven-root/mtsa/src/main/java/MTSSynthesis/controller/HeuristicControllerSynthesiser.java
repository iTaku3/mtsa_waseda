...
public class HeuristicControllerSynthesiser<S, A> {
  ...
  public MTS<S, A> applyHeuristics(MTS<S, A> controller, MTS<S, A> env, ControllerGoal<A> goal) {
    ...
    if (goal.isNonTransient()) {
      ...
      heuristicSolution =
          generateBestController(
              perfectSolution,
              realEnvironment,
              getActivityDefinition(goal.getActivityFluents()),
              getFinalStates(goal, perfectSolution),
    ...
    if (goal.isReachability()) {
      GR1toReachability.transform(
          heuristicSolution, cp.getGRGame().getGoal().getGuarantee(1).getStateSet());
    }
  ...
  public MTS<S, A> applyReachabilityPruning(MTS<S, A> controller, ControllerGoal<A> goal) {
    ...
    GR1toReachability.transform(
        new LTSAdapter<S, A>(controller, TransitionType.REQUIRED),
        cgame.getGoal().getGuarantee(1).getStateSet());
    return controller;
  }

  private void compareControllers(
      ControllerGoal<A> goal,
      LTS<S, A> realEnvironment,
      LTS<S, A> heuristicSolution,
    ...
    compareControllers(
        ...
        heuristicSolution,
        perfectSolution,
        heuristicFinalStates,
        perfectFinalStates,
        false);
  }

  private Set<S> getFinalStates(ControllerGoal<A> goal, LTS<S, A> heuristicSolution) {
    GRGame<S> heuristicGRGame =
        new GRGameBuilder<S, A>().buildGRGameFrom(new MTSAdapter<S, A>(heuristicSolution), goal);
  ...
  private void compareControllers(
      ...
      LTS<S, A> heuristicSolution,
      LTS<S, A> perfectSolution,
      Set<S> heuristicFinalStates,
      Set<S> perfectFinalStates,
      boolean sameControllers) {

    LTS<Pair<S, S>, A> heuristicComposition =
        transformToReachability1(realEnvironment, heuristicSolution, heuristicFinalStates);
    LTS<Pair<S, S>, A> perfectComposition =
        transformToReachability1(realEnvironment, perfectSolution, perfectFinalStates);
    ...
    Set<Pair<S, S>> heuristicComposedFinalStates =
        compositionFinalStates(heuristicFinalStates, heuristicComposition.getStates());
    Set<Pair<S, S>> perfectComposedFinalStates =
        compositionFinalStates(perfectFinalStates, perfectComposition.getStates());

    pruneRealEnvironment(perfectComposition, realEnvironment);

    if (sameControllers)
      compareWithTheSameSchedulers(
          goal,
          realEnvironment,
          heuristicComposition,
          perfectComposition,
          controllableActions,
          activityDefinitions,
          translator,
          heuristicComposedFinalStates,
          perfectComposedFinalStates);
    else
      compareWithParticularSchedulers(
          goal,
          realEnvironment,
          heuristicComposition,
          perfectComposition,
  ...
  private void compareWithTheSameSchedulers(
      ControllerGoal<A> goal,
      LTS<S, A> realEnvironment,
      LTS<Pair<S, S>, A> heuristicComposition,
      LTS<Pair<S, S>, A> perfectComposition,
      ...
      Translator<S, Pair<S, S>> translator,
      Set<Pair<S, S>> heuristicComposedFinalStates,
      Set<Pair<S, S>> perfectComposedFinalStates) {
    Set<GenericChooser<S, A, Pair<S, S>>> schedulers =
        generateSchedulers(
            goal.getMaxSchedulers() - 1, realEnvironment, controllableActions, activityDefinitions);
    LatencyPresetEvaluator<Pair<S, S>, A, S> evaluator =
        new LatencyPresetEvaluator<Pair<S, S>, A, S>(
  ...
  private void compareWithParticularSchedulers(
      ControllerGoal<A> goal,
      LTS<S, A> realEnvironment,
      LTS<Pair<S, S>, A> heuristicComposition,
      LTS<Pair<S, S>, A> perfectComposition,
      ...
      Set<Pair<S, S>> heuristicComposedFinalStates,
      Set<Pair<S, S>> perfectComposedFinalStates) {
    LatencyNotPresetEvaluator<S, A> evaluator =
        new LatencyNotPresetEvaluator<S, A>(
            new MTSAdapter<Pair<S, S>, A>(heuristicComposition),
            new MTSAdapter<Pair<S, S>, A>(perfectComposition),
            heuristicComposedFinalStates,
            perfectComposedFinalStates,
  ...
  private Set<GenericChooser<S, A, Pair<S, S>>> generateSchedulers(
      Integer maxSchedulers,
      LTS<S, A> realEnvironment,
      Set<A> controllableActions,
      ActivityDefinitions<A> activityDefinitions) {
    SchedulerGenerator<S, A> schedulerGenerator =
        new SchedulerGenerator<S, A>(realEnvironment, controllableActions, activityDefinitions);
  ...
  private void pruneRealEnvironment(
      LTS<Pair<S, S>, A> perfectComposition, LTS<S, A> realEnvironment) {
  ...
  private LTS<Pair<S, S>, A> transformToReachability1(
      LTS<S, A> realEnvironment, LTS<S, A> perfectSolution, Set<S> perfectFinalStates) {
    GR1toReachability.transform(perfectSolution, perfectFinalStates);
    ParallelComposer<S, A, S> perfectComposer =
        new ParallelComposer<S, A, S>(realEnvironment, perfectSolution);
    LTS<Pair<S, S>, A> perfectComposition = perfectComposer.compose();
    return perfectComposition;
  }

  private Set<Pair<S, S>> compositionFinalStates(
      Set<S> originalFinalStates, Set<Pair<S, S>> compositionStates) {
  ...
  private MTS<StrategyState<S, Integer>, A> getResult(
      MTS<S, A> plant, GameSolver<S, Integer> gameSolver, GRGameSolver<S> grSolver) {
    Strategy<S, Integer> strategy = gameSolver.buildStrategy();

    Set<Pair<StrategyState<S, Integer>, StrategyState<S, Integer>>> worseRank =
        grSolver.getWorseRank();
    ...
    MTS<StrategyState<S, Integer>, A> result =
        GameStrategyToMTSBuilder.getInstance().buildMTSFrom(plant, strategy, worseRank, lazyness);
    return result;
  }
  ...
  private LTS<S, A> generateBestController(
      LTS<S, A> controller,
      LTS<S, A> environment,
      ActivityDefinitions<A> activityDefinitions,
      Set<S> finalStates,
    ...
    // TODO: Think about changing the implementation to use Pair<S,S> as state, so we can use the
    // comparison framework straight.
    ...
    ControllerChoicesBuilder<S, A> builder =
        new ControllerChoicesBuilder<S, A>(bestController, controllableActions, finalStates);
    ...
    while (!pruningCandidates.isEmpty()) {
      ...
      if (!visited.contains(state)
          ...
          && isReachable(bestController, state, finalStates)
  ...
  private void addNonVisitedStates(
      S state, DoubleLinkedLTS<S, A> controller, Queue<S> pruningCandidates, Set<S> visited) {
  ...
  private Choice<A> getAChoice(
      LTS<S, A> controller, LTS<S, A> environment, Map<S, List<Choice<A>>> choices, S state) {
  ...
  private Result partialComparison(
      S state,
      Choice<A> aChoice,
      Choice<A> otherChoice,
      LTS<S, A> controller,
      LTS<S, A> environment) {
