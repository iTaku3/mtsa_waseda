...
public class NondetControlProblem<S, A>
    implements ControlProblem<StrategyState<Set<S>, Integer>, A> {
  ...
  public NondetControlProblem(MTS<S, A> env, ControllerGoal<A> goal) {
    ...
    // MTSToGRGameBuilder
    FluentStateValuation<Set<S>> valuation =
        fluentUtils.buildValuation(perfectInfoGame, goal.getFluents());
    Assumptions<Set<S>> assumptions =
        this.formulasToAssumptions(perfectInfoGame.getStates(), goal.getAssumptions(), valuation);
    Guarantees<Set<S>> guarantees =
        this.formulasToGuarantees(perfectInfoGame.getStates(), goal.getGuarantees(), valuation);
    Set<Set<S>> faults =
        this.formulaToStateSet(perfectInfoGame.getStates(), goal.getFaults(), valuation);
    ...
    game =
        new KnowledgeGRGame<S, A>(
            initialStates, env, perfectInfoGame, goal.getControllableActions(), grGoal);
  }
  ...
  public LTS<StrategyState<Set<S>, Integer>, A> solve() {
    ...
    GRRankSystem<Set<S>> system =
        new GRRankSystem<Set<S>>(
            game.getStates(),
    ...
    if (solver.isWinning(perfectInfoGame.getInitialState())) {
      ...
      Set<Pair<StrategyState<Set<S>, Integer>, StrategyState<Set<S>, Integer>>> worseRank =
          solver.getWorseRank();
      MTS<StrategyState<Set<S>, Integer>, A> result =
  ...
  private Set<Set<S>> formulaToStateSet(
      Set<Set<S>> states, List<Formula> formulas, FluentStateValuation<Set<S>> valuation) {
  ...
  private void validateActions(MTS<Set<S>, A> mts, ControllerGoal<A> goal) {
    ...
    if (!actions.containsAll(goal.getControllableActions())) {
      ...
      //			System.out.println("WARNING: " + "The following actions in the controller " + "Goal does
      // not belong to the mts action set.\n" + controllableNotIn);
    }
  ...
  private Assumptions<Set<S>> formulasToAssumptions(
      Set<Set<S>> states, List<Formula> formulas, FluentStateValuation<Set<S>> valuation) {
  ...
  private Guarantees<Set<S>> formulasToGuarantees(
      Set<Set<S>> states, List<Formula> formulas, FluentStateValuation<Set<S>> valuation) {
