...
public class OppositeSafeGameSolver<S, A> extends StateSpaceCuttingGameSolver<S, A> {
  ...
  public OppositeSafeGameSolver(
      LTS<S, A> env,
      Set<A> controllable,
      Set<Set<S>> goalStates,
      Set<S> assumptionsStates,
      boolean relaxAllControllables,
      boolean relaxOnAssumptions,
      boolean relaxSelfLoops) {
    ...
    for (S state : game.getStates()) {
      ...
      if (!relaxAllControllables) {
        for (S s : currentUncontrollableSuccesors) {
          if ((!relaxOnAssumptions
              || (!assumptionsStates.contains(s) || assumptionsStates.contains(state))
                  && (!relaxSelfLoops || (!s.equals(state))))) currentSet.add(s);
        }
      }
    ...
    for (Set<S> actualGoalSet : this.goalStates) {
      ...
      while (!losing.isEmpty()) {
        ...
        for (S pred : predecessors) {
          if (losing.contains(pred) || currentLosingStates.contains(pred)) continue;
          ...
          if (!isUncontrollable[i]) {
            ...
            if (uncontrollableSuccessors.get(i).isEmpty()) losing.add(pred);
          }
        }
      }
  ...
  public void solveGame4() {
    ...
    while (!losing.isEmpty()) {
      ...
      Set<S> predecessors = this.game.getPredecessors(state);
      for (S pred : predecessors) {
        if (losing.contains(pred) || losingStates.contains(pred)) continue;
    ...
    while (!losing.isEmpty()) {
      ...
      for (S pred : predecessors) {
        ...
        if (!isUncontrollable[i]) {
          ...
          if (uncontrollableLosingCount[i] == uncontrollableCount[i]) losing.add(pred);
        }
      }
    }
    ...
    while (!losing.isEmpty()) {
      ...
      for (S pred : predecessors) {
        if (!(game.isUncontrollable(pred))) {
          if (!losingStates.contains(pred) && !losing.contains(pred)) losing.add(pred);
        } else {
          ...
          for (S succ : uncontrollableSuccesors) {
            ...
            if (losing.contains(succ) || losingStates.contains(succ) || succ == pred) {
              losingSuccesors++;
            }
          }
