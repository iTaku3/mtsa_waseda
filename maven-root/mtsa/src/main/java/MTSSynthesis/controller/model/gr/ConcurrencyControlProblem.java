...
import MTSSynthesis.controller.gr.concurrency.ConcurrencyFunction;
import MTSSynthesis.controller.gr.concurrency.ConcurrencyGameSolver;
import MTSSynthesis.controller.gr.concurrency.DoubleRankFunction;
...
public class ConcurrencyControlProblem<S, A, M> extends GRGameControlProblem<S, A, M> {
  ...
  public ConcurrencyControlProblem(LTS<S, A> environment, ControllerGoal<A> controllerGoal) {
  ...
  public LTS<StrategyState<S, Integer>, A> rawSolve() {
    ...
    DoubleRankFunction<S> function =
        new ConcurrencyFunction<S>(
            environment.getStates(), cgame.getGoal().getConcurrencyInformation());
    ConcurrencyGameSolver<S, A> solver =
        new ConcurrencyGameSolver<S, A>(
            cgame,
            environment,
            function,
            cgame.getGoal().getGuarantee(cgame.getGoal().getGuarantees().size()).getStateSet());
    LTS<StrategyState<S, Integer>, A> result =
        GameStrategyToLTSBuilder.getInstance().buildLTSFrom(environment, solver.buildStrategy());
