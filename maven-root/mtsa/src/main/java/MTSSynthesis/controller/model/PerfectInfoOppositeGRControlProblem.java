...
public class PerfectInfoOppositeGRControlProblem<S, A>
    extends StateSpaceCuttingControlProblem<S, A> {

  public PerfectInfoOppositeGRControlProblem(
      LTS<S, A> originalEnvironment, ControllerGoal<A> controllerGoal, S trapState) {
  ...
  protected LabelledGameSolver<S, A, Integer> buildGameSolver() {
    ...
    GRLabelledGame<S, A> game =
        new GRLabelledGame<>(environment, controllerGoal.getControllableActions(), grGoal);
    ...
    PerfectInfoOppositeGRLabelledGameSolver<S, A> perfectInfoGRGameSolver =
        new PerfectInfoOppositeGRLabelledGameSolver<S, A>(game, grRankSystem);
