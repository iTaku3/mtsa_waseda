...
public class ConcurrencyGRControlProblem<S, A, M> extends GRGameControlProblem<S, A, M> {
  ...
  public LTS<StrategyState<S, Integer>, A> rawSolve() {
    perfectInfoGRControlProblem =
        new PerfectInfoGRControlProblem<S, A>(environment, controllerGoal);
    LTS<S, A> env = perfectInfoGRControlProblem.solve();
    ConcurrencyControlProblem<S, A, M> ccp =
        new ConcurrencyControlProblem<S, A, M>(env, controllerGoal);
