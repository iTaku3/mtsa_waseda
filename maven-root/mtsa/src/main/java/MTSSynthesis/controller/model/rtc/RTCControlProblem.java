...
public class RTCControlProblem implements ControlProblem<StrategyState<Long, Integer>, String> {
  ...
  public RTCControlProblem(
    ...
    // controllableStateAction and uncontrollableStateAction are marks on states to that after
    // composing we can
    ...
    this.game =
        new BGRGameBuilder<>(synchronous, goal)
            .buildBGRGameForRTC(
                envYieldsAction,
                contYieldsAction,
                controllableStateAction,
  ...
  private MTS<Long, String> getYieldComposition(
      MTS<Long, String> env,
      ControllerGoal<String> goal,
    ...
    // There may be issues with actions being considered uncontrollable and added to this DLTS,
    // "tau" for example.
    Set<String> actions =
        env.getTransitions(REQUIRED).values().stream()
    ...
    Predicate<Long> onlyYieldsBack =
        (s) ->
            mtsComposition.getTransitions(s, REQUIRED).stream()
                .allMatch(
                    (t) ->
    ...
    // We remove all envYields and contYields transitions that can afterwards only yield back
    // (live(E||Y))
    mtsComposition
        .getTransitions(REQUIRED)
        .forEach(
            (from, transitions) -> {
              transitions.getImage(contYieldsAction).stream()
                  .filter(onlyYieldsBack)
                  .forEach(
                      to ->
                          toRemove.add(
                              () ->
                                  mtsComposition.removeTransition(
                                      from, contYieldsAction, to, REQUIRED)));
              transitions.getImage(envYieldsAction).stream()
                  .filter(onlyYieldsBack)
                  .forEach(
                      to ->
                          toRemove.add(
                              () ->
                                  mtsComposition.removeTransition(
                                      from, envYieldsAction, to, REQUIRED)));
            });
