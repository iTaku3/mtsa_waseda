...
public abstract class StateSpaceCuttingControlProblem<S, A> extends GRControProblem<S, A, Integer> {
  ...
  public LabelledGameSolver<S, A, Integer> getGameSolver() {
    if (gameSolver == null) gameSolver = buildGameSolver();
    return gameSolver;
  }
  ...
  public StateSpaceCuttingControlProblem(
      LTS<S, A> env, ControllerGoal<A> grControlGoal, S trapState) {
    super(env, grControlGoal);
    this.trapState = trapState;
    if (!env.getStates().contains(trapState)) env.addState(trapState);
  }
    ...
    for (S s : gameStates) {
      for (Pair<A, S> p : environment.getTransitions(s)) {
        if (!winningStates.contains(p.getSecond())) {
          environment.removeTransition(s, p.getFirst(), p.getSecond());
  ...
  public LTS<S, A> removeStates() {
    return removeInnerStates(
        Sets.difference(gameSolver.getGame().getStates(), gameSolver.getWinningStates()));
  }

  public LTS<S, A> removeInnerStates(Set<S> innerStates) {

    if (!environment.getStates().contains(trapState)) environment.addState(trapState);
    // remove transitions from trapState, just in case
    for (Pair<A, S> p : environment.getTransitions(trapState)) {
      environment.removeTransition(trapState, p.getFirst(), p.getSecond());
    }
  ...
  protected GRGoal<S> getGRGoalFromControllerGoal(
      LTS<S, A> environment, ControllerGoal<A> controllerGoal) {
    GRGameBuilder<S, A> grGameBuilder = new GRGameBuilder<S, A>();
    return grGameBuilder
        .buildGRGameFrom(new MTSAdapter<S, A>(environment), controllerGoal)
        .getGoal();
  }
}
