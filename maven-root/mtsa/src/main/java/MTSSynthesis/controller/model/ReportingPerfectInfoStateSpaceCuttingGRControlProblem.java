...
public class ReportingPerfectInfoStateSpaceCuttingGRControlProblem
  ...
  public class CompositeReport {
    ...
    public CompositeReport(
        ...
        List<CutterReport> cutterReports,
        LTS<Long, String> mut,
        LTS<Long, String> exp) {
    ...
    public CompositeReport(
      ...
      for (String action : mut.getActions())
        if (!expActions.contains(action)) exp.addAction(action);
    ...
    protected void removeUnusedActionsFromLTS(LTS<Long, String> machine) {
      ...
      for (String action : machine.getActions()) {
        ...
        if (!hasAction) actionsToRemove.add(action);
      }
      for (String actionToRemove : actionsToRemove) machine.removeAction(actionToRemove);
    }
    ...
    public String getHeader(
        String columnSeparator, String sectionSeparator, String rowSeparator, boolean forLatex) {
      String cutsHeader = "";
      String cutsAlignment = "";
      if (forLatex) {
        cutsAlignment = "\\begin{tabular}{ | c | c c c c c c c | ";
        for (int i = 0; i < cutterReports.size(); i++) cutsAlignment += "c c c c c | ";
        cutsAlignment += "}" + rowSeparator;
      }
      for (CutterReport report : cutterReports)
        cutsHeader +=
            columnSeparator + sectionSeparator + report.getHeader(columnSeparator, "", forLatex);
      if (forLatex) {
        return cutsAlignment
            + "\\rot{test name}"
            + columnSeparator
            + "\\rot{$c'=c$}"
            + columnSeparator
    ...
    public String toString(
      ...
      String cutsString = "";
      for (CutterReport report : cutterReports)
        cutsString += columnSeparator + report.toString(false, columnSeparator, "", forLatex);
      returnString =
          returnString
              + testName
              + columnSeparator
              + boolToString(isEquivalent())
              + columnSeparator
              + boolToString(simulatesController)
              + columnSeparator
              + boolToString(simulatedByController)
              + columnSeparator
              + assumptionsCount
              + columnSeparator
              + guaranteesCount
              ...
              + failuresCount
              + columnSeparator
              + boolToString(isCompatible)
              + cutsString
              + rowSeparator;
      if (printFooter) returnString += getFooter(columnSeparator, rowSeparator, forLatex);
      return returnString;
    }
  }
  ...
  public class CutterReport {
    ...
    public CutterReport(String cutterName, int initialStatesCount) {
      this(
          cutterName, System.nanoTime(), System.nanoTime(), initialStatesCount, initialStatesCount);
    }

    public CutterReport(String cutterName, long initialTime, int initialStatesCount) {
      this(cutterName, initialTime, initialTime, initialStatesCount, initialStatesCount);
    }
    ...
    public String toString(
        boolean printHeader, String columnSeparator, String rowSeparator, boolean forLatex) {
      String returnString = "";
      if (printHeader) returnString = getHeader(columnSeparator, rowSeparator, forLatex);
      returnString =
          returnString
              + cutterName
              + columnSeparator
              + initialStatesCount
              + columnSeparator
              + finishingStatesCount
              + columnSeparator
              + getStatesCountDelta()
              + columnSeparator
              + (initialStateWinning ? "T" : "F")
              + columnSeparator
              + getTimeDelta()
              + rowSeparator;
      return returnString;
  ...
  public String getReportString(
      ...
      String rowSeparator,
      boolean forLatex) {
    if (generalReport == null) return "";
    return generalReport.toString(
        hasHeader, hasFooter, columnSeparator, sectionSeparator, rowSeparator, forLatex);
  }
  ...
  public ReportingPerfectInfoStateSpaceCuttingGRControlProblem(
      ...
      ControllerGoal<String> controllerGoal,
  ...
  protected LTS<Long, String> primitiveSolve() {
    ...
    System.gc();
    GRGame<Long> game =
        new GRGameBuilder<Long, String>()
            .buildGRGameFrom(new MTSAdapter<Long, String>(envForController), controllerGoal);
    GRRankSystem<Long> system =
        new GRRankSystem<Long>(
            game.getStates(),
            game.getGoal().getGuarantees(),
            game.getGoal().getAssumptions(),
            game.getGoal().getFailures());

    PerfectInfoGRGameSolver<Long> originalGameSolver =
        new PerfectInfoGRGameSolver<Long>(game, system);

    CutterReport originalSolverReport = new CutterReport("GR", environment.getStates().size());
    ...
    originalSolverReport.initialStateWinning =
        originalGameSolver.getWinningStates().containsAll(game.getInitialStates());
    System.gc();

    // WHOLE PROBLEM
    CutterReport grReport = new CutterReport("GR+CUT", environment.getStates().size());
    reports.add(grReport);

    // GR AFTER CUT
    CutterReport onlyGRAfterCutReport = new CutterReport("GR.only", environment.getStates().size());
    ...
    if (runNoG) {
      if (removeSafeStatesFirst) {
        SafeGameSolver<Long, String> safeGameSolver =
            new SafeGameSolver<Long, String>(environment, controllable);

        CutterReport safeReport = new CutterReport("Safe", environment.getStates().size());
        ...
        safeReport.initialStateWinning =
            !(safeGameSolver
                .getWinningStates()
                .containsAll(safeGameSolver.getGame().getInitialStates()));

        CutterReport safeRemoveReport =
            new CutterReport("SafeRemove", environment.getStates().size());
        reports.add(safeRemoveReport);

        Set<Long> safeLosingStates =
            Sets.difference(environment.getStates(), safeGameSolver.getWinningStates());
        ...
        safeRemoveReport.initialStateWinning =
            !(safeGameSolver
                .getWinningStates()
                .containsAll(safeGameSolver.getGame().getInitialStates()));
      }

      OppositeSafeControlProblem safeControlProblem =
          new OppositeSafeControlProblem(
              environment,
              controllerGoal,
              trapState,
              relaxAllControllables,
      ...
      CutterReport oppositeSafeReport =
          new CutterReport("Opp.Safe", environment.getStates().size());
      ...
      //            System.out.println("TIME DELTA for safety on " + testName + ":" +
      // oppositeSafeReport.getTimeDelta() + " size: " + winningNoG.size());
      try {
        File innerfile = new File(outputFileName + ".rep");
        FileOutputStream innerfout = new FileOutputStream(innerfile);
        // now convert the FileOutputStream into a PrintStream
        PrintStream innerOutput = new PrintStream(innerfout);
        innerOutput.println(
            "TIME DELTA for safety on "
                + testName
                + ":"
                + oppositeSafeReport.getTimeDelta()
                + " size: "
                + winningNoG.size());
        innerOutput.close();
        innerfout.close();
      ...
      oppositeSafeReport.initialStateWinning =
          !(safeControlProblem
              .getWinningStates()
              .containsAll(safeControlProblem.getGameSolver().getGame().getInitialStates()));
      // return null;
    }
    ...
    if (runAssumptions) {
      PerfectInfoOppositeGRControlProblem<Long, String> oppositeGRControlProblem =
          new PerfectInfoOppositeGRControlProblem<Long, String>(
              environment, controllerGoal, trapState);

      CutterReport oppositeGRReport = new CutterReport("Opp.GR", environment.getStates().size());
      ...
      oppositeGRReport.initialStateWinning =
          oppositeGRControlProblem
              .getWinningStates()
              .containsAll(oppositeGRControlProblem.getGameSolver().getGame().getInitialStates());
    }

    Set<Long> losingStates = null;
    if (runAssumptions && runNoG) {
      CutterReport intersectionReport =
          new CutterReport("Intersection", environment.getStates().size());
    ...
    if (losingStates != null) environment = removeInnerStates(losingStates);
    ...
    onlyGRAfterCutReport.initialStateWinning =
        gameSolver.getWinningStates().containsAll(gameSolver.getGame().getInitialStates());

    System.gc();

    LTS<Long, String> result = buildStrategy();
    generalReport = new CompositeReport(testName, controllerGoal, reports, result, expectedResult);

    // LTS<Long, String> result = new LTSImpl<Long, String>(1L);

    generalReport =
        new CompositeReport(testName, controllerGoal, reports, result, expectedResult, true);
