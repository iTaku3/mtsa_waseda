...
public abstract class RankBasedGameSolver<S, M> implements GameSolver<S, M> {
  ...
  protected abstract void addPredecessorsTo(
      Queue<StrategyState<S, M>> pending, StrategyState<S, M> strategyState, Rank bestRank);
  ...
  private void parallelRankingSolving(Integer nThreads, Queue<StrategyState<S, M>> pending) {
    ...
    // result,
    // you can declare types of the form Future<?> and return null as a result of the underlying
    // task.
    ...
    for (int i = 0; i < nThreads; i++) {
      futureArray[i] =
          taskPool.submit(
              new RankBasedGameSolverWorker<S, M>(
                  pending, this, running, waitingElement, allFinish));
    }
  ...
  private void sequentialRankingSolving(Queue<StrategyState<S, M>> pending) {
    RankBasedGameSolverWorker<S, M> sequentialWorker =
        new RankBasedGameSolverWorker<S, M>(pending, this);
