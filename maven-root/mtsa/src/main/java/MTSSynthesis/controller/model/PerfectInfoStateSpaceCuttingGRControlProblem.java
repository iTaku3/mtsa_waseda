...
public class PerfectInfoStateSpaceCuttingGRControlProblem<S, A>
    extends GRControProblem<S, A, Integer> {

  protected List<String> stateSpaceCuttingControlProblems;
  protected PerfectInfoGRGameSolver<S> perfectInfoGRControlProblem;

  public PerfectInfoStateSpaceCuttingGRControlProblem(
      LTS<S, A> originalEnvironment, ControllerGoal<A> controllerGoal) {
  ...
  protected StateSpaceCuttingControlProblem<S, A> createStateSpaceCuttingControlProblem(
    ...
    StateSpaceCuttingControlProblem<S, A> instance =
        (StateSpaceCuttingControlProblem<S, A>)
            constructor.newInstance(environment, controllerGoal);
    return instance;
  }
  ...
  protected LTS<S, A> primitiveSolve() {
    ...
    for (String s : stateSpaceCuttingControlProblems) {
      try {
        StateSpaceCuttingControlProblem<S, A> cuttingControlProblem =
            createStateSpaceCuttingControlProblem(s);
    ...
    GRCGame<S> perfectInfoGRGame =
        new GRGameBuilder<S, A>()
            .buildGRCGameFrom(new MTSAdapter<S, A>(environment), controllerGoal);
    GRRankSystem<S> grRankSystem =
        new GRRankSystem<S>(
            perfectInfoGRGame.getStates(), perfectInfoGRGame.getGoal().getGuarantees(),
            perfectInfoGRGame.getGoal().getAssumptions(),
                perfectInfoGRGame.getGoal().getFailures());
    ...
    LTS<StrategyState<S, Integer>, A> result =
        GameStrategyToLTSBuilder.getInstance()
            .buildLTSFrom(environment, perfectInfoGRControlProblem.buildStrategy());
