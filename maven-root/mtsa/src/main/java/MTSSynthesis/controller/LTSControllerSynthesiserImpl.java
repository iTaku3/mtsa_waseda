...
public class LTSControllerSynthesiserImpl<S, A> implements LTSControllerSynthesiser<S, A> {
    ...
    if (solver.isWinning(plant.getInitialState())) {
      ...
      MTS<StrategyState<S, Integer>, A> result =
          GameStrategyToMTSBuilder.getInstance()
              .buildMTSFrom(plant, strategy, worseRank, maxLazyness);
  ...
  public boolean checkGRAssumptionsCompatibility(MTS<S, A> plant, ControllerGoal<A> goal) {
    ...
    if (!goal.getFaults().isEmpty()) {
      Set<Symbol> symbols =
          FluentUtils.getInstance().getInitiatingSymbols(goal.getFluentsInFaults());
    ...
    Logger.getAnonymousLogger()
        .log(
            Level.INFO,
            "Environment model with" + (stateRemoved ? "" : "out") + " unreachable states");
