...
public class MTSA {
  ...
  // refinement
  public <State, Action> boolean isRefinement(
      MTS<State, Action> refined, MTS<State, Action> refines, Set<Action> silentActions) {
    return new WeakSemantics(silentActions).isARefinement(refined, refines);
  }

  // merge
  public <State, Action> MTS<Object, Action> merge(
      MTS<State, Action> mtsA, MTS<State, Action> mtsB, Set<Action> silentActions) {
