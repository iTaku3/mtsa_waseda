...
public class ClientProject extends JFrame
    implements IMessageListener, ITransitionEventListener<String> {
  /** */
  ...
  protected void initGui() {
    ...
    try {
      inputLtsDocument.insertString(
          ...
          // "TEST=(follow -> success -> follow -> success -> turnRight -> success -> follow ->
          // success -> follow -> success ->  follow -> success -> turnLeft -> success -> follow ->
          // success -> turnLeft -> success -> follow -> success -> turnLeft -> success -> follow ->
          // success -> TEST)."
          // "LOOP = (follow -> (success -> (turnLeft -> (success -> (follow -> (success ->
          // (turnLeft -> (success -> (follow -> (success -> (turnLeft -> (success -> (follow ->
          // (success -> (follow -> (success -> (turnRight -> (success -> (follow -> (success ->
          // (turnRight -> (success -> (follow -> (success -> (turnRight -> (success -> (follow ->
          // (success -> LOOP | lost -> LOST)) | lost -> LOST)) | lost -> LOST)) | lost -> LOST)) |
          // lost -> LOST)) | lost -> LOST)) | lost -> LOST)) | lost -> LOST)) | lost -> LOST)) |
          // lost -> LOST)) | lost -> LOST)) | lost -> LOST)) | lost -> LOST)) | lost -> LOST))."
          // "READ = (readSensors -> (lost -> LOST | readSensorsReply -> READ))."
          // "FOLLOW = (follow -> (success -> TURN | lost -> LOST)), TURN = (turnLeft -> (success ->
          // FOLLOW | lost -> LOST))."
          ...
          */
          // "CONTROL = (follow -> Q1), Q1 = (success -> CONTROL | failure -> FAIL | lost ->
          // LOST))."
          ,
          null);
    ...
    inputLtsAcceptButton.addActionListener(
        new ActionListener() {
    ...
    inputLtsStopButton.addActionListener(
        new ActionListener() {
    ...
    saveRunsButton.addActionListener(
        new ActionListener() {
        ...
        if (chooser.getSelectedFile() != null) {
          ...
          if (f.isDirectory()) {
            ...
            for (RobotRun<String> run : runs) {
              ...
              run.saveFilesToLocation(
                  f.getAbsolutePath(), chooser.getSelectedFile().getName() + runIndex++);
            }
            ...
            for (Integer transitionToFailure : firstFailure) {
              if (transitionToFailure >= 0) totalActions += transitionToFailure;
            }
  ...
  public void eventHandler(MessageEvent messageEvent) {
    ...
    try {
      ltsOutputDocument.insertString(
          ltsOutputDocument.getLength(),
          ((Message) messageEvent.getSource()).getPackedMessage() + "\n",
  ...
  protected void initializeScheduler(LTSAdapter<Long, String> ltsAdapter) {
    ...
    controllerScheduler =
        new TakeFirstController<Long, String>(
            "controller scheduler", ltsAdapter, controllableActions);
    n6 =
        new N6Robot<Long, String>(
            "n6",
            "success",
            "failure",
            "lost",
  ...
  public void handleTransitionEvent(TransitionEvent<String> transitionEvent) throws Exception {
    ...
    ltsOutputDocument.insertString(
        ltsOutputDocument.getLength(), transitionEvent.getAction() + "\n", null);
    ltsOutputPane.invalidate();
    ltsOutputPane.repaint();
  }
}
