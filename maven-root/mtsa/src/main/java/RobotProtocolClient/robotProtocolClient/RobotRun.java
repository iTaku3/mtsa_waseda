...
public class RobotRun<Action> {
  ...
  public void addFailureAction(Action action) throws Exception {
    ...
    if (failureActions.contains(action)) return;
  ...
  public void addControllableAction(Action action) throws Exception {
    ...
    if (controllableActions.contains(action)) return;
  ...
  public List<ActionRun<Action>> getControllableTrace() {
    ...
    for (ActionRun<Action> actionRun : trace) {
      if (controllableActions.contains(actionRun.getAction())) controllableTrace.add(actionRun);
    }
  ...
  public void addAction(Action action) {
    ...
    if (bufferedImage != null) trace.add(new ActionRun<Action>(action, bufferedImage));
    else trace.add(new ActionRun<Action>(action));
  ...
  public String getFormattedString() {
    ...
    if (trace.size() > 1) {
      long secs =
          trace.get(trace.size() - 1).getStartingTime().getTime()
              - trace.get(0).getStartingTime().getTime();
          ...
          imageFile =
              new File(
                  location
                      ...
                      + prefix
                      + "_"
                      + actionRun.getStartingTime().toString()
                      + "_"
                      + actionRun.getAction().toString()
                      + ".png");
