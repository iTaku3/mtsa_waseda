...
/*implements IMessageListener*/ {
  ...
  protected void primitiveHandleTransitionEvent(TransitionEvent<Action> transitionEvent)
    ...
    if (transitionEvent.getAction().equals(takeoff)
    ...
    } else if (transitionEvent.getAction().equals(moveTo0)
        || transitionEvent.getAction().equals(moveToOLD0)) {
      //			System.out.println("MOVE TO 0");
      moveTo(0, 0);
    } else if (transitionEvent.getAction().equals(moveTo1)
        || transitionEvent.getAction().equals(moveToOLD1)) {
      //			System.out.println("MOVE TO 1");
      moveTo(0, 2);
    } else if (transitionEvent.getAction().equals(moveTo2)
        || transitionEvent.getAction().equals(moveToOLD2)) {
      moveTo(2, 2);
    } else if (transitionEvent.getAction().equals(moveTo3)
        || transitionEvent.getAction().equals(moveToOLD3)) {
      ...
      drone.downSimply();
    } else if (transitionEvent.getAction().equals(reset)
        || transitionEvent.getAction().equals(resetOLD)) {
  ...
  private boolean isLabel(String text) {
    return !(tWin.text.equalsIgnoreCase("Tag 2")
        || tWin.text.equalsIgnoreCase("Tag 1")
        || tWin.text.equalsIgnoreCase("Tag 0"));
  }
  ...
  //				e.printStackTrace();
  //			}
  //		}else if(source instanceof DestinationReachedMessage || messageEvent.getReply() instanceof
  ...
  //				e.printStackTrace();
  //			}
  //		}else if(source instanceof ReadIRSensorsReplyMessage || messageEvent.getReply() instanceof
  // ReadIRSensorsReplyMessage ){
  //			try {
  //				fireTransitionEvent(readValue0);
  //			} catch (Exception e) {
  //				// TODO Auto-generated catch block
