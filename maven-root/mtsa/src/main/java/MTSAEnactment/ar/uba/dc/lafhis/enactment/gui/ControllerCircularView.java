...
public class ControllerCircularView<State, Action> extends JPanel
  ...
  public synchronized void initialize(
      ...
      State initialState,
      TransitionDispatcher<Action> transitionDispatcher) {
  ...
  public void handleTransitionEvent(TransitionEvent<Action> transitionEvent) throws Exception {
    //		//System.out.println("taking action " + transitionEvent.getAction() + " from " +
    // transitionEvent.getSource());
    Iterator<State> iterator =
        lts.getTransitions(currentState).getImage(transitionEvent.getAction()).iterator();
    if (iterator.hasNext()) {
  ...
  protected int getStateIndex(State state) {
    if (lts == null) return -1;
    Set<State> states = lts.getStates();
    int i = 0;
    for (State s : states) {
      if (state.equals(s)) return i;
      i++;
    }
    return -1;
  }
  ...
  protected int getStateRadius(int stateIndex) {
    if (lts == null) return (int) 0;
    int i = 0;
    Set<State> states = lts.getStates();
    for (State state : states) {
      if (i == stateIndex) {
        if (state.equals(initialState)) {
          // Deshabilito animacion para estado inicial. Usuarios tienden a confundirlo con estado
          // vigente
  ...
  protected Color getStateColor(int stateIndex) {
    if (lts == null) return Color.BLACK;
    int i = 0;
    Set<State> states = lts.getStates();
    for (State state : states) {
      if (i == stateIndex) {
