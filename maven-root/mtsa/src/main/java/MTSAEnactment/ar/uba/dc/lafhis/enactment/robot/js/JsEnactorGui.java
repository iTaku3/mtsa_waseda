...
/**
 * The graphical user interface for the {@link JsEnactor enactor}. Shows the drone's {@link JsCamera
 * video stream} and the result of the {@link JsVision image processing}. Also allows choosing the
 * {@link JsCamera#setMinColor(Color) minimum} and {@link JsCamera#setMaxColor(Color) maximum
 * color}.
 *
...
public class JsEnactorGui extends JFrame {
  ...
  /**
   * A panel showing an image. Functions as a video display by repeatedly updating the image and
   * then repainting.
   *
  ...
  public JsEnactorGui(JsCamera cam, JsVision vis) {
    ...
    imgPanelIn =
        new ImagePanel(
            new BufferedImage(IMG_SIZE.width, IMG_SIZE.height, BufferedImage.TYPE_3BYTE_BGR));
    imgPanelOut =
        new ImagePanel(
            new BufferedImage(IMG_SIZE.width, IMG_SIZE.height, BufferedImage.TYPE_BYTE_GRAY));

    // Create the color frame.
    colorFrame.setTitle("Target Color");
    colorFrame
        .getContentPane()
        .setLayout(new BoxLayout(colorFrame.getContentPane(), BoxLayout.X_AXIS));
