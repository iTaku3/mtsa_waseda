...
public class DroneAPI {
  ...
  public void move(int m, int n) throws ARDroneException {
    fm.fileWrite(
        "============start move" + Integer.toString(m) + "," + Integer.toString(n) + "===========");
    ...
    while (timecounta < 200) {
      ...
      if (ex < th && ex > -th && ey < th && ey > -th) {
        ...
        //				System.out.println("Success!!target x:"+targetPosition[0]+"target
        // y:"+targetPosition[1]);
        wait(1000);
        return;
      } else {
...
class FileManager {
    ...
    f =
        new File(
            "./log_drone.txt"
                ...
                + "_"
                + Integer.toString(Calendar.HOUR)
  ...
  void fileOpen() {
    ...
    int m = t.get(Calendar.MONTH) + 1,
        d = t.get(Calendar.DATE),
        h = t.get(Calendar.HOUR),
        mi = t.get(Calendar.MINUTE),
