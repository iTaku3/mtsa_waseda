...
/**
 * Processes images from the drone's {@link JsCamera camera}. Finds target locations in the image. A
 * target is identified through a color range.
 *
...
public class JsVision {
  ...
  public class ImageProcessedEvent extends Event {
    ...
    public ImageProcessedEvent(
        long time, ImageReceivedEvent imageReceivedEvent, Mat imgOut, Point2D.Double targetImgPos) {
      super(time, imageReceivedEvent);
      this.imgOut = imgOut;
  ...
  private static final Point2D.Double DRONE_ORIGIN =
      new Point2D.Double(IMG_SIZE.getWidth() / 2, IMG_SIZE.getHeight() + 70);

  /**
   * The transformation matrix defining the homography between image plane and bird's eye view. Each
   * pixel in the bird's eye view has a width and height of 1 millimeter. Thus, the bird's eye view
   * can be used to measure distances and angles in physical relationship to the drone.
   */
  ...
  static {
    ...
    final Mat translate =
        Mat.zeros(
            3,
            3,
            CvType.CV_64F); // shifts drone position in image from image origin to drone origin
    translate.put(0, 0, 1);
    translate.put(1, 1, 1);
    translate.put(2, 2, 1);
  ...
  // image processing
  /**
   * The width of the vertical strip in the center of the image to be considered for target
   * detection.
   */
  private static final int IMG_CROP_WIDTH = 500;

  /**
   * Rectangle for cropping the image so that only a narrow vertical strip is considered for target
   * detection.
   */
  private static final Rect IMG_CROP =
      new Rect(
  ...
  private static final Mat MORPH_KERNEL =
  ...
  private void processImage(ImageReceivedEvent imageReceivedEvent) {
    ...
    notifyImageProcessed(
        new ImageProcessedEvent(
            System.currentTimeMillis(), imageReceivedEvent, imgMorph, targetImgPos));
  }
  ...
  private Point2D.Double findTargetImagePos() {
    ...
    final int labelCount =
        Imgproc.connectedComponentsWithStats(imgCrop, imgLabels, stats, centroids);
    ...
    for (int label = 1; label < labelCount; label++) {
      ...
      final Point2D.Double centroid =
          new Point2D.Double(
              centroids.get(label, 0)[0] + IMG_CROP.x, centroids.get(label, 1)[0] + IMG_CROP.y);
