...
/**
 * {@link Enactor} for a Parrot Jumping Sumo drone. The drone {@link JsMovement moves} along a 2D
 * grid. {@link JsVision Processes images} from the {@link JsCamera camera} to detect target
 * positions. Then {@link JsVisionMovement moves to the target}.
 *
 * <p>The enactor may be configured using a properties file called <code>JumpingSumo.properties
 * </code>. It supports the following entries:
 *
 * <table>
 * <tr>
 * <th>Key</th>
...
public class JsEnactor<S, A> extends Enactor<S, A> {
  ...
  /**
   * A direction in which the drone can move. The direction in which the drone is facing at the
   * start defines {@link #NORTH North}. After that, the compass stays the same independently of the
   * drone.
   *
   * @author Timo G&uuml;nther
   */
  private enum Direction {
    ...
    /**
     * Returns the heading from this direction to another. That is how many directions one would
     * have to turn clockwise to get from the former to the latter. For example, the heading from
     * {@link #SOUTH East} to {@link #SOUTH South} is 1.
     *
     * <p>The returned value is normalized. For example, the heading from {@link #NORTH North} to
     * {@link #WEST West} is not 3 but -1. That prevents unnecessary turning.
     *
  ...
  public JsEnactor(String name, A goE, A goW, A goN, A goS, A arrive, A obstacle) {
    super(name);
    this.goE = goE;
    this.goW = goW;
    this.goN = goN;
  ...
  @Override
  protected void primitiveHandleTransitionEvent(TransitionEvent<A> transitionEvent)
      throws Exception {
