...
public class RobotProtocolSession extends MessageDispatcher implements IMessageListener, Runnable {
  ...
  public synchronized boolean sendMessage(Message message) {
    if (!(message instanceof CommunicationStartMessage
        || message instanceof CommunicationStopMessage
        || isOpen)) return false;
  ...
  public void run() {
    while (true) {
      if (serial == null || !serial.isOpened()) return;
      if (messageOnCourse == null && pendingQueue.size() > 0) sendMessage(pendingQueue.remove(0));
      processSerial();
      processTimeout();
      try {
  ...
  protected synchronized void processTimeout() {
    if (messageOnCourse == null) return;
      ...
      if (availableBytes > 0) {
        ...
        if (queueLength >= Message.MESSAGE_HEADER_LENGTH) {
          ...
          if (Message.isMessageComplete(rawMessage)) {
            ...
            //						System.out.println("|pops|<<" + firstRawMessage + (incomingMessage == null? "no
            // incoming message" : " |replies to|<<" + incomingMessage.getPackedMessage()));
            if (incomingMessage != null) {
              ...
              else outgoingEvent = new MessageEvent(incomingMessage, null);
