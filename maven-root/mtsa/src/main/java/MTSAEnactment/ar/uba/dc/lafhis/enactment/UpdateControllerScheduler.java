...
public class UpdateControllerScheduler extends BaseController<Long, String> {
  ...
  public UpdateControllerScheduler(
      String name, LTS<Long, String> lts, Set<String> controllableActions) {
  ...
  private void makeControllableButtons() {
    ...
    for (final String action : this.optControllableActions)
      UpdatingControllersAnimatorUtils.addControllableButton(
          action.toString(),
          new ActionListener() {
  ...
  private void makeEnvironmentButtons() {
    ...
    for (String action : this.optEnvironmentActions)
      UpdatingControllersAnimatorUtils.addEnvironmentButton(
          action.toString(),
          new ActionListener() {
  ...
  private List<Pair<String, Long>> getActualActions() {
    ...
    if (!stateIterator.hasNext()) {
      ...
      logger.error(
          String.format(
              "[Scheduler::primitiveHandleTransitionEvent] there is no reachable state from"
                  + " currentState %s",
  ...
  private void executeRandom() throws Exception {
    ...
    if (envSpeed > 0 && UpdatingControllersAnimatorUtils.pauseEnvironment == 0)
      possibles.addAll(optEnvironmentActions);
  ...
  private void doHotSwap() {
    ...
    LTSAdapter<Long, String> toLTS =
        new LTSAdapter<Long, String>(
            UpdatingControllersAnimatorUtils.updateController, MTS.TransitionType.REQUIRED);
    setLts(
        toLTS,
        UpdatingControllersAnimatorUtils.getActualStateFromTheUpdateController(executedActions),
        UpdatingControllersAnimatorUtils.updControllableActions);
