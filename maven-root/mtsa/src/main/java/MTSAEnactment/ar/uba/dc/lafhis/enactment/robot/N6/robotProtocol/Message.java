...
public abstract class Message extends MessageDispatcher {
  ...
  public static boolean isMessageComplete(String rawMessage) {
    if (rawMessage.length() < MESSAGE_HEADER_LENGTH) return false;
    int expectedLength = getLengthFromRawMessage(rawMessage);
    if (expectedLength > rawMessage.length()) return false;
    return true;
  }

  public static int getLengthFromRawMessage(String rawMessage) {
    if (rawMessage.length() < MESSAGE_HEADER_LENGTH) return -1;
    return Integer.parseInt(
        rawMessage.substring(HEAD_FIELD_LENGTH, HEAD_FIELD_LENGTH + LENGTH_FIELD_LENGTH));
  }
  ...
  public static Message deserializeMessage(String rawMessage) {
    ...
    List<Class> subClasses =
        ReflectionUtils.findSubClasses(
            Message.class.getPackage().getName(), Message.class.getName(), 1, 4);
    subClasses.addAll(
        ReflectionUtils.findSubClasses(
            Message.class.getPackage().getName(), Message.class.getName(), 1, 4, 911));
    subClasses.addAll(
        ReflectionUtils.findSubClasses(
            Message.class.getPackage().getName(), Message.class.getName(), 1, 4, 911, 911));
    subClasses.addAll(
        ReflectionUtils.findSubClasses(
            Message.class.getPackage().getName(), Message.class.getName(), 1, 4, 911, 911, 911));
    subClasses.addAll(
        ReflectionUtils.findSubClasses(
  ...
  private String writeHeader() {
    return String.format(
        "%s%04d%04d%03d%01d%04d%04d",
        MESSAGE_HEAD_VALUE, getMessageLength(), messageID, revision, flags, to, from);
  }
