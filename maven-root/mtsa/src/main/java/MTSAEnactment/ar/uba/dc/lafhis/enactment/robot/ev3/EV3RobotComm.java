...
public class EV3RobotComm {
  ...
  private void setupConnection() throws EV3RobotCommException {
    if (socket != null) throw new EV3RobotCommException("Connection already created");
  ...
  private void sendCommand(int command) throws EV3RobotLostException, EV3RobotCommException {
    ...
    try {
      ...
      if (this.isSyncCalls()) {
        ...
        if (cmdResp == RobotDriverCommCommands.SUCCESS) return;
  ...
  private void sendCommand(int command, double value)
      throws EV3RobotLostException, EV3RobotCommException {
    if (!isConnected()) setupConnection();
    ...
    try {
      ...
      if (this.isSyncCalls()) {
        ...
        if (cmdResp == RobotDriverCommCommands.SUCCESS) return;
          ...
          throw new EV3RobotLostException("Robot lost");
        else throw new EV3RobotCommException(String.valueOf(cmdResp));
      }
