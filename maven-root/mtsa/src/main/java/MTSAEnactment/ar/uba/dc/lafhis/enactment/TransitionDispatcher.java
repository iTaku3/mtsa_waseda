...
public abstract class TransitionDispatcher<Action> {
  ...
  public synchronized void removeTransitionEventListener(
      ITransitionEventListener<Action> listener) {
  ...
  protected void fireTransitionEvent(TransitionEvent<Action> transitionEvent) throws Exception {
    TransitionEvent<Action> event =
        new TransitionEvent<Action>(transitionEvent.getSource(), transitionEvent.getAction());
