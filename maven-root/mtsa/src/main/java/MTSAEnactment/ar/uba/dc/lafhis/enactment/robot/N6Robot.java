...
/**
 * Created with IntelliJ IDEA. User: tommy Date: 2/23/13 Time: 6:06 PM To change this template use
...
public class N6Robot<State, Action> extends RobotAdapter<State, Action>
  ...
  private void activateOptions(Action triggerAction) {
    if (n6RobotGUI == null) this.setUp();
    if (!n6RobotGUI.isVisible()) n6RobotGUI.setVisible(true);
  ...
  protected void primitiveHandleTransitionEvent(TransitionEvent<Action> transitionEvent)
    ...
    if (transitionEvent.getAction().equals(follow)) {
      ...
      session.sendMessage(
          new ReadIRSensorsReplyMessage(session.getRobotId(), session.getMasterId(), 0, 0, 0));
      ...
      session.sendMessage(
          new ReadUSSensorReplyMessage(session.getRobotId(), session.getMasterId(), 0));
  ...
  public void eventHandler(MessageEvent messageEvent) {
    ...
    } else if (source instanceof ReadUSSensorReplyMessage
        || messageEvent.getReply() instanceof ReadUSSensorReplyMessage) {
      try {
  ...
  private class ArrowBinding extends AbstractAction {
    ...
    public void actionPerformed(ActionEvent e) {
      ...
      switch (direction) {
        case UP:
          session.sendMessage(
              new SetMotorSpeedMessage(
                  session.getRobotId(), session.getMasterId(), FORWARD_SPEED, FORWARD_SPEED));
          break;
        case DOWN:
          session.sendMessage(
              new SetMotorSpeedMessage(
                  session.getRobotId(), session.getMasterId(), BACK_SPEED, BACK_SPEED));
          break;
        case LEFT:
          session.sendMessage(
              new SetMotorSpeedMessage(
                  session.getRobotId(), session.getMasterId(), -TURN_SPEED, TURN_SPEED));
          break;
        case RIGHT:
          session.sendMessage(
              new SetMotorSpeedMessage(
        ...
        case SPACE:
          session.sendMessage(
              new SetMotorSpeedMessage(session.getRobotId(), session.getMasterId(), 0, 0));
          break;
      }
    }
  }
