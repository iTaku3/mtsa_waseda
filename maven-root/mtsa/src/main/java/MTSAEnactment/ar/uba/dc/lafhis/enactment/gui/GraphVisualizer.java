 ...
 * @author Julio
 */
public class GraphVisualizer<State, Action> extends JPanel
    implements ITransitionEventListener<Action> {
  ...
  public void initialize(
      LTS<State, Action> lts, State initialState, BaseController<State, Action> scheduler) {
  ...
  private synchronized void applyStateStyles() {
    ...
    try {
      ...
      if (prevCell != null) {
        ...
        else this.getGraphComponent().getGraph().setCellStyle(STATE_STYLE_INITIAL, cells);
      }
  ...
  private void drawGraph2() {
    ...
    if (getScheduler().getLts() == null) return;
    // this.removeAll();
    ...
    try {
      ...
      if (this.getPreviousState() != null) {
        if (model.getCell(getStateCellId(this.getPreviousState())) == null) {
          graph.insertVertex(
              parent,
              getStateCellId(this.getPreviousState()),
              this.getPreviousState().toString(),
              this.nextNodeX,
              y,
              60,
              60,
      ...
      if (this.getCurrentState() != null) {
        if (model.getCell(getStateCellId(this.getCurrentState())) == null) {
          graph.insertVertex(
              parent,
              getStateCellId(this.getCurrentState()),
      ...
      if (this.getStateIterator() != null) {
        ...
        while (stateIterator.hasNext()) {
          ...
          if (mxGraphModel.getEdgesBetween(
                      model,
                      model.getCell(getStateCellId(this.getCurrentState())),
                      model.getCell(getStateCellId(nextState)))
                  .length
              == 0)
            graph.insertEdge(
                parent,
                null,
                currentPair.getFirst().toString(),
                model.getCell(getStateCellId(this.getCurrentState())),
                model.getCell(getStateCellId(nextState)),
                "MY_EDGE");
        }
  ...
  private void drawGraph() {
    ...
    try {
      ...
      for (State state : states) {
        ...
        graph.insertVertex(
            parent, getStateCellId(state), state.toString(), x, y, 60, 60, styleToApply);
        x += 60;
      }
      for (State state : states) {
        Iterator<Pair<Action, State>> actionIterator =
            getScheduler().getLts().getTransitions(state).iterator();
        String stateName = "";
   ...
   */
  private synchronized void setStateIterator(Iterator<Pair<Action, State>> stateIterator) {
