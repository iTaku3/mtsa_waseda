...
public class UpdatePanel extends JFrame {
    ...
    LTSOutput ltsOutput =
        new LTSOutput() {
          ...
          public void outln(String str) {
            this.out(
                (str != null ? str : GeneralConstants.EMPTY_STRING) + GeneralConstants.LINE_BREAK);
          }
  ...
  private JComboBox<String> initCombo(HPWindow input, String currentDirectory) {
    ...
    if (updateNames.isEmpty()) {
      ...
      LTSCompiler compiler =
          new LTSCompiler(input.ltsInputString, input.ltsOutput, currentDirectory);
  ...
  private void compileUpdateOfController(Object selectedItem, LTSCompiler compiler) {
    compiler.compile();
    UpdatingControllersDefinition def =
        UpdatingControllersAnimatorUtils.updateDefinitions.get(selectedItem);
    UpdatingControllerCompositeState updContCompositeState =
        (UpdatingControllerCompositeState) compiler.continueCompilation(def.getName().getName());
    ...
    if (updContCompositeState.getComposition().getName().endsWith(ControlConstants.NO_CONTROLLER)) {
      throw new LTSException(
          "update controller could not be generated because controller "
              + def.getName()
              + " could "
              + "not be generated");
    }
