...
public class JsVisionMovement {
  ...
  /**
   * The number of consecutive processed frames without target found until the target is considered
   * lost.
   */
  ...
  /**
   * The number of consecutive processed frames that need to have similar angles to the target in
   * order to commit to turning.
   */
  ...
  /**
   * The lock for {@link #onTargetFound(ImageProcessedEvent)} and {@link
   * #onTargetNotFound(ImageProcessedEvent)}.
   */
  ...
  private void onTargetFound(ImageProcessedEvent e) {
    ...
    synchronized (moveToTargetLock) {
      if (moveToTarget) {
        ...
        closeToTarget |=
            ds < TARGET_DISTANCE; // if close, stop considering camera input to avoid skipping over
        // the target
   ...
   * This is the case iff all of them are similar. If false, the yaw cannot be relied upon due to
   * noise or being in the middle of turning.
   *
