...
public class JsMovement extends JsSpeedometer.SynchronizedDecorator implements Runnable {
  ...
  /**
   * Models a change in the movement function of the drone. Each node allows interpolating the
   * movement properties over time up until the newer node.
   *
   * <p>Assumes that the acceleration per node is constant. As a result, the velocity is linear. In
   * turn, the distance over time can be deduced.
   *
   * @author Timo G&uuml;nther
   */
  private static class SpeedNode implements JsSpeedometer {
     ...
     */
    public SpeedNode(long t, double speed, double a, SpeedNode older) {
     ...
     * @param older the previous node in time
     */
    private SpeedNode(long t, double speed, double a, double v, double d, SpeedNode older) {
      this.t = t;
      this.speed = speed;
      this.a = a;
      this.v = v;
  ...
  /**
   * Sends movement commands to the drone in regular intervals. This is necessary as otherwise the
   * drone interprets the lack of movement commands as a drop in bandwidth, causing the frame rate
   * of the {@link JsCamera video stream} to drop.
   */
  @Override
  public void run() {
    try {
   ...
   *
   * <ul>
   *   <li>A value of 0 causes the drone to stop moving longitudinally.
   *   <li>A value greater than 0 causes the drone to move forward, with maximum speed at 1.
   *   <li>A value smaller than 0 causes the drone to move backward, with maximum speed at -1.
   * </ul>
   *
  ...
  public void setSpeed(long t, double speed) {
    synchronized (moveLock) {
      ...
      if (speed > 0 && old.speed < 0 || speed < 0 && old.speed > 0) { // reverse direction
        ...
        old =
            new SpeedNode(
                t, 0, a, old); // brake before accelerating in opposite of current direction
  ...
  private boolean setSpeedFromDestination(long t) {
    ...
    final double speed =
        destinationReached ? speedEnd : Maths.scaleLinear(progress, speedStart, speedEnd);
    setSpeed(t, speed);
    return destinationReached;
  }

  /**
   * Returns the physical velocity that would be reached after setting the drone to the given speed
   * for long enough.
   *
  ...
  /**
   * Returns the speed that the drone would need to be set to eventually reach the given physical
   * velocity.
   *
