 ...
 * Base Controller
 *
 * <p>Extend this class to implement controllers who react and update the lts state
 *
 * @author Julio
 */
public abstract class BaseController<State, Action> extends Enactor<State, Action> {
  ...
  @Override
  public synchronized void primitiveHandleTransitionEvent(TransitionEvent<Action> transitionEvent)
      throws Exception {
      ...
      logger.error(
          String.format(
              "[Scheduler::primitiveHandleTransitionEvent] there is no reachable state from"
                  + " currentState %s taking incoming action %s",
              currentState.toString(), action.toString(), exceptionString));
