...
public class RobotGui extends JFrame implements ActionListener {
  ...
  /**
   * Calcula la distancia entre dos colores Funcion temporal de prueba. debe eliminarse de la GUI
   *
   * @param e1
   * @param e2
   * @return
  ...
  private void openLTS() {
    try {
      ...
      //					MTSCompiler.getInstance().compileMTS("controller", "FOLLOW = (follow -> (success ->
      // TURN | lost -> LOST)), TURN = (turnLeft -> (success -> FOLLOW | lost -> LOST)).")
      ...
      //			nxtRobot = new NXTRobot<String, String>("Robot1", nxtRobotComm, actionSuccess,
      // actionFailure, actionLost, actionFollow, actionTurnLeft, actionTurnRight, actionTurnBack,
      // actionCalibrar);
    ...
    try {
      ...
      if (enactmentCommand != null) {
        if (nxtRobotComm == null) {
          ...
          // TODO: Temporal calibrar color action
          // nxtRobot.readColor = actionReadColor;
          nxtRobot.addTransitionEventListener(
              new ITransitionEventListener<String>() {

                @Override
                public void handleTransitionEvent(TransitionEvent<String> transitionEvent)
                    throws Exception {
                  //							System.out.println("Event received - " + transitionEvent.getAction());
                  nocontrolableTextArea.append(
                      "No controlable received: " + transitionEvent.getAction() + "\n");
                }
              });
        }
