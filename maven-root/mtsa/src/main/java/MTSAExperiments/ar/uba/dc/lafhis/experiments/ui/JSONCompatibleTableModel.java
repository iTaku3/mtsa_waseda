...
public class JSONCompatibleTableModel implements TableModel {
  ...
  public JSONCompatibleTableModel(List<String> keys, List<List<JSONCompatible>> data)
      throws IllegalArgumentException {
    for (List<JSONCompatible> row : data)
      if (keys.size() != row.size())
        throw new IllegalArgumentException(
            "JSONCompatibleTableModel::JSONCompatibleTableModel data and keys should be of the same"
                + " size");
    this.data = data;
    this.keys = keys;
  ...
  public Object getValueAt(int row, int col) {
    ...
    if (jsonObject instanceof JSONCompatibleInt) return ((JSONCompatibleInt) jsonObject).getValue();
  ...
  public boolean isCellEditable(int row, int col) {
    if (getValueAt(row, col) instanceof JSONCompatibleObject)
      if (((JSONCompatibleObject) getValueAt(row, col)).getValue()
          instanceof DirectedSparseMultigraph) return true;
    return true;
  }

  public void setValueAt(Object value, int row, int col) {
    if (!(value instanceof JSONCompatible)) return;
