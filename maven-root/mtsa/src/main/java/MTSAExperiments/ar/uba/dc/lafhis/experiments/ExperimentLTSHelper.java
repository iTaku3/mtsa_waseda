...
public class ExperimentLTSHelper {
  ...
  public static ExperimentLTSHelper getInstance() {
    if (instance == null) instance = new ExperimentLTSHelper();
    return instance;
  }

  public CompositeState getCompositeFromFile(String ltsFilename, String controllerName) {
  ...
  public LTS<Long, String> getLTSFromFile(String ltsFilename, String modelName) {
    // parse input file
    File ltsFile = new File(ltsFilename);
    try {
      LTSInput input = new FileInput(ltsFile);
      ...
      if (c == null) {
        ...
        if (compo != null) c = compo.composition;
      }
      // get model lts
      LTSAdapter<Long, String> env =
          new LTSAdapter<Long, String>(
              AutomataToMTSConverter.getInstance().convert(c), TransitionType.POSSIBLE);
      return env;
    } catch (Exception e) {
      return null;
    }
  ...
  public Set<LTS<Long, String>> getSafetyProcessesFromFile(
    ...
    try {
      ...
      ControllerGoalDefinition goalDefinition =
          ControllerGoalDefinition.getDefinition(controllerSymbol);

      Set<LTS<Long, String>> safetyReqs = new HashSet<LTS<Long, String>>();
      for (Symbol safetyDef : goalDefinition.getSafetyDefinitions()) {
        CompactState c = compiler.getProcessCompactStateByName(safetyDef.getName());
        // TODO:what happens with assertions? they are not automata
        if (c == null) c = AssertDefinition.compileConstraint(output, safetyDef.getName());

        LTSAdapter<Long, String> safe =
            new LTSAdapter<Long, String>(
                AutomataToMTSConverter.getInstance().convert(c), TransitionType.POSSIBLE);
        safetyReqs.add(safe);
      }
  ...
  public Set<String> getControllableActionsFromFile(String ltsFilename, String controllerName) {
    // parse input file
    File ltsFile = new File(ltsFilename);
    try {
      LTSInput input = new FileInput(ltsFile);
  ...
  public GRGoal<Long> getGRGoalFromFile(String ltsFilename, String controllerName) {
    // parse input file
    File ltsFile = new File(ltsFilename);
    try {
      LTSInput input = new FileInput(ltsFile);
      ...
      GRGame<Long> grGame =
          grGameBuilder.buildGRGameFrom(
              AutomataToMTSConverter.getInstance().convert(c.composition), c.goal);
  ...
  public ControllerGoal<String> getGRControllerGoalFromFile(
