...
public class GamePrunningExperiment extends Experiment {
  ...
  @Override
  public Dictionary<String, JSONCompatible> primitiveRunExperiment(
      Dictionary<String, JSONCompatible> parameters) throws NullArgumentException {
    if (parameters.get(SHORT_NAME_PARAM) == null
        || parameters.get(NO_G_PARAM) == null
        || parameters.get(ASSUMPTIONS_PARAM) == null
        || parameters.get(GR_PARAM) == null
        || parameters.get(LTS_LOC_PARAM) == null
        || parameters.get(ENV_NAME_PARAM) == null
        || parameters.get(EXP_NAME_PARAM) == null
        || parameters.get(CTRL_NAME_PARAM) == null
        || parameters.get(GOAL_NAME_PARAM) == null
        || parameters.get(ALL_CONTROLLABLES_PARAM) == null
        || parameters.get(RELAX_PARAM) == null
        || parameters.get(SELF_LOOP_PARAM) == null) {
      throw new NullArgumentException(
          "GamePrunningExperiment::primitiveRunExperiment Input parameter missing");
    }

    JSONCompatibleBoolean runNoG = (JSONCompatibleBoolean) (parameters.get(NO_G_PARAM));
    JSONCompatibleBoolean runAssumptions =
        (JSONCompatibleBoolean) (parameters.get(ASSUMPTIONS_PARAM));
    JSONCompatibleBoolean runGR = (JSONCompatibleBoolean) (parameters.get(GR_PARAM));

    JSONCompatibleBoolean allControllables =
        (JSONCompatibleBoolean) (parameters.get(ALL_CONTROLLABLES_PARAM));
    JSONCompatibleBoolean relaxOnAssumptions =
        (JSONCompatibleBoolean) (parameters.get(RELAX_PARAM));
    JSONCompatibleBoolean relaxSelfLoops =
        (JSONCompatibleBoolean) (parameters.get(SELF_LOOP_PARAM));
    ...
    JSONCompatibleString goalsDefinitionName =
        (JSONCompatibleString) (parameters.get(GOAL_NAME_PARAM));

    LTS<Long, String> env =
        ExperimentLTSHelper.getInstance()
            .getLTSFromFile(filename.getValue(), environmentName.getValue());
    ControllerGoal<String> controllerGoal =
        ExperimentLTSHelper.getInstance()
            .getGRControllerGoalFromFile(filename.getValue(), controllerName.getValue());

    Set<LTS<Long, String>> safetyReqs =
        ExperimentLTSHelper.getInstance()
            .getSafetyProcessesFromFile(
                filename.getValue(), controllerName.getValue(), goalsDefinitionName.getValue());

    CompositeState safetyComposite =
        new CompositeState("safetyComposite", new Vector<CompactState>());
    if (safetyReqs == null) System.out.println("no safetyRequirement");
    else
      for (LTS<Long, String> safetyReq : safetyReqs) {
        safetyComposite.machines.add(
            MTSToAutomataConverter.getInstance()
                .convert(new MTSAdapter(safetyReq), "safety_automata"));
      }
    safetyComposite.machines.add(
        MTSToAutomataConverter.getInstance().convert(new MTSAdapter(env), "environment"));
    safetyComposite.compose(new StandardOutput());
    MTS<Long, String> envMTS =
        AutomataToMTSConverter.getInstance().convert(safetyComposite.composition);
    env =
        new LTSAdapter<Long, String>(
            ControllerUtils.embedFluents(envMTS, controllerGoal, new StandardOutput()),
            TransitionType.POSSIBLE);

    MTS<Long, String> envForCtrlMTS =
        AutomataToMTSConverter.getInstance().convert(safetyComposite.composition);

    LTS<Long, String> envForController =
        new LTSAdapter<Long, String>(
            ControllerUtils.embedFluents(envForCtrlMTS, controllerGoal, new StandardOutput()),
            TransitionType.POSSIBLE);
    ...
    StateSpaceCuttingPerfectInfoOppositeGrControlProblem grControlProblem =
        new StateSpaceCuttingPerfectInfoOppositeGrControlProblem(
            env,
            controllerGoal,
            trapState,
            runNoG.getValue(),
            runAssumptions.getValue(),
            runGR.getValue(),
            allControllables.getValue(),
            relaxOnAssumptions.getValue(),
            relaxSelfLoops.getValue());
    LTS<Long, String> controller = grControlProblem.solve();

    results.put(
        GAME_GRAPH_RESULT,
        new JSONCompatibleObject(
            ExperimentJUNGHelper.getInstance()
                .getGameGraph(
                    (LabelledGameSolver<Long, String, Integer>) grControlProblem.getGameSolver(),
                    grControlProblem.getGRGoal(),
                    grControlProblem.getWinningStates())));
    results.put(
        ENV_GRAPH_RESULT,
        new JSONCompatibleObject(
            ExperimentJUNGHelper.getInstance()
                .getLTSGraph(env, controllerGoal.getControllableActions())));
    ...
    results.put(
        INIT_STATE_RESULT,
        new JSONCompatibleInt(grControlProblem.getGameSolver().getGame().getStates().size()));
    results.put(
        END_STATE_RESULT,
        new JSONCompatibleInt(grControlProblem.getGameSolver().getWinningStates().size()));
    ...
    results.put(
        ...
        new JSONCompatibleObject(
            ExperimentJUNGHelper.getInstance()
                .getLTSGraph(controller, controllerGoal.getControllableActions())));
