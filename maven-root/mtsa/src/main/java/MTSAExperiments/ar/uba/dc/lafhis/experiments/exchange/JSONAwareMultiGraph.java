...
public class JSONAwareMultiGraph<V, E> extends DirectedSparseMultigraph<V, E> implements JSONAware {
  ...
  public String toJSONString() {
    ...
    for (V vertex : getVertices()) {
      if (verticesString != null) verticesString += ",";
      else verticesString = "[";
      verticesString +=
          (vertex instanceof JSONAware) ? ((JSONAware) vertex).toJSONString() : vertex.toString();
    }
    if (verticesString != null) verticesString += "]";
    else verticesString = "[]";
    for (E edge : getEdges()) {
      if (edgesString != null) edgesString += ",";
      else edgesString = "[";
      edgesString +=
          (edge instanceof JSONAware) ? ((JSONAware) edge).toJSONString() : edge.toString();
    }
    if (edgesString != null) edgesString += "]";
    else edgesString = "[]";
