...
public class SymbolicBuilder {
  ...
  protected void buildSpecification() {
    ...
    for (String key : fluents.keySet()) {
      ...
      currentNode =
          new BinaryNode(
              equalOperator,
              stateSignals.get(FLUENT_PREFIX + fluent.getName()),
      ...
      specification.addSystemInitialFormula(
          new Formula(
              "init sys fluent " + fluent.getName() + " formula",
              initialFormula(currentNode, initiatingSignals, fluentActionSignals),
              new ControllableType(true)));

      // adding transition formulas
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " initiating sys trans",
              transitionFormula(currentNode, initiatingSignals, nextNode),
              controllableType));
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " terminating sys trans",
              transitionFormula(nextNode, terminatingSignals, currentNode),
              controllableType));
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " external event supressing trans",
              selfLoopFormula(currentNode, externalSignals),
              controllableType));

      // adding enabling formulas
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " initiating env enabling",
              enablingFormula(
                  currentNode,
                  initiatingSignals,
                  terminatingNonControllableSignals,
                  initiatingNonControllableSignals),
              nonControllableType));
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " initiating sys enabling",
              enablingFormula(
                  currentNode,
                  initiatingSignals,
                  terminatingNonControllableSignals,
                  terminatingControllableSignals,
                  initiatingControllableSignals),
              controllableType));
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " terminating env enabling",
              enablingFormula(
                  nextNode,
                  terminatingSignals,
                  initiatingNonControllableSignals,
      ...
      specification.addSystemSafetyFormula(
          new Formula(
              fluent.getName() + " terminating sys enabling",
              enablingFormula(
                  nextNode,
                  terminatingSignals,
                  initiatingNonControllableSignals,
    ...
    for (String key : ltss.keySet()) {
      ...
      for (Long fromState : lts.getTransitions().keySet()) {
        ...
        Iterator<Pair<String, Long>> currentRelationIterator =
            lts.getTransitions().get(fromState).iterator();
        ...
        while (currentRelationIterator.hasNext()) {
          ...
          specification.addSystemSafetyFormula(
              new Formula(
                  key
                      + " "
                      + String.valueOf(fromState)
                      + " "
                      + currentAction
                      + " sys trans to "
                      + String.valueOf(toState),
                  transitionFormula(currentNode, actionSignals.get(currentAction), nextNode),
                  controllableType));
        }
        // add self loop transition
        specification.addSystemSafetyFormula(
            new Formula(
                key + " " + String.valueOf(fromState) + " external event supressing trans",
                selfLoopFormula(currentNode, externalSignals),
                controllableType));
  ...
  private BinaryNode initializeSignal(SignalNode signalNode, boolean value) {
    return new BinaryNode(
        equalOperator, signalNode, new SignalNode(value ? trueSignal : falseSignal));
  }

  private BinaryNode initializeNumericSignal(SignalNode signalNode, Long value) {
    return new BinaryNode(
        equalOperator, signalNode, new SignalNode(new Signal(String.valueOf(value), true)));
  }
  ...
  private FormulaNode applySetFormula(
      Operator operator, Set<FormulaNode> nodes, boolean encloseFormula) {
  ...
  private FormulaNode exclusiveDisjunctSignals(Set<SignalNode> signalNodes) {
    ...
    for (SignalNode node : signalNodes) {
      ...
      for (SignalNode node2 : signalNodes) {
        if (node2.getSignal().getID().equals(node2.getSignal().getID())) continue;
  ...
  private FormulaNode initialFormula(
      FormulaNode currentStateNode, Set<SignalNode> outgoingNodes, Set<SignalNode> completeNodes) {
  ...
  // G(s_i && one event to s_j up -> X(only one of the outgoing if there's anay and none of the
  // others)) 	 FOR ENV THETA
  private FormulaNode enablingFormula(
      FormulaNode currentStateNode,
      SignalNode outgoingNode,
      Set<SignalNode> exclusiveEnvNodes,
      Set<SignalNode> disabledEnvNodes) {
  ...
  // G(s_i && (at least one event to s_j up) -> X(only one of the outgoing if there's any and none
  // of the others)) 	 FOR ENV THETA
  private FormulaNode enablingFormula(
      FormulaNode currentStateNode,
      Set<SignalNode> outgoingNodes,
      Set<SignalNode> exclusiveEnvNodes,
      Set<SignalNode> disabledEnvNodes) {
    Set<FormulaNode> leftSideNodes = new HashSet<FormulaNode>();
    leftSideNodes.add(currentStateNode);
    ...
    if (exclusiveEnvNodes.size() > 0) { // set one exclusive outgoing event
      if (disabledEnvNodes.size() > 0) {
        rightSideNode =
            next(
                conjunct(
                    exclusiveDisjunctSignals(exclusiveEnvNodes),
                    conjunctSignals(disabledEnvNodes, false)));
      } else {
  ...
  // G(s_i && one event to s_j up -> X(only one of the outgoing while none of the others || at least
  // one of the others)) 	FOR SYS THETA
  private FormulaNode enablingFormula(
      FormulaNode currentStateNode,
      SignalNode outgoingNode,
      Set<SignalNode> envNodes,
      Set<SignalNode> exclusiveSysNodes,
      Set<SignalNode> disabledSysNodes) {
    Set<SignalNode> dummySet = new HashSet<SignalNode>();
    dummySet.add(outgoingNode);

    return enablingFormula(
        currentStateNode, dummySet, envNodes, exclusiveSysNodes, disabledSysNodes);
  }

  // G(s_i && (at least one event to s_j up) -> X(only one of the outgoing while none of the others
  // || at least one of the others)) 	FOR SYS THETA
  private FormulaNode enablingFormula(
      FormulaNode currentStateNode,
      Set<SignalNode> outgoingNodes,
      Set<SignalNode> envNodes,
        ...
        if (disabledSysNodes.size() > 0) {
          rightSideNode =
              next(
                  conjunct(
                      imply(
                          conjunctSignals(envNodes, false),
                          exclusiveDisjunctSignals(exclusiveSysNodes)),
  ...
  private FormulaNode transitionFormula(
      FormulaNode currentStateNode, Set<SignalNode> outgoingNodes, FormulaNode nextStateNode) {
    Set<FormulaNode> leftSideNodes = new HashSet<FormulaNode>();
    leftSideNodes.add(currentStateNode);
  ...
  private FormulaNode transitionFormula(
      FormulaNode currentStateNode, SignalNode outgoingNode, FormulaNode nextStateNode) {
    Set<FormulaNode> leftSideNodes = new HashSet<FormulaNode>();
    leftSideNodes.add(currentStateNode);
