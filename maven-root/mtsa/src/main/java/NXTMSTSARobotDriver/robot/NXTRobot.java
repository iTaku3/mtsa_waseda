...
/**
 * Created with IntelliJ IDEA. User: tommy Date: 2/23/13 Time: 5:44 PM To change this template use
...
public abstract class NXTRobot extends Robot {
  ...
  private void doblar(int sentido) {
    ...
    // Giro
    if (sentido == 1) getPilot().rotate(-this.getRotateAngle());
    else if (sentido == -1) getPilot().rotate(this.getRotateAngle());
    else if (sentido == 0) getPilot().rotate(this.turnBackAngle);

    double correctionAngle;
    if (sentido == 1) correctionAngle = -1;
    else if (sentido == -1) correctionAngle = 1;
    else correctionAngle = 2;
    ...
    while (calculatePathState(PathState.intersection) != PathState.intersection
        && attemp < maxCorrection) {
  ...
  /** Inicia rutina de calibracion. Captura de colores: Camino - Interseccion */
  ...
  public void avanzarHastaGris() throws RobotLostException {
    ...
    if (currentState == PathState.intersection) exitIntersection();
    ...
    if (currentState != PathState.path)
      if (!tryToFind(PathState.path)) throw new RobotLostException();
    ...
    if (calculatePathState(PathState.none) == PathState.outside)
      if (!tryToFind(PathState.intersection)) throw new RobotLostException();
  ...
  public void avanzar(double distancia) {
    ...
    getPilot()
        .travel(
            distancia); // distancia debe estar en las mismas unidades que la instancia del pilot
  }
  ...
  private void verificarMovimientoYParar() {
    if (getPilot().isMoving()) getPilot().stop(); // tambien se puede usar quickStop
  }
  ...
  private void exitIntersection() {
    ...
    while (!Button.ESCAPE.isDown()
        && calculatePathState(PathState.intersection) == PathState.intersection) {
  ...
  private void followPath(boolean reverse) throws RobotLostException {
    ...
    double linea =
        this.getPathColor(); // - extraRango;                // lectura del sensor de luz sobre la
    // linea
    double suelo = this.getOutOfPath(); // lectura del sensor de luz fuera de la linea
    double velMax =
        this.getVelocidadMaxima(); // velocidad (grados por segundo) maxima de los motores
