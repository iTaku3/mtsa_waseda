...
public class MyConstraint {
  ...
  private void buildSafety(Spec toParse, String clockKind, Integer propertyNumber) {
    ...
    while ((toParse instanceof SpecExp)
      ...
      this.SubConstraints.add(
          new Pair<String, String>(name, "[](" + clockKind + " -> " + LTLProp + ")"));
    }
  ...
  private void buildInitial(Spec toParse, String clockKind, Integer propertyNumber) {
    ...
    String LTLProp =
        "(!" + clockKind + " W (" + clockKind + " && " + subParse(toParse, clockKind) + "))";
    this.SubConstraints.add(new Pair<String, String>(name, LTLProp));
  }
    ...
    switch (e.getOperator()) {
      ...
      case EQUALS:
        if (!isPrimitiveEqual(e)) {
          ...
          //					return changeNotOrder(new SpecExp(Operator.IFF, e.getChildren()[0],
          // e.getChildren()[1]), insideNot);
        }
        if (insideNot) {
      ...
      for (String f : leftFluents) {
        ...
        if (hasNoNext) {
          if (insideNot) {
            ...
            // and  !(a <-> b) is equivalent to !(a -> b) || !(b -> a).     !(a -> b) is equivalent
            // to a && !b
            equals.add(
                "(("
                    + leftAnswer
