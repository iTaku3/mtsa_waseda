...
public class PathBuilder2Test extends TestCase {
  ...
  public void testAll() {
    ...
    System.out.println("\nLTS_01");
    this.getAllPath(
        new MTSAdapter<Integer, String>(LTSExamples.LTS_01),
        Collections.singleton(MTSExamples.TAU),
        8);
  }
  ...
  public <S, A> void getAllPath(MTS<S, A> mts, Set<A> silentActions, int expectedAmount) {
    ...
    Map<TransitionType, PathBuilder2<S, A>> builders =
        new EnumMap<TransitionType, PathBuilder2<S, A>>(TransitionType.class);
