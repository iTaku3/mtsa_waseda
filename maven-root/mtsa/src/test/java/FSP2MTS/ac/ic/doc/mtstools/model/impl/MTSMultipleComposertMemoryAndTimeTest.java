  ...
  public void testComponeMTSA() throws Exception {
    String sourceString =
        "PHIL = (sitdown?->right.get->left.get->eat->left.put->right.put->arise->PHIL).\n"
            + " FORK = (get -> put -> FORK).\n"
            + " ||DINERS(N=5)=forall"
            + " [i:0..N-1](phil[i]:PHIL||{phil[i].left,phil[((i-1)+N)%N].right}::FORK).menu RUN ="
            + " {phil[0..5].{sitdown,eat}}";
    ...
    System.out.println(
        "Memoria usada : " + ((finalMemoryUsed - initialMemoryUsed) / (1024 * 1024)) + "Mb.");
  }

  public static void main(String[] args) throws Exception {
    MTSMultipleComposertMemoryAndTimeTest multipleComposertMemoryAndTimeTest =
        new MTSMultipleComposertMemoryAndTimeTest();
