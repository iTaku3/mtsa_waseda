  ...
  public void testUnLTSenMTSAyEnLTSA() throws Exception {
    String sourceString =
        "PHIL = (sitdown->right.get->left.get->eat->left.put->right.put->arise->PHIL).\n"
            + " FORK = (get -> put -> FORK).\n"
            + " ||DINERS(N=4)=forall"
            + " [i:0..N-1](phil[i]:PHIL||{phil[i].left,phil[((i-1)+N)%N].right}::FORK).\n"
    ...
    new MTSMultipleComposer(new CompositionRuleApplier())
        .compose(buildMTSCollection(compositeByMTSA));
    ...
    System.out.println(
        "Diferencia de memoria : " + (Math.abs(memoriaMTSA - memoriaLTSA) / (1024 * 1024)) + "Mb.");
