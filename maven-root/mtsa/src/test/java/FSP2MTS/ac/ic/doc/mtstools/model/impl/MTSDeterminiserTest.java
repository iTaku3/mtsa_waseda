...
public class MTSDeterminiserTest extends MTSTestBase {
  ...
  protected void testFor(
      String sourceString,
      String sourceModel,
      String expectedSourceString,
      String expectedModel,
      boolean forkSemantics)
      throws Exception {
    ...
    if (!forkSemantics) {
      ...
      MTS<Long, String> toDet =
          AutomataToMTSConverter.getInstance().convert(composite.getComposition());
      MTS<Long, String> deterministic = new MTSDeterminiser(toDet, false).determinize();
      CompactState detAutomata =
          MTSToAutomataConverter.getInstance().convert(deterministic, composite.getName());
    ...
    CompositeState expectedComposite =
        LTSATestUtils.buildAutomataFromSource(expectedSourceString, expectedModel);
