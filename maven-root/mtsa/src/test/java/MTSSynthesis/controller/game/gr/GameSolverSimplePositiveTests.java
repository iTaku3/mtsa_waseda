...
public class GameSolverSimplePositiveTests extends GRGameSolverBaseTestCase {
  ...
  public void testSimpleGame21() throws Exception {
    ...
    assertEquals(
        Collections.singleton(new StrategyState<Long, Integer>(state2, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state1, 1)));
    assertEquals(
        Collections.singleton(new StrategyState<Long, Integer>(state3, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state2, 1)));
    assertEquals(
        Collections.singleton(new StrategyState<Long, Integer>(state1, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state3, 1)));
  }
}
