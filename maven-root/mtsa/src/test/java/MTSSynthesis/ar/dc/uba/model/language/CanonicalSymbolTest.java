...
public class CanonicalSymbolTest {
  ...
  public void testCanonicalSymbolEquals() {
    ...
    assertEquals(
        this.createSymbolAndEmptyConditions("tt"), this.createSymbolAndEmptyConditions("tt"));
    ...
    assertEquals(
        this.createNoSymbolAndConditions(conditions), this.createNoSymbolAndConditions(conditions));
    assertEquals(
        this.createNoSymbolAndConditions(conditions),
        this.createNoSymbolAndConditions(otherConditions));

    assertEquals(
        this.createSymbolAndConditions("test", conditions),
        this.createSymbolAndConditions("test", conditions));
    assertEquals(
        this.createSymbolAndConditions("test", conditions),
        this.createSymbolAndConditions("test", otherConditions));
  }

  @Test
  public void testCanonicalSymbolNotEquals() {
    assertFalse(
        this.createNoSymbolAndEmptyConditions().equals(this.createSymbolAndEmptyConditions("tt")));

    Set<String> conditions = new HashSet<String>();
    conditions.add("c1");

    assertFalse(
        this.createSymbolAndEmptyConditions("tt")
            .equals(this.createNoSymbolAndConditions(conditions)));

    assertFalse(
        this.createNoSymbolAndConditions(conditions)
            .equals(this.createSymbolAndConditions("test", conditions)));
  }
}
