...
public class TestBlockingDCS {
  ...
  private static final String[] RESOURCE_FOLDERS = {
    "Blocking/ControllableFSPs", "Blocking/NoControllableFSPs"
  };
  ...
  public void testControllability() throws Exception {
    ...
    if (isControllable) {
      assertTrue(
          "we are returning an empty controller", compositeStateC.composition.ntransitions() > 0);

      String asserted = "Check";
      CompositeState ltlProperty = AssertDefinition.compile(output, asserted);
      CompositeState not_ltlProperty =
      ...
      if (ltlProperty != null) {
        ...
        TransitionSystemDispatcher.checkFLTL(sys, ltlProperty, not_ltlProperty, false, output);
        String result = output.toString();
        if (result.contains("No LTL Property violations detected")) {
          assertTrue(sys.getErrorTrace() == null || sys.getErrorTrace().isEmpty());
        }

        if (result.contains("Trace to property violation in Check")
        ...
        LegalityAnalyser a =
            new LegalityAnalyser(
                ...
                "DirectedController",
                "Plant",
                goalDefinition.getControllableActionSet());
        a.composeNoHide();
