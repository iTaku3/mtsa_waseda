      ...
      if (!compositeState.name.contains("#")) {
        ...
        if (compositeState != null && ltlProperty != null) {
          ...
          TransitionSystemDispatcher.checkFLTL(
              compositeState, ltlProperty, not_ltl_property, false, output);
          String result = output.toString();
          if (result.contains("No LTL Property violations detected")) {
            assertTrue(
                compositeState.getErrorTrace() == null || compositeState.getErrorTrace().isEmpty());
          }

          if (result.contains("Trace to property violation in TESTGOAL")
