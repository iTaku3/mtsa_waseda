...
public class TestTranslationFromSynchronous {
  ...
  private static final String[] RESOURCE_FOLDERS = {
    "GR1FromReactive/Controllable", "GR1FromReactive/Uncontrollable"
  };
  ...
  public void TestBisimilarAfterTranslation() throws Exception {
    ...
    assertTrue(
        "One FSP is controllable and the other is not",
    ...
    if (isControllable) {
      ...
      CompositeState not_ltlProperty1 =
          AssertDefinition.compile(new EmptyLTSOuput(), AssertDefinition.NOT_DEF + assertAsterisk);
      CompositeState ltlProperty2 = AssertDefinition.compile(output, assertAlphabet);
      CompositeState not_ltlProperty2 =
          AssertDefinition.compile(new EmptyLTSOuput(), AssertDefinition.NOT_DEF + assertAlphabet);
      ...
      if (ltlProperty1 != null && ltlProperty2 != null) {
        ...
        TransitionSystemDispatcher.checkFLTL(sys1, ltlProperty1, not_ltlProperty1, false, output);
        System.out.println(
            "\n ==================Does Sys with * fulfil LTL without *?==================\n");
        TransitionSystemDispatcher.checkFLTL(sys1, ltlProperty2, not_ltlProperty2, false, output);
        System.out.println(
            "\n ==================Does Sys without * fulfil LTL with *?==================\n");
        TransitionSystemDispatcher.checkFLTL(sys2, ltlProperty1, not_ltlProperty1, false, output);
        System.out.println(
            "\n ==================Does Sys without * fulfil LTL without *?==================\n");
        TransitionSystemDispatcher.checkFLTL(sys2, ltlProperty2, not_ltlProperty2, false, output);
        String result = output.toString();
        if (result.contains("No LTL Property violations detected")) {
          assertTrue(
              (sys1.getErrorTrace() == null || sys1.getErrorTrace().isEmpty())
