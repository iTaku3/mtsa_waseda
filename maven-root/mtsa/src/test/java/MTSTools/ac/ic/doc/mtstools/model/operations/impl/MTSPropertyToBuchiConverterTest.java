...
public class MTSPropertyToBuchiConverterTest extends TestCase {
  public void testBuildProperty() throws Exception {
    ...
    assertEquals(
        "El automata no tiene los ciclos sobre el estado trampa",
        2,
        testProperty.getTransitions(2L, TransitionType.REQUIRED).size());
    ...
    for (Iterator it = testProperty.getStates().iterator(); it.hasNext(); ) {
      ...
      for (Iterator it2 = testProperty.getTransitions(state, TransitionType.REQUIRED).iterator();
          it2.hasNext(); ) {
