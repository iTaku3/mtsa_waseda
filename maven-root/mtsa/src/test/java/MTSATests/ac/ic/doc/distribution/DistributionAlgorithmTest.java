...
public class DistributionAlgorithmTest {
  ...
  public void testDistributionFeedbackOnFullAlphabetNotPresent() throws Exception {
    DistributionAlgorithm<Long, String> distAlg =
        new DistributionAlgorithmDefaultImpl<Long, String>();
    ...
    Predicate isDistributionFeedbackOnFullAlphabet =
        PredicateUtils.instanceofPredicate(DistributionFeedbackOnFullAlphabet.class);

    Assert.assertTrue(
        !CollectionUtils.exists(distResult.getFeedback(), isDistributionFeedbackOnFullAlphabet));
  ...
  @SuppressWarnings("unchecked")
  private void testNonDeterminismFeedback(AlphabetDistribution<String> alphabetDistribution)
      throws Exception {
    DistributionAlgorithm<Long, String> distAlg =
        new DistributionAlgorithmDefaultImpl<Long, String>();
    DistributionResult<Long, String> distResult;
    ...
    @SuppressWarnings("rawtypes")
    Predicate isDistributionFeedbackOnNonDeterminism =
        PredicateUtils.instanceofPredicate(DistributionFeedbackOnNonDeterminism.class);

    Predicate<MTS<Long, String>> isDeterministic =
    ...
    for (MTS<Long, String> mts : allTestModels) {
      ...
      Assert.assertTrue(
          "Invalid feedback on non determinism",
          isDeterministic.evaluate(mts)
              != CollectionUtils.exists(
                  distResult.getFeedback(), isDistributionFeedbackOnNonDeterminism));
      ...
      if (CollectionUtils.exists(
        ...
        for (DistributionFeedbackItem distributionFeedbackItem : feedback) {
          if (isDistributionFeedbackOnNonDeterminism.evaluate(distributionFeedbackItem)) {
            DistributionFeedbackOnNonDeterminism<Long, String> feedbackOnNonDeterminism =
                (DistributionFeedbackOnNonDeterminism<Long, String>) distributionFeedbackItem;
            Set<MTSTrace<String, Long>> tracesToNonDeterministicStates =
                feedbackOnNonDeterminism.getTracesToNonDeterministicStates();
            for (MTSTrace<String, Long> mtsTrace : tracesToNonDeterministicStates) {
              ...
              HasNonDeterministicTransitionsStatePredicate<Long, String>
                  hasNonDetTransitionsFromStatePredicate =
                      new HasNonDeterministicTransitionsStatePredicate<Long, String>(
              ...
              Assert.assertTrue(
                  "Trace does not lead to non deterministic state. Trace: "
                      + mtsTrace
                      + "\n. MTS: "
                      + mts,
                  hasNonDetTransitionsFromStatePredicate.evaluate(traceLastState));
            }
          }
        }
      }
  ...
  private void assertIsValidTrace(MTS<Long, String> mts, MTSTrace<String, Long> mtsTrace) {
    ...
    for (MTSTransition<String, Long> mtsTransition : mtsTrace) {
      ...
      BinaryRelation<String, Long> transitionsFromCurrentState =
          mts.getTransitions(currentState, TransitionType.POSSIBLE);
      Set<Long> statesAfterTransition =
          transitionsFromCurrentState.getImage(mtsTransition.getEvent());
      Assert.assertTrue(
          "Trace is not valid for the mts. MTS: " + mts + "\n Trace: " + mtsTrace,
          statesAfterTransition.contains(mtsTransition.getStateTo()));
      // get the next state
  ...
  public void testSimpleModelNonDetDifferentModalities() throws Exception {
    ...
    //////////////////// CHECK MODEL 12 ////////////////////////////////////////////////
    DistributionResult<Long, String> distResult =
        distAlg.tryDistribute(mi12, alphaDist, MTSConstants.TAU);
  ...
  public void testComplexModelDifferentModalities() throws Exception {
    ...
    //////////////////// CHECK MODEL 11 ////////////////////////////////////////////////
    DistributionResult<Long, String> distResult =
        distAlg.tryDistribute(mi11, alphaDist, MTSConstants.TAU);
  ...
  private void checkMissmatches(
      List<DistributionFeedbackItem> feedback,
      Map<String, Long> expectedMissmatchingActions,
      MTS<Long, String> systemModel) {

    Set<DeterminisationModalityMismatch<Long, String>> actualMismatches =
        new HashSet<DeterminisationModalityMismatch<Long, String>>();
    ...
    @SuppressWarnings("rawtypes")
    Predicate isDistributionFeedbackOnModalityMismatch =
        PredicateUtils.instanceofPredicate(DistributionFeedbackOnModalityMismatch.class);

    Collection feedbackOnMismatches =
        CollectionUtils.select(feedback, isDistributionFeedbackOnModalityMismatch);
    for (Object feedbackItemObj : feedbackOnMismatches) {
      DistributionFeedbackOnModalityMismatch<Long, String> feedbackItem =
          (DistributionFeedbackOnModalityMismatch<Long, String>) feedbackItemObj;
    ...
    for (DeterminisationModalityMismatch<Long, String> mismatch : actualMismatches) {
      ...
      Assert.assertTrue(
          !systemModel
              .getTransitions(stateWithReqTransition, TransitionType.REQUIRED)
              .getImage(mismatch.getAction())
              .isEmpty());

      Long stateWithMayTransition = mismatch.getStateByModality(TransitionType.MAYBE);
      Assert.assertTrue(
          !systemModel
              .getTransitions(stateWithMayTransition, TransitionType.MAYBE)
              .getImage(mismatch.getAction())
              .isEmpty());
    }
  }
