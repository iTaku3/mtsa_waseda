...
public class MTSUtilsTest {
  ...
  @Test
  public void testSizeForGetAllTracesToStateSatisfyingPredicateNonDeterministicStates()
      throws Exception {
    Set<MTSTrace<String, Long>> traces =
        this.getAllTracesToNonDeterministicStates(0, TransitionType.POSSIBLE);
  ...
  private Set<MTSTrace<String, Long>> getAllTracesToNonDeterministicStates(
      int i, TransitionType transitionType) throws Exception {
    return this.getAllTracesToNonDeterministicStates(
        TestModels.getInstance().getMTS(i), transitionType);
  }

  private Set<MTSTrace<String, Long>> getAllTracesToNonDeterministicStates(
      MTS<Long, String> mts, TransitionType transitionType) throws Exception {
    Predicate<Long> nonDeterministicStatePredicate =
        new HasNonDeterministicTransitionsStatePredicate<Long, String>(mts, transitionType);
    return MTSUtils.getAlltracesToStatesSatisfyingPredicate(
        mts, nonDeterministicStatePredicate, transitionType);
  }

  @Test
  public void testTracesSatisfyPredicateFromGetAllTracesToStateSatisfyingPredicate()
      throws Exception {
    TestModels testModels = TestModels.getInstance();
    Set<MTSTrace<String, Long>> traces;
    MTS<Long, String> mts;
    ...
    for (int i = 0; i < testModels.getSize(); i++) {
      ...
      Predicate<Long> hasNonDetTransitionsStatePredicate =
      ...
      for (MTSTrace<String, Long> traceToStateSatifyingPredicate : traces) {
        ...
        if (traceToStateSatifyingPredicate.size() == 0) {
          ...
          finalState =
              traceToStateSatifyingPredicate
                  .get(traceToStateSatifyingPredicate.size() - 1)
                  .getStateTo();
        }
      ...
      for (Long state : states) {
        ...
        traces =
            MTSUtils.getAlltracesToStatesSatisfyingPredicate(
                mts, isThisStatePredicate, TransitionType.POSSIBLE);
        assertTrue(
            "There is only one state satysying the condition so there should only be one trace.\n"
                + " MTS: "
                + i
                + ".\n Traces = "
                + traces.toString()
        ...
        if (trace.size() == 0) {
          assertTrue(
              "The empty trace was returned so the initial state should be the particular state",
              state.equals(mts.getInitialState()));
        } else {
          assertTrue(
              "The trace must lead to the state \n MTS: "
                  + i
                  + ".\n Trace = "
                  + trace.get(trace.size() - 1)
                  + ".\n State = "
                  + state,
              trace.get(trace.size() - 1).getStateTo().equals(state));
        }
      }
    }
  }
