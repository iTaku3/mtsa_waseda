...
public class NonDetControllerSynthesisTests {
  ...
  public void test1() throws Exception {
    CompositeState model =
        compiler.compileCompositeState("C", TestConstants.fileFrom("nondet1.lts"));
  }
  ...
  public void test5() throws Exception {
    CompositeState model =
        compiler.compileCompositeState("C", TestConstants.fileFrom("nondet5.lts"));
  ...
  public void test6() throws Exception {
    CompositeState model =
        compiler.compileCompositeState("C", TestConstants.fileFrom("nondet6.lts"));
  }
   ...
   * @param fileName the location of the file containing both the expected and the to be synthesize
   *     definitions
   * @throws IOException
   */
  private void testSynthesisedController(String resultName, String expectedName, String fileName)
      throws IOException {
    CompositeState resultController =
        compiler.compileCompositeState(resultName, TestConstants.fileFrom(fileName));
    MTS<Long, String> result =
        AutomataToMTSConverter.getInstance().convert(resultController.getComposition());
    String name = resultController.getComposition().name;
    assertTrue("There is no controller for C", !name.contains(ControlConstants.NO_CONTROLLER));
    CompositeState expectedController =
        compiler.compileCompositeState(expectedName, TestConstants.fileFrom(fileName));
    MTS<Long, String> expected =
        AutomataToMTSConverter.getInstance().convert(expectedController.getComposition());
  ...
  public void testN() throws Exception {
    ...
    assertEquals(
        Collections.singleton(Pair.create(state1, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state0, 1)));
    assertEquals(
        Collections.singleton(Pair.create(state2, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state1, 1)));
    assertEquals(
        Collections.singleton(Pair.create(state0, 1)),
        strategy.getSuccessors(new StrategyState<Long, Integer>(state2, 1)));

    MTS<StrategyState<Long, Integer>, String> synthesiseController =
        new ControllerSynthesisFacade<Long, String, Integer>().synthesiseController(mts, goal);
    MTS<Long, String> synMTS =
        new GenericMTSToLongStringMTSConverter<StrategyState<Long, Integer>, String>()
            .transform(synthesiseController);
    ...
    boolean refinement =
        TransitionSystemDispatcher.isRefinement(
            synMTS, " synthesised ", mts, " original ", weakSemantics, new StandardOutput());
