...
public class UpdatingControllersTests {
  ...
  private void environmentTest(String filename) throws Exception {
    ...
    MTS<Long, String> expectedEnv =
        MTSCompiler.getInstance().compileMTS("UpdEnv", String.valueOf(updEnv));
    ...
    for (String action : actionsFromCompiled) {
      if (action.contains(UpdateConstants.OLD_LABEL)
          && !expectedEnv.getActions().contains(action)) {
  ...
  // This test check that the file in targetA can compose UpdCont and check some formulas defined
  // there
  ...
  public void graphGenerator_producesCorrectGraph(String fileName)
    ...
    UpdateGraph updateGraph =
        UpdateGraphGenerator.generateGraph("Graph", compiler, new StandardOutput());
  ...
  //
  //		// LC: LowBeforeUpdate; NGFC: updateBeforeStopOldSpec; NSC: NoScanWhileUpdate; NMC:
  // NoMoveWhileUpdate; GC:
  // Ghezzi
  ...
  //	private void controllerComparisonTest(String targetA, String targetB, String filename) throws
  // Exception{
  //		MTS<Long, String> compiledA = MTSCompiler.getInstance().compileMTS(targetA,
  // getFile(filename));
  //		MTS<Long, String> compiledB = MTSCompiler.getInstance().compileMTS(targetB,
  // getFile(filename));
  //
  //		LTSSimulationSemantics simulationSemantics = new LTSSimulationSemantics();
