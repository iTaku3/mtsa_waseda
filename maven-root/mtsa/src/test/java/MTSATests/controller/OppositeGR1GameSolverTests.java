...
public class OppositeGR1GameSolverTests {
  ...
  private void OppositeGR1GameTest(
      String filename,
      String mutName,
      String expName,
      String controllerName,
      String controllerGoalName,
    ...
    LTS<Long, String> mut =
        getMut(filename, mutName, controllerName, controllerGoalName, resultFileName);

    if (mut == null) Assert.assertTrue(false, "mut was null");
  ...
  protected LTS<Long, String> getMut(
      String filename,
      String mutName,
      String controllerName,
      String controllerGoalName,
      String resultFileName) {
    try {
      LTS<Long, String> env = LTSTestHelper.getInstance().getLTSFromFile(filename, mutName);

      // IMPORTANT: to use the assumptionsAsGoals version of the goal
      ControllerGoal<String> controllerGoal =
          LTSTestHelper.getInstance()
              .getGRControllerGoalFromFile(filename, controllerName)
              .cloneWithAssumptionsAsGoals();

      Set<LTS<Long, String>> safetyReqs =
          LTSTestHelper.getInstance()
              .getSafetyProcessesFromFile(filename, controllerName, controllerGoalName);
      ...
      PerfectInfoOppositeGRControlProblem<Long, String> grControlProblem =
          new PerfectInfoOppositeGRControlProblem<Long, String>(env, controllerGoal, -1l);
  ...
  protected LTS<Long, String> getExpected(String filename, String expectedName) {
    try {
      return LTSTestHelper.getInstance().getLTSFromFile(filename, expectedName);
    } catch (Exception e) {
      System.out.print(e.getStackTrace().toString());
      return null;
    }
