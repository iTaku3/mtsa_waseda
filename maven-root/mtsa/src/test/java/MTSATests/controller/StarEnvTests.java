...
public class StarEnvTests extends TestCase {
  ...
  public void testSimpleStarredModel() throws Exception {
    ...
    MTS<Pair<Long, Set<String>>, String> generateStarredEnvModel =
        ControllerUtils.generateStarredEnvModel(mts);
    ...
    BinaryRelation<String, Pair<Long, Set<String>>> transitionsFrom0Bis =
        generateStarredEnvModel.getTransitions(state0Bis, TransitionType.POSSIBLE);
    ...
    PowerSet<String> ps =
        new PowerSet<String>(
            BinaryRelationUtils.getDomain(mts.getTransitions(state0, TransitionType.MAYBE)));
    ...
    for (Long state : mts.getStates()) {
      if (!state.equals(state0)) {
        ...
        BinaryRelation<String, Pair<Long, Set<String>>> transitions =
            generateStarredEnvModel.getTransitions(stateBis, TransitionType.POSSIBLE);
  ...
  public void testStateWithNoReqs() throws Exception {
    ...
    MTS<Pair<Long, Set<String>>, String> expected =
        new MTSImpl<Pair<Long, Set<String>>, String>(initialState);
