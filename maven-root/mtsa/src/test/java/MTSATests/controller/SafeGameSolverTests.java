...
public class SafeGameSolverTests {
  ...
  private LTS<StrategyState<Long, Integer>, String> getLTSSafeGameStrategyFromFile(
      String ltsFilename, String modelName, String controllerName) {
    // parse input file
    LTS<Long, String> env = LTSTestHelper.getInstance().getLTSFromFile(ltsFilename, modelName);

    Set<String> controllableActions =
        LTSTestHelper.getInstance().getControllableActionsFromFile(ltsFilename, controllerName);

    SafeGameSolver<Long, String> gameSolver =
        new SafeGameSolver<Long, String>(env, controllableActions);
    ...
    if (gameSolver.isWinning(env.getInitialState())) {
      ...
      LTS<StrategyState<Long, Integer>, String> result =
          GameStrategyToLTSBuilder.getInstance().buildLTSFrom(env, strategy);
  ...
  public Object[][] safeGameParameters() {
    return new Object[][] {
      {
        "SafetyCuts/Tests/safe_game_test.lts",
        "MUT",
        "EXP",
        "C",
        "G1",
        "SafetyCuts/Results/safe_game_test.results"
      }
    };
  }

  @Test(dataProvider = "safeGameTest")
  private void SafeGameTest(
      String filename,
      String mutName,
      String expName,
  ...
  protected LTS<Long, String> getMut(
      String filename, String mutName, String controllerName, String controllerGoalName) {
    LTS<StrategyState<Long, Integer>, String> strategyMut =
        getLTSSafeGameStrategyFromFile(filename, mutName, controllerName);
    LTS<Long, String> mut =
        new GenericLTSToLongStringLTSConverter<StrategyState<Long, Integer>, String>()
            .transform(strategyMut);
    LTSTestHelper.getInstance().removeUnusedActionsFromLTS(mut);
    return mut;
  }
