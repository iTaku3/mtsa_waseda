...
/**
 * This class tests the functionality of the property keyword by comparing the compose automata with
 * the expected automata.
 *
 * @author lnahabedian
 */
public class PropertyTests {
  ...
  public void testPropertyInComposite() throws Exception {
    ...
    CompositeState expected = compiler.compileCompositeState("PL", expectedFSP);

    WeakSimulationSemantics weakSimulationSemantics =
        new WeakSimulationSemantics(Collections.emptySet());

    // Check LTS simulation through MTS refinement.
    boolean refinement =
        TransitionSystemDispatcher.isRefinement(
            converter.convert(compiled.getComposition()),
            " original ",
            converter.convert(expected.getComposition()),
            " synthesised ",
            weakSimulationSemantics,
            new StandardOutput());
  ...
  public void testPropertyInCompositeWithOthersOperations() throws Exception {
    AutomataToMTSConverter converter = AutomataToMTSConverter.getInstance();
    String fsp =
        "LOWER=(a ->b-> LOWER)@{a,b}.HIGHER=(b->a->HIGHER)@{a,b}.property ||PL = LOWER.||CHECK ="
            + " (PL||HIGHER).";
    MTSCompiler compiler = MTSCompiler.getInstance();
    ...
    String expectedFSP =
        "LOWER=(a->b->LOWER).PL=(a->P1|b->ERROR),P1=(b->PL|a->ERROR).HIGHER=(b->a->HIGHER).";
    CompositeState expected = compiler.compileCompositeState("PL", expectedFSP);
  ...
  public void testPropertyInCompositeWithAlotOfStates() throws Exception {
    ...
    String expectedFSP =
        "A=(a->AA|b->B|c->C|d->D),B=(a->A|{b,c,d}->ERROR),C=(a->A|{b,c,d}->ERROR),D=(a->A|{b,c,d}->ERROR),AA=(b->AAA|{a,c,d}->ERROR),AAA=(c->AAAA|{a,b,d}->ERROR),AAAA=(d->A|{a,b,c}->ERROR).";
    CompositeState expected = compiler.compileCompositeState("PL", expectedFSP);
